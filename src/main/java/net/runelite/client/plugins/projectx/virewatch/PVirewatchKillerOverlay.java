...
public class PVirewatchKillerOverlay extends Overlay {
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        if (plugin.startingLocation != null) {
            LocalPoint startTile = LocalPoint.fromWorld(ProjectX.getClient(), plugin.startingLocation);
            if (startTile != null) {
                Polygon safeSpotPoly = Perspective.getCanvasTileAreaPoly(ProjectX.getClient(), startTile, 1);
        ...
        if (!config.disableNPCOutline()) {
            for (var npcModel : ProjectX.getRs2NpcCache().query().withName("Vyrewatch Sentinel").where(n -> !n.isDead()).toList()) {
