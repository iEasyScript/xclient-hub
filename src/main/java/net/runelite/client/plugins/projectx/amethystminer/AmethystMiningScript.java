package net.runelite.client.plugins.projectx.amethystminer;
...
import net.runelite.client.plugins.projectx.amethystminer.enums.AmethystCraftingOption;
import net.runelite.client.plugins.projectx.amethystminer.enums.MiningSpot;
import net.runelite.client.plugins.projectx.amethystminer.enums.Status;
...
public class AmethystMiningScript extends Script {
    ...
    private void executeTask() {
        try {
            if (!super.run() || !ProjectX.isLoggedIn()) {
            ...
            if (pickAxeInInventory.isEmpty() && config.pickAxeInInventory()) {
                ProjectX.showMessage("Pickaxe was not found in your inventory");
                sleep(5000);
                return;
            }
            ...
            if (Rs2Player.isAnimating() || ProjectX.getClient().getLocalPlayer().isInteracting()) return;
            ...
            ProjectX.log("Error in AmethystMiningScript: " + e.getMessage());
        }
    }
    ...
    private void chiselAmethysts() {
        ...
        if (ProjectX.getClient().getRealSkillLevel(Skill.CRAFTING) >= requiredLevel ) {
            ...
            ProjectX.showMessage("You do not have the required crafting level to make " + craftingOption.getDisplayName());
            status = Status.BANKING;
        }
    }
    ...
    private int distanceToPlayer(WallObject wallObject) {
        ...
        return ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation().distanceTo2D(closestWalkableNeighbour));
    }
