package net.runelite.client.plugins.projectx.TzhaarVenatorBow;
...
public class TzhaarVenatorBowScript extends Script {
    ...
    private void handleFighting(TzHaarVenatorBowConfig config) {
        ...
        if (shouldRetreat(config)) {
            ProjectX.log("Retreating due to low health or no food/prayer potions");
            BOT_STATUS = State.TRAVELLING;
            TRAVEL_STATUS = TravelStatus.TO_BANK;
        }
    }
    ...
    private boolean EnsureMagerSafety() {
        ...
        if (!invalidNpcs.isEmpty()) {
            ProjectX.log("Under attack from mager, focusing it");
    ...
    private void handleBanking(TzHaarVenatorBowConfig config) {
        ...
        if (Rs2Bank.openBank()) {
            ...
            if (Rs2Inventory.count("Prayer potion(4)") == 0) {
                ProjectX.log("Out of prayer potions, stopping script.");
                shutdown();
            }
            BOT_STATUS = State.TRAVELLING;
            TRAVEL_STATUS = TravelStatus.TO_TZHAAR;
        } else {
            ProjectX.log("Failed to open bank, trying to get back to bank.");
            BOT_STATUS = State.TRAVELLING;
            TRAVEL_STATUS = TravelStatus.TO_BANK;
        }
    }
