...
public class AutoFiremakingPlusScript extends Script {
    ...
    public boolean run(AutoFiremakingPlusConfig config) {
        ...
        startSkillXp = ProjectX.getClientThread().runOnClientThreadOptional(() ->
                ProjectX.getClient().getSkillExperience(Skill.FIREMAKING)).orElse(0);
        startSkillLevel = ProjectX.getClientThread().runOnClientThreadOptional(() ->
                ProjectX.getClient().getRealSkillLevel(Skill.FIREMAKING)).orElse(1);
        actionsCompleted = 0;
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (ProjectX.pauseAllScripts.get()) {
                    ProjectX.status = "[PAUSED]";
                    return;
                }

                if (config.stopAfterMinutes() > 0 && !shutdownAfterCleanup
                        && (System.currentTimeMillis() - startTimeMillis) / 60000 >= config.stopAfterMinutes()) {
                    ProjectX.log("AutoFiremakingPlus: reached stopAfterMinutes. Banking then shutting down.");
                    shutdownAfterCleanup = true;
                    state = State.BANKING;
                }
                if (config.stopAfterXp() > 0 && !shutdownAfterCleanup) {
                    int currentXp = ProjectX.getClientThread().runOnClientThreadOptional(() ->
                            ProjectX.getClient().getSkillExperience(Skill.FIREMAKING)).orElse(startSkillXp);
                    if (currentXp - startSkillXp >= config.stopAfterXp()) {
                        ProjectX.log("AutoFiremakingPlus: reached stopAfterXp. Banking then shutting down.");
                        shutdownAfterCleanup = true;
                        state = State.BANKING;
                    }
                }
                if (config.targetLevel() > 0 && !shutdownAfterCleanup) {
                    int currentLevel = ProjectX.getClientThread().runOnClientThreadOptional(() ->
                            ProjectX.getClient().getRealSkillLevel(Skill.FIREMAKING)).orElse(startSkillLevel);
                    if (currentLevel >= config.targetLevel()) {
                        ProjectX.log("AutoFiremakingPlus: reached targetLevel (" + currentLevel
                ...
                if (activeLog == null || !activeLog.hasRequiredLevel()) {
                    ProjectX.status = "Firemaking level too low for "
                ...
                ProjectX.logStackTrace("AutoFiremakingPlusScript", ex);
            }
    ...
    private void runCampfire(AutoFiremakingPlusConfig config) {
        ...
        if (Rs2Player.isMoving()) {
            ProjectX.status = "Returning to campfire";
            return;
        }
        ...
        if (target == null) {
            ...
            if (!Rs2Inventory.hasItem(TINDERBOX_NAME)) {
                ProjectX.status = "No tinderbox - banking for one";
                lastLogCount = -1;
                handleBanking(config);
                return;
            }
            ProjectX.status = "Lighting own fire";
            ...
            if (Rs2Player.waitForXpDrop(Skill.FIREMAKING, 5000)) {
                ...
                ProjectX.status = "Can't light here - repositioning";
        ...
        if (lastLogCount >= 0 && count < lastLogCount) {
            ...
            ProjectX.status = "Burning logs (" + count + " left)";
            return;
        }
        ...
        ProjectX.status = "Adding logs to campfire";
        Rs2Inventory.use(activeLog.getItemId());
        if (!sleepUntil(Rs2Inventory::isItemSelected, 2000)) {
            ProjectX.log("[Firemaking] log did not select; retrying next tick");
            return;
        }
        ...
        if (sleepUntil(() -> Rs2Widget.getWidget(BURN_INTERFACE_WIDGET) != null, 5000)) {
            ...
            ProjectX.log("[Firemaking] burn dialog did NOT open; retrying next tick");
        }
    }
    ...
    private Rs2TileObjectModel findCampfire(WorldPoint anchor, int radius) {
        return ProjectX.getClientThread().runOnClientThreadOptional(() -> {
            Rs2TileObjectModel t = ProjectX.getRs2TileObjectCache().query()
                    .withNameContains("ampfire")
                    .nearest(anchor, radius);
            if (t == null) {
                t = ProjectX.getRs2TileObjectCache().query()
    ...
    private void lineScan(AutoFiremakingPlusConfig config) {
        ProjectX.status = "Scanning for open space";
        ...
        if (line == null) {
            emptyScans++;
            if (emptyScans >= 5) {
                ProjectX.status = "No open line found - move me to open ground";
                emptyScans = 0;
                blockedLineY = Integer.MIN_VALUE; // reset so banking->return can retry fresh
                state = State.BANKING;
            } else {
                ProjectX.status = "No open space found - retrying";
            }
            return;
        }
        emptyScans = 0;
        currentLine = line;
        ProjectX.status = "Found line: " + line.getLength() + " tiles";
    ...
    private void lineWalkTo() {
        ...
        ProjectX.status = "Walking to line";
    ...
    private void lineBurn() {
        ...
        if (Rs2Player.isMoving()) {
            ProjectX.status = "Walking after lighting";
            return;
        }
        if (Rs2Player.isAnimating()) {
            ProjectX.status = "Lighting animation";
            return;
        }
        if (!Rs2Inventory.hasItem(TINDERBOX_NAME)) {
            state = State.BANKING;
        ...
        if (TileScanner.hasFire(playerPos)) {
            ...
            if (!Rs2Tile.isWalkable(westTile)) {
                ...
                ProjectX.status = "Blocked west - rescanning";
                state = State.SCANNING;
                return;
            }
        ...
        if (!Rs2Tile.isWalkable(playerPos)) {
            ...
            ProjectX.status = "Blocked tile - rescanning";
            state = State.SCANNING;
            return;
        }

        ProjectX.status = "Lighting " + activeLog.getLogName();
    ...
    private void handleBanking(AutoFiremakingPlusConfig config) {
        ...
        if (shutdownAfterCleanup) {
            Rs2Bank.closeBank();
            ProjectX.log("AutoFiremakingPlus: target reached, banked, shutting down.");
            super.shutdown();
            return;
        }
        ...
        if (needTinderbox && !Rs2Inventory.hasItem(TINDERBOX_NAME)) {
            if (!Rs2Bank.hasItem(TINDERBOX_ID)) {
                ProjectX.showMessage("No tinderbox in the bank!");
                super.shutdown();
                return;
            }
        ...
        if (!Rs2Bank.hasItem(activeLog.getItemId())) {
            ProjectX.showMessage("No " + activeLog.getLogName() + " in the bank!");
            super.shutdown();
            return;
        }
        ProjectX.status = "Withdrawing logs";
