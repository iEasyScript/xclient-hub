package net.runelite.client.plugins.projectx.valetotems.handlers;

import net.runelite.client.plugins.projectx.ProjectX;
import net.runelite.client.plugins.projectx.valetotems.ValeTotemConfig;
import net.runelite.client.plugins.projectx.valetotems.enums.BankLocation;
import net.runelite.client.plugins.projectx.valetotems.enums.GameObjectId;
import net.runelite.client.plugins.projectx.valetotems.enums.GameState;
import net.runelite.client.plugins.projectx.valetotems.utils.CoordinateUtils;
import net.runelite.client.plugins.projectx.valetotems.utils.GameObjectUtils;
import net.runelite.client.plugins.projectx.valetotems.utils.InventoryUtils;
...
import net.runelite.client.plugins.projectx.valetotems.enums.TotemLocation;
import net.runelite.client.plugins.projectx.valetotems.models.GameSession;
...
public class BankingHandler {
    ...
    public static boolean navigateToBank() {
        try {
            ...
            ProjectX.log("[Banking] navigateToBank: alreadyNear=" + alreadyNear + " pos=" + CoordinateUtils.getPlayerLocation());

            if (alreadyNear) {
                if (!CoordinateUtils.isAtBankingPosition()) {
                    ProjectX.log("[Banking] Walking to optimal banking tile");
                    return Rs2Walker.walkTo(BankLocation.PLAYER_STANDING_TILE.getLocation());
                }
                return true;
            }

            ProjectX.log("[Banking] Walking to bank booth at " + BankLocation.BANK_BOOTH.getLocation());
            boolean walked = Rs2Walker.walkTo(BankLocation.BANK_BOOTH.getLocation());
            ProjectX.log("[Banking] walkTo bank result=" + walked);
            if (walked) {
            ...
            ProjectX.log("[Banking] Error navigating to bank: " + e.getMessage());
            return false;
        }
    }
    ...
    public static boolean openBank() {
        try {
            if (Rs2Bank.isOpen()) {
                ProjectX.log("[Banking] Bank already open");
                return true;
            }

            if (!CoordinateUtils.isNearBank(BANK_INTERACTION_DISTANCE)) {
                ProjectX.log("[Banking] openBank: not near bank, navigating first");
                if (!navigateToBank()) {
                    return false;
                }
            }

            ProjectX.log("[Banking] Interacting with bank booth (id=" + GameObjectId.BANK_BOOTH.getId() + " at " + BankLocation.BANK_BOOTH.getLocation() + ")");
            ...
            ProjectX.log("[Banking] Bank booth interact result=" + interacted);
            ...
            if (interacted) {
                ...
                ProjectX.log("[Banking] Bank opened=" + opened);
                return opened;
            }

            return false;
        } catch (Exception e) {
            ProjectX.log("[Banking] Error opening bank: " + e.getMessage());
            return false;
        }
    }
    ...
    public static boolean depositAllExceptKnife() {
        try {
            ...
            ProjectX.log("Error depositing items: " + e.getMessage());
            return false;
        }
    }
    ...
    public static boolean withdrawRequiredItems(GameSession gameSession) {
        try {
            ...
            if (!Rs2Bank.isOpen() && !openBank()) {
                ProjectX.log("Cannot withdraw items - failed to open bank");
                return false;
            }
            ...
            if (logsToWithdraw <= 0) {
                ProjectX.log("Already have sufficient logs in inventory - no withdrawal needed");
            ...
            ProjectX.log("Withdrawing " + logsToWithdraw + " " + logTypeName + " from bank");
            ...
            if (!Rs2Bank.withdrawX(logId, logsToWithdraw)) {
                ProjectX.log("Failed to queue log withdrawal");
                return false;
            }
            if (!sleepUntil(() -> InventoryUtils.getLogCount() >= expectedLogs, 3000)) {
                ProjectX.log("Log withdrawal timed out (have " + InventoryUtils.getLogCount() + "/" + expectedLogs + ")");
                return false;
            }
            ProjectX.log("Successfully withdrew " + logsToWithdraw + " " + logTypeName);
            return InventoryUtils.hasRequiredItems();

        } catch (Exception e) {
            ProjectX.log("Error withdrawing required items: " + e.getMessage());
            return false;
        }
    }
    ...
    public static boolean fixZoom() {
        try {
            ...
            ProjectX.log("Error fixing zoom: " + e.getMessage());
            return false;
        }
    }
    ...
    public static TotemLocation.RouteType detectRouteTypeWithBankOpen() {
        try {
            ...
            if (InventoryUtils.hasLogBasket()) {
                ProjectX.log("Log basket detected in inventory - enabling Extended Route (8 totems)");
                return TotemLocation.RouteType.EXTENDED;
            }

            // Ensure bank is open before checking
            if (!Rs2Bank.isOpen() && !openBank()) {
                ProjectX.log("Could not open bank for route detection - defaulting to Standard Route");
                return TotemLocation.RouteType.STANDARD;
            }

            // Check bank for log basket
            if (Rs2Bank.hasItem(InventoryUtils.LOG_BASKET_ID)) {
                ProjectX.log("Log basket detected in bank - enabling Extended Route (8 totems)");
                return TotemLocation.RouteType.EXTENDED;
            }

            ProjectX.log("No log basket detected in inventory or bank - using Standard Route (5 totems)");
            return TotemLocation.RouteType.STANDARD;

        } catch (Exception e) {
            ProjectX.log("Error detecting route type: " + e.getMessage());
            ProjectX.log("Defaulting to Standard Route due to error");
            return TotemLocation.RouteType.STANDARD;
        }
    }
    ...
    public static boolean performUnifiedBankingCycle(GameSession gameSession) {
        try {
            ...
            ProjectX.log("[Banking] Start cycle. Player=" + playerPos + " distToBank=" + distToBank + " nearBank=" + nearBank);

            if (!nearBank) {
                ProjectX.log("[Banking] Not near bank, navigating...");
                boolean navResult = navigateToBank();
                ProjectX.log("[Banking] Navigate result=" + navResult + " newPos=" + CoordinateUtils.getPlayerLocation());
                if (!navResult) {
                    return false;
                }
            }

            ProjectX.log("[Banking] Opening bank...");
            if (!openBank()) {
                ProjectX.log("[Banking] Failed to open bank");
                return false;
            }
            ProjectX.log("[Banking] Bank opened successfully");
            ...
            if (detectedRouteType == TotemLocation.RouteType.EXTENDED) {
                ProjectX.log("Performing extended route banking cycle");
                bankingSuccess = performExtendedRouteBankingOperations(gameSession);
            } else {
                ProjectX.log("Performing standard route banking cycle");
            ...
            if (bankingSuccess) {
                ...
                Rs2Bank.closeBank();
                fixZoom();
                ProjectX.log("Banking cycle completed successfully using " + detectedRouteType.getDescription());
            }

            return bankingSuccess;

        } catch (Exception e) {
            ProjectX.log("Error during unified banking cycle: " + e.getMessage());
            return false;
        }
    }
    ...
    private static boolean performStandardBankingOperations(GameSession gameSession) {
        try {
            ...
            if (!ensureKnifeInInventory(gameSession)) {
                ProjectX.log("Failed to ensure knife availability for standard route");
                return false;
            }

            // Step 2: Deposit all except knife (and stamina potion if we have one)
            if (!depositAllExceptKnife()) {
                ProjectX.log("Failed to deposit items for standard route");
                return false;
            }

            // Step 3: Check if we have enough materials
            if (!hasSufficientMaterials(1)) {
                ProjectX.log("Not enough materials for standard route - stopping bot");
            ...
            if (!withdrawRequiredItems(gameSession)) {
                ProjectX.log("Failed to withdraw required items for standard route");
                return false;
            }

            // Step 5: Handle stamina potion withdrawal if enabled
            if (!handleStaminaPotionWithdrawal()) {
                ProjectX.log("Warning: Failed to handle stamina potions - continuing without");
            }

            return true;
        } catch (Exception e) {
            ProjectX.log("Error during standard banking operations: " + e.getMessage());
            return false;
        }
    }
    ...
    private static boolean performExtendedRouteBankingOperations(GameSession gameSession) {
        try {
            ...
            if (!hasSufficientMaterialsForExtendedRoute(gameSession, 1)) {
                ProjectX.log("Not enough materials for extended route - stopping bot");
                gameSession.setState(GameState.STOPPING);
                return false;
            }
            ...
            if (!depositAllExceptKnifeAndLogBasket()) {
                ProjectX.log("Failed to deposit items while preserving knife and log basket");
                return false;
            }

            // Step 5: Perform log basket filling operation
            if (!performLogBasketFillingOperation(gameSession)) {
                ProjectX.log("Failed to perform log basket filling operation");
                return false;
            }
            ...
            ProjectX.log("Extended route banking operations completed successfully");
            return true;

        } catch (Exception e) {
            ProjectX.log("Error during extended route banking operations: " + e.getMessage());
            return false;
        }
    }
    ...
    private static void handleRandomAFK() {
        ...
        if (roll < LONG_AFK_PROBABILITY) {
            ...
            ProjectX.log("Performing a long AFK...");
            ProjectX.status = "Long AFK";
            Rs2Antiban.moveMouseOffScreen(90);
            sleep(30000, 90000);
        } else if (roll < MEDIUM_AFK_PROBABILITY) {
            // Medium AFK: 10 to 30 seconds
            ProjectX.log("Performing a medium AFK...");
            ProjectX.status = "Medium AFK";
            Rs2Antiban.moveMouseOffScreen(90);
            sleep(10000, 30000);
        } else if (roll < SHORT_AFK_PROBABILITY) {
            // Short AFK: 2 to 8 seconds
            ProjectX.log("Performing a short AFK...");
            ProjectX.status = "Short AFK";
    ...
    public static boolean hasSufficientMaterials(int roundsPlanned) {
        try {
            if (!Rs2Bank.isOpen() && !openBank()) {
                ProjectX.log("Unable to check materials - cannot open bank");
                return false;
            }
            ...
            if (hasSufficientMaterials) {
                ProjectX.log("Material check passed - sufficient materials for " + roundsPlanned + " rounds");
            } else {
                ProjectX.log("=== MATERIAL SHORTAGE DETECTED ===");
                ProjectX.log("Standard Route Requirements:");
                ProjectX.log(shortageDetails.toString().trim());
                ProjectX.log("==================================");
                ProjectX.showMessage("Material Shortage: " + shortageDetails.toString().trim());
            }

            return hasSufficientMaterials;

        } catch (Exception e) {
            ProjectX.log("Error checking bank materials: " + e.getMessage());
            return false;
        }
    }
    ...
    private static void handleCriticalMaterialShortage(GameSession gameSession, String missingItem) {
        ...
        ProjectX.log("=== CRITICAL MATERIAL SHORTAGE DETECTED ===");
        ProjectX.log("Issue: " + missingItem);
        ProjectX.log("Bot Status: Stopping due to insufficient materials");
        ProjectX.log("Action Required: Please restock materials in bank before restarting");
        ProjectX.log("=============================================");

        ProjectX.showMessage("Critical Material Shortage: " + missingItem);
     ...
     */
    public static boolean hasSufficientMaterialsForExtendedRoute(net.runelite.client.plugins.projectx.valetotems.models.GameSession gameSession, int roundsPlanned) {
        try {
            if (!Rs2Bank.isOpen() && !openBank()) {
                ProjectX.log("Unable to check materials for extended route - cannot open bank");
                return false;
            }
            ...
            // Log material status
            if (hasSufficientMaterials) {
                ProjectX.log("Extended route material check passed - sufficient materials");
            } else {
                ProjectX.log("=== MATERIAL SHORTAGE DETECTED (EXTENDED ROUTE) ===");
                ProjectX.log("Extended Route Requirements (8 totems per round):");
                ProjectX.log(shortageDetails.toString().trim());
                ProjectX.log("==================================================");
                ProjectX.showMessage("Material Shortage: " + shortageDetails.toString().trim());
            }
            ...
            ProjectX.log("Error checking bank materials for extended route: " + e.getMessage());
            return false;
        }
    }
    ...
    public static boolean ensureLogBasketAvailable() {
        try {
            ...
            if (Rs2Bank.hasItem(InventoryUtils.LOG_BASKET_ID)) {
                ProjectX.log("Withdrawing log basket from bank for extended route");
                if (!Rs2Bank.withdrawOne(InventoryUtils.LOG_BASKET_ID)) {
                    ProjectX.log("Failed to queue log basket withdrawal");
                    return false;
                }
                return sleepUntil(InventoryUtils::hasLogBasket, 3000);
            }

            ProjectX.log("No log basket found in bank");
            return false;
            
        } catch (Exception e) {
            ProjectX.log("Error ensuring log basket availability: " + e.getMessage());
            return false;
        }
    }
    ...
    public static boolean depositAllExceptKnifeAndLogBasket() {
        try {
            ...
            ProjectX.log("Error depositing items (preserving knife and log basket): " + e.getMessage());
            return false;
        }
    }
    ...
    private static boolean ensureKnifeInInventory(GameSession gameSession) {
        try {
            if (!Rs2Bank.isOpen() && !openBank()) {
                ProjectX.log("Cannot ensure knife availability - failed to open bank");
                return false;
            }
            ...
            if (Rs2Bank.hasItem(InventoryUtils.FLETCHING_KNIFE_ID)) {
                ProjectX.log("Withdrawing Fletching knife from bank (prioritized)");
                if (Rs2Bank.withdrawOne(InventoryUtils.FLETCHING_KNIFE_ID)) {
                    if (sleepUntil(InventoryUtils::hasKnife, 3000)) {
                        ProjectX.log("Successfully withdrew Fletching knife");
                        return true;
                    }
                    ProjectX.log("Fletching knife withdrawal timed out");
                    return false;
                }
                ProjectX.log("Failed to queue Fletching knife withdrawal");
            }

            // Try to withdraw regular knife as fallback
            if (Rs2Bank.hasItem(InventoryUtils.KNIFE_ID)) {
                ProjectX.log("Withdrawing regular knife from bank");
                if (Rs2Bank.withdrawOne(InventoryUtils.KNIFE_ID)) {
                    if (sleepUntil(InventoryUtils::hasKnife, 3000)) {
                        ProjectX.log("Successfully withdrew regular knife");
                        return true;
                    }
                    ProjectX.log("Regular knife withdrawal timed out");
                    return false;
                }
                ProjectX.log("Failed to queue regular knife withdrawal");
            }
            ...
            ProjectX.log("Error ensuring knife in inventory: " + e.getMessage());
            return false;
        }
    }
    ...
    private static boolean ensureKnifeAndLogBasketInInventory(GameSession gameSession) {
        try {
            if (!Rs2Bank.isOpen() && !openBank()) {
                ProjectX.log("Cannot ensure knife and log basket availability - failed to open bank");
                return false;
            }
            ...
            if (!InventoryUtils.hasKnife()) {
                if (Rs2Bank.hasItem(InventoryUtils.FLETCHING_KNIFE_ID)) {
                    ...
                    ProjectX.log("Withdrawing regular knife from bank");
                    if (!Rs2Bank.withdrawOne(InventoryUtils.KNIFE_ID)) {
                        ProjectX.log("Failed to queue knife withdrawal");
                        return false;
                    }
                } else {
                    handleCriticalMaterialShortage(gameSession, "No knife available (checked inventory and bank for both Fletching knife and regular knife)");
                ...
                if (!sleepUntil(InventoryUtils::hasKnife, 3000)) {
                    ProjectX.log("Knife withdrawal timed out");
                    return false;
                }
            }
            ...
            if (!InventoryUtils.hasLogBasket()) {
                ...
                if (!sleepUntil(InventoryUtils::hasLogBasket, 3000)) {
                    ProjectX.log("Log basket withdrawal timed out");
                    return false;
                }
            }
            ...
            ProjectX.log("Error ensuring knife and log basket in inventory: " + e.getMessage());
            return false;
        }
    }
    ...
    private static boolean handleStaminaPotionWithdrawal() {
        ...
        if (potionToWithdraw == null) {
            ProjectX.log("No stamina or energy potions available in bank");
            return true;
        }

        if (needsDrink) {
            ProjectX.log("Run energy low - withdrawing and drinking potion");
            ...
            ProjectX.log("Withdrawing potion for navigation");
            Rs2Bank.withdrawOne(potionToWithdraw);
            Rs2Inventory.waitForInventoryChanges(1800);
        }
     ...
     */
    private static boolean performLogBasketFillingOperation(net.runelite.client.plugins.projectx.valetotems.models.GameSession gameSession) {
        try {
            ProjectX.log("Starting log basket filling operation");
            ...
            if (!Rs2Bank.withdrawX(logId, logsToWithdraw)) {
                ProjectX.log("Failed to queue log withdrawal for basket-fill step");
                return false;
            }

            int expectedAfterFirst = logsBeforeFirst + logsToWithdraw;
            if (!sleepUntil(() -> InventoryUtils.getLogCount() >= expectedAfterFirst, 3000)) {
                ProjectX.log("Log withdrawal timed out in basket-fill step");
                return false;
            }
            ...
            if (!InventoryUtils.fillLogBasket(gameSession)) {
                ProjectX.log("Failed to fill log basket");
                return false;
            }

            // Step 4: Open bank again
            if (!openBank()) {
                ProjectX.log("Failed to reopen bank after filling basket");
                return false;
            }
            ...
            if (logsStillNeeded > 0) {
                ProjectX.log("Withdrawing additional " + logsStillNeeded + " logs for extended route");
                int logsBeforeSecond = InventoryUtils.getLogCount();
                if (!Rs2Bank.withdrawX(logId, logsStillNeeded)) {
                    ProjectX.log("Failed to queue additional log withdrawal");
                    return false;
                }
                ...
                if (!sleepUntil(() -> InventoryUtils.getLogCount() >= expectedAfterSecond, 3000)) {
                    ProjectX.log("Additional log withdrawal timed out");
                    return false;
                }
            }

            ProjectX.log("Log basket filling operation completed successfully");
            return true;

        } catch (Exception e) {
            ProjectX.log("Error during log basket filling operation: " + e.getMessage());
            return false;
        }
    }
}