...
public class EmoteClueTask extends ClueTask {
    ...
    private void interactWithUri() {
        ProjectX.getClientThread().invoke(() -> ProjectX.getRs2NpcCache().query().withName("Uri").interact("Talk-to"));
    ...
    public void onNpcSpawned(NpcSpawned event) {
        ...
        if (npc.getName() != null) {
            if (state == State.WAITING_FOR_ENEMY_SPAWN && npc.getName().equalsIgnoreCase(DOUBLE_AGENT_NAME)) {
                doubleAgent = ProjectX.getRs2NpcCache().query().where(n -> n.getIndex() == npc.getIndex()).nearest();
