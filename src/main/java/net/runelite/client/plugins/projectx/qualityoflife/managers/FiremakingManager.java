...
import net.runelite.client.plugins.projectx.qualityoflife.enums.FiremakingLogs;
...
public class FiremakingManager {
    ...
    private void lightLogsOnClick(MenuEntry event) {
        ...
        if (logToLight.isEmpty()) {
            ProjectX.log("<col=5F1515>No logs available or level too low to light logs</col>");
            return;
        }

        Rs2ItemModel logItem = logToLight.get(0);
        if (logItem == null) {
            ProjectX.log("<col=5F1515>Could not find the selected log in inventory</col>");
            return;
        }

        ProjectX.log("<col=245C2D>Lighting: " + logItem.getName() + "</col>");
