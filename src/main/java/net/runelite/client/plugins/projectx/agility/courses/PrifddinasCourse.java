...
public class PrifddinasCourse implements AgilityCourseHandler
{
	...
	{
		var portalModel = ProjectX.getRs2TileObjectCache().query().withIds(PORTAL_OBSTACLE_IDS.stream().mapToInt(Integer::intValue).toArray()).within(10).nearest();
		if (portalModel != null && ProjectX.getClientThread().runOnClientThreadOptional(portalModel::getClickbox).isPresent())
		{
	...
	public boolean handleHealthMonitoring()
	{
		...
		if (currentHealth < lastKnownHealth)
		{
			...
			ProjectX.log("Fell from agility course, walking back to start");
