package net.runelite.client.plugins.projectx.combathotkeys;
    ...
    public void onMenuEntryAdded(MenuEntryAdded event)
    {
        if (event.getOption().equals("Walk here") && config.yesDance())
        {
            ProjectX.getClient().getMenu().createMenuEntry(-1)
                    ...
                    .onClick(e -> {
                        final var target = ProjectX.getClient().getTopLevelWorldView().getSelectedSceneTile();
                        if (target != null)
                        {
                            final var location = target.getWorldLocation();
                            ProjectX.getConfigManager().setConfiguration(
                                    "combathotkeys",
                                    "tile2",
                                    location
                            );
