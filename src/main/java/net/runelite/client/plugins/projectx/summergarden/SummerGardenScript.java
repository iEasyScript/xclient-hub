package net.runelite.client.plugins.projectx.summergarden;
...
import  net.runelite.client.plugins.projectx.summergarden.ElementalCollisionDetector;
import  net.runelite.client.plugins.projectx.summergarden.SummerGardenConfig;
...
public class SummerGardenScript extends Script {
    ...
    public boolean run( net.runelite.client.plugins.projectx.summergarden.SummerGardenConfig config) {
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            if (!super.run()) return;
            try {
                if (ProjectX.getClient().getGameState() != GameState.LOGGED_IN) {
                    return;
                }
                ...
                if (config.autoHandInAndReset()) {
                    ...
                    if (botState == BotState.RUN) {
                        ...
                        if (config.sendInvFullNotification() && !sendRs2InventoryFullNotification) {
                            ...
                            //ProjectX.getClient().playSoundEffect(OUT_OF_SUPPLY_SOUND, SoundEffectVolume.HIGH);
                            ProjectX.log("You need a beer glass and a pestle and mortar to make sq'irk juice.");
    ...
    private boolean isInAlKharid() {
        return ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation().getRegionID()) == REGION_ALKHARID;
    }

    private boolean isInGarden() {
        return ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation().getRegionID()) == REGION_GARDEN;
    }

    private boolean isInHouseArea() {
        WorldPoint loc = ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation());
    ...
    private WallObject getHouseDoor() {
        ...
        LocalPoint doorLp = LocalPoint.fromWorld(ProjectX.getClient(), 3321, 3142);
        if (doorLp == null) {
            return null;
        }

        Scene scene = ProjectX.getClient().getScene();
    ...
    private boolean goInsideHouse() {
        ...
        if (!isInAlKharid() || ProjectX.getClient().getTopLevelWorldView().getPlane() != 0) {
            return false;
        }
        ...
        if (!isInHouseArea()) {
            ...
            if (getHouseDoor() != null) {
                ProjectX.getRs2TileObjectCache().query().interact(OBJECT_HOUSE_DOOR_CLOSED);
    ...
    private void exitGarden() {
        ...
        if (ProjectX.getRs2TileObjectCache().query().withId(12941).nearest() == null) {
            return;
        }

        // If the player is still in the garden then click the fountain to exit and wait until the player is teleported out.
        if (isInGarden()) {
            ProjectX.getRs2TileObjectCache().query().interact(12941);
    ...
    private void doMaze(SummerGardenConfig config) {
        ...
        // Click tree
        if (WORLD_POINT_MAZE_STARTING_LOCATION.equals(ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation()))) {
            if (config.waitForOneClick() || ElementalCollisionDetector.getTicksUntilStart() == 0) {
                ProjectX.getRs2TileObjectCache().query().interact(OBJECT_SUMMER_TREE);
                sleepUntil(() -> Rs2Player.isMoving());
                sleepUntil(() -> !Rs2Player.isMoving(), 30000);
                sleepUntilOnClientThread(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation().getY() < 5481);
        ...
        if (ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation().getY()) >= 5481) {
            return;
        }

        // Click Gate
        Rs2TileObjectModel gate = ProjectX.getRs2TileObjectCache().query().withId(ObjectID.GATE_11987).nearest();
        if (gate != null) {
            gate.click();
            sleepUntil(Rs2Player::isMoving);
            sleepUntil(() -> !Rs2Player.isMoving());
            sleepUntilOnClientThread(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation().equals(WORLD_POINT_MAZE_STARTING_LOCATION));
        }
    }
    ...
    private void completeAndReset() {
        ...
        var npcOsman = ProjectX.getRs2NpcCache().query().withName(NPC_NAME_OSMAN).nearestOnClientThread();
        if (npcOsman == null) {
            var osmanLocalLocation = LocalPoint.fromWorld(ProjectX.getClient(), osmanLocation);
        ...
        if (lastInteractedActor == null || !Objects.equals(lastInteractedActor.getName(), NPC_NAME_OSMAN)) {
            ProjectX.getClientThread().invoke(() -> ProjectX.getRs2NpcCache().query().withName(NPC_NAME_OSMAN).interact("Talk-to"));
        ...
        if (Objects.equals(lastInteractedActor.getName(), NPC_NAME_OSMAN)) {
            var npcDialogueText = ProjectX.getClient().getWidget(WidgetInfo.DIALOG_NPC_TEXT);
            ...
            //sleepUntil(() -> ProjectX.getClient().getWidget(WidgetInfo.DIALOG_OPTION_OPTIONS) != null, 2000);
            var playerDialogueOptionsWidget = ProjectX.getClient().getWidget(WidgetInfo.DIALOG_OPTION_OPTIONS);
            ...
            //sleepUntil(() -> ProjectX.getClient().getWidget(WidgetInfo.DIALOG_PLAYER_TEXT) != null, 2000);
            var playerDialogueText = ProjectX.getClient().getWidget(WidgetInfo.DIALOG_PLAYER_TEXT);
            ...
            //sleepUntil(() -> ProjectX.getClient().getWidget(WidgetInfo.DIALOG_SPRITE_TEXT) != null, 2000);
            var npcDialogueSpriteText = ProjectX.getClient().getWidget(WidgetInfo.DIALOG_SPRITE_TEXT);
    ...
    private void handleReturnToHouse() {
        ...
        if (!isInHouseArea()) {
            var outsideHouseDoorLocalLocation = LocalPoint.fromWorld(ProjectX.getClient(), WORLD_POINT_OUTSIDE_HOUSE_DOOR);
            if (outsideHouseDoorLocalLocation != null && ProjectX.getClient().getLocalPlayer().getLocalLocation().distanceTo(outsideHouseDoorLocalLocation) >= MAX_DISTANCE) {
        ...
        while (Rs2Inventory.count("Beer glass") < 25) {
            ProjectX.getRs2TileObjectCache().query().interact(OBJECT_BEER_GLASS_SHELF);
            sleep(3000);
        }
        ...
        var npcApprentice = ProjectX.getRs2NpcCache().query().withName(NPC_NAME_APPRENTICE).nearestOnClientThread();
        ...
        if (lastInteractedActor == null || !Objects.equals(lastInteractedActor.getName(), NPC_NAME_APPRENTICE)) {
            ProjectX.getClientThread().invoke(() -> ProjectX.getRs2NpcCache().query().withName(NPC_NAME_APPRENTICE).interact("Teleport"));
    ...
    private void makeLastJuice() {
        ...
        while (Rs2Inventory.count("Summer sq'irkjuice") != 26) {
            ProjectX.getRs2TileItemCache().query().withName("Summer sq'irkjuice").interact("Take");
