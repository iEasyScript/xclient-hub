...
import net.runelite.client.plugins.projectx.blastoisefurnace.enums.Bars;
import net.runelite.client.plugins.projectx.blastoisefurnace.enums.State;
...
public class BlastoiseFurnaceScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (!Rs2GameObject.exists(BLAST_FURNACE_DISPENSER)) {
                    ...
                    ProjectX.getRs2TileObjectCache().query().interact(DWARF_KELDAGRIM_FACTORY_STAIRS);
                    return;
                }

                if (!init) {
                    int inCoffer = ProjectX.getVarbitValue(BLAST_FURNACE_COFFER);
                    ...
                    case BANKING:
                        ...
                        if (config.getBars().isRequiresCoalBag() && !Rs2Inventory.contains(coalBag)) {
                            if (!Rs2Bank.hasItem(coalBag)) {
                                ProjectX.showMessage("No coal bag found in inventory and bank.");
                        ...
                        if (config.getBars().isRequiresGoldsmithGloves()) {
                            ...
                            if (!hasGauntlets) {
                                if (!Rs2Bank.hasItem(GAUNTLETS_OF_GOLDSMITHING)) {
                                    ProjectX.showMessage("No goldsmith gauntlets found.");
                                    ProjectX.stopPlugin(plugin);
                                    return;
                                }
                        ...
                        if (ProjectX.getClient().getEnergy() < 8100) {
                            useStaminaPotions();
                        }
    ...
    private void handleTax() {
        ...
        var blastie = ProjectX.getRs2NpcCache().query().withName("Blast Furnace Foreman").nearestOnClientThread();
    ...
    private void handleDispenserLooting() {
        if (!Rs2Inventory.isFull()) {
            ...
            if (!Rs2Equipment.isWearing(ICE_GLOVES) && !Rs2Equipment.isWearing(SMITHING_UNIFORM_GLOVES_ICE)) {
                ...
                if (!equipped) {
                    ProjectX.showMessage("Ice gloves or smith gloves required to loot the hot bars.");
                    Rs2Player.logout();
                    ProjectX.stopPlugin(plugin);
                    return;
                }
            ...
            ProjectX.getRs2TileObjectCache().query().interact(BLAST_FURNACE_DISPENSER, "Take");
    ...
    private void retrieveItemsForCurrentFurnaceInteraction() {
        ...
        final int coal = ProjectX.getVarbitValue(
    ...
    private boolean putOreOnConveyorBelt() {
        ...
        if (!ProjectX.getRs2TileObjectCache().query().interact(BLAST_FURNACE_CONVEYER_BELT_CLICKABLE, "Put-ore-on")) {
    ...
    public void setTenMinuteTimer(){
        ...
        TimerTask task = new TimerTask() {
            @Override
            public void run() {
                ProjectX.log("Time's up! 10 minutes have passed.");
                timeIsUp = true;
                timerStarted = false;
                timer.cancel();
            }
        ...
        ProjectX.log("Timer started for 10 minutes...");
    ...
    private int fallbackBar(int primary) {
        int value = ProjectX.getVarbitValue(primary);
        return value > 0 ? value : ProjectX.getVarbitValue(net.runelite.api.gameval.VarbitID.BLAST_FURNACE_GOLD_BARS);
    }

    private int getBars(int varbitId) {
        return ProjectX.getVarbitValue(varbitId);
    }
    ...
    public boolean dispenserContainsBars() {
        return Arrays.stream(new int[]{
                ...
                BLAST_FURNACE_ADAMANTITE_BARS,
                BLAST_FURNACE_RUNITE_BARS
        }).anyMatch(id -> ProjectX.getVarbitValue(id) > 0);
    }
    ...
    public void checkAndTopOffCoffer() {
        int lvl = ProjectX.getClient().getRealSkillLevel(Skill.SMITHING);
        if (lvl < 60) return;
        ...
        ProjectX.getRs2TileObjectCache().query().interact(BLAST_FURNACE_AUTOMATA_COFFER, "use");
    ...
    public boolean fullCoffer() {
        int coffer = ProjectX.getVarbitValue(BLAST_FURNACE_COINSINCOFFER);
        return coffer == 1;
    }
}
