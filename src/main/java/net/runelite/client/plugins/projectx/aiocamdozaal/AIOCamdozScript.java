package net.runelite.client.plugins.projectx.aiocamdozaal;
        ...
        switch (state) {
            case WALKING_TO_ROCKS:
                ...
                if (!Rs2Walker.isInArea(barroniteLocation, tileDistance)) {
                    ...
                    ProjectX.status = "Walking to Barronite rocks";
                } else {
                    state = State.MINING;
                }
                break;
                        ...
                        if (sleepUntil(() -> Rs2Player.getAnimation() != targetMiningAnimationID, sleepBetween)) {
                            ...
                            if (lastInteractedBarroniteID == 41547) {
                                ...
                                for (int miningObjectID : miningObjectIDsReverse) {
                                    mineableRocks.addAll(ProjectX.getRs2TileObjectCache().query().withId(41548).toList());
                                }
                                ...
                                for (int miningObjectID : miningObjectIDs) {
                                    mineableRocks.addAll(ProjectX.getRs2TileObjectCache().query().withId(41547).toList());
                                }
                            }
                            ...
                            if (!mineableRocks.isEmpty()) {
                                ...
                                ProjectX.status = "Mining Barronite rocks";
                            }
                        }
                    }
                }
            ...
            case WALKING_TO_ANVIL:
                ...
                if (!Rs2Walker.isInArea(barroniteCrusherLocation, tileDistance)) {
                    ...
                    ProjectX.status = "Walking to Barronite Crusher";
                } else {
                ...
                if (Rs2Player.getAnimation() != smithingAnimationID) {
                    if (Rs2Inventory.hasItem(ItemID.CAMDOZAAL_BARRONITE_DEPOSIT)) {
                        if (!sleepUntil(() -> Rs2Player.waitForXpDrop(Skill.SMITHING), 3000)) {
                            ProjectX.getRs2TileObjectCache().query().interact(crusherID, "Smith");
                            ProjectX.status = "Smithing Barronite deposits";
                        }
                    } else {
                        state = State.WALKING_TO_BANK;
                    }
        ...
        switch (state) {
            case WALKING_TO_FISH:
                ...
                if (!Rs2Walker.isInArea(fishingLocation, 5)) {
                    //ProjectX.log("Trying to walk to fishingLocation");
                    Rs2Walker.walkTo(fishingLocation);
                } else {
                    state = State.FISHING;
                }
                ProjectX.status = "Walking to fishing location";
                break;
            ...
            case FISHING:
                ...
                if (!Rs2Player.isInteracting() && !Rs2Player.isMoving()) {
                    if (netToUse == 303) {
                        ProjectX.getRs2NpcCache().query().withId(10686).interact("Small Net");
                    } else if (netToUse == 305) {
                        ProjectX.getRs2NpcCache().query().withId(10686).interact("Big Net");
            ...
            case PROCESSING:
                        ...
                        if (Rs2Player.getAnimation() != AnimationID.HUMAN_COOKING) {
                            if (Rs2Inventory.hasItem(ItemID.RAW_GUPPY) || Rs2Inventory.hasItem(ItemID.RAW_CAVEFISH)
                                ...
                                if (Rs2Widget.isProductionWidgetOpen()) {
                                    ...
                                    ProjectX.getRs2TileObjectCache().query().interact(41545, "Prepare-fish");
                        ...
                        if (Rs2Player.getAnimation() != AnimationID.HUMAN_BONE_SACRIFICE) {
                            if (Rs2Inventory.hasItem(ItemID.RUINED_GUPPY) || Rs2Inventory.hasItem(ItemID.RUINED_CAVEFISH)
                                ...
                                if (Rs2Widget.isProductionWidgetOpen()) {
                                    ...
                                    ProjectX.getRs2TileObjectCache().query().interact(41546, "Offer-fish");
                                }
                            } else {
                ...
                ProjectX.status = "Processing fish";
                break;
    ...
    private void FightGolem(AIOCamdozConfig _config) {
        ...
        if (!Rs2Player.eatAt(_config.EatFoodAtPercent())) {
            ...
            if (foods.isEmpty() && (state != State.WALKING_TO_BANK && state != State.BANKING) && healthPercent < 50) {
                state = State.WALKING_TO_BANK;
                ProjectX.log("[AIO Camdozaal]: Out of food & less than half HP! Walking to bank!]");
            } else {
    ...
    private void HandleGolemFightingState(WorldPoint _golemLocation, int _rubbleID, int _golemID, AIOCamdozConfig _config) {
        switch (state) {
            ...
            case PROCESSING:
                ...
                golemsAttackingPlayer = ProjectX.getRs2NpcCache().query().where(n -> n.isInteractingWithPlayer()).toList();
                ...
                if (!Rs2Combat.inCombat()) {
                    ...
                    golems = ProjectX.getRs2NpcCache().query().withId(_golemID).toList();
                    rubbles = ProjectX.getRs2NpcCache().query().withId(_rubbleID).toList();
                ...
                if (!Rs2Bank.isOpen()) {
                    ...
                    if (_config.GetFood()) {
                        ...
                        if (foodWithdrawn) {
                            ...
                            ProjectX.showMessage("Not enough food found in bank!");
                            shutdown();
                        }
                    } else {
                        Rs2Bank.closeBank();
