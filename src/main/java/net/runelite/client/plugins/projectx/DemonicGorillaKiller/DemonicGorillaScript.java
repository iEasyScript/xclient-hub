...
public class DemonicGorillaScript extends Script {
    ...
    private void preflightChecks(DemonicGorillaConfig config) {
        if (config.useMagicStyle() && config.magicGear() == null) {
            ProjectX.showMessage("You've selected magic combatstyle, but your magic inventory setup is null. Please set it in the config. If you already have one selected. Select another inventory setup and then select your magic setup again.");
            shutdown();
            return;
        }
        if (config.useRangeStyle() && config.rangeGear() == null) {
            ProjectX.showMessage("You've selected ranged combatstyle, but your range inventory setup is null. Please set it in the config. If you already have one selected. Select another inventory setup and then select your range setup again.");
            shutdown();
            return;
        }
        if (config.useMeleeStyle() && config.meleeGear() == null) {
            ProjectX.showMessage("You've selected melee combatstyle, but your melee inventory setup is null. Please set it in the config. If you already have one selected. Select another inventory setup and then select your melee setup again.");
            shutdown();
            return;
        }
        if (config.gearSetup() == null) {
            ProjectX.showMessage("Your banking gear setup is null. Please set it in the config. If you already have one selected. Select another inventory setup and then select your gear setup again.");
            shutdown();
            return;
        }
        ...
        Plugin qolPlugin = ProjectX.getActiveProjectXPlugins().stream().filter(p -> p.getClass().getSimpleName().contains("QoLPlugin"))
                .findFirst().orElse(null);

        if (qolPlugin != null) {
            ProjectX.stopPlugin(qolPlugin.getClass());
        }
    }
            ...
            case LOAD_INVENTORY:
                ...
                if (equipmentLoaded && inventoryLoaded) {
                    ...
                    while (Rs2Player.getHealthPercentage() < 70 || ateFood || drankPrayerPot) {
                        ...
                        ProjectX.log("Eating food or drinking prayer potion before going to gorillas.");
        ...
        if (!Rs2Player.isInCombat()) {
            if (outOfCombatTime == null) {
                ...
                if (currentTarget != null) {
                    ...
                    currentTarget = getTarget(true);
                    if (currentTarget == null) {
                        ProjectX.getClientThread().invoke(() -> ProjectX.getRs2NpcCache().query().withName("Demonic gorilla").interact("Attack"));
                    }
                }
    ...
    private void retreatToSafety(DemonicGorillaConfig config) {
        ...
        ProjectX.log("Changing to default gear");
    ...
    private void handleDemonicGorillaAttacks(DemonicGorillaConfig config) {
        ...
        if (currentTarget != null && !currentTarget.getNpc().isDead()) {
            ...
            if (currentAnimation == DEMONIC_GORILLA_AOE_ATTACK && demonicGorillaRockPosition != null) {
                ...
                dangerousWorldPoints.add(ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation()));
            ...
            if (!dodgedRock) {
                if ((currentGear == ArmorEquiped.RANGED || currentGear == ArmorEquiped.MAGIC) && (currentAnimation != DEMONIC_GORILLA_MELEE_ATTACK && currentAnimation != -1 && currentDefensivePrayer != Rs2PrayerEnum.PROTECT_MELEE)) {
                    var isMeleeDist = currentTarget.getNpc().getWorldArea().isInMeleeDistance(ProjectX.getClient().getLocalPlayer().getWorldArea());
                    if (isMeleeDist) {
                        moveAwayFromTarget();
                    }
                }
    ...
    public Rs2NpcModel getTarget(boolean force) {
        ...
        if (interacting != null) {
            if (Objects.equals(interacting.getName(), "Demonic gorilla")) {
                var match = ProjectX.getRs2NpcCache().query().withName("Demonic gorilla")
                        .where(n -> n.isInteractingWithPlayer()).nearest();
                if (match != null) return match;
            }
        }
        var playerLocation = ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation());

        var alreadyInteractingNpcs = ProjectX.getRs2NpcCache().query().withName("Demonic gorilla")
        ...
        List<Rs2NpcModel> demonicGorillas = ProjectX.getRs2NpcCache().query().withName("Demonic gorilla").toListOnClientThread();
    ...
    private WorldPoint findSafeTile(WorldPoint playerLocation, List<WorldPoint> dangerousWorldPoints) {
        ...
        for (WorldPoint tile : nearbyTiles) {
            final LocalPoint location = LocalPoint.fromWorld(ProjectX.getClient(), tile);
