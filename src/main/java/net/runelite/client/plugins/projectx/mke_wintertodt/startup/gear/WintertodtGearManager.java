...
public class WintertodtGearManager {
    ...
    public boolean setupOptimalGear() {
        try {
            ProjectX.log("Setting up optimal Wintertodt gear using database analysis...");
            ...
            ProjectX.log("Error setting up optimal gear: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
    ...
    private boolean analyzeOptimalGearFromDatabase() {
        try {
            ProjectX.log("Analyzing gear using comprehensive database...");
            ...
            ProjectX.log("Error analyzing gear from database: " + e.getMessage());
            return false;
        }
    }
        ...
        try {
            ...
            for (Quest quest : Quest.values()) {
                try {
                    ...
                    ProjectX.log("Failed to check quest: " + quest.name());
                }
            }
            ...
            ProjectX.log("Error checking quest states: " + e.getMessage());
    ...
    private boolean equipOptimalGear() {
        try {
            ProjectX.log("Equipping optimal gear setup...");
            ...
            ProjectX.log("Optimal gear equipped successfully!");
            return true;
            
        } catch (Exception e) {
            ProjectX.log("Error equipping gear: " + e.getMessage());
            return false;
        }
    }
    ...
    private void equipGearItem(WintertodtGearItem gearItem) {
        ...
        if (Rs2Equipment.isWearing(itemId)) {
            ProjectX.log("Already wearing: " + gearItem.getItemName());
            return;
        }
        ...
        if (!Rs2Inventory.hasItem(itemId)) {
            if (Rs2Bank.hasItem(itemId)) {
                ...
                ProjectX.log("Warning: Cannot find " + gearItem.getItemName() + " in bank");
                return;
            }
        }
        
        // Equip the item
        ProjectX.log("Equipping: " + gearItem.getItemName());
    ...
    private void bankCurrentGear() {
        try {
            ProjectX.log("Banking current gear (preserving optimal tool placement)...");
            ...
            ProjectX.log("Error banking gear: " + e.getMessage());
        }
    }
    ...
    private boolean ensureBankAccess() {
        if (!Rs2Bank.isOpen()) {
            ...
            if (!Rs2Bank.openBank()) {
                ProjectX.log("Failed to open bank for gear setup");
                return false;
            }
            Rs2Player.waitForWalking();
            if (!Rs2Bank.isOpen()) {
                ProjectX.log("Bank not open for gear setup");
                return false;
            }
        }
        return true;
    ...
    private void logDetailedGearAnalysis() {
        ProjectX.log("=== DETAILED GEAR ANALYSIS RESULTS ===");
        ...
        ProjectX.log("=== GEAR SUMMARY ===");
        ProjectX.log("Total gear pieces: " + optimalGear.size());
        ProjectX.log("Warmth-providing items: " + warmthItems + "/4 (max cold protection)");
        ProjectX.log("Pyromancer pieces: " + pyromancerPieces + "/4 (for XP bonus)");
        ProjectX.log("Total weight reduction: " + Math.abs(totalWeight) + " kg");
        
        if (warmthItems >= 4) {
            ProjectX.log("MAXIMUM COLD PROTECTION ACHIEVED");
        } else {
            ProjectX.log("Only " + warmthItems + "/4 warmth items - consider getting more warm gear");
        }
        
        if (pyromancerPieces >= 4) {
            ProjectX.log("Pyromancer XP bonus active ( 2.5% bonus)");
        }
    }
    ...
    public void reset() {
        optimalGear.clear();
        gearAnalysisLog.clear();
        ProjectX.log("Gear manager reset - ready for fresh analysis");
    }
