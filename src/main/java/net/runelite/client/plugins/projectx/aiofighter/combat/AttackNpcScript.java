...
import net.runelite.client.plugins.projectx.aiofighter.enums.AttackStyle;
import net.runelite.client.plugins.projectx.aiofighter.enums.AttackStyleMapper;
...
public class AttackNpcScript extends Script {
    ...
    public void run(AIOFighterConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                if (!ProjectX.isLoggedIn() || !super.run())
                    return;
                ...
                if (config.useSpecialAttack()
                    ...
                    ProjectX.getSpecialAttackConfigs().useSpecWeapon();
                }
                ...
                if (config.reanimateEnsouledHeads()) {
                    ...
                    if (head != null) {
                        boolean prevPause = ProjectX.pauseAllScripts.getAndSet(true);
                        try {
                            if (head.reanimate()) {
                                sleepUntil(() -> findReanimatedHeadOnPlayer() != null, 15000);
                            }
                        } finally {
                            ProjectX.pauseAllScripts.set(prevPause);
                        }
                    }
                ...
                final WorldView worldView = ProjectX.getClient().getTopLevelWorldView();
                ...
                if (config.toggleWaitForLoot() && !AIOFighterPlugin.isWaitingForLoot() && cachedTargetNpcIndex != -1) {
                    ...
                    Rs2NpcModel cachedNpcModel = ProjectX.getRs2NpcCache().query()
                    ...
                    if (cachedNpcModel != null && (cachedNpcModel.isDead() || (cachedNpcModel.getHealthRatio() == 0 && cachedNpcModel.getHealthScale() > 0))) {
                        ...
                        ProjectX.status = "Waiting for loot...";
                        ProjectX.log("NPC died, waiting for loot...");
                ...
                if (config.toggleWaitForLoot() && AIOFighterPlugin.isWaitingForLoot()) {
                    ...
                    if (timeSinceKill >= timeoutMs) {
                        ...
                        ProjectX.status = "Waiting for loot... " + secondsLeft + "s";
                        return;
                    }
                }
                ...
                if (config.toggleCenterTile() && config.centerLocation().getX() == 0
                    ...
                    if (!messageShown) {
                        ProjectX.showMessage("Please set a center location");
                        messageShown = true;
                    }
                    return;
                }
                ...
                if (!attackableNpcs.isEmpty()) {
                    ...
                    npc.click("attack");
                    ProjectX.status = "Attacking " + npc.getName();
                    ...
                    if (Rs2Player.getWorldLocation().isInArea(attackableArea)) {
                        ProjectX.log(Level.INFO, "No attackable NPC found");
                        noNpcCount++;
                        if (noNpcCount > 60 && config.slayerMode()) {
                            ProjectX.log(Level.INFO, "No attackable NPC found for 60 ticks, resetting slayer task");
    ...
    private void handleItemOnNpcToKill(AIOFighterConfig config) {
        ...
        if (ProjectX.getVarbitValue(SLAYER_AUTOKILL_DESERTLIZARDS) == 0 && lizardVariants.contains(npc.getName()) && npc.getHealthRatio() < 5) {
            Rs2Inventory.useItemOnNpc(ItemID.SLAYER_ICY_WATER, legacyNpc);
            Rs2Player.waitForAnimation();
        } else if (ProjectX.getVarbitValue(SLAYER_AUTOKILL_ROCKSLUGS) == 0 && npc.getName().equalsIgnoreCase("rockslug") && npc.getHealthRatio() < 5) {
            Rs2Inventory.useItemOnNpc(ItemID.SLAYER_BAG_OF_SALT, legacyNpc);
            Rs2Player.waitForAnimation();
        } else if (ProjectX.getVarbitValue(SLAYER_AUTOKILL_GARGOYLES) == 0 && npc.getName().equalsIgnoreCase("gargoyle") && npc.getHealthRatio() < 3) {
    ...
    private static WorldPoint npcWorldLocation(Rs2NpcModel npc, boolean instanced) {
        if (instanced) {
            ...
            if (lp != null) {
                return WorldPoint.fromLocalInstance(ProjectX.getClient(), lp);
            }
        }
