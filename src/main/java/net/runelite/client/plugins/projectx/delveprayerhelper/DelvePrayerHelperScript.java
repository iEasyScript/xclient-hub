package net.runelite.client.plugins.projectx.delveprayerhelper;

import lombok.extern.slf4j.Slf4j;
import net.runelite.api.Projectile;
import net.runelite.client.plugins.projectx.ProjectX;
import net.runelite.client.plugins.projectx.Script;
import net.runelite.client.plugins.projectx.delveprayerhelper.enums.DelvePrayerHelperState;
import net.runelite.client.plugins.projectx.delveprayerhelper.enums.DelvePrayerHelperProjectile;
...
public class DelvePrayerHelperScript extends Script {
    ...
    void handleProjectiles() {
        int currentCycle = ProjectX.getClient().getGameCycle();
        Rs2NpcModel boss = ProjectX.getRs2NpcCache().query().where(n -> n.getName() != null && n.getName().contains("Doom of Mokhaiotl")).nearest();
        ...
        if (!bossAlive) {
            ...
            ProjectX.status = state.toString();
    ...
    private void toggleOffensivePrayer() {
        if(config.offensivePrayer()) {
            var doom = ProjectX.getRs2NpcCache().query().where(n -> n.getName() != null && n.getName().contains("Doom of Mokhaiotl")).nearest();
