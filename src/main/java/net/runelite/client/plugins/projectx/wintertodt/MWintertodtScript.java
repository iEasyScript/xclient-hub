package net.runelite.client.plugins.projectx.wintertodt;
...
import net.runelite.client.plugins.projectx.wintertodt.enums.State;
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (!init) {
                    ...
                    state = State.BANKING;
                    sleep(2000);
                    ProjectX.log("Validating inventory...");
                    if (config.axeInInventory()) {
                        if (!Rs2Inventory.hasItem("axe")) {
                            ProjectX.showMessage("It seems that you selected axeInInventory option but no axe was found in your inventory.");
                            sleep(5000);
                            return;
                        }
                        ...
                        ProjectX.showMessage("Please wear an axe OR If you'd like to have an axe in your inventory, you can enable the setting 'Axe In Inventory'.");
                        sleep(5000);
                        return;
                    }
                    init = true;
                ...
                brazierModel = ProjectX.getRs2TileObjectCache().query().withId(BRAZIER_29312)
                        .where(o -> o.getWorldLocation().equals(config.brazierLocation().getOBJECT_BRAZIER_LOCATION())).nearest();
                Rs2TileObjectModel brokenBrazierModel = ProjectX.getRs2TileObjectCache().query().withId(ObjectID.BRAZIER_29313)
                        .where(o -> o.getWorldLocation().equals(config.brazierLocation().getOBJECT_BRAZIER_LOCATION())).nearest();
                Rs2TileObjectModel fireBrazierModel = ProjectX.getRs2TileObjectCache().query().withId(ObjectID.BURNING_BRAZIER_29314)
                        .where(o -> o.getWorldLocation().equals(config.brazierLocation().getOBJECT_BRAZIER_LOCATION())).nearest();
                ...
                switch (state) {
                    ...
                    case CHOP_ROOTS:
                        ...
                        if (!Rs2Player.isAnimating()) {
                            ProjectX.getRs2TileObjectCache().query().interact(ObjectID.BRUMA_ROOTS, "Chop");
                    ...
                    case FLETCH_LOGS:
                        ...
                        break;
                    case BURN_LOGS:
                        if (!ProjectX.isGainingExp || resetActions) {
                            Rs2TileObjectModel burningBrazier = ProjectX.getRs2TileObjectCache().query().withId(BURNING_BRAZIER_29314).nearest();
                            if (brokenBrazierModel != null && config.fixBrazier()) {
                                brokenBrazierModel.click("fix");
                                ProjectX.log("Fixing brazier");
...
//            }
            ...
            if (brazierModel != null && brazierModel.getClickbox() != null) {
                ProjectX.getMouse().move(brazierModel.getClickbox().getBounds());
            }
        }
    }
    ...
    private void dodgeOrbDamage() {
        for (GraphicsObject graphicsObject : ProjectX.getClient().getGraphicsObjects()) {
            if (!resetActions && graphicsObject.getId() == 502
                    && WorldPoint.fromLocalInstance(ProjectX.getClient(),
                    graphicsObject.getLocation()).distanceTo(Rs2Player.getWorldLocation()) == 1) {
                //walk south
                List<Rs2TileObjectModel> gameObjects = ProjectX.getRs2TileObjectCache().query().within(5).toList();
    ...
    private boolean handleBankLogic(MWintertodtConfig config) {
        if (config.rejuvenationPotions()) {
            ...
            if (Rs2Inventory.hasItemAmount("Rejuvenation potion ", config.foodAmount())) {
                ...
                Rs2TileObjectModel crate = ProjectX.getRs2TileObjectCache().query()
                ...
                Rs2TileObjectModel roots = ProjectX.getRs2TileObjectCache().query()
        ...
        if (!Rs2Bank.hasBankItem(config.food().getName(), config.foodAmount(), true)) {
            ProjectX.showMessage("Insufficient food supply");
    ...
    public int getWarmthLevel() {
        ...
        if (warmthWidgetText == null || warmthWidgetText.isEmpty()) {
            ProjectX.log("No Warmth Level Found");
