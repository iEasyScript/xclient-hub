...
public class NmzScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (!initialized) {
                    ...
                    boolean isInNmzInstance = ProjectX.getClient().getLocalPlayer().getWorldLocation().getY() > 4500;
                    if (!isInNmzInstance) {
                ...
                useOverload = ProjectX.getClient().getBoostedSkillLevel(Skill.RANGED) == ProjectX.getClient().getRealSkillLevel(Skill.RANGED) && config.overloadPotionAmount() > 0;
                if (isOutsideNmz) {
    ...
    public void handleOutsideNmz() {
        boolean hasStartedDream = ProjectX.getVarbitValue(VarbitID.NZONE_PURCHASEDDREAM) > 0;
    ...
    public void manageSelfHarm() {
        ...
        int currentRangedLevel = ProjectX.getClient().getBoostedSkillLevel(Skill.RANGED);
        int realRangedLevel = ProjectX.getClient().getRealSkillLevel(Skill.RANGED);
    ...
    public void useOverloadPotion() {
        if (useOverload && Rs2Inventory.hasItem("overload") && ProjectX.getClient().getBoostedSkillLevel(Skill.HITPOINTS) > 50) {
    ...
    public void useAbsorptionPotion() {
        if (ProjectX.getVarbitValue(VarbitID.NZONE_ABSORB_POTION_EFFECTS) < minAbsorption && Rs2Inventory.hasItem("absorption")) {
    ...
    public void consumeEmptyVial() {
        if (ProjectX.getClientThread().runOnClientThreadOptional(() ->
        ...
        if (!ProjectX.getClientThread().runOnClientThreadOptional(widget::isHidden).orElse(false)) {
    ...
    public void handleStore() {
        ...
        int overloadAmt = ProjectX.getVarbitValue(VarbitID.NZONE_POTION_3);
        int absorptionAmt = ProjectX.getVarbitValue(VarbitID.NZONE_POTION_4);
        ...
        int nmzPoints = ProjectX.getVarbitPlayerValue(VarPlayerID.NZONE_REWARDPOINTS);

        if (nmzPoints < totalCost) {
            ProjectX.showMessage("BOT SHUTDOWN: Not enough points to buy potions (have " + nmzPoints + ", need " + totalCost + ")");
            ProjectX.stopPlugin(plugin);
            return;
        }
