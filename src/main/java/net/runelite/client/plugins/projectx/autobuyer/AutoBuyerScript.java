package net.runelite.client.plugins.projectx.autobuyer;
...
import net.runelite.client.plugins.projectx.util.grandexchange.GrandExchangeAction;
import net.runelite.client.plugins.projectx.util.grandexchange.GrandExchangeRequest;
...
public class AutoBuyerScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (getQuestHelperPlugin().getSelectedQuest() != null && config.buyQuest()) {
                    final List<Integer> idsList = ProjectX.getClientThread().runOnClientThreadOptional(getQuestHelperPlugin()::itemsToTag).orElse(new ArrayList<>());
                    ...
                    if (!listOfItemsToBuy.isEmpty()) ProjectX.log("Using quest helper item list: " + listOfItemsToBuy);
                    else ProjectX.log("Requirements already met check bank");
                ...
                itemsList.forEach((itemName, quantity) -> {
                    ...
                    if (!hasFreeSlots()) {
                        if (canFreeUpSlots()) {
                            ...
                            ProjectX.log("Items bought from G.E. are collected to your bank.");
                        } else {
                            ProjectX.log("All G.E. slots are in use, either abort or wait until one comes available.");
                            return;
                        }
                    }
                    ...
                    if (!result) {
                        ProjectX.log("Could not buy '" + itemName + "'. Skipping it.");
                ...
                ProjectX.getClientThread().runOnClientThreadOptional(() -> {
                    ...
                    if (config.buyQuest()) {
                        ProjectX.getClient().addChatMessage(ChatMessageType.ENGINE, "", "Auto-buying quest items enabled! Churr bro ! - Wassupzzz", "Wassupzzz", false);
                    }

                    return null;
                });
                ProjectX.log("Finished buying.");
                shutdown();
            } catch (Exception ex) {
    ...
    private Map<String, Integer> mapItems(String[] items) {
        ...
        for (String item : items) {
            try {
                ...
                ProjectX.log(item + " has an invalid quantity. Quantity must be a number.");
                shutdown();
            }
        }
    ...
    public String getItemName(int itemId) {
        ItemComposition item = ProjectX.getClientThread().runOnClientThreadOptional(() -> ProjectX.getItemManager().getItemComposition(itemId)).orElse(null);
        return item.getName();
    }

    public List<ItemRequirement> getItemRequirements() {
        return ProjectX.getClientThread().runOnClientThreadOptional(() -> getQuestHelperPlugin().getSelectedQuest().getItemRequirements()).orElse(null);
    }
