package net.runelite.client.plugins.projectx.mining;
...
import net.runelite.client.plugins.projectx.mining.data.LocationOption;
import net.runelite.client.plugins.projectx.mining.data.MiningRockLocations;
import net.runelite.client.plugins.projectx.mining.data.Rocks;
...
public class AutoMiningScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (activeRock == null || !activeRock.hasRequiredLevel()) {
                    ProjectX.log("You do not have the required mining level to mine this ore.");
                    return;
                }
                ...
                if (maxPlayers > 0) {
                    ...
                    long nearbyPlayers = ProjectX.getClientThread().runOnClientThreadOptional(() ->
                                    ProjectX.getClient().getTopLevelWorldView().players().stream()
                    ...
                    if (nearbyPlayers >= maxPlayers) {
                        ProjectX.status = "Too many players nearby. Hopping...";
                        ...
                        if (hopped) {
                            ProjectX.status = "Hopped to world: " + world;
                    ...
                    case MINING:
                        ...
                        if (initialPlayerLocation != null) {
                            ...
                            if (distanceFromStart > config.distanceToStray()) {
                                ProjectX.status = "Walking back to mining location...";
    ...
    private void updateStatus() {
        ...
        String locationName = (activeLocation != null && activeLocation.getName() != null)
                ...
                : "current area";
        ProjectX.status = "Mining " + oreName + " @ " + locationName;
    }
}
