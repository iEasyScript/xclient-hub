package net.runelite.client.plugins.projectx.gildedaltar;
...
public class GildedAltarScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (!Rs2Inventory.hasItem(995)) {
                    ProjectX.showMessage("No gp found in your inventory");
                    shutdown();
                    return;
                }
                if (!hasNotedBones() && !hasUnNotedBones()) {
                    ProjectX.showMessage("No bones found in your inventory");
                    shutdown();
                    return;
                }
    ...
    public void leaveHouse() {
        ...
        if(usePortal) {
            Rs2TileObjectModel portalObject = ProjectX.getRs2TileObjectCache().query().withId(HOUSE_PORTAL_OBJECT).nearest();
    ...
    public void unnoteBones() {
        ...
        } else if (ProjectX.getClient().getWidget(14352385) != null) {
    ...
    private void enterHouse() {
        ...
        if (visitedOnce) {
            ProjectX.getRs2TileObjectCache().query().interact(ObjectID.HOUSE_ADVERTISEMENT, "Visit-Last");
            sleep(2400, 3000);
            return;
        }
        ...
        if (!isAdvertisementWidgetOpen) {
            ProjectX.getRs2TileObjectCache().query().interact(ObjectID.HOUSE_ADVERTISEMENT, "View");
            sleep(1200, 1800);
        }
    ...
    public void bonesOnAltar() {
        ...
        Rs2TileObjectModel altar = ProjectX.getRs2TileObjectCache().query().withName("Altar").nearestOnClientThread();
        if (altar != null) {
