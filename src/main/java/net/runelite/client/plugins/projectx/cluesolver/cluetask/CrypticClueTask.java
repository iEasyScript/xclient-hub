...
public class CrypticClueTask extends ClueTask {
    ...
    private void transitionToNextState() {
        ...
        } else if (clue.getNpc(clueScrollPlugin) != null && ProjectX.getRs2NpcCache().query().withName(clue.getNpc(clueScrollPlugin)).nearestOnClientThread() != null) {
    ...
    private boolean killEnemy() {
        Rs2NpcModel enemy = ProjectX.getRs2NpcCache().query().withName(clue.getEnemy().name()).nearestOnClientThread();
    ...
    private boolean lootGroundItem() {
        var groundItems = ProjectX.getRs2TileItemCache().query().within(10).toList();
    ...
    private boolean interactWithObject() {
        ...
        if (ProjectX.getRs2TileObjectCache().query().interact(targetObject, "Search")
                || ProjectX.getRs2TileObjectCache().query().interact(targetObject, "Investigate")
                || ProjectX.getRs2TileObjectCache().query().interact(targetObject, "Examine")
                || ProjectX.getRs2TileObjectCache().query().interact(targetObject, "Look-at")
                || ProjectX.getRs2TileObjectCache().query().interact(targetObject, "Open")) {
    ...
    private boolean interactWithNpc() {
        Rs2NpcModel targetNpc = ProjectX.getRs2NpcCache().query().withName(clue.getNpc(clueScrollPlugin)).nearestOnClientThread();
        if (targetNpc == null) {
