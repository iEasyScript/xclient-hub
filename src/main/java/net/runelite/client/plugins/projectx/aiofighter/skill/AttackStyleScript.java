package net.runelite.client.plugins.projectx.aiofighter.skill;
...
public class AttackStyleScript extends Script {
    ...
    private void scheduledTask(AIOFighterConfig config) {
        try {
            ...
            if (!ProjectX.isLoggedIn() || !super.run() || !config.toggleAllowStyleSwitch() || disableIfMaxed(config.toggleDisableOnMaxCombat()))
                return;
            ...
            if (attackStyle == AttackStyle.LONGRANGE || attackStyle == AttackStyle.RANGING) {
                ...
                if (weaponAttackType != null && weaponAttackType.getAttackOptions().stream().anyMatch(attackOption -> attackOption.getStyle().equals("Rapid"))) {
                    ...
                    int attackStyleVarbit = ProjectX.getVarbitPlayerValue(VarPlayer.ATTACK_STYLE);
            ...
            ProjectX.log("Current Attack Style: " + attackStyle.getName());
            ProjectX.log("Attack Style to Train: " + attackStyleToTrain.getName());
            // Change attack style if needed
            if (attackStyle != attackStyleToTrain) {
                changeAttackStyle(config, componentToDisplay);
            }
        } catch (Exception ex) {
            ProjectX.logStackTrace("AttackStyleScript", ex);
        }
    ...
    private void updateAttackStyleInfo() {
        int attackStyleVarbit = ProjectX.getVarbitPlayerValue(VarPlayer.ATTACK_STYLE);
        equippedWeaponTypeVarbit = ProjectX.getVarbitValue(Varbits.EQUIPPED_WEAPON_TYPE);
        int castingModeVarbit = ProjectX.getVarbitValue(Varbits.DEFENSIVE_CASTING_MODE);
    ...
    private AttackStyle[] getWeaponTypeStyles(int weaponType) {
        int weaponStyleEnum = ProjectX.getEnum(EnumID.WEAPON_STYLES).getIntValue(weaponType);
        ...
        for (int style : weaponStyleStructs) {
            String attackStyleName = ProjectX.getStructComposition(style).getStringValue(ParamID.ATTACK_STYLE_NAME);
    ...
    private int getSkillLevel(Skill skill) {
        return ProjectX.getClient().getRealSkillLevel(skill);
    }
