...
public class QoLOverlay extends OverlayPanel {
    ...
    private void renderNpcs(Graphics2D graphics) {
        ...
        npcs = ProjectX.getClientThread().runOnClientThreadOptional(() ->
                ProjectX.getRs2NpcCache().query().where(npc -> npc.getName() != null).toList())
        ...
        for (Rs2NpcModel npc : npcs) {
            if (npc != null && npc.getLocalLocation() != null) {
                try {
                    ...
                    Point textLocation = Perspective.getCanvasTextLocation(ProjectX.getClient(), graphics, lp, text, npc.getNpc().getLogicalHeight());
    ...
    private void renderPlayers(Graphics2D graphics) {
        for (Rs2PlayerModel player : Rs2Player.getPlayersInCombat()) {
            ...
            Point textLocation = Perspective.getCanvasTextLocation(ProjectX.getClient(), graphics, lp, text, player.getLogicalHeight());
            if (textLocation == null) {
                continue;
            }
            textLocation = new Point(textLocation.getX(), textLocation.getY() - 25);
