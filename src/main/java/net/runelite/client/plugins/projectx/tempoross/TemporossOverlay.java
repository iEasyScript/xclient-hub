...
import static net.runelite.client.plugins.projectx.tempoross.TemporossScript.workArea;
...
public class TemporossOverlay extends Overlay {
    ...
    private void renderWorldPoint(Graphics2D graphics, WorldPoint point, Color color, String label) {
        ...
        //WorldPoint pl = WorldPoint.toLocalInstance(ProjectX.getClient().getTopLevelWorldView(), point).stream().findFirst().orElse(null);
        LocalPoint localPoint = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), point);
        if (localPoint == null) {
            return;
        }

        Polygon poly = Perspective.getCanvasTilePoly(ProjectX.getClient(), localPoint);
        if (poly != null) {
            OverlayUtil.renderPolygon(graphics, poly, color);

            // Draw the label
            Point textLocation = Perspective.getCanvasTextLocation(ProjectX.getClient(), graphics, localPoint, label, 0);
