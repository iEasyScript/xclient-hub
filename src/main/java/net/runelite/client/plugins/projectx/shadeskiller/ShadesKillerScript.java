...
import net.runelite.client.plugins.projectx.shadeskiller.enums.State;
...
import static net.runelite.client.plugins.projectx.shadeskiller.enums.State.USE_TELEPORT_TO_BANK;
...
public class ShadesKillerScript extends Script {
    ...
    private boolean withdrawRequiredItems() {
        ...
        if (key.isEmpty()) {
            ProjectX.showMessage("You are missing a silver key.");
            sleep(5000);
            return false;
        }
    ...
    public boolean run(ShadesKillerConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (ProjectX.getClient().getBoostedSkillLevel(Skill.HITPOINTS) <= config.hpTreshhold() && Rs2Inventory.hasItem(config.teleportItemToBank())) {
                ...
                switch (state) {
                    ...
                    case FIGHT_SHADES:
                        ...
                        if (npc != null && !ProjectX.getClient().getLocalPlayer().isInteracting()) {
                            npc.click("Attack");
                            return;
                        }
                        if (!Rs2Combat.inCombat() && !isLooting) {
                            ProjectX.getClientThread().invoke(() -> ProjectX.getRs2NpcCache().query()
