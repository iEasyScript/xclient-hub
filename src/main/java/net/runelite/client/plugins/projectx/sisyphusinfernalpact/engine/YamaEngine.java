package net.runelite.client.plugins.projectx.sisyphusinfernalpact.engine;
...
import net.runelite.client.plugins.projectx.sisyphusinfernalpact.SisyphusInfernalPactConfig;
...
public class YamaEngine {
    ...
    public boolean tick(SisyphusInfernalPactConfig config) {
        ...
        ProjectX.log("[Yama] fire=" + fireLocs.size()
     ...
     * Queries both the ProjectX NPC cache (manual ID filter) and the raw
    ...
    private List<WorldPoint> getAllFireLocations() {
        ...
        // Source 1: ProjectX NPC cache
        try {
            List<Rs2NpcModel> cached = ProjectX.getRs2NpcCache().query().toList();
            if (cached != null) {
        ...
        try {
            List<NPC> clientNpcs = ProjectX.getClient().getNpcs();
            if (clientNpcs != null) {
