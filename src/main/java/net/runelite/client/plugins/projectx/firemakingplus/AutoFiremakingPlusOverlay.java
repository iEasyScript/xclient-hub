...
public class AutoFiremakingPlusOverlay extends OverlayPanel {
    ...
    AutoFiremakingPlusOverlay(AutoFiremakingPlusPlugin plugin, Client client, AutoFiremakingPlusConfig config) {
        ...
        pauseButton.setOnClick(() -> {
            ProjectX.pauseAllScripts.set(!ProjectX.pauseAllScripts.get());
    ...
    public Dimension render(Graphics2D graphics) {
        try {
            ...
            panelComponent.getChildren().add(LineComponent.builder()
                    .left("Status:")
                    .right(ProjectX.status == null ? "Idle" : ProjectX.status)
            ...
            if (script != null && script.getStartTimeMillis() > 0) {
                ...
                if (config.logType() != null && runtimeMillis > 1000) {
                    int logPrice = ProjectX.getItemManager().getItemPrice(config.logType().getItemId());
                    if (logPrice > 0) {
            ...
            pauseButton.setText(ProjectX.pauseAllScripts.get() ? "Resume" : "Pause");
