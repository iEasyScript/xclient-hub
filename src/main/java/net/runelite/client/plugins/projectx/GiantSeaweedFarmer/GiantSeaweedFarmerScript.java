...
public class GiantSeaweedFarmerScript extends Script {
    ...
    private void getToFossilIsland() {
        ...
        if (config.DigsiteInHouse()) {
            if (Rs2Inventory.hasItem("Farming cape", false) || Rs2Inventory.hasItem("Farming cape(t)", false)) {
                ...
                sleep(550, 750);
                ProjectX.getRs2TileObjectCache().query().interact(FARMGUILD_SPIRITTREE, "Travel");
                sleep(550, 750);
    ...
    private void handleBanking() {
        ...
        if (!readyToFarm) {
            ...
            ProjectX.log(missingItemsStr + " - shutting down");
            shutdownSequence();
        }
        BankSuccess = true;
    }
    ...
    private void handleDiving() {
        ProjectX.getRs2TileObjectCache().query().interact(BOAT, "Dive");
        sleepUntil(() -> Rs2Player.getWorldLocation().getPlane() == 1, 5000);
        if (Rs2Player.getWorldLocation().getPlane() != 1) {
            ProjectX.log("We failed to get underwater - Make sure to handle the warning dialog manually once");
            shutdownSequence();
            return;
        }
    ...
    private void handleFarmNull(){
        if (config.modeType().equals(GiantSeaweedFarmerConfig.modeType.RUN_ONCE)) {
            ProjectX.log("Finished farming, stopping...");
            shutdownSequence();
            return;
        }
        if (config.modeType().equals(GiantSeaweedFarmerConfig.modeType.RUN_ONCE_THEN_BANK)) {
            ...
            ProjectX.log("Finished farming, returning to wreck");
            return;
        }
    ...
    private boolean handlePatch(int patchId) {
        ...
        var objModel = ProjectX.getRs2TileObjectCache().query().withId(patchId).nearest();

        if (objModel == null) {
            objModel = ProjectX.getRs2TileObjectCache().query().where(o -> {
        ...
        switch (state) {
            ...
            case "Dead":
                ...
                return false;
            case "Diseased":
                ProjectX.showMessage("Diseased patch! Please turn off the script and then cure me manually as i cant do this automatically yet.");
                return false;
            case "Growing":
                return true;
            default:
    ...
    private void returnToBank() {
        ...
        if (Rs2Player.getWorldLocation().getPlane() == 1) {
            ...
            sleep(300, 500);
            ProjectX.getRs2TileObjectCache().query().interact(UNDERWATER_ANCHOR, "Climb");
            sleepUntil(() -> Rs2Player.getWorldLocation().getPlane() == 0, 7000);
        }


        if (Rs2Player.getWorldLocation().getPlane() != 0) {
            ProjectX.log("We failed to get back to the surface");
        }
        else {
            ProjectX.log("We're back at the bankt");
            if (config.modeType().equals(GiantSeaweedFarmerConfig.modeType.RUN_ONCE_THEN_BANK)) {
                shutdownSequence();
            }
        }
    ...
    private void shutdownSequence(){
        ProjectX.stopPlugin(giantSeaweedPlugin);
    }
