...
public class FortisGemStallThievingSpot implements IStallThievingSpot {
    ...
    public void thieve() {
        ...
        final Rs2TileObjectModel stall = ProjectX.getRs2TileObjectCache().query().withId(STALL_ID).within(SAFESPOT, 2).nearest();
        if (stall == null) {
            boolean started = ProjectX.hopToWorld(Login.getRandomWorld(Rs2Player.isMember()));
            if (started) {
                Global.sleepUntil(ProjectX::isLoggedIn, 15000);
            }
            return;
        }
