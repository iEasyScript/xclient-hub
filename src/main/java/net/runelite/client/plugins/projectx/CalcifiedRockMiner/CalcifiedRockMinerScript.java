package net.runelite.client.plugins.projectx.CalcifiedRockMiner;
...
import net.runelite.client.plugins.projectx.CalcifiedRockMiner.CalcifiedRockMinerConfig;
...
public class CalcifiedRockMinerScript extends Script {
    ...
    public boolean run(net.runelite.client.plugins.projectx.CalcifiedRockMiner.CalcifiedRockMinerConfig config) {
    ...
    private void handleMining(net.runelite.client.plugins.projectx.CalcifiedRockMiner.CalcifiedRockMinerConfig config) {
        ...
        if (config.focusCrackedWaterDeposits() && !Rs2Player.isMoving()) {
            Rs2TileObjectModel weepingRock = ProjectX.getRs2TileObjectCache().query()
                    .withId(WEEPING_ROCK)
                    .within(Rs2Player.getWorldLocation(), 15)
                    .first();
            if (weepingRock != null) {
                Rs2TileObjectModel tearRock = ProjectX.getClientThread().invoke(() ->
        ...
        Rs2TileObjectModel rock = ProjectX.getClientThread().invoke(() ->
                ProjectX.getRs2TileObjectCache().query()
        ...
        if (rock != null && shouldTryMiningAgain) {
            ...
            ProjectX.log("No calcified rock found. Waiting...");
        }
    }

    private boolean hopIfTooManyPlayersNearby(net.runelite.client.plugins.projectx.CalcifiedRockMiner.CalcifiedRockMinerConfig config) {
    ...
    private void handleCrushing(net.runelite.client.plugins.projectx.CalcifiedRockMiner.CalcifiedRockMinerConfig config) {
        if (config.crushDeposits() && hasHammer() && Rs2Inventory.hasItem(29088)) {
            if (Rs2Player.getWorldLocation().distanceTo(ANVIL) < 1) {
                ...
                Rs2TileObjectModel anvil = ProjectX.getClientThread().invoke(() ->
                        ProjectX.getRs2TileObjectCache().query()
                                .within(ANVIL, 3)
                                .withName("Anvil")
                                .nearestReachable());
    ...
    private void MoveCameraToRock(WorldPoint rock) {
        ...
        Rs2Camera.resetZoom();
        Rs2Camera.turnTo(LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), rock));
    }
