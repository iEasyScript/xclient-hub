...
import net.runelite.client.plugins.projectx.*;
import net.runelite.client.plugins.Plugin;
import net.runelite.client.plugins.projectx.util.bank.Rs2Bank;
import net.runelite.client.plugins.projectx.util.inventory.*;
...
import net.runelite.client.plugins.projectx.globval.WidgetIndices;
...
public final class WildernessAgilityScript extends Script {
    ...
    public boolean run(WildernessAgilityConfig config) {
        ...
        wildyItems = new WildernessAgilityItems(ProjectX.getItemManager());
        
        if (config.debugMode()) {
            currentState = ObstacleState.valueOf(config.debugStartState().name());
            ProjectX.log("[DEBUG MODE] Starting in state: " + currentState);
            ...
            if (currentState == ObstacleState.EMERGENCY_ESCAPE) {
                ...
                ProjectX.log("[DEBUG MODE] Emergency escape variables initialized");
            }
        } else {
            currentState = ObstacleState.START;
        }
        startTime = System.currentTimeMillis();
        ProjectX.log("[WildernessAgilityScript] startup called");
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (Rs2Player.getHealthPercentage() <= 0 || deathDetected) {
                    if (deathDetected) {
                        ProjectX.log("[WildernessAgility] Death detected - triggering death handler");
                    }
                    handlePlayerDeath();
            ...
            if (currentPoseAnimation != 737) {
                ...
                if (config.phoenixEscape() && !emergencyEscapeTriggered) {
                    if (!hasPhoenixNecklace()) {
                        ProjectX.log("[WildernessAgility] Phoenix necklace missing - triggering escape!");
                ...
                if (config.leaveAtHealthPercent() > 0 && !emergencyEscapeTriggered) {
                    if (Rs2Player.getHealthPercentage() <= config.leaveAtHealthPercent()) {
                        ProjectX.log("[WildernessAgility] Health dropped to " + (int)Rs2Player.getHealthPercentage() + "% - triggering emergency escape!");
                        triggerPhoenixEscape();
                    }
                }
    ...
    public void shutdown() {
        ...
        ProjectX.log("[WildernessAgilityScript] shutdown called");
        super.shutdown();
    }
    ...
    public boolean runeliteClientPluginsProjectXWildernessagilityWildernessAgilityScript_run(WildernessAgilityConfig config) {
        return run(config);
    }
    ...
    private Rs2TileObjectModel getDispenserObj() {
        return ProjectX.getRs2TileObjectCache().query().withId(DISPENSER_ID).nearest();
    }
    private Rs2TileObjectModel getObstacleObj(int index) {
        return ProjectX.getRs2TileObjectCache().query().withId(obstacles.get(index).getObjectId()).nearest();
    }
    private boolean isInUndergroundPit() {
        return ProjectX.getRs2TileObjectCache().query().withId(UNDERGROUND_OBJECT_ID).nearest() != null;
    }
    private void recoverFromPit() {
        ...
        if (isInUndergroundPit()) {
            ...
            List<Rs2TileObjectModel> ladders = ProjectX.getRs2TileObjectCache().query().withId(17385).toList();
            ...
            if (ladderObj != null && Rs2Player.getWorldLocation().distanceTo(ladderObj.getWorldLocation()) <= 50) {
                ...
                if (now - lastLadderInteractTime > 2000) {
                    ...
                    List<Rs2TileObjectModel> laddersNow = ProjectX.getRs2TileObjectCache().query().withId(17385).toList();
            ...
            switch (pitRecoveryTarget) {
                case ROPE:
                    ...
                    if (rope != null && !Rs2Player.isMoving()) {
                        isWaitingForRope = false;
                        ropeStartXp = ProjectX.getClient().getSkillExperience(AGILITY);
                ...
                case LOG:
                    ...
                    Rs2TileObjectModel log = ProjectX.getRs2TileObjectCache().query()
    ...
    private void handlePipe() {
        ...
        if (!Rs2Player.isAnimating() && !Rs2Player.isMoving()) {
            ...
            Rs2TileObjectModel pipe = ProjectX.getRs2TileObjectCache().query()
            ...
            if (interacted) {
                ...
                pipeStartXp = ProjectX.getClient().getSkillExperience(AGILITY);
    ...
    private void handleRope() {
        ...
        if (isWaitingForRope) {
            ...
            if (ProjectX.getClient().getSkillExperience(AGILITY) > ropeStartXp) {
                isWaitingForRope = false;
    ...
    private void handleStones() {
        if (isWaitingForStones) {
            WorldPoint loc = Rs2Player.getWorldLocation();
            int currentXp = ProjectX.getClient().getSkillExperience(AGILITY);
        ...
        if (!Rs2Player.isAnimating() && !Rs2Player.isMoving() && !isWaitingForStones) {
            ...
            if (stones != null) {
                ...
                if (interacted) {
                    ...
                    stonesStartXp = ProjectX.getClient().getSkillExperience(AGILITY);
                    lastObstacleInteractTime = System.currentTimeMillis();
                    lastObstaclePosition = Rs2Player.getWorldLocation();
                }
            }
    ...
    private void handleLog() {
        if (isWaitingForLog) {
            ...
            boolean xpPassed = ProjectX.getClient().getSkillExperience(AGILITY) > logStartXp;
        ...
        if (!Rs2Player.isAnimating() && !Rs2Player.isMoving()) {
            ...
            if (log == null) {
                log = ProjectX.getRs2TileObjectCache().query()
                        .withId(obstacles.get(3).getObjectId()).nearest();
            }
            if (log != null) {
                boolean interacted = log.click();
                if (interacted) {
                    isWaitingForLog = true;
                    logStartXp = ProjectX.getClient().getSkillExperience(AGILITY);
                    lastObstacleInteractTime = System.currentTimeMillis();
                    lastObstaclePosition = Rs2Player.getWorldLocation();
                }
            }
    ...
    private void handleRocks() {
        ...
        if (!Rs2Player.isAnimating() && !Rs2Player.isMoving()) {
            ...
            Rs2TileObjectModel targetRockObj = ProjectX.getRs2TileObjectCache().query()
            ...
            if (targetRockObj != null && targetRockObj.click("Climb")) {
                ...
                if (transitioned) {
                    ...
                    int startExp = ProjectX.getClient().getSkillExperience(AGILITY);
                    if (waitForXpChange(startExp, 3000)) { // Shorter timeout for fallback
                        ProjectX.log("[WildernessAgility] XP fallback successful, transitioning to dispenser");
                        Rs2TileObjectModel freshDispenser = getDispenserObj();
                        cachedDispenserObj = freshDispenser;
                        lastObjectCheck = System.currentTimeMillis();
                        currentState = ObstacleState.DISPENSER;
                        if (freshDispenser != null) {
                            ...
                            // Start monitoring rock climb pose animation
                            ProjectX.log("[WildernessAgility] Starting rock climb pose animation monitoring...");
                            waitingForRockClimbCompletion = true;
                            // Don't interact with dispenser yet - wait for pose animation 737 to finish
                        }
                        return;
    ...
    private void handleConfigChecks() {
        ...
        if (config.bankAfterDispensers() > 0 && dispenserLoots >= config.bankAfterDispensers()) {
            ...
            if (config.enablePlayerMonitor()) {
                try {
                    Plugin playerMonitor = ProjectX.getPluginManager().getPlugins().stream()
                            .filter(x -> x.getClass().getName().equals("net.runelite.client.plugins.projectx.playermonitor.PlayerMonitorPlugin"))
                            .findFirst()
                            .orElse(null);
                    if (playerMonitor != null) {
                        ProjectX.startPlugin(playerMonitor);
                        ProjectX.log("[WildernessAgility] Player Monitor enabled for banking safety");
                    } else {
                        ProjectX.log("[WildernessAgility] Player Monitor plugin not found - continuing without it");
                    }
                } catch (Exception e) {
                    ProjectX.log("[WildernessAgility] Failed to start Player Monitor: " + e.getMessage());
                }
            }
    ...
    private void handleStart() {
        ...
        if (!(forceStartAtCourse || config.startAtCourse())) {
            ...
            if (coinCount < 150000) {
                ProjectX.log("[WildernessAgility] Not enough coins to deposit into dispenser (" + coinCount + " < 150000) - going to bank");
            ...
            if (dispenserObj != null) {
                ProjectX.log("[WildernessAgility] Attempting to deposit " + coinCount + " coins into dispenser");
                ...
                ProjectX.log("[WildernessAgility] Dispenser object not found!");
            }
            currentState = ObstacleState.PIPE;
            return;
        } else {
    ...
    private void handleLocationTracking() {
        ...
        if (timeSinceLastMove >= LOCATION_STUCK_TIMEOUT && lastStateBeforeStuck == null) {
            ...
            ProjectX.log("[Location Tracking] Player stuck for " + (LOCATION_STUCK_TIMEOUT/1000) + " seconds in " + currentState + ", will retry previous state");
        }
        
        // If stuck for 8 seconds, retry the current state
        if (timeSinceLastMove >= LOCATION_STUCK_TIMEOUT && lastStateBeforeStuck != null) {
            ProjectX.log("[Location Tracking] Retrying state " + currentState + " due to being stuck");
            retryCurrentState();
    ...
    private void handlePositionTimeout() {
        if (!isInRetryMode) {
            ...
            if (currentState == ObstacleState.PIT_RECOVERY) {
                ProjectX.log("[Position Timeout] Player stuck in pit for " + getPositionTimeout() + "ms. Retrying ladder interaction...");
            } else {
                ProjectX.log("[Position Timeout] Player stuck in " + currentState + " for " + getPositionTimeout() + "ms. Retrying...");
            }
            isInRetryMode = true;
            ...
            if (currentState == ObstacleState.PIT_RECOVERY) {
                ProjectX.log("[Position Timeout] Still stuck in pit after retry. Forcing exit from pit recovery...");
            } else {
                ProjectX.log("[Position Timeout] Retry failed for " + currentState + ". Moving to next state...");
            }
    ...
    private void forceProgressToNextState() {
        switch (currentState) {
            case PIPE:
                ProjectX.log("[Force Progress] Moving from PIPE to ROPE");
                currentState = ObstacleState.ROPE;
                isWaitingForPipe = false;
                break;
            case ROPE:
                ProjectX.log("[Force Progress] Moving from ROPE to STONES");
                currentState = ObstacleState.STONES;
                isWaitingForRope = false;
                break;
            case STONES:
                ProjectX.log("[Force Progress] Moving from STONES to LOG");
                currentState = ObstacleState.LOG;
                isWaitingForStones = false;
                break;
            case LOG:
                ProjectX.log("[Force Progress] Moving from LOG to ROCKS");
                currentState = ObstacleState.ROCKS;
                isWaitingForLog = false;
                break;
            case ROCKS:
                ProjectX.log("[Force Progress] Moving from ROCKS to DISPENSER");
                currentState = ObstacleState.DISPENSER;
                break;
            case DISPENSER:
                ProjectX.log("[Force Progress] Moving from DISPENSER to CONFIG_CHECKS");
                currentState = ObstacleState.CONFIG_CHECKS;
                waitingForDispenserLoot = false;
                dispenserLootAttempts = 0;
                break;
            case PIT_RECOVERY:
                ProjectX.log("[Force Progress] Stuck in pit recovery, forcing exit and continuing...");
                // Force the player to move to the recovery target or default to ROPE
                if (pitRecoveryTarget != null) {
                    currentState = pitRecoveryTarget;
                    ProjectX.log("[Force Progress] Returning to recovery target: " + pitRecoveryTarget);
                } else {
                    currentState = ObstacleState.ROPE;
                    ProjectX.log("[Force Progress] No recovery target set, defaulting to ROPE");
                }
                ...
                break;
            case BANKING:
                ProjectX.log("[Force Progress] Moving from BANKING to POST_BANK_CONFIG");
                currentState = ObstacleState.POST_BANK_CONFIG;
                break;
            case SWAP_BACK:
                ProjectX.log("[Force Progress] Moving from SWAP_BACK to PIPE");
                currentState = ObstacleState.PIPE;
                break;
            case CONFIG_CHECKS:
                ProjectX.log("[Force Progress] Moving from CONFIG_CHECKS to PIPE");
                currentState = ObstacleState.PIPE;
                break;
            case START:
                ProjectX.log("[Force Progress] Moving from START to PIPE");
                currentState = ObstacleState.PIPE;
                break;
            case WORLD_HOP_2:
                ProjectX.log("[Force Progress] Moving from WORLD_HOP_2 to WALK_TO_LEVER");
                currentState = ObstacleState.WALK_TO_LEVER;
                break;
            case INTERACT_LEVER:
                ProjectX.log("[Force Progress] Moving from INTERACT_LEVER to BANKING (web walking handles lever)");
                currentState = ObstacleState.BANKING;
                break;
            case POST_BANK_CONFIG:
                ProjectX.log("[Force Progress] Moving from POST_BANK_CONFIG to WALK_TO_COURSE");
                currentState = ObstacleState.WALK_TO_COURSE;
                break;
            case WALK_TO_COURSE:
                ProjectX.log("[Force Progress] Moving from WALK_TO_COURSE to PIPE");
                currentState = ObstacleState.PIPE;
                break;
            case INIT:
                ProjectX.log("[Force Progress] Moving from INIT to PIPE");
                currentState = ObstacleState.PIPE;
                break;
            case WALK_TO_LEVER:
                ProjectX.log("[Force Progress] Moving from WALK_TO_LEVER to BANKING (web walking handles lever)");
                webWalkStartTime = 0; // Reset web walk timeout
                currentState = ObstacleState.BANKING;
                break;
            case WORLD_HOP_1:
                ProjectX.log("[Force Progress] Moving from WORLD_HOP_1 to WORLD_HOP_2");
                currentState = ObstacleState.WORLD_HOP_2;
                break;
            default:
                ProjectX.log("[Force Progress] No progression defined for state: " + currentState);
                break;
        }
    }

    private boolean waitForXpChange(int startXp, int timeoutMs) {
        return sleepUntil(() -> ProjectX.getClient().getSkillExperience(AGILITY) > startXp, timeoutMs);
    }
    ...
    private void triggerPhoenixEscape() {
        ...
        ProjectX.log("[WildernessAgility] EMERGENCY ESCAPE TRIGGERED - Activating robust escape sequence");
        ...
        if (config.enablePlayerMonitor()) {
            try {
                ...
                if (playerMonitor != null) {
                    ProjectX.startPlugin(playerMonitor);
                    ProjectX.log("[WildernessAgility] Player Monitor enabled for Emergency Escape");
                } else {
                    ProjectX.log("[WildernessAgility] Player Monitor plugin not found - continuing escape without it");
                }
            } catch (Exception e) {
                ProjectX.log("[WildernessAgility] Failed to start Player Monitor for Emergency Escape: " + e.getMessage());
            }
        }
    ...
    private void handleEmergencyEscape() {
        ...
        if (timeSinceStart > EMERGENCY_ESCAPE_TIMEOUT) {
            ProjectX.log("[WildernessAgility] Emergency Escape timed out after " + (EMERGENCY_ESCAPE_TIMEOUT/1000) + " seconds - logging out");
            Rs2Player.logout();
            return;
        }
        ...
        if (!hasEquippedPhoenixNecklace && Rs2Inventory.hasItem("Phoenix necklace")) {
            ProjectX.log("[WildernessAgility] Emergency Escape Step 1: Equipping Phoenix necklace");
        ...
        if (!hasClimbedRocks) {
            ...
            if (stepElapsedTime > ESCAPE_STEP_TIMEOUT) {
                ProjectX.log("[WildernessAgility] Emergency Escape Step 2: Timeout - forcing to next step");
            ...
            if (isInArea) {
                ProjectX.log("[WildernessAgility] Emergency Escape Step 2: Climbing rocks");
                ProjectX.getRs2TileObjectCache().query().interact(ROCKS_OBJECT_ID, "Climb"); // Climb rocks
                sleep(1200);
                sleepUntil(() -> !Rs2Player.isMoving(), 5000);
                hasClimbedRocks = true;
                ...
                if (currentLoc != null && currentLoc.distanceTo(GATE_AREA) <= 3) {
                    ProjectX.log("[WildernessAgility] Emergency Escape Step 2: Arrived at gate area (within 3 tiles)");
                    hasClimbedRocks = true;
                    escapeStep2StartTime = 0; // Reset timer
                    return;
                }
                
                ProjectX.log("[WildernessAgility] Emergency Escape Step 2: Walking to gate area");
        ...
        if (!hasOpenedGate) {
            ProjectX.log("[WildernessAgility] Emergency Escape Step 3: Opening gate");
            sleepUntilOnClientThread(() -> ProjectX.getRs2TileObjectCache().query().withId(GATE_OBJECT_ID).nearest() != null); // Wait for Gate
            ProjectX.getRs2TileObjectCache().query().interact(GATE_OBJECT_ID, "Open");
            hasOpenedGate = true;
            return; // Wait for next loop iteration
        }
        
        // Step 4: Walk to Mage Bank (our approach - keep Player Monitor on)
        ProjectX.log("[WildernessAgility] Emergency Escape Step 4: Walking to Mage Bank");
        ...
        if (currentLoc != null && currentLoc.distanceTo(new WorldPoint(2534, 4712, 0)) <= 10) {
            ProjectX.log("[WildernessAgility] Successfully reached Mage Bank - logging out");
            // Logout until successful (Netoxic's approach)
            while (ProjectX.isLoggedIn()) {
                Rs2Player.logout();
                sleepUntil(() -> !ProjectX.isLoggedIn(), 300);
            }
            // Reset escape state
            emergencyEscapeTriggered = false;
            emergencyEscapeStartTime = 0;
    ...
    private boolean attemptWorldHop(int targetWorld, String context) {
        ...
        if (worldHopRetryCount >= MAX_WORLD_HOP_RETRIES || timeSinceStart > WORLD_HOP_RETRY_TIMEOUT) {
            ProjectX.log("World hop to " + targetWorld + " failed after " + worldHopRetryCount + " attempts in " + context);
        ...
        ProjectX.cantHopWorld = false;
        
        ProjectX.log("Attempting world hop to " + targetWorld + " (attempt " + worldHopRetryCount + "/" + MAX_WORLD_HOP_RETRIES + ") in " + context);
        
        boolean hopSuccess = ProjectX.hopToWorld(targetWorld);
        if (!hopSuccess) {
            ProjectX.log("Failed to initiate world hop to " + targetWorld + " in " + context);
            return false; // Will retry on next call
        }
        
        boolean hopConfirmed = sleepUntil(() -> Rs2Player.getWorld() == targetWorld, 8000);
        if (!hopConfirmed) {
            ProjectX.log("World hop to " + targetWorld + " not confirmed in " + context + ", current world: " + Rs2Player.getWorld());
            return false; // Will retry on next call
        }
        
        // Success! Reset retry counter
        ProjectX.log("Successfully hopped to world " + targetWorld + " in " + context);
        worldHopRetryCount = 0;
        return true;
    }
    ...
    private void handleWalkToLever() {
        ...
        if (webWalkStartTime == 0) {
            ...
            ProjectX.log("[WildernessAgility] Starting web walk to Mage Bank");
        }
        
        // Check for timeout
        long timeSinceStart = System.currentTimeMillis() - webWalkStartTime;
        if (timeSinceStart > WEB_WALK_TIMEOUT) {
            ProjectX.log("[WildernessAgility] Web walk to Mage Bank timed out after " + (WEB_WALK_TIMEOUT/1000) + " seconds, forcing to banking state");
        ...
        if (walkSuccess) {
            ProjectX.log("[WildernessAgility] Successfully reached Mage Bank area");
            webWalkStartTime = 0; // Reset for next attempt
            currentState = ObstacleState.BANKING;
        } else {
            ...
            if (currentLoc != null && currentLoc.distanceTo(mageBankTile) <= 10) {
                ProjectX.log("[WildernessAgility] Close enough to Mage Bank, proceeding to banking");
                webWalkStartTime = 0; // Reset for next attempt
                currentState = ObstacleState.BANKING;
            } else {
                // Stay in this state to retry, but log progress
                if (timeSinceStart % 10000 < 100) { // Log every 10 seconds
                    ProjectX.log("[WildernessAgility] Still walking to Mage Bank... (" + (timeSinceStart/1000) + "s elapsed)");
                }
            }
        }
    }
    ...
    private void handleInteractLever() {
        ...
        ProjectX.log("[WildernessAgility] handleInteractLever called but using web walking instead");
        currentState = ObstacleState.BANKING;
    }
    ...
    private void handleWalkToCourse() {
        ...
        if (dispenserObj != null) {
            int coinCount = Rs2Inventory.itemQuantity(COINS_ID);
            if (coinCount >= 150000) {
                ProjectX.log("[WildernessAgility] [WALK_TO_COURSE] Attempting to deposit " + coinCount + " coins into dispenser");
                Rs2Inventory.use(COINS_ID);
                sleep(400);
                dispenserObj.click("Use");
                sleep(getActionDelay());
                ...
                ProjectX.log("[WildernessAgility] [WALK_TO_COURSE] Not enough coins (" + coinCount + " < 150000)");
            }
        } else {
            ProjectX.log("[WildernessAgility] [WALK_TO_COURSE] Dispenser object not found!");
        }
        currentState = ObstacleState.PIPE;
    }
    ...
    private void handleBanking() {
        ...
        if (config.enablePlayerMonitor()) {
            try {
                Plugin playerMonitor = ProjectX.getPluginManager().getPlugins().stream()
                        ...
                        .orElse(null);
                if (playerMonitor != null) {
                    ProjectX.stopPlugin(playerMonitor);
                    ProjectX.log("[WildernessAgility] Player Monitor disabled - safely reached bank");
                } else {
                    ProjectX.log("[WildernessAgility] Player Monitor plugin not found - nothing to disable");
                }
            } catch (Exception e) {
                ProjectX.log("[WildernessAgility] Failed to stop Player Monitor: " + e.getMessage());
            }
        }
        
        // Check if we have an open looting bag and deposit its contents first
        if (Rs2Inventory.hasItem(LOOTING_BAG_OPEN_ID)) {
            ProjectX.log("[WildernessAgility] Depositing looting bag contents (value: " + NumberFormat.getIntegerInstance().format(lootingBagValue) + "gp)");
            Rs2Bank.depositLootingBag();
            sleep(getActionDelay());
            
            // Reset looting bag value after deposit
            lootingBagValue = 0;
            ProjectX.log("[WildernessAgility] Looting bag value reset after deposit");
        }
        ...
        if (config.withdrawLootingBag() && !Rs2Inventory.hasItem(LOOTING_BAG_CLOSED_ID) && !Rs2Inventory.hasItem(LOOTING_BAG_OPEN_ID)) {
            ...
            if (closedSuccess) {
                ...
                ProjectX.log("[WildernessAgility] Successfully withdrew closed looting bag");
            } else {
                // If closed bag withdrawal failed, try open bag
                ProjectX.log("[WildernessAgility] Closed looting bag not available, trying open version");
                Rs2Bank.withdrawOne(LOOTING_BAG_OPEN_ID);
                boolean openSuccess = sleepUntil(() -> Rs2Inventory.hasItem(LOOTING_BAG_OPEN_ID), 3000);
                
                if (openSuccess) {
                    ProjectX.log("[WildernessAgility] Successfully withdrew open looting bag");
                    needsLootingBagActivation = false; // No need to activate, already open
                } else {
                    ProjectX.log("[WildernessAgility] Failed to withdraw looting bag in any state");
                }
            }
        }
        ...
        if (needsLootingBagActivation && Rs2Inventory.hasItem(LOOTING_BAG_CLOSED_ID)) {
            ProjectX.log("[WildernessAgility] Activating looting bag");
    ...
    private void attemptLogoutUntilLoggedOut() {
        ...
        int attempts = 0;
        while (!"LOGIN_SCREEN".equals(ProjectX.getClient().getGameState().toString()) && attempts < maxAttempts) {
            Rs2Player.logout();
    ...
    private void clearInventoryIfNeeded() {
        ...
        if (attempts >= maxAttempts) {
            ProjectX.log("clearInventoryIfNeeded() reached max attempts, breaking to prevent infinite loop");
        }
    }
    ...
    private void addLootingBagValue(int qty1, String item1, int qty2, String item2) {
        ...
        int value1 = ProjectX.getItemManager().getItemPrice(itemId1) * qty1;
        int value2 = ProjectX.getItemManager().getItemPrice(itemId2) * qty2;
    ...
    private void addLootingBagValue(int qty1, String item1, int qty2, String item2, int qty3, String item3) {
        ...
        int value1 = ProjectX.getItemManager().getItemPrice(itemId1) * qty1;
        int value2 = ProjectX.getItemManager().getItemPrice(itemId2) * qty2;
        int value3 = ProjectX.getItemManager().getItemPrice(itemId3) * qty3;
    ...
    private void syncLootingBagFromContainer(net.runelite.api.ItemContainer container) {
        if (container == null) {
            ProjectX.log("[WildernessAgility] Looting bag is empty (container null)");
            lootingBagValue = 0;
        ...
        for (net.runelite.api.Item item : container.getItems()) {
            if (item.getId() > 0) { // Valid item
                int itemValue = ProjectX.getItemManager().getItemPrice(item.getId()) * item.getQuantity();
        ...
        ProjectX.log("[WildernessAgility] Synced looting bag from container: " + 
    ...
    public void checkLootingBagOnStartup() {
        ...
        ProjectX.log("[WildernessAgility] Checking looting bag for initial sync...");
        ...
        try {
            ...
            ProjectX.log("[WildernessAgility] Looting bag interface closed, inventory refreshed");
        } catch (NullPointerException e) {
            // Known issue: Items with null action data in inventory can cause Rs2Inventory operations to crash
            ProjectX.log("[WildernessAgility] Inventory error during looting bag check - will retry next cycle");
            waitingForLootingBagSync = false;
