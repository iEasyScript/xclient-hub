...
public class TemporossWorkArea
{
    ...
    {
        return ProjectX.getRs2TileObjectCache().query().withId(ObjectID.BUCKETS).within(bucketPoint, 2).nearest();
    }

    public Rs2TileObjectModel getPump()
    {
        return ProjectX.getRs2TileObjectCache().query().withId(ObjectID.WATER_PUMP_41000).within(pumpPoint, 2).nearest();
    }

    public Rs2TileObjectModel getRopeCrate()
    {
        return ProjectX.getRs2TileObjectCache().query().withId(ObjectID.ROPES).within(ropePoint, 2).nearest();
    }

    public Rs2TileObjectModel getHammerCrate()
    {
        return ProjectX.getRs2TileObjectCache().query().withId(ObjectID.HAMMERS_40964).within(hammerPoint, 2).nearest();
    }

    public Rs2TileObjectModel getHarpoonCrate()
    {
        return ProjectX.getRs2TileObjectCache().query().withId(ObjectID.HARPOONS).within(harpoonPoint, 2).nearest();
    }

    public Rs2TileObjectModel getMast() {
    return ProjectX.getRs2TileObjectCache().query().withIds(NullObjectID.NULL_41352, NullObjectID.NULL_41353).within(mastPoint, 10).nearest();
}

    public Rs2TileObjectModel getBrokenMast() {
    return ProjectX.getRs2TileObjectCache().query().withIds(ObjectID.DAMAGED_MAST_40996, ObjectID.DAMAGED_MAST_40997).within(mastPoint, 10).nearest();
    }

    public Rs2TileObjectModel getTotem() {
    return ProjectX.getRs2TileObjectCache().query().withIds(NullObjectID.NULL_41355, NullObjectID.NULL_41354).within(totemPoint, 10).nearest();
}

    public Rs2TileObjectModel getBrokenTotem() {
    return ProjectX.getRs2TileObjectCache().query().withIds(ObjectID.DAMAGED_TOTEM_POLE, ObjectID.DAMAGED_TOTEM_POLE_41011).within(totemPoint, 10).nearest();
    }

    public Rs2TileObjectModel getRange()
    {
        return ProjectX.getRs2TileObjectCache().query().withId(ObjectID.SHRINE_41236).within(rangePoint, 2).nearest();
    }
