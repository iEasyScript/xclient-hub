...
public class StunTeleAlchScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (plugin.getTeleportSpell() == null) {
                    ProjectX.showMessage("Set a teleport spell in config");
                    shutdown();
                    return;
                }
                ...
                if (plugin.getAlchItemNames().isEmpty()
                    ...
                    ProjectX.log("Missing alchemy items...");
                    return;
                }
                ...
                var target = ProjectX.getRs2NpcCache().query()
                ...
                if (target.getWorldLocation() == null) {
                    ProjectX.log("NPC has no valid world location: " + configuredTargetNpc);
                    return;
                }
                if (!Rs2Magic.cast(plugin.getStunSpell().getRs2Spell().getMagicAction())) {
                    ProjectX.log("Failed to select stun spell: " + plugin.getStunSpell().name());
                    return;
                }
                ...
                if (!stunned) {
                    ProjectX.log("Failed to cast stun on NPC: " + configuredTargetNpc);
                    return;
                }
                // 2) ALCHEMY
                if (Rs2AntibanSettings.naturalMouse) {
                ...
                ProjectX.log("Total time for loop " + totalTime);
            } catch (Exception ex) {
                ProjectX.log("Stun TeleAlchemy loop failed: " + ex.getMessage());
            }
    ...
    private void attemptNpcRecoveryTeleport(String configuredTargetNpc) {
        ...
        if (!recovered) {
            ProjectX.log("Recovery teleport failed for missing NPC: " + configuredTargetNpc);
        }
    }
    ...
    private boolean castConfiguredTeleportAndWait(String context) {
        ...
        if (!Rs2Magic.cast(plugin.getTeleportSpell().getRs2Spell().getMagicAction())) {
            ProjectX.log("Failed to cast teleport (" + context + "): " + plugin.getTeleportSpell().name());
            return false;
        }
        ...
        boolean teleportStarted = sleepUntil(() ->
                ...
                2400);
        if (!teleportStarted) {
            ProjectX.log("Teleport did not start in time (" + context + "): " + plugin.getTeleportSpell().name());
            return false;
        }
        ...
        if (!teleportFinished) {
            ProjectX.log("Teleport did not finish in time (" + context + "): " + plugin.getTeleportSpell().name());
            return false;
        }
