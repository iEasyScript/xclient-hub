package net.runelite.client.plugins.projectx.astralrc;
...
public class AstralRunesScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if(!isLunar()) {
                    ...
                    ProjectX.showMessage("Set spellbook to Lunar Spellbook");
                    shutdown();
                    return;
                }
                ...
                if(!dreamMentorComplete && !(Rs2Inventory.hasItem(ItemID.LUNAR_SEAL_OF_PASSAGE) || Rs2Equipment.isWearing(ItemID.LUNAR_SEAL_OF_PASSAGE))) {
                    ...
                    ProjectX.showMessage("No Seal of Passage found equipped or in inventory");
                    shutdown();
                    return;
                }

                if( !Rs2Inventory.hasRunePouch() ) {
                    ProjectX.showMessage("No Rune pouch found");
                    shutdown();
                    return;
                }

                if( !Rs2Bank.isOpen() && !canCastMoonclanTeleport ) {
                    ProjectX.showMessage("Required runes not found, make sure Dust staff is equipped and Rune pouch contains Law and Astral runes.");
                    shutdown();
                    return;
                }

                if( !Rs2Bank.isOpen() && Rs2Inventory.contains(ItemID.RCU_POUCH_COLOSSAL) && !canCastMoonclanTeleport ) {
                    ProjectX.showMessage("Required runes not found, make sure Dust staff is equipped and Rune pouch contains Law, Astral, and Cosmic runes.");
                    shutdown();
                    return;
                }
                ...
                switch (state) {
                    case REPAIRING:
                        if( Rs2Inventory.hasDegradedPouch() ) {
                            ...
                            if( Rs2Magic.hasRequiredRunes(Rs2Spells.NPC_CONTACT) ) {
                                ...
                                ProjectX.showMessage("No runes found for NPC Contact");
                                shutdown();
                                return;
                            }
                        } else {
                    ...
                    case BANKING:
                        ...
                        if( !Rs2Bank.isOpen() ) {
                            ...
                            var bankTileModel = ProjectX.getRs2TileObjectCache().query().within(bankTileLoc, 0).nearest();
                        ...
                        if( !Rs2Bank.hasItem(ItemID.BLANKRUNE_HIGH) ) {
                            ProjectX.showMessage("No pure essence found in bank");
                            shutdown();
                            return;
                        }

                        boolean staminaPotNeeded = Rs2Bank.isOpen() && ProjectX.getClient().getEnergy() < 6000 && !Rs2Player.hasStaminaBuffActive();
                        boolean foodNeeded = Rs2Player.getHealthPercentage() < 70;

                        if( staminaPotNeeded ) {
                            if( !Rs2Bank.hasItem("Stamina Potion") ) {
                                ProjectX.showMessage("No Stamina Potions found in bank");
                                shutdown();
                                return;
                            }
                        ...
                        if( foodNeeded ) {
                            if( !Rs2Bank.hasItem(config.foodType().getId()) ) {
                                ProjectX.showMessage("No food found in bank (Item ID: " + config.foodType().getId() + ")");
                                shutdown();
                                return;
                            }
    ...
    private boolean handleAutoSetup(AstralRunesConfig config) {
        ...
        if (!inventorySetup.doesInventoryMatch() || !inventorySetup.doesEquipmentMatch()) {
            if(!openBank()){
                ProjectX.showMessage("Failed to open bank for auto setup! Move closer to a bank or disable auto setup in config");
                return false;
            }
            if (!inventorySetup.loadEquipment() || !inventorySetup.loadInventory()) {
                ProjectX.showMessage("Failed to auto equip inventory! Disable auto setup in config");
                return false;
            }
        }
        ...
        if(isLunar() && !canCastMoonclanTeleport) {
            ProjectX.showMessage("Equipment is correct, but unable to cast Moonclan Teleport! Check if Rune Pouch contains correct runes or disable auto setup in config");
            return false;
        }
        ...
        if(!isLunar() && !isLunarIsleRegion()) {
            ...
            if(!Rs2Bank.hasItem(ItemID.TELEPORTSCROLL_LUNARISLE)) {
                ProjectX.showMessage("No teleport scroll for Lunar Isle found! Make sure bank has Lunar Isle teleport or disable auto setup in config");
                return false;
            }
    ...
    private void setSpellbookLunarAltar() {
        if( isLunarIsleRegion() ) {
            ...
            var altarModel = ProjectX.getRs2TileObjectCache().query().withId(ASTRAL_ALTAR_ID).nearest();
            if( altarModel != null ) {
    ...
    private static void doAltarCraft() {
        var altarModel = ProjectX.getRs2TileObjectCache().query().within(ASTRAL_ALTAR_WORLD_POINT, 0).nearest();
    ...
    private void updateRuneStates() {
        ...
        if (initialRuneCount == null) {
            ...
            ProjectX.log("Baseline rune count set to %d", initialRuneCount);
            return;
        }
