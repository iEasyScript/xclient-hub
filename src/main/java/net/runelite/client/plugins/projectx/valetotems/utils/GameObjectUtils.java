package net.runelite.client.plugins.projectx.valetotems.utils;
...
public class GameObjectUtils {
    ...
    private static GameObject getCachedObjectAtLocationByName(String searchTerm, WorldPoint location) {
        ...
        if (cached != null && !cached.isExpired()) {
            ProjectX.log("Cache HIT for location: " + location + " (searchTerm: " + searchTerm + ")");
            return cached.getGameObject();
        }
        
        // Cache miss or expired - perform expensive search
        ProjectX.log("Cache MISS for location: " + location + " (searchTerm: " + searchTerm + ") - performing search");
        long startTime = System.currentTimeMillis();
        Rs2TileObjectModel tileObjModel = ProjectX.getClientThread().invoke(() ->
                ProjectX.getRs2TileObjectCache().query().withNameContains(searchTerm).nearest(location, 10));
        GameObject gameObject = toGameObject(tileObjModel);
        long searchTime = System.currentTimeMillis() - startTime;
        ProjectX.log("Search completed in " + searchTime + "ms for location: " + location);
    ...
    private static GameObject toGameObject(Rs2TileObjectModel model) {
        ...
        return ProjectX.getClientThread().invoke(() -> {
            var tile = net.runelite.client.plugins.projectx.util.tile.Rs2Tile.getTile(wx, wy);
            if (tile != null) {
    ...
    public static GameObject findNearestObject(int objectId) {
        return toGameObject(ProjectX.getRs2TileObjectCache().query().withId(objectId).nearest());
    }
    ...
    public static GameObject findNearestObjectByName(String searchTerm) {
        Rs2TileObjectModel model = ProjectX.getClientThread().invoke(() ->
                ProjectX.getRs2TileObjectCache().query().withNameContains(searchTerm).nearest());
    ...
    public static GameObject findObjectAtLocation(int objectId, WorldPoint location) {
        return toGameObject(ProjectX.getRs2TileObjectCache().query().withId(objectId).nearest(location, 3));
    }
    ...
    public static List<GameObject> findGameObjects(int objectId, WorldPoint location, int radius) {
        return ProjectX.getRs2TileObjectCache().query().withId(objectId).within(location, radius).toList().stream()
    ...
    public static List<GameObject> findGameObjectsByName(String searchTerm, WorldPoint location, int radius) {
        List<Rs2TileObjectModel> models = ProjectX.getClientThread().invoke(() ->
                ProjectX.getRs2TileObjectCache().query().withNameContains(searchTerm).within(location, radius).toList());
    ...
    public static boolean interactWithObject(GameObject gameObject, String action) {
        var obj = ProjectX.getRs2TileObjectCache().query().withId(gameObject.getId()).nearest(gameObject.getWorldLocation(), 1);
    ...
    public static boolean findAndInteract(int objectId, String action) {
        return ProjectX.getRs2TileObjectCache().query().interact(objectId, action);
    }
    ...
    public static boolean findAndInteractByName(String searchTerm, String action) {
        var obj = ProjectX.getClientThread().invoke(() ->
                ProjectX.getRs2TileObjectCache().query().withNameContains(searchTerm).nearest());
        return obj != null && obj.click(action);
    }
    ...
    public static boolean findAndInteractAtLocation(int objectId, WorldPoint location, String action) {
        var obj = ProjectX.getRs2TileObjectCache().query().withId(objectId).nearest(location, 3);
        return obj != null && obj.click(action);
    }
    ...
    public static boolean objectExistsAtLocation(int objectId, WorldPoint location) {
        return ProjectX.getRs2TileObjectCache().query().withId(objectId).nearest(location, 3) != null;
    }
    ...
    public static boolean objectExistsAtLocationByName(String searchTerm, WorldPoint location) {
        return ProjectX.getClientThread().invoke(() ->
                ProjectX.getRs2TileObjectCache().query().withNameContains(searchTerm).nearest(location, 10)) != null;
    }
    ...
    public static boolean objectExists(int objectId) {
        return ProjectX.getRs2TileObjectCache().query().withId(objectId).nearest() != null;
    }
    ...
    public static int getDistanceToNearestObject(int objectId) {
        var obj = ProjectX.getRs2TileObjectCache().query().withId(objectId).nearest();
        if (obj == null) {
            return -1;
        }
    ...
    public static GameObjectId getTotemStateAtLocation(WorldPoint location) {
        var totemModel = ProjectX.getRs2TileObjectCache().query()
                .where(obj -> GameObjectId.isTotemObject(obj.getId()) || discoveredTotemIds.contains(obj.getId()))
                .nearest(location, 10);

        if (totemModel == null) {
            totemModel = ProjectX.getClientThread().invoke(() ->
            ...
            if (totemModel != null) {
                ...
                ProjectX.log("[ValeTotem] Discovered totem id=" + totemModel.getId()
    ...
    public static GameObjectId getOfferingsStateNearLocation(WorldPoint location, int radius) {
        var nearbyObjects = ProjectX.getRs2TileObjectCache().query()
    ...
    public static GameObject findClaimableOfferings(WorldPoint location, int radius) {
        var model = ProjectX.getRs2TileObjectCache().query()
                .where(obj -> GameObjectId.isOfferingsPile(obj.getId()) || discoveredOfferingIds.contains(obj.getId()))
                .within(location, radius).nearest();

        if (model == null) {
            model = ProjectX.getClientThread().invoke(() ->
                    ProjectX.getRs2TileObjectCache().query()
                            .withNameContains(GameObjectId.OFFERINGS_MANY.getSearchTerm())
                            .within(location, radius).nearest());
            if (model != null) {
                discoveredOfferingIds.add(model.getId());
                ProjectX.log("[ValeTotem] Discovered offering id=" + model.getId()
    ...
    public static boolean waitForObjectAtLocationByName(String searchTerm, WorldPoint location, long timeoutMs) {
        ...
        while (System.currentTimeMillis() - startTime < timeoutMs) {
            if (ProjectX.getClientThread().invoke(() ->
                    ProjectX.getRs2TileObjectCache().query().withNameContains(searchTerm).nearest(location, 10)) != null) {
                return true;
            }
    ...
    public static boolean waitForObjectAtLocation(int objectId, WorldPoint location, long timeoutMs) {
        ...
        while (System.currentTimeMillis() - startTime < timeoutMs) {
            if (ProjectX.getRs2TileObjectCache().query().withId(objectId).nearest(location, 3) != null) {
                return true;
            }
    ...
    public static boolean canReachObject(GameObject gameObject) {
        var obj = ProjectX.getRs2TileObjectCache().query().withId(gameObject.getId()).nearest();
