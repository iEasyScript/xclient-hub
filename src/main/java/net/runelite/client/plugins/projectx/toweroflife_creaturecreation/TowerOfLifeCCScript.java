package net.runelite.client.plugins.projectx.toweroflife_creaturecreation;
...
public class TowerOfLifeCCScript extends Script {
        ...
        switch (currentState)
        {
            ...
            case MOVING_TO_ALTAR:
                if (!arrivedInTower)
                {
                    ...
                    {
                        ProjectX.log("Walking to basement ladder.");
                        Rs2Walker.walkTo(basementLadderLocation);
                    }
                    else
                    {
                        arrivedInTower = true;
                        ProjectX.log("Arrived in tower!");
                    }
                }
                else
                {
                    ...
                    {
                        Rs2TileObjectModel trapdoor = ProjectX.getRs2TileObjectCache().query().within(new WorldPoint(2648, 3212, 0), 1).nearest();
                        if (trapdoor != null)
                        {
                            ProjectX.log("Trapdoor id: " + trapdoor.getId());
                            trapdoor.click("Climb-down");
                        }
                        else
                        {
                            ProjectX.log("Trapdoor null");
                        }
                    }
                    else if (inBasement)
                    {
                        ...
                        {
                            ProjectX.log("Arrived in basement! Switching state");
    ...
    void HandleSummonIngredientAtBank(int disposableItemID, int secondaryItemID, ToLCreature _creature)
    {
        ...
        {
            ProjectX.showMessage("Missing items in bank! Shutting down.");
            ProjectX.stopPlugin(TowerOfLifeCCPlugin.class);
            return;
        }

        if (ProjectX.getVarbitValue(VarbitID.ARDOUGNE_DIARY_MEDIUM_COMPLETE) == 1)
        {
            int numItemsToWithdraw = (int)Math.floor(Rs2Inventory.emptySlotCount() * 0.5);

            //ProjectX.log("Number of secondary in bank: " + Rs2Bank.count(secondaryItemID));
    ...
    void HandleCreature(LootingParameters params, int disposableItem, int secondaryItem)
    {
        ...
        if (summonedCreature == null)
        {
            ...
            if (System.currentTimeMillis() - lastKillTime < 2000) {
                //ProjectX.log("Waiting a couple seconds since kill time before acting");
                return;
            }
            ...
            if (Rs2Inventory.hasItem(disposableItem))
            {
                ...
                //Rs2Inventory.waitForInventoryChanges(3000);
                ProjectX.getRs2TileObjectCache().query().interact(altarObjectId, "Activate");
                sleepUntil(() -> { summonedCreature = ProjectX.getRs2NpcCache().query()
                        .where(npc -> npc.getNpc() != null
                                && (npc.getNpc().getInteracting() == null || npc.getNpc().getInteracting() == ProjectX.getClient().getLocalPlayer()))
                        .nearest();
                    return summonedCreature != null;
                }, 5000);
                //ProjectX.log("Summoned creature");
            }
            else
            {
