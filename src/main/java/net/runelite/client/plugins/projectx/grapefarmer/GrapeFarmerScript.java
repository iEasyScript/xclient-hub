...
import net.runelite.client.plugins.projectx.grapefarmer.GrapeFarmerConfig;
...
public class GrapeFarmerScript extends Script {
            ...
            try {
                ...
                for (Map.Entry<Integer, Integer> entry : patchMap.entrySet()) {
                    ...
                    while (true) {
                        ...
                        int currentVarbitValue = ProjectX.getVarbitValue(varbitKey);
    ...
    private void plantSeed(int gameObjectId) {
        ...
        if (Rs2Inventory.use(grapeSeed)) {
            ProjectX.getRs2TileObjectCache().query().withId(gameObjectId).interact();
            Rs2Player.waitForXpDrop(Skill.FARMING);
        }
    }

    private void addSaltpetre(int gameObjectId) {
        if (ProjectX.getRs2TileObjectCache().query().withId(gameObjectId).interact()) {
            sleep(4000, 4500);
        }
    ...
    private void pickGrapes(int gameObjectId) {
        Rs2NpcModel leprechaun = ProjectX.getRs2NpcCache().query().withId(0).nearest();
        if (leprechaun != null) {
        ...
        if (!Rs2Inventory.isFull() && ProjectX.getRs2TileObjectCache().query().withId(gameObjectId).interact()) {
