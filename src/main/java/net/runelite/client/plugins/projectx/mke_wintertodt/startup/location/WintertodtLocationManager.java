package net.runelite.client.plugins.projectx.mke_wintertodt.startup.location;
...
public class WintertodtLocationManager {
    ...
    public static boolean attemptLeaveWintertodt()
    {
        ...
        if (Rs2Player.getWorldLocation().distanceTo(doorTile) > 4) {
            ProjectX.log("Walking closer to Wintertodt exit door...");
        ...
        if (Rs2Widget.hasWidget("Leave and lose")
            ...
            ProjectX.log("Confirming leave dialog...");
        ...
        Rs2TileObjectModel exitDoor = ProjectX.getRs2TileObjectCache().query().withId(ObjectID.DOORS_OF_DINH).within(6).nearest();
        if (exitDoor != null) {
            ProjectX.log("Clicking Wintertodt exit door...");
            ...
            ProjectX.log("Could not find Wintertodt exit door, trying to walk closer...");
        ...
        ProjectX.log("Still inside Wintertodt, retrying leave sequence...");
        return false;
    }
    ...
    public boolean navigateToWintertodt() {
        try {
            ...
            if (isAtWintertodt()) {
                ...
                ProjectX.log("Player is already at Wintertodt - no navigation needed.");
                return true;
            }
            ...
            if (isInsideGameRoom()) {
                ...
                ProjectX.log("Player is inside Wintertodt game room, attempting custom door exit...");
                
                // Use custom door leaving logic - never webwalker
                if (attemptLeaveWintertodt()) {
                    ProjectX.log("Successfully left Wintertodt game room");
                    walkToWintertodtBank = true;
                } else {
                    ProjectX.log("Still attempting to leave Wintertodt game room...");
            ...
            if (tilesToWintertodBank < FAR_FROM_BANK_THRESHOLD && tilesToWintertodBank > 0) {  
                ProjectX.log("Already close to Wintertodt - only walking to Wintertodt required (tilesToWintertodBank: " + tilesToWintertodBank + ")");
                walkToWintertodtBank = true;
            }
            ...
            if (walkToWintertodtBank) {
                ...
                if (sleepUntilTrue(this::isAtWintertodt, 1000, 180000)) { // 3 minute for walking
                    ...
                    ProjectX.log("Successfully walked to Wintertodt");
                    return true;
                } else {
                    ProjectX.log("Timeout while walking to Wintertodt - may still be travelling");
                }
            }
            ...
            if (hasGamesNecklace()) {
                ...
                ProjectX.log("Found Games necklace - using direct teleport to Wintertodt");
                ...
                if (sleepUntilTrue(this::isAtWintertodt, 1000, 20000)) { // 20 seconds for teleport
                    ...
                    ProjectX.log("Successfully teleported to Wintertodt");
                    return true;
                }
            }
            ...
            if (!handleBankingPhase(tilesToWintertodBank)) {
                ProjectX.log("Failed to complete banking phase");
            ...
            ProjectX.log("Using Rs2Walker to navigate to Wintertodt - will automatically use best route and teleports");
            ...
            if (sleepUntilTrue(this::isAtWintertodt, 1000, 240000)) { // 4 minute timeout
                ...
                ProjectX.log("Successfully navigated to Wintertodt using webwalker");
                return true;
            } else {
                ProjectX.log("Timeout while navigating to Wintertodt - may still be travelling");
                return false;
            }
            
        } catch (Exception e) {
            ProjectX.log("Error during navigation: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
        ...
        try {
            ...
            for (int gamesNecklaceId : GAMES_NECKLACE_ITEMS) {
                if (Rs2Inventory.hasItem(gamesNecklaceId)) {
                    ProjectX.log("Found Games necklace (ID: " + gamesNecklaceId + ") in inventory");
                    return true;
                }
            }
            ...
            for (int gamesNecklaceId : GAMES_NECKLACE_ITEMS) {
                if (Rs2Equipment.isWearing(gamesNecklaceId)) {
                    ProjectX.log("Found Games necklace (ID: " + gamesNecklaceId + ") equipped");
                    return true;
                }
            }
            ...
            ProjectX.log("Error checking for Games necklace: " + e.getMessage());
            return false;
        }
    }
    ...
    private boolean handleBankingPhase(int tilesToWintertodBank) {
        try {
            ...
            if (tilesToWintertodBank < tilesToClosestBank + 50) {
                ProjectX.log("Its faster to walk to Wintertodt than to walk other bank.");
                return true;
            }
            
            if (!Rs2Bank.walkToBank()) {
                ProjectX.log("Failed to reach bank, maybe missclicked");
            ...
            if (!bankUnnecessaryItems()) {
                ProjectX.log("Failed to bank items");
                return false;
            }
            ...
            if (!ensureTeleportItems()) {
                ProjectX.log("Failed to get teleport items");
                return false;
            }
            ...
            ProjectX.log("Error in banking phase: " + e.getMessage());
            return false;
        }
    }
    ...
    private boolean bankUnnecessaryItems() {
        try {
            ...
            ProjectX.log("Error banking unnecessary items: " + e.getMessage());
            return false;
        }
    }
    ...
    private boolean ensureTeleportItems() {
        try {
            ...
            if (!hasGamesNecklace) {
                ProjectX.log("No Games necklace available - getting emergency teleports");
            ...
            ProjectX.log("Error ensuring teleport items: " + e.getMessage());
            return false;
        }
    }
    ...
    public void reset() {
        ...
        ProjectX.log("Location manager state reset - ready for fresh navigation");
    }
