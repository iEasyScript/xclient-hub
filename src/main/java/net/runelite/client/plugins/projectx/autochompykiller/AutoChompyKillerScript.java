package net.runelite.client.plugins.projectx.autochompykiller;
...
public class AutoChompyKillerScript extends Script {
    ...
    private boolean isBloatedToadOnGround() {
        long toadCount = ProjectX.getRs2NpcCache().query().withId(NpcID.BLOATED_TOAD).where(element -> element.getWorldLocation().equals(Rs2Player.getWorldLocation())).count();

        return toadCount > 0;
    }

    private boolean isDeadChompyNearby() {
        long deadChompyCount = ProjectX.getRs2NpcCache().query().withId(NpcID.CHOMPYBIRD_DEAD).where(element -> Rs2Player.getWorldLocation().distanceTo(element.getWorldLocation()) <= 5).count();
    ...
    private Rs2NpcModel getNearestReachableNpc(int npcId) {
        ...
        return ProjectX.getRs2NpcCache().query().withId(npcId).toList().stream()
    ...
    private void manageRunEnergy(AutoChompyKillerConfig config) {
        if (ProjectX.getClient().getEnergy() >= 20) return;
    ...
    private void handleLogoutOnCompletion(String reason) {
        ProjectX.showMessage(reason + " - waiting to logout...");
        ProjectX.status = "Waiting to logout";
        
        sleepUntil(() -> !Rs2Player.isInCombat(), 30000);
        
        if (Rs2Player.isInCombat()) {
            ProjectX.showMessage("Still in combat after 30 seconds - logging out anyway...");
        }
        
        ProjectX.status = "Logging out";
        Rs2Player.logout();
        sleepUntil(() -> !ProjectX.isLoggedIn(), 5000);
        
        if (ProjectX.isLoggedIn()) {
            ProjectX.showMessage("Logout failed - stopping script...");
        }
    ...
    private boolean checkStopConditions(AutoChompyKillerConfig config) {
        if (config.stopOnKillCount() && chompyKills >= config.killCount()) {
            ...
            if (config.logoutOnCompletion()) {
                ...
                ProjectX.showMessage(reason + " - stopping");
                ProjectX.status = "IDLE";
            }
            return true;
        }
    ...
    private boolean validateConfig() {
        if (config == null) {
            ...
            ProjectX.showMessage("Configuration error");
            return false;
        }
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (!Rs2Equipment.isWearing(EquipmentInventorySlot.AMMO)) {
                    ...
                    ProjectX.showMessage("No ammo - aborting...");
                    ProjectX.status = "IDLE";
                ...
                if (!Rs2Equipment.isWearing(ItemID.OGRE_BOW) && !Rs2Equipment.isWearing(ItemID.ZOGRE_BOW)) {
                    ...
                    ProjectX.showMessage("No ogre bow equipped - aborting...");
                    ProjectX.status = "IDLE";
                    // wait briefly before shutdown to display message
                    sleepUntil(() -> true, 3000);
                    shutdown();
                ...
                switch (state) {
                    case FILLING_BELLOWS:
                        ...
                        ProjectX.status = "Filling bellows";
                        if (ProjectX.getRs2TileObjectCache().query().interact(ObjectID.SWAMPBUBBLES, "Suck")) {
                    ...
                    case INFLATING:
                        ...
                        ProjectX.status = "Inflating toads";
                    ...
                    case ATTACKING:
                        ...
                        ProjectX.status = "Attacking chompy";
                    ...
                    case PLUCKING:
                        ...
                        ProjectX.status = "Plucking chompy";
    ...
    public void handleNotMyChompy() {
        ProjectX.showMessage("Someone else is hunting chompys in this world - aborting...");
        ProjectX.status = "IDLE";
        shutdown();
    }

    public void handleBowNotPowerfulEnough() {
        ProjectX.showMessage("Your bow isn't powerful enough for those arrows - aborting...");
        ProjectX.status = "IDLE";
        shutdown();
    }
    ...
    public void handleCantReachBubbles() {
        var bubbles = ProjectX.getRs2TileObjectCache().query().withId(ObjectID.SWAMPBUBBLES).toList();
        if (bubbles != null && !bubbles.isEmpty()) {
            ...
            if (bubble != null) {
                ...
                ProjectX.showMessage("No bubbles available - stopping");
                ProjectX.status = "IDLE";
                shutdown();
            }
        } else {
            ProjectX.showMessage("No bubbles found - stopping");
            ProjectX.status = "IDLE";
            shutdown();
        }
    }
