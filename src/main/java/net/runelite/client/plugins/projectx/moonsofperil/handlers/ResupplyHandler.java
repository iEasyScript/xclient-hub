...
public class ResupplyHandler implements BaseHandler {
    ...
    private boolean checkSupplies(int moonlightPotionMinimum, int cookedBreamMinimum) {
        if ((countMoonlightPotions() < moonlightPotionMinimum)
            ...
            if (debugLogging) {ProjectX.log("No need to resupply right now.");}
        }
        return false;
    }

    private void walkToSupplies() {
        if (debugLogging) {ProjectX.log("Attempting to walk to supply area 1");}
        Rs2Walker.walkWithState(supplyLocation, 0);
        if (debugLogging) {ProjectX.log("Arrived at supply area 1");}
    }

    private void makeMoonlightPotions(int moonlightPotionsQuantum) {
        if (debugLogging) {ProjectX.log("Need a total of " + moonlightPotionsQuantum + " Moonlight potions");}
        int amountToCreate = checkPotionQuantum(moonlightPotionsQuantum);
        if (debugLogging) {ProjectX.log("Need to create " + amountToCreate + " Moonlight potions");}
        ...
        if ((amountToCreate > 0) && (Rs2Player.distanceTo(supplyLocation) <= 10)) {
            ...
            while (Rs2Inventory.count(ItemID.VIAL_WATER) < amountToCreate) {
                if (debugLogging) {ProjectX.log("Take herblore supplies from supply crate");}
                if (ProjectX.getRs2TileObjectCache().query().interact(ObjectID.PMOON_SUPPLY_CRATE, "Take from")) {
            ...
            while (Rs2Inventory.count(ItemID.MOONLIGHT_GRUB)
                ...
                if (debugLogging) {ProjectX.log("Collect Moonlight Grub");}
                if (ProjectX.getRs2TileObjectCache().query().interact(ObjectID.PMOON_GRUB_SAPLING, "Collect-from")) {
            ...
            if (Rs2Inventory.contains(ItemID.PESTLE_AND_MORTAR)) {
                ...
                if (debugLogging) {ProjectX.log("Need a pestle and mortar!");}
            }
    ...
    private boolean ensureBigNet() {
        ...
        while (!Rs2Inventory.contains(ItemID.BIG_NET) && System.currentTimeMillis() - start < 15_000) {
            ...
            if (ProjectX.getRs2TileObjectCache().query().interact(ObjectID.PMOON_SUPPLY_CRATE, "Take from")) { // interact exists :contentReference[oaicite:1]{index=1}
        ...
        if (!Rs2Inventory.contains(ItemID.BIG_NET)) {
            if (debugLogging) ProjectX.log("Failed to obtain BIG_NET (timeout).");
            return false;
        }
    ...
    private void fishBream() {
        ...
        while (!Rs2Inventory.isFull()) {
            ...
            if (!Rs2Inventory.contains(ItemID.BIG_NET)) {
                if (debugLogging) ProjectX.log("BIG_NET missing while fishing; returning to crate.");
                return;
            }

            if (!Rs2Player.isAnimating()) {
                ProjectX.getRs2TileObjectCache().query().interact(51367, "Fish");
                sleep(3000, 4000);
            }
            sleep(300, 500);
        }

        if (debugLogging) ProjectX.log("Inventory should now be full of fish");
        sleep(600, 900);
    ...
    private void cookBream() {
        if (debugLogging) {ProjectX.log("Walking to cooking stove");}
        Rs2Walker.walkFastCanvas(new WorldPoint(1512, 9693, 0));

        while (Rs2Inventory.contains(ItemID.BREAM_FISH_RAW)) {
            if (!Rs2Player.isAnimating()) {
                if (ProjectX.getRs2TileObjectCache().query().interact(ObjectID.PMOON_RANGE, "Cook")) {
                    sleep(600, 900);
                }
            }
            sleep(900, 1200);
        }
        if (debugLogging) {ProjectX.log("Finished cooking bream.");}
    }
    ...
    private int checkPotionQuantum(int target)
    {
        ...
        if (debugLogging) {ProjectX.log("Required free inventory slots: " + requiredSlots);}
        int freeSlots     = Rs2Inventory.emptySlotCount();
        if (debugLogging) {ProjectX.log("Current free inventory slots: " + freeSlots);}
