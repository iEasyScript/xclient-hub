...
import net.runelite.client.plugins.projectx.util.gameobject.Rs2BankID;
...
public class BanksShopperScript extends Script {
    ...
    public boolean run(BanksShopperConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                if (!super.run() || !ProjectX.isLoggedIn() || Rs2AntibanSettings.actionCooldownActive) return;
                ...
                switch (state) {
                    case SHOPPING:
                        ...
                        if (missingAllRequiredItems && plugin.getSelectedAction() == Actions.SELL) {
                            ProjectX.status = "[Shutting down] - Reason: Not enough supplies.";
                        ...
                        if (Rs2Shop.isOpen()) {
                            ...
                            for (String itemName : plugin.getItemNames()) {
                                if (!isRunning() || ProjectX.pauseAllScripts.get()) break;
    ...
    private boolean tryDirectBankDeposit() {
        ...
        if (!alreadyOpen) {
            ...
            if (bankObject != null) {
                ProjectX.log("[BanksShopper] direct-click bank id=" + bankObject.getId()
                ...
                if (!sleepUntil(() -> Rs2Bank.isOpen() || Rs2DepositBox.isOpen(), 15_000)) {
                    ProjectX.log("[BanksShopper] click sent but no bank/deposit UI opened");
                    return false;
                }
            } else {
    ...
    private void hopWorld() {
        ...
        sleepUntil(() -> ProjectX.hopToWorld(world), 15000);
