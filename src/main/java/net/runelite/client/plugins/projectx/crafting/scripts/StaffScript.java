...
import net.runelite.client.plugins.projectx.crafting.enums.Staffs;
...
public class StaffScript extends Script implements ICraftingScript {
    ...
    private void verifyItemInBank(String item) {
        if (Rs2Bank.isOpen() && !Rs2Bank.hasItem(item)) {
            Rs2Bank.closeBank();
            ProjectX.status = "[Shutting down] - Reason: " + item + " not found in the bank.";
            ProjectX.getNotifier().notify(ProjectX.status);
            shutdown();
        }
    }
    ...
    public ProgressiveStaffmakingModel calculateItemToCraft() {
        int craftingLvl = ProjectX.getClient().getRealSkillLevel(Skill.CRAFTING);
