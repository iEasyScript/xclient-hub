package net.runelite.client.plugins.projectx.shootingstar.enums;
...
public enum ShootingStarLocation
{
	...
	public boolean hasRequirements()
	{
		...
		boolean hasLineOfSight = Boolean.TRUE.equals(ProjectX.getClientThread().runOnClientThreadOptional(() -> {
			if (ProjectX.getClient().getLocalPlayer() == null || ProjectX.getClient().getLocalPlayer().getWorldArea() == null)
			{
				return false;
			}
			return ProjectX.getClient().getLocalPlayer().getWorldArea().hasLineOfSightTo(ProjectX.getClient().getTopLevelWorldView(), this.getWorldPoint());
		}).orElse(false));
		switch (this)
		{
			case CRAFTING_GUILD:
			...
			case CUSTODIA_MOUNTAINS:
				...
				return ProjectX.getVarbitValue(VarbitID.VARLAMORE_VISITED) == 1;
			case CORSAIR_COVE:
