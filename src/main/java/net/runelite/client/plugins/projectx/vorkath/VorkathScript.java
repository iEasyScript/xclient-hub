...
package net.runelite.client.plugins.projectx.vorkath;
...
public class VorkathScript extends Script {
    ...
    private static void walkToCenter() {
        Rs2Walker.walkFastLocal(
                LocalPoint.fromScene(48, 58, ProjectX.getClient().getTopLevelWorldView().getScene())
        );
    }
    ...
    private void calculateState() {
        if (ProjectX.getRs2NpcCache().query().withId(NpcID.VORKATH).first() != null) {
            state = State.FIGHT_VORKATH;
            return;
        }
        if (ProjectX.getRs2NpcCache().query().withId(NpcID.VORKATH_SLEEPING).first() != null) {
            state = State.PREPARE_FIGHT;
            return;
        }
        if (ProjectX.getRs2TileObjectCache().query().withId(ObjectID.UNGAEL_CRATER_ENTRANCE).first() != null) {
        ...
        if (ProjectX.getRs2NpcCache().query().withId(NpcID.TORFINN_COLLECT_UNGAEL).first() != null) {
            state = State.WALK_TO_VORKATH;
        }
    }
            ...
            try {
                ...
                if (init) {
                    ...
                    if (!rs2InventorySetup.hasSpellBook()) {
                        ProjectX.showMessage("Your spellbook is not matching the inventory setup.");
                        sleep(10000);
                        return;
                    }
                    calculateState();
                ...
                switch (state) {
                    case BANKING:
                        ...
                        if (!init && Rs2Equipment.get(EquipmentInventorySlot.AMMO) == null) {
                            ProjectX.showMessage("Out of ammo!");
                            sleep(5000);
                            return;
                        }
                    ...
                    case TELEPORT_TO_RELLEKKA:
                        ...
                        if (!isCloseToRelleka()) {
                            ...
                            if (config.pohInRellekka()) {
                                teleToPoh();
                                sleepUntil(() -> ProjectX.getRs2TileObjectCache().query().withId(4525).first() != null);
                                Rs2TileObjectModel pohPortal = ProjectX.getRs2TileObjectCache().query().withId(4525).first();
                                if (pohPortal != null) {
                    ...
                    case WALK_TO_VORKATH_ISLAND:
                        ...
                        Rs2NpcModel torfin = ProjectX.getRs2NpcCache().query().withId(NpcID.TORFINN_COLLECT_RELLEKKA).first();
                        if (torfin != null) {
                            torfin.click("Ungael");
                            sleepUntil(() -> ProjectX.getRs2NpcCache().query().withId(NpcID.TORFINN_COLLECT_UNGAEL).first() != null);
                        }
                        if (ProjectX.getRs2NpcCache().query().withId(NpcID.TORFINN_COLLECT_UNGAEL).first() != null) {
                            state = State.WALK_TO_VORKATH;
                        }
                    ...
                    case WALK_TO_VORKATH:
                        ...
                        Rs2TileObjectModel iceChunks = ProjectX.getRs2TileObjectCache().query().withId(ObjectID.UNGAEL_CRATER_ENTRANCE).first();
                        if (iceChunks != null) {
                            iceChunks.click("Climb-over");
                            sleepUntil(() -> ProjectX.getRs2TileObjectCache().query().withId(ObjectID.UNGAEL_CRATER_ENTRANCE).first() == null);
                        }
                        if (ProjectX.getRs2TileObjectCache().query().withId(ObjectID.UNGAEL_CRATER_ENTRANCE).first() == null) {
                            state = State.PREPARE_FIGHT;
                        }
                    ...
                    case PREPARE_FIGHT:
                        ...
                        if (result) {
                            Rs2NpcModel sleepingVorkath = ProjectX.getRs2NpcCache().query().withId(NpcID.VORKATH_SLEEPING).first();
                            if (sleepingVorkath != null) {
                                sleepingVorkath.click("Poke");
                                Rs2Player.waitForWalking();
                                sleepingVorkath = ProjectX.getRs2NpcCache().query().withId(NpcID.VORKATH_SLEEPING).first();
                                if (sleepingVorkath != null) {
                                    sleepingVorkath.click("Poke");
                                }
                            }
                            ...
                            Rs2Player.waitForWalking();
                            handlePrayer();
                            sleepUntil(() -> ProjectX.getRs2NpcCache().query().withId(NpcID.VORKATH).first() != null);
                        ...
                        break;
                    case FIGHT_VORKATH:
                        vorkath = ProjectX.getRs2NpcCache().query().withId(NpcID.VORKATH).first();
                        if (vorkath == null || vorkath.isDead()) {
                            ...
                            togglePrayer(false);
                            sleepUntil(() -> ProjectX.getRs2TileItemCache().query().withName("Superior dragon bones").within(20).first() != null, 15000);
                            return;
                        }
                        if (ProjectX.getClient().getBoostedSkillLevel(Skill.HITPOINTS) <= 0) {
                        ...
                        if (!Rs2Combat.inCombat() && vorkath.click("Attack"))
                            sleep(600);
                        if (ProjectX.getClient().getLocalPlayer().getLocalLocation().getSceneY() >= 59) {
                            walkToCenter();
                        }
                        drinkPotions();
                        handlePrayer();
                    ...
                    case ZOMBIE_SPAWN:
                        ...
                        Rs2Player.eatAt(80);
                        drinkPrayer();
                        Rs2NpcModel zombieSpawn = ProjectX.getRs2NpcCache().query().withName(ZOMBIFIED_SPAWN).firstOnClientThread();
                        if (zombieSpawn != null) {
                            Rs2NpcModel currentSpawn;
                            while ((currentSpawn = ProjectX.getRs2NpcCache().query().withName(ZOMBIFIED_SPAWN).firstOnClientThread()) != null
                                    && !currentSpawn.isDead()
                            ...
                            state = State.FIGHT_VORKATH;
                            sleepUntil(() -> ProjectX.getRs2NpcCache().query().withName(ZOMBIFIED_SPAWN).firstOnClientThread() == null);
                            if (doesProjectileExistById(redProjectileId)) {
                                handleRedBall();
                                sleep(300);
                            }
                    ...
                    case LOOT_ITEMS:
                        ...
                        var vorkathHead = ProjectX.getRs2TileItemCache().query().withName("Vorkath's head").within(20).first();
                        if (vorkathHead != null) {
                            vorkathHead.pickup();
                        }
                        ...
                        if (!Rs2GroundItem.isItemBasedOnValueOnGround(config.priceOfItemsToLoot(), 20)
                                && ProjectX.getRs2TileItemCache().query().withName("Vorkath's head").within(20).first() == null) {
                            if (config.KillsPerTrip() > 0 && kcPerTrip >= config.KillsPerTrip()) {
                                leaveVorkath();
                            }
                    ...
                    case DEAD_WALK:
                        if (isCloseToRelleka()) {
                            Rs2Walker.walkTo(new WorldPoint(2640, 3693, 0));
                            torfin = ProjectX.getRs2NpcCache().query().withId(NpcID.TORFINN_COLLECT_RELLEKKA).first();
                            if (torfin != null) {
    ...
    private boolean checkSellingItems(VorkathConfig config) {
        ...
        LootTrackerRecord lootRecord = ProjectX.getAggregateLootRecords("vorkath");
        if (lootRecord != null) {
    ...
    private void healAndDrinkPrayerPotion() {
        ...
        while (ProjectX.getClient().getRealSkillLevel(Skill.PRAYER) != ProjectX.getClient().getBoostedSkillLevel(Skill.PRAYER) && Rs2Inventory.hasItem(Rs2Potion.getPrayerPotionsVariants().toArray(String[]::new))) {
            Rs2Bank.closeBank();
            Rs2Inventory.interact(Rs2Potion.getPrayerPotionsVariants().toArray(String[]::new), "drink");
            Rs2Player.waitForAnimation();
            hasInventory = false;
    ...
    private void leaveVorkath() {
        ...
        switch (config.teleportMode()) {
            ...
            case JEWELLERY_BOX:
                ...
                if (config.rejuvinationPool()) {
                    Rs2TileObjectModel pool = ProjectX.getRs2TileObjectCache().query().withId(29241).first();
                    if (pool != null) {
                        pool.click("Drink");
                        sleepUntil(() -> Rs2Player.isFullHealth());
                    }
                }
                Rs2TileObjectModel jewelleryBox = ProjectX.getRs2TileObjectCache().query().withId(29156).first();
        ...
        Rs2Player.waitForAnimation();
        sleepUntil(() -> !ProjectX.getClient().isInInstancedRegion());
    ...
    private boolean drinkPotions() {
        ...
        if (!ProjectX.getClient().getLocalPlayer().isInteracting() && state == State.PREPARE_FIGHT && (drinkRangePotion || drinkAntiFire || drinkAntiVenom))
            Rs2Player.waitForAnimation();
    ...
    public void togglePrayer(boolean onOff) {
        ...
        if (ProjectX.getClient().getRealSkillLevel(Skill.PRAYER) >= 74 && ProjectX.getClient().getRealSkillLevel(Skill.DEFENCE) >= 70 && config.activateRigour()) {
    ...
    private void handleRedBall() {
        if (doesProjectileExistById(redProjectileId)) {
            redBallWalk();
            Rs2NpcModel vorkathNpc = ProjectX.getRs2NpcCache().query().withName("Vorkath").firstOnClientThread();
            if (vorkathNpc != null) {
    ...
    private boolean doesProjectileExistById(int id) {
        for (Projectile projectile : ProjectX.getClient().getProjectiles()) {
    ...
    private boolean isCloseToRelleka() {
        if (ProjectX.getClient().getLocalPlayer() == null) return false;
        return ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation().distanceTo(new WorldPoint(2670, 3634, 0))) < 80;
    }
    ...
    private void redBallWalk() {
        WorldPoint currentPlayerLocation = ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation());
        ...
        Rs2Walker.walkFastLocal(LocalPoint.fromWorld(ProjectX.getClient(), _sideStepLocation));
        Rs2Player.waitForWalking();
        sleepUntil(() -> _sideStepLocation.equals(ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation())));
    }
    ...
    private void handleAcidWalk() {
        Object[] clientState = ProjectX.getClientThread().invoke(() -> {
            ...
            List<WorldPoint> acidTiles = ProjectX.getRs2TileObjectCache().query()
            ...
            WorldPoint playerLoc = ProjectX.getClient().getLocalPlayer().getWorldLocation();
        ...
        if (safeTile != null) {
            if (playerLocation.equals(safeTile)) {
                ...
                Rs2Player.eatAt(60);
                Rs2Walker.walkFastLocal(LocalPoint.fromWorld(ProjectX.getClient(), safeTile));
            }
        }
    }

    private void testWooxWalk() {
        vorkath = ProjectX.getRs2NpcCache().query().withId(NpcID.VORKATH_SLEEPING).first();
