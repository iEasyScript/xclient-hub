package net.runelite.client.plugins.projectx.TearsOfGuthix;
...
public class TearsOfGuthixScript extends Script {
    ...
    public boolean run(TearsOfGuthixConfig config) {
        ProjectX.log("Starting Tears of Guthix script");
                ...
                switch (BOT_STATUS) {
                    case TRAVELLING:
                        if (Rs2Player.getWorldLocation().getRegionID() == TOG_REGION) {
                            ProjectX.log("Already in Tears of Guthix region, collecting tears...");
                            BOT_STATUS = State.GETTING_WORLD;
                            break;
                        }
                        var junaLocation = new WorldPoint(3251, 9516, 2);
                        if (!Rs2Inventory.hasItem("Games necklace")) {
                            ProjectX.log("No games necklace in inventory, going to bank...");
                            Rs2Bank.walkToBank();
                            Rs2Bank.openBank();
                            ...
                            if (Rs2Inventory.hasItem("Games necklace")) {
                                ...
                                ProjectX.log("No games necklace in bank, stopping script.");
                                shutdown();
                            }
                        } else {
                            ProjectX.log("Travelling to Tears of Guthix...");
                    ...
                    case GETTING_WORLD:
                        if (config.queryForOptimalWorld()) {
                            ProjectX.log("Querying for optimal world...");
                            getBestWorld();
                        } else {
                            ProjectX.log("Skipping world query, using current world.");
    ...
    private void getTears() {
        ...
        if (Rs2Player.getWorldLocation().getRegionID() != TOG_REGION) {
            if (hasBeenRunning) {
                ProjectX.log("Done collecting tears - shutting down.");
                shutdown();
            }
            ProjectX.log("Not in Tears of Guthix region, travelling...");
    ...
    private void collectTears() {
        ...
        if (currentStream != null && currentStreamSpawnTime != null) {
            ...
            if (timeRemaining > 0) {
                ...
                ProjectX.log(status + " stream: ~" + timeRemaining + "s remaining" + timeSinceClick);
            }
        }
    ...
    private void clickStream(DecorativeObject stream, long timeRemaining) {
        ...
        ProjectX.getClientThread().invoke(() -> {
            ProjectX.getMouse().click(stream.getCanvasLocation());
            ProjectX.log("Switching to blue tear stream (Time left: ~" + timeRemaining + "s, Distance: " + distance + " tiles)");
            return true;
        });
    }
    ...
    private void getBestWorld() {
        if (bestWorld != 0) {
            if (ProjectX.getClient().getWorld() == bestWorld) {
                ProjectX.log("Already in the best world: " + bestWorld);
                BOT_STATUS = State.GETTING_TEARS;
                return;
            }
            boolean isHopped = ProjectX.hopToWorld(bestWorld);
            if (!isHopped) return;
        ...
        ProjectX.log("Fetching optimal world for tears collection...");
        ...
        try {
            ...
            ProjectX.log("Fetched data for " + worldData.size() + " worlds");
            ...
            if (optimalWorld != null) {
                ...
                ProjectX.log("Found optimal world: " + bestWorld +
                        " (Hits: " + optimalWorld.getHits() +
                        ", Pattern: " + optimalWorld.getStream_order() + ")");

            } else {
                ProjectX.log("No optimal world found, continuing with current world");
                BOT_STATUS = State.GETTING_TEARS;
            }
        } catch (Exception e) {
            ProjectX.log("Error fetching TOG world data: " + e.getMessage());
    ...
    public void shutdown() {
        ...
        ProjectX.log("Shutting down Tears of Guthix script");
    }
