package net.runelite.client.plugins.projectx.mke_wintertodt;
...
import net.runelite.client.plugins.projectx.mke_wintertodt.enums.HealingMethod;
...
import net.runelite.client.plugins.projectx.mke_wintertodt.enums.State;
import net.runelite.client.plugins.projectx.mke_wintertodt.startup.WintertodtStartupManager;
import net.runelite.client.plugins.projectx.mke_wintertodt.startup.gear.WintertodtAxeManager;
import net.runelite.client.plugins.projectx.mke_wintertodt.startup.gear.WintertodtGearManager;
import net.runelite.client.plugins.projectx.mke_wintertodt.startup.location.WintertodtLocationManager;
import net.runelite.client.plugins.projectx.mke_wintertodt.startup.inventory.WintertodtInventoryManager;
...
import static net.runelite.client.plugins.projectx.util.inventory.Rs2Inventory.isItemSelected;
import static net.runelite.client.plugins.projectx.util.player.Rs2Player.eatAt;
...
public class MKE_WintertodtScript extends Script {
    ...
    public static class FletchingState {
        ...
        public void startFletching() {
            ...
            ProjectX.log("Fletching started - tracking " + rootsAtStart + " roots");
        }
        ...
        public void stopFletching(FletchingInterruptType reason) {
            if (isActive) {
                ...
                ProjectX.log("Fletching stopped: " + reason.getDescription() + " (Duration: " + duration + "ms)");
            }
        }
    ...
    public static class FeedingState {
        ...
        public void startFeeding() {
            ...
            warmthAtStart = getWarmthLevel();
            lastInterruptType = null;
            ProjectX.log("Feeding started - tracking " + itemsAtStart + " items");
        }
        ...
        public void stopFeeding(FeedingInterruptType reason) {
            if (isActive) {
                ...
                lastInterruptTime = System.currentTimeMillis();
                long duration = lastInterruptTime - startTime;
                ProjectX.log("Feeding stopped: " + reason.getDescription() + " (Duration: " + duration + "ms)");
            }
        }
    ...
    public static void handleChatInterruption(String message) {
        ...
        if (fletchingState.isActive()) {
            ...
            if (message.startsWith("The cold of")) {
                ...
                ProjectX.log("Brazier fixed - fletching can resume");
                ...
                ProjectX.log("Brazier lit - fletching can resume");
                return; // Don't stop fletching, just log
            }
        ...
        if (feedingState.isActive()) {
            ...
            if (message.startsWith("The cold of")) {
                ...
                ProjectX.log("Brazier fixed - feeding can resume");
                return; // Don't stop feeding, just log
            } else if (message.startsWith("You light the brazier")) {
                // Brazier was lit - allow feeding to resume
                ProjectX.log("Brazier lit - feeding can resume");
                return; // Don't stop feeding, just log
            }
    ...
    private static void parseRewardCartMessage(String message) {
        try {
            ...
            if (parts.length >= 2) {
                ...
                if (rewardSplit.length >= 1) {
                    ...
                    if (newRewardCount != currentRewardCartRewards) {
                        ...
                        ProjectX.log("Reward Cart: " + previousRewards + " -> " + currentRewardCartRewards + " rewards owed");
            ...
            ProjectX.log("Failed to parse reward cart message: " + message + " - " + e.getMessage());
        }
    }
    ...
    public static void onHitsplatApplied(HitsplatApplied hitsplatApplied) {
        ...
        if (actor != ProjectX.getClient().getLocalPlayer()) {
            return;
        }
    ...
    public boolean run(MKE_WintertodtConfig config, MKE_WintertodtPlugin plugin) {
        ...
        lastStateChange = System.currentTimeMillis();
        ProjectX.log("Plugin started - all state reset to fresh start");
            ...
            try {
                ...
                if (!ProjectX.isLoggedIn()) {
                    ...
                    if (!WintertodtBreakManager.isLogoutBreakActive() && !BreakHandlerScript.isBreakActive() && !wasOnBreak) {
                        ...
                        ProjectX.log("Player logged out - reset initialization flag");
                ...
                if (!breakManager.isInSafeLocationForBreak()) {
                    if (!BreakHandlerScript.isLockState()) {
                        ...
                        ProjectX.log("Locking break handler");
                    }
                }
                ...
                if (wasOnBreak) {
                    ProjectX.log("Resuming from break");
                    if (wasOnBreakHandlerBreak) {
                        ProjectX.log("Sleeping for 10 seconds to ensure client is fully loaded");
                        sleep(10000);
                    ...
                    ProjectX.log("Resuming from break, resetting state timer, world check, and action plan.");
                    wasOnBreak = false;
                    return;
                }
    ...
    private void resetAllScriptState() {
        ProjectX.log("=== COMPLETE SCRIPT STATE RESET ===");
        ...
        ProjectX.log("All script state variables reset to default values");
    }
    ...
    private void configureAntibanSettings() {
        try {
            ProjectX.log("Configuring antiban settings for Wintertodt...");
            ...
            ProjectX.log("=== Antiban Configuration ===");
            ProjectX.log("Activity: " + Rs2Antiban.getActivity().getMethod());
            ProjectX.log("Play Style: " + Rs2Antiban.getPlayStyle().getName());
            ProjectX.log("Micro Breaks: " + (Rs2AntibanSettings.takeMicroBreaks ? "Enabled" : "Disabled"));
            ProjectX.log("Action Cooldown: " + (Rs2AntibanSettings.usePlayStyle
                    ? "Enabled (chance " + Rs2AntibanSettings.actionCooldownChance + ")"
                    : "Disabled"));
            ProjectX.log("Mouse Randomization: " + (Rs2AntibanSettings.moveMouseRandomly ? "Enabled" : "Disabled"));
            ProjectX.log("============================");
            
            ProjectX.log("Antiban system configured for Wintertodt with firemaking profile");
            
        } catch (Exception e) {
            ProjectX.log("Error configuring antiban settings: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private boolean executeStartupSequence() {
        try {
            ProjectX.log("Executing smart startup sequence...");
            
            // Update ProjectX status
            ProjectX.status = startupManager.getCurrentPhase().getDescription();
            
            // Execute the startup sequence
            if (startupManager.executeStartupSequence()) {
                startupCompleted = true;
                ProjectX.log("Smart startup sequence completed successfully!");
                return true;
            } else {
                ProjectX.log("Smart startup sequence failed");
                return false;
            }
            
        } catch (Exception e) {
            ProjectX.log("Error during startup sequence: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
    ...
    private boolean initializeScript(MKE_WintertodtConfig config, MKE_WintertodtPlugin plugin) {
        try {
            ProjectX.log("Initializing Enhanced Wintertodt Script...");

            // Ensure lastStateChange is current (should already be set in run() method)
            lastStateChange = System.currentTimeMillis();
            ProjectX.log("Script initialization - lastStateChange confirmed reset");

            // Close bank if it's open at script start
            if (Rs2Bank.isOpen()) {
                ProjectX.log("Bank is open at script start - closing it");
                Rs2Bank.closeBank();
            ...
            ProjectX.log("Script initialized successfully!");
            
            // Fix camera settings on script start
            ProjectX.log("Checking and adjusting camera settings...");
            ...
            ProjectX.log("Initial state decided: " + state);

            return true;

        } catch (Exception e) {
            ProjectX.log("Failed to initialize script: " + e.getMessage());
            return false;
        }
    }
    ...
    private boolean validateEquipmentSetup() {
        ...
        ProjectX.log("Automatic axe validation: " + axeDecision.toString());
        
        if (axeDecision.shouldEquipAxe()) {
            if (!Rs2Equipment.isWearing(axeDecision.getAxeId())) {
                if (Rs2Inventory.hasItem(axeDecision.getAxeId())) {
                    ProjectX.log("Equipping axe from inventory: " + axeDecision.getAxeName());
                ...
                ProjectX.showMessage("No suitable axe found! The startup system should have handled this.");
                return false;
            }
            ProjectX.log("Using equipped axe: " + axeDecision.getAxeName());
        } else {
            if (!Rs2Inventory.hasItem(axeDecision.getAxeId())) {
                ProjectX.showMessage("Axe should be in inventory but not found! Startup system issue.");
                return false;
            }
            ProjectX.log("Using axe from inventory: " + axeDecision.getAxeName());
        }
    ...
    private boolean validateInventorySetup() {
        ...
        if (config.fletchRoots() && !Rs2Inventory.hasItem(WintertodtInventoryManager.knifeToUse)) {
            ProjectX.log("Fletching enabled but no knife in inventory - this is okay, will get one from bank");
        }

        if (config.fixBrazier() && !Rs2Inventory.hasItem(ItemID.HAMMER)) {
            ProjectX.log("Brazier fixing enabled but no hammer in inventory - this is okay, will get one from bank");
        }
        ...
        if (!Rs2Equipment.isWearing(ItemID.BRUMA_TORCH) &&
            ...
            ProjectX.log("No bruma torch equipped and no tinderbox in inventory - this is okay, will get one from bank");
        }
    ...
    private GameState analyzeGameState() {
        ...
        try {
            ...
            gameState.brazier        = ProjectX.getRs2TileObjectCache().query().withId(BRAZIER_29312)
                                          .within(brazierAnchor, 3).nearest();
            gameState.brokenBrazier  = ProjectX.getRs2TileObjectCache().query().withId(BRAZIER_29313)
                                          .within(brazierAnchor, 3).nearest();
            gameState.burningBrazier = ProjectX.getRs2TileObjectCache().query().withId(BURNING_BRAZIER_29314)
                                          .within(brazierAnchor, 3).nearest();
    ...
    private boolean handleEmergencySituations(GameState gameState) {
        ...
        if (BreakHandlerScript.isBreakActive() || Rs2AntibanSettings.microBreakActive) {
            ...
            if (Rs2Widget.hasWidget("Leave and lose all progress")) {
                ProjectX.log("Critical dialog detected during break, handling...");
        ...
        if (Rs2Widget.hasWidget("Leave and lose all progress")) {
            ProjectX.log("Detected leave dialog, selecting option 1");
            Rs2Keyboard.typeString("1");
            sleepGaussian(1600, 400);
            return true;
        }
        ...
        if ((BreakHandlerScript.isBreakActive() || WintertodtBreakManager.isBreakActive() || Rs2AntibanSettings.microBreakActive) && gameState.playerWarmth < 30) {
            ProjectX.log("EMERGENCY: Very low warmth (" + gameState.playerWarmth + ") during break!");
            ...
            if (Rs2Inventory.hasItem(config.food().getName()) || (usesPotions && !Rs2Inventory.getPotions().isEmpty())) {
                ProjectX.log("Emergency eating during break due to low warmth");
            ...
            if (playerLocation != null && playerLocation.distanceTo(BOSS_ROOM) > 6) {
                ProjectX.log("Emergency walk to safe spot (boss room) during break");
        ...
        if (System.currentTimeMillis() - lastStateChange > 120000 && !BreakHandlerScript.isLockState()) {
            ProjectX.log("Possible stuck state detected after 2 minutes, resetting...");
            resetActions = true;
    ...
    private void fixCameraPitchIfNeeded() {
        try {
            ...
            if (currentPitch < 230) { // Adjust this threshold as needed
                ProjectX.log("Camera pitch too low (" + currentPitch + "), adjusting...");
                ...
                ProjectX.log("Camera pitch adjusted from " + currentPitch + " to " + newPitch);
            }
    ...
    private void executeMainGameLogic(GameState gameState) {
        ...
        if (state == State.BANKING && isInsideWintertodtArea()) {
            ...
            ProjectX.log("Emergency banking exit in progress - executing banking logic directly");
        ...
        if ((BreakHandlerScript.isBreakActive() || Rs2AntibanSettings.microBreakActive) && 
            ...
            ProjectX.log("Skipping main game logic due to active break");
            return;
        }
        ...
        if (gameState.needPotions && usesPotions &&
            ...
            ProjectX.log("EMERGENCY: Need potions for survival - interrupting current activity (state: " + state + ")");
            
            // Force unlock state locks for emergency potion creation
            if (lockState) {
                setLockState(state, false);
                ProjectX.log("Emergency unlock of state lock for potion creation");
            }
            
            // Stop active fletching/feeding if needed
            if (fletchingState.isActive()) {
                fletchingState.stopFletching(FletchingInterruptType.MANUAL_STOP);
                ProjectX.log("Stopped fletching for emergency potion creation");
            }
            if (feedingState.isActive()) {
                feedingState.stopFeeding(FeedingInterruptType.MANUAL_STOP);
                ProjectX.log("Stopped feeding for emergency potion creation");
            }
    ...
    private void handleMainGameLoop(GameState gameState)
    {
        ...
        if (shouldPriorizeBrazierAtStart) {
            ProjectX.log("Round start priority active - checking if brazier needs lighting");
            if (shouldLightBrazier(gameState)) {
                ProjectX.log("Prioritizing brazier lighting at round start (state: " + state + ")");
                ...
                shouldPriorizeBrazierAtStart = false;
                ProjectX.log("Brazier lighting priority completed - already burning (state: " + state + ")");
            }
        }
        ...
        if (estimatedSecondsLeft < 15)
        {
            if (gameState.hasItemsToBurn) {
                if (shouldBurnLogs(gameState)) {
                    ProjectX.log("Time is almost over and we have items to burn, lets just burn them!");
                    return;
                }
            }
        ...
        try {
            ...
            if (!isInsideWintertodtArea()) {
                ...
                if (currentPoints >= 500) {
                    ProjectX.log("Have " + currentPoints + " points - waiting near door for round to end naturally");
                ...
                if (!WintertodtLocationManager.attemptLeaveWintertodt()) {
                    ...
                    if (consecutiveExitFailures >= MAX_EXIT_FAILURES) {
                        ProjectX.log("CRITICAL: Too many exit failures (" + consecutiveExitFailures + ") - emergency logout to prevent infinite loop");
                        Rs2Player.logout();
                        shutdown();
                        return;
                    }
                    ProjectX.log("Exit attempt failed (" + consecutiveExitFailures + "/" + MAX_EXIT_FAILURES + ") - retrying next tick");
            ...
            if (usesPotions) {
                 ...
                 */
                ProjectX.log("Using rejuvenation potions - ensuring gear/tools via bank setup");
                ...
                if (!checkAndUpgradeGear()) {
                    ProjectX.log("Gear setup failed for rejuvenation potion mode");
                    return;
                }
                ...
                ProjectX.log("Rejuvenation potion gear/tool setup completed - proceeding to potion creation");
        ...
        try {
            ...
            if (gameState.brazier != null) {
                if (gameState.brazier.click("light")) {
                    ProjectX.log("Lighting brazier");
                    
                    // Reset priority flag after successful lighting attempt
                    if (shouldPriorizeBrazierAtStart) {
                        shouldPriorizeBrazierAtStart = false;
                        ProjectX.log("Brazier lighting priority completed - resuming normal flow");
                    }
                    ...
                    if (gameState.hasItemsToBurn) {
                        ...
                        ProjectX.log("Transitioning to BURN_LOGS after lighting brazier");
                    } else if (!gameState.inventoryFull) {
                        changeState(State.CHOP_ROOTS);
                        ProjectX.log("Transitioning to CHOP_ROOTS after lighting brazier");
                    } else {
                        changeState(State.WAITING);
                        ProjectX.log("Transitioning to WAITING after lighting brazier");
                    }
                }
            }
        } catch (Exception e) {
    ...
    private void handleFletchLogsState(GameState gameState) {
        try {
            ...
            if (gameState.brokenBrazier != null && config.fixBrazier()) {
                ...
                ProjectX.log("Fixing broken brazier (priority during fletching)");
                resetActions = true;
                actionsPerformed++;
            ...
            if (gameState.burningBrazier == null && gameState.brazier != null &&
                ...
                ProjectX.log("Relighting brazier (priority during fletching)");
                resetActions = true;
                actionsPerformed++;
            ...
            if (rootCount > lastInventoryCount) {
                ...
                ProjectX.log("New inventory cycle detected, knife threshold: " + knifePreselectThreshold);
            }
    ...
    private void handleBurnLogsState(GameState gameState) {
        try {
            ...
            if (gameState.brokenBrazier != null && config.fixBrazier()) {
                ...
                gameState.brokenBrazier.click("fix");
                ProjectX.log("Fixing broken brazier");
                resetActions = true;
                actionsPerformed++;
                return; // Wait for next tick after the fix attempt
            }
            ...
            if (burningBrazier == null && gameState.brazier != null &&
                ...
                gameState.brazier.click("light");
                ProjectX.log("Relighting brazier");
                resetActions = true;
                actionsPerformed++;
                return; // Wait for next tick after the relight attempt
            }
            ...
            if (!isCurrentlyFeeding() &&
                ...
                if (burningBrazier.click("feed")) {
                    ...
                    resetActions = false;
                    actionsPerformed++;
                    ProjectX.log("Started feeding brazier");
                    maybeNudgeMouse();
    ...
    private void handleGetConcoctionsState(GameState gameState) {
        try {
            ...
            ProjectX.log("CONCOCTIONS: Need " + needed + " more (potions: " + currentPotions + ", concoctions: " + currentConcoctions + ", target: " + config.healingAmount() + ")");
            
            if (needed <= 0) {
                ProjectX.log("Have enough total healing items (" + totalHealingItems + "/" + config.healingAmount() + "), moving to herbs");
                changeState(State.GET_HERBS);
                return;
            }
            ...
            if (emptySlots == 0) {
                ProjectX.log("Inventory full - dropping bruma items to make space for concoctions");
                ...
                // If no roots, drop bruma kindlings
                    ...
                    ProjectX.log("No bruma items to drop - inventory management issue");
                }
                return; // Try again next tick after dropping
            }

            // Find and interact with crate
            Rs2TileObjectModel crate = ProjectX.getRs2TileObjectCache().query().withId(CRATE_OBJECT_ID)
            ...
            if (crate != null) {
                if (crate.click("Take-concoction")) {
                    ...
                    actionsPerformed++;
                    ProjectX.log("Got concoction!");
                } else {
                    ProjectX.log("Failed to interact with crate, retrying...");
                }
            } else {
                ProjectX.log("Could not find crate");
    ...
    private void handleGetHerbsState(GameState gameState) {
        try {
            ...
            ProjectX.log("HERBS: Currently have " + totalHealingItems + "/" + config.healingAmount() + " healing items");
            ProjectX.log("HERBS: After combining would have " + totalAfterCombining + "/" + config.healingAmount() + " (concoctions: " + currentConcoctions + ", herbs: " + currentHerbs + ")");
            
            // If we already have enough total healing items, go straight to combining
            if (totalAfterCombining >= config.healingAmount()) {
                ProjectX.log("Have enough herbs for combining, moving to make potions");
                changeState(State.MAKE_POTIONS);
                return;
            }
            ...
            if (Rs2Player.isAnimating()) {
                ProjectX.log("Picking herbs... (need " + herbsNeeded + " more)");
                return;
            }
            ...
            int emptySlots = Rs2Inventory.emptySlotCount();
            if (emptySlots == 0) {
                ProjectX.log("Inventory full - dropping bruma items to make space for herbs");
            ...
            Rs2TileObjectModel roots = ProjectX.getRs2TileObjectCache().query().withId(SPROUTING_ROOTS_OBJECT_ID)
            ...
            if (roots != null) {
                ...
                if (roots.click("Pick")) {
                    ...
                    ProjectX.log("Failed to interact with sprouting roots");
                }
            } else {
                ProjectX.log("Could not find sprouting roots");
    ...
    private void handleMakePotionsState(GameState gameState) {
        try {
            ...
            ProjectX.log("COMBINING: potions: " + currentPotions + ", concoctions: " + currentConcoctions + ", herbs: " + currentHerbs + ", target: " + config.healingAmount());
            
            // Check if we have enough potions now
            if (currentPotions >= config.healingAmount()) {
                ProjectX.log("Have enough rejuvenation potions (" + currentPotions + "/" + config.healingAmount() + "), cleaning up and organizing inventory");
            ...
            if (currentConcoctions <= 0 && currentPotions < config.healingAmount()) {
                ProjectX.log("Need more concoctions to reach target");
                changeState(State.GET_CONCOCTIONS);
                return;
            }
            
            if (currentHerbs <= 0 && currentConcoctions > 0 && currentPotions < config.healingAmount()) {
                ProjectX.log("Need more herbs to combine");
                changeState(State.GET_HERBS);
                return;
            }
            ...
            if (currentConcoctions > 0 && currentHerbs > 0) {
                if (canManuallyMake) {
                    ...
                    ProjectX.log("Using manual combination method (will make " + potionsToMake + " potions)");
                    if (Rs2Inventory.combineClosest(ItemID.REJUVENATION_POTION_UNF, ItemID.BRUMA_HERB)) {
                        ...
                        actionsPerformed++;
                        ProjectX.log("Manual combination attempted - checking results next tick");
                    } else {
                        ProjectX.log("Failed to start manual combining - retrying next tick");
                        sleepGaussian(1000, 300);
                    }
                } else {
                    // Use Brew'ma NPC method
                    ProjectX.log("Player cannot manually combine potions - using Brew'ma NPC method");
                    if (useBreadmaNpcForPotions(currentConcoctions, currentHerbs)) {
                        ProjectX.log("Brew'ma NPC combination completed");
                    } else {
                        ProjectX.log("Failed to use Brew'ma NPC - retrying next tick");
                        sleepGaussian(1000, 300);
                    }
                }
            } else {
                // Shouldn't get here, but handle it
                ProjectX.log("No valid combinations possible - checking what we need");
                if (currentPotions >= config.healingAmount()) {
    ...
    private boolean canManuallyMakePotions() {
        try {
            ...
            ProjectX.log("Error checking druidic ritual, defaulting to NPC method: " + e.getMessage());
            return false;
        }
    }
    ...
    private boolean useBreadmaNpcForPotions(int concoctions, int herbs) {
        try {
            ...
            if (Rs2Player.getWorldLocation().distanceTo(BREWMA_NPC_LOCATION) > 10) {
                ProjectX.log("Walking to Brew'ma NPC at " + BREWMA_NPC_LOCATION);
            ...
            // Find Brew'ma NPC
            Rs2NpcModel brewmaNpc = ProjectX.getRs2NpcCache().query().withName("Brew'ma")
                    .where(n -> n.hasLineOfSight()).nearestOnClientThread();
            if (brewmaNpc == null) {
                ProjectX.log("Could not find Brew'ma NPC - walking closer");
            ...
            ProjectX.log("Interacting with Brew'ma NPC to convert " + concoctions + " concoctions and " + herbs + " herbs");
            ...
            if (Rs2Inventory.getSelectedItemId() != ItemID.BRUMA_HERB && !Rs2Inventory.interact(ItemID.BRUMA_HERB, "Use")) {
                ProjectX.log("Failed to select herb from inventory");
                return false;
            }
            ...
            if (Rs2Inventory.getSelectedItemId() != ItemID.BRUMA_HERB) {
                ProjectX.log("Failed to select herb from inventory, retrying...");
                return false;
            }

            ProjectX.log("Selected herb from inventory");
            
            // Then click on the Brew'ma NPC while herb is selected
            if (brewmaNpc.click("Use")) {
                ProjectX.log("Used herb on Brew'ma NPC - waiting for conversion");
                ...
                if (potionsAfterInteraction > potionsBeforeInteraction) {
                    ProjectX.log("Brew'ma NPC successfully converted ingredients! Potions: " + 
                               potionsBeforeInteraction + " -> " + potionsAfterInteraction);
                    actionsPerformed++;
                    return true;
                } else {
                    ProjectX.log("Brew'ma NPC interaction may have failed - no potion increase detected");
                    return false;
                }
            } else {
                ProjectX.log("Failed to use herb on Brew'ma NPC");
                return false;
            }

        } catch (Exception e) {
            ProjectX.log("Error using Brew'ma NPC for potions: " + e.getMessage());
            return false;
        }
    }
    ...
    private void organizeRejuvenationPotionsInInventory() {
        try {
            ProjectX.log("Cleaning up extra resources before organizing potions...");
            
            // Clean up first to make space
            cleanupExtraResources();
            sleepGaussian(500, 200); // Wait for cleanup to complete
            
            ProjectX.log("Organizing rejuvenation potions in inventory...");
            ...
            if (allPotions.isEmpty()) {
                ProjectX.log("No rejuvenation potions found to organize");
                return;
            }
            ...
            for (Rs2ItemModel potion : allPotions) {
                ...
                if (potion.getSlot() != targetSlot) {
                    ProjectX.log("Moving " + potion.getName() + " from slot " + potion.getSlot() + " to slot " + targetSlot);
                    ...
                    ProjectX.log("Potion " + potion.getName() + " already in target slot " + targetSlot);
                }
                
                slotIndex++;
            }
            
            ProjectX.log("Inventory organization completed - potions arranged in leftmost column starting from slot 0");
            
        } catch (Exception e) {
            ProjectX.log("Error organizing rejuvenation potions: " + e.getMessage());
        }
    }
    ...
    private void cleanupExtraResources() {
        try {
            ProjectX.log("Cleaning up extra resources...");
            ...
            ProjectX.log("Cleanup completed - final potion count: " + finalPotions);
            
        } catch (Exception e) {
            ProjectX.log("Error during cleanup: " + e.getMessage());
        }
    }
    ...
    private boolean handleBrazierMaintenance(GameState gameState) {
        ...
        if (gameState.brokenBrazier != null && config.fixBrazier()) {
            ...
            deselectSelectedItem();
            gameState.brokenBrazier.click("fix");
            ProjectX.log("Fixing broken brazier (priority)");
            resetActions = true;
            actionsPerformed++;
            return true;
        }
        ...
        if (gameState.burningBrazier == null && gameState.brazier != null
            ...
            deselectSelectedItem();
            gameState.brazier.click("light");
            ProjectX.log("Relighting brazier (priority)");
            resetActions = true;
            actionsPerformed++;
            return true;
        }
    ...
    private boolean shouldStartOrContinueChopping(GameState gameState) {
        ...
        if (targetRootsForThisRun == 0) {
            ...
            ProjectX.log("New Action Plan: Chop " + targetRootsForThisRun + " roots.");
    ...
    private boolean shouldLightBrazier(GameState gameState) {
        ...
        if (gameState.brazier == null || gameState.burningBrazier != null) {
            ...
            if (shouldPriorizeBrazierAtStart && gameState.burningBrazier != null) {
                shouldPriorizeBrazierAtStart = false;
                ProjectX.log("Brazier priority reset - brazier already lit");
            }
            return false;
        }
    ...
    private void dodgeSnowfallDamage(GameState gameState) {
        try {
            if (!resetActions) {
                ...
                Deque<Projectile> projectiles = ProjectX.getClient().getProjectiles();
                ...
                for (Projectile projectile : projectiles) {
                    if (projectile.getId() == 501 && projectile.getHeight() == 150) { //Somewhere is said this -1268, by testing runelite shows 150
                        ...
                        for (GraphicsObject graphicsObject : ProjectX.getClient().getGraphicsObjects()) {
                            if (graphicsObject.getId() == 502) {
                                WorldPoint dangerZone = WorldPoint.fromLocalInstance(
                                        ProjectX.getClient(), graphicsObject.getLocation());
                        ...
                        if (hasNearbyGraphics) {
                            ...
                            if (random.nextInt(100) < 80) {
                                ...
                                resetActions = true;
                                ProjectX.log("Dodged snowfall damage (80% chance triggered)");
                                ...
                                ProjectX.log("Snowfall detected but purposely NOT dodging (20% chance - staying put for realism ;) )");
                            }
    ...
    private boolean handleFoodAndHealing() {
        try {
            ...
            if (!Rs2Bank.hasBankItem(foodName, requiredAmount, true)) {
                ProjectX.showMessage("Insufficient food in bank! Please restock.");
    ...
    private void trackPerformance(long loopStartTime) {
        ...
        if (consecutiveFailures > 10) {
            ProjectX.log("Too many consecutive failures, resetting state");
            setLockState(state, false);
            resetActions = true;
            consecutiveFailures = 0;
        }
    ...
    private void handleBreakTrigger() {
        try {
            ...
            if (breakManager != null && WintertodtBreakManager.isBreakActive()) {
                ...
                ProjectX.log("Custom break system active, pausing script");
                return;
            }
            ...
            if (BreakHandlerScript.isBreakActive()) {
                wasOnBreakHandlerBreak = true;
                ProjectX.log("Break handler break active, pausing script");
                return;
            }
    ...
    private boolean handleBreakOpportunity() {
        try {
            ...
            if (BreakHandlerScript.isLockState()) {
                ...
                ProjectX.log("Unlocking break handler for break opportunity");
    ...
    private void handleScriptException(Exception ex) {
        ProjectX.log("Script error: " + ex.getMessage());
        ex.printStackTrace();
        consecutiveFailures++;
    ...
    private boolean checkAndUpgradeGear() {
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ProjectX.log("Running gear + inventory setup (attempt " + attempt + "/" + maxRetries + ")...");

                // Ensure bank is still open
                if (!Rs2Bank.isOpen()) {
                    ProjectX.log("Bank closed during gear setup - reopening...");
                    if (!Rs2Bank.openBank()) {
                        ProjectX.log("Failed to reopen bank on attempt " + attempt);
                        if (attempt < maxRetries) {
                ...
                if (!gearPassOk) {
                    ProjectX.log("Gear setup failed on attempt " + attempt);
                    if (attempt < maxRetries) {
                        sleepGaussian(retryDelay, 200);
                        continue;
                    }
                    ProjectX.log("Gear setup failed after " + maxRetries + " attempts");
                    return false;
                }

                ProjectX.log("Gear setup completed successfully");
                ...
                if (inventoryManager != null) {
                    ...
                    if (!invOk) {
                        ProjectX.log("Inventory setup failed on attempt " + attempt);
                        if (attempt < maxRetries) {
                            sleepGaussian(retryDelay, 200);
                            continue;
                        }
                        ProjectX.log("Inventory setup failed after " + maxRetries + " attempts");
                        return false;
                    }
                    
                    ProjectX.log("Inventory setup completed successfully");
                } else {
                    ProjectX.log("Warning: Inventory manager is null - cannot setup inventory");
                    return false;
                }
                ...
                // All steps succeeded!
                ProjectX.log("Complete gear + inventory setup successful!");
                return true;

            } catch (Exception e) {
                ProjectX.log("Error during gear/inventory setup (attempt " + attempt + "): " + e.getMessage());
                e.printStackTrace();
                ...
                ProjectX.log("Gear setup failed with exception after " + maxRetries + " attempts");
                return false;
            }
        }
        ...
        try {
            ...
            for (int world : wintertodtWorlds) {
                if (currentWorld == world) {
                    ProjectX.log("Already on Wintertodt world " + currentWorld + " - continuing");
            ...
            if (!isSafeToHopWorlds()) {
                ProjectX.log("Not safe to hop worlds currently (combat/interaction) - will try again later");
                return true; // Don't fail, just retry later
            }
            
            // Not on a Wintertodt world, need to hop
            ProjectX.log("Not on a Wintertodt world (current: " + currentWorld + ") - hopping to a Wintertodt world...");
            
            boolean hopSuccess = hopToWintertodtWorldSafely(wintertodtWorlds);
            
            if (!hopSuccess) {
                // Hop failed - stop script and logout
                ProjectX.log("CRITICAL: Failed to hop to Wintertodt world - stopping script and logging out");
                stopScriptAndLogout();
                return false;
            }
            ...
            ProjectX.log("Error checking/hopping to Wintertodt world: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
    ...
    private boolean isSafeToHopWorlds() {
        try {
            ...
            ProjectX.log("Error checking if safe to hop: " + e.getMessage());
    ...
    private void stopScriptAndLogout() {
        try {
            ProjectX.log("Stopping script due to critical error...");
            ...
            // Logout the player
            if (ProjectX.isLoggedIn()) {
                ProjectX.log("Logging out player...");
                Rs2Player.logout();
            }
            
        } catch (Exception e) {
            ProjectX.log("Error during script stop and logout: " + e.getMessage());
        }
    }
    ...
    private boolean hopToWintertodtWorldSafely(int[] wintertodtWorlds) {
        try {
            ...
            ProjectX.log("Attempting to hop to Wintertodt world " + targetWorld + "...");
            
            // Perform the world hop (NOTE: ProjectX.hopToWorld() has a bug - returns false even on success)
            ProjectX.hopToWorld(targetWorld);
            ...
            ProjectX.log("World hop initiated, waiting for completion...");
            ...
            boolean hopCompleted = sleepUntilTrue(() -> {
                ...
                if (currentWorld == targetWorld && ProjectX.isLoggedIn()) {
                    return true;
                }
                // Also accept if we ended up on any Wintertodt world
                for (int wintertodtWorld : wintertodtWorlds) {
                    if (currentWorld == wintertodtWorld && ProjectX.isLoggedIn()) {
                        return true;
                    }
                }
                return false;
            ...
            if (hopCompleted) {
                ProjectX.log("Successfully hopped to Wintertodt world " + finalWorld);
                return true;
            } else if (finalWorld != originalWorld) {
                // We hopped somewhere, but not to a Wintertodt world - this is partial success
                ProjectX.log("Hopped to world " + finalWorld + " but not a Wintertodt world - trying to hop again");
                
                // Try once more to a Wintertodt world
                int retryWorld = wintertodtWorlds[Rs2Random.between(0, wintertodtWorlds.length - 1)];
                ProjectX.log("Attempting second hop to Wintertodt world " + retryWorld + "...");
                ProjectX.hopToWorld(retryWorld);
                ...
                if (retryCompleted) {
                    ProjectX.log("Successfully hopped to Wintertodt world " + Rs2Player.getWorld() + " on second attempt");
                    return true;
                } else {
                    ProjectX.log("Second hop attempt failed - still on world " + Rs2Player.getWorld());
                }
            } else {
                ProjectX.log("World hop failed - still on original world " + originalWorld + " after 15 seconds");
            }
            ...
            if (wintertodtWorlds.length > 1) {
                ...
                ProjectX.log("Final attempt - trying Wintertodt world " + finalAttemptWorld + "...");
                ProjectX.hopToWorld(finalAttemptWorld);
                ...
                if (finalCompleted) {
                    ProjectX.log("Successfully hopped to Wintertodt world " + Rs2Player.getWorld() + " on final attempt");
                    return true;
                }
            }
            
            ProjectX.log("CRITICAL: Failed to hop to any Wintertodt world after multiple attempts");
            return false;
            
        } catch (Exception e) {
            ProjectX.log("Error during world hop: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
    ...
    public void shutdown() {
        ProjectX.log("Shutting down Enhanced Wintertodt Script");
        ...
        if (BreakHandlerScript.isLockState()) {
            BreakHandlerScript.setLockState(false);
            ProjectX.log("Unlocked break handler state during shutdown");
        }
        ...
        super.shutdown();
        ProjectX.log("Script shutdown completed with full state reset");
    }
        ...
        try {
            ...
            if (remainingSeconds > 0) {
                ...
                if (roundEndTime == -1) {
                    ...
                    isHoverBeforeStartTimeCalculated = false;
                    hoverBeforeStartTime = 0;
                    ProjectX.log("New Wintertodt round ended, next round starts in " + remainingSeconds + " seconds");
                    ...
                    if (currentRoundStartTime > 0) {
                        ...
                        if (roundDuration > 30000 && roundDuration < 600000) { // 30s-10min sanity check
                            ...
                            ProjectX.log("Logged round duration: " + (roundDuration / 1000) + "s. History size: " + previousRoundDurations.size());
                        }
                        currentRoundStartTime = 0;
                    }
                }
                ...
                ProjectX.log("Wintertodt round started! Prioritizing brazier lighting.");
            }
    ...
    private Rs2TileObjectModel getNextInteractiveObject(GameState gameState) {
        try {
            ...
            if (gameState.brokenBrazier != null && config.fixBrazier()) {
                ProjectX.log("DEBUG: Will hover over broken brazier for fixing");
            ...
            if (gameState.brazier != null && gameState.burningBrazier == null && !gameState.needBanking && !gameState.needPotions) {
                ProjectX.log("DEBUG: Will hover over unlit brazier for lighting");
            ...
            if (gameState.burningBrazier != null && gameState.hasItemsToBurn) {
                ProjectX.log("DEBUG: Will hover over burning brazier for feeding");
            ...
            if (!gameState.inventoryFull && !gameState.hasItemsToBurn && !gameState.needBanking && !gameState.needPotions) {
                Rs2TileObjectModel root = getOwnSideRoot();
                if (root != null) {
                    ProjectX.log("DEBUG: Will hover over bruma roots for chopping");
                    return root;
                }
            }
            ...
            if (gameState.burningBrazier != null) {
                ProjectX.log("DEBUG: Fallback to burning brazier");
                return gameState.burningBrazier;
            }
            if (gameState.brazier != null) {
                ProjectX.log("DEBUG: Fallback to unlit brazier");
                return gameState.brazier;
            }
            if (gameState.brokenBrazier != null) {
                ProjectX.log("DEBUG: Fallback to broken brazier");
                return gameState.brokenBrazier;
            }
            
            ProjectX.log("DEBUG: No interactive objects found for hovering");
            return null;
    ...
    private void handleRoundTimerMouseBehavior(GameState gameState) {
        try {
            ...
            if (!isHoverBeforeStartTimeCalculated && timeUntilStart > 1000) {
                ...
                ProjectX.log("Will hover " + (hoverBeforeStartTime / 1000.0) + " seconds before round starts");
            }
            ...
            if (!hoveredForNextRound && !spamClickingActive && timeUntilStart > 0 && timeUntilStart <= hoverBeforeStartTime) {
                ...
                if (nextObject != null) {
                    ...
                    ProjectX.log("Hovering over next interactive object: " + nextObject.getId() + 
                               " (" + (timeUntilStart / 1000.0) + "s before round start)");
                } else {
                    ProjectX.log("Could not find interactive object to hover over");
                }
            }
    ...
    private void determineInitialState(GameState gs)
    {
        ...
        if (playerLocation.getY() > 3967 && // Inside game room 
            ...
            ProjectX.log("Starting in game room - skipping banking phase and going directly to game logic");
            
            // If using rejuvenation potions and we don't have enough, we need to get them first
            if (usesPotions) {
                int currentPotions = getTotalRejuvenationPotions();
                if (currentPotions < config.minHealingItems()) {
                    ProjectX.log("Using rejuvenation potions but don't have enough - need to make them");
                    changeState(State.GET_CONCOCTIONS);
                    return;
                }
            }
        ...
        if (usesPotions) {
            int currentPotions = getTotalRejuvenationPotions();
            if (currentPotions < config.minHealingItems()) {
                ProjectX.log("Need rejuvenation potions - starting potion creation workflow");
                changeState(State.GET_CONCOCTIONS);
                return;
            }
        }
    ...
    private void maybeNudgeMouse()
    {
        ...
        try
        {
            Point start = ProjectX.getMouse().getMousePosition();
     ...
     * ProjectX.getMouse().move(x, y) dispatches a teleport MOUSE_MOVED event
    ...
    {
        if (x <= 1 || y <= 1) return;
        if (ProjectX.naturalMouse != null) {
            ProjectX.naturalMouse.moveTo(x, y);
        } else {
            ProjectX.getMouse().move(x, y);
        }
    }
    ...
    {
        ProjectX.log("-- Resetting action plan & cooldowns");
    ...
    private void setupSpamClickingForRoundStart() {
        try {
            ...
            ProjectX.log("Spam clicking scheduled: " + (preStartDuration / 1000.0) + "s before start, " + 
        ...
        try {
            ...
            if (currentTime < spamClickStartTime || currentTime > spamClickEndTime) {
                if (spamClickingActive) {
                    spamClickingActive = false;
                    ProjectX.log("Spam clicking finished. Performed " + spamClicksPerformed + " clicks.");
                }
                return;
            }
            ...
            if (!spamClickingActive) {
                ...
                ProjectX.log("Started spam clicking on: " + 
    ...
    private void performSpamClick() {
        try {
            ...
            ProjectX.log("Spam clicking on: " + spamClickTarget.getId());
    ...
    private void depositUnnecessaryItems() {
        ...
        sleepGaussian(300, 100);
        ProjectX.log("Deposited unnecessary items from inventory.");
    }
    ...
    private void handleExitingForRewardsState(GameState gameState) {
        try {
            ...
            ProjectX.log("Successfully exited Wintertodt for reward collection");
    ...
    private void handleWalkingToRewardsBankState(GameState gameState) {
        try {
            ...
            if (playerLocation.distanceTo(BANK_LOCATION) > 6) {
                ...
                ProjectX.log("Arrived at bank for reward collection");
    ...
    private void handleBankingForRewardsState(GameState gameState) {
        try {
            ...
            if (!Rs2Inventory.isEmpty()) {
                Rs2Bank.depositAll();
                sleepGaussian(600, 100);
                ProjectX.log("Deposited all items for reward collection");
                return;
            }
    ...
    private void handleWalkingToRewardCartState(GameState gameState) {
        try {
            ...
            if (playerLocation.distanceTo(targetTile) > 1) {
                ...
                ProjectX.log("Arrived at reward cart location");
    ...
    private void handleLootingRewardCartState(GameState gameState) {
        try {
            ...
            if (isRewardCartLootingFinished()) {
                ...
                ProjectX.log("Reward cart looting finished - no more rewards available");
                changeState(State.RETURNING_FROM_REWARDS);
                return;
            }
            ...
            if (Rs2Inventory.isFull()) {
                ProjectX.log("Inventory full - banking rewards and returning");
                changeState(State.WALKING_TO_REWARDS_BANK);
                return;
            }
    ...
    private static void calculateRewardThreshold() {
        ...
        ProjectX.log("Reward cart threshold set to: " + targetRewardThreshold + " rewards (base: " + baseRewards + ", variance: " + randomVariance + ")");
    }
    ...
    private void startRewardCartLooting() {
        ...
        ProjectX.log("Starting reward cart looting sequence - current rewards: " + currentRewardCartRewards + "/" + targetRewardThreshold);
    ...
    private void finishRewardCartLooting() {
        ...
        ProjectX.log("Reward cart looting completed - returning to normal execution");
        ...
        ProjectX.log("Forcing complete startup sequence after reward collection to ensure proper setup");
    ...
    private boolean interactWithRewardCart() {
        ...
        var rewardCart = ProjectX.getRs2TileObjectCache().query().withName("Reward").nearestOnClientThread();
        if (rewardCart != null && rewardCart.click("Big-search")) {
            ProjectX.status = "Interacting with reward cart";
            return true;
        }

        ProjectX.status = "No reward cart found to interact with";
        return false;
    }
    ...
    private boolean isRewardCartInteractionComplete() {
        ...
        if (timeSinceInteraction > 30000) {
            ProjectX.log("Reward cart interaction timeout");
            return true;
        }
