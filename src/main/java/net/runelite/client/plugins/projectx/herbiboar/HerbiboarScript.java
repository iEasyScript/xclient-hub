...
public class HerbiboarScript extends Script {
    ...
    private void withdrawDosesDescending(int[] ids, int[] doses, int targetDoses) {
        ProjectX.log(Level.INFO, "Withdrawing potions to reach "+targetDoses+" total doses");
        ...
        for (int i = 0; i < ids.length; i++) {
            ...
            ProjectX.log(Level.INFO,"Already have "+count+"x "+doses[i]+"-dose potions = "+(count * doses[i])+" doses");
        }

        int remaining = targetDoses - current;
        ProjectX.log(Level.INFO,"Current total: "+current+" doses, Need: "+remaining+" more doses to reach target of "+targetDoses);

        if (remaining <= 0) {
            ProjectX.log(Level.INFO,"Already have enough doses, no need to withdraw more");
            return;
        }
        ...
        for (int i = 0; i < ids.length && remaining > 0; i++) {
            ...
            if (potionsToWithdraw <= 0) {
                ProjectX.log(Level.INFO,"Don't need any {}-dose potions (remaining doses: {})", dose, remaining);
                continue;
            }

            ProjectX.log(Level.INFO,"Attempting to withdraw {}x {}-dose potions (id: {})", potionsToWithdraw, dose, id);
            ...
            if (withdrawn > 0) {
                ...
                ProjectX.log(Level.INFO,"Successfully withdrawn {}x {}-dose potions = {} doses", withdrawn, dose, dosesWithdrawn);
                ProjectX.log(Level.INFO,"Remaining doses needed: {}", remaining);
                ...
                ProjectX.log(Level.INFO,"Current total: {} doses", current);

                // If we've reached or exceeded our target, stop withdrawing
                if (current >= targetDoses) {
                    ProjectX.log(Level.INFO,"Reached or exceeded target doses ({}), stopping withdrawal", targetDoses);
                    break;
                }
            } else {
                ProjectX.log(Level.INFO,"Failed to withdraw any {}-dose potions", dose);
            }
        }

        // Log final counts
        current = 0;
        ProjectX.log(Level.INFO,"--- Final inventory after withdrawal ---");
        for (int i = 0; i < ids.length; i++) {
            int count = Rs2Inventory.count(ids[i]);
            current += count * doses[i];
            ProjectX.log(Level.INFO,"{}x {}-dose potions = {} doses", count, doses[i], count * doses[i]);
        }
        ProjectX.log(Level.INFO,"Final total: {} doses (target was {})", current, targetDoses);
    }
    ...
    private int getEnergyDoseCount(HerbiboarConfig.RunEnergyOption option) {
        ...
        ProjectX.log(Level.INFO,"Checking inventory for all potion types");
        int totalDoses = 0;
        ...
        ProjectX.log(Level.INFO,"Found in inventory: stamina={}, superEnergy={}, energy={}, fruit={}",
    ...
    private void ensureEnergyDoses(HerbiboarConfig.RunEnergyOption option, int targetDoses) {
        ...
        ProjectX.log(Level.INFO,"Ensuring energy doses for {}: current={}, target={}", option, currentDoses, targetDoses);

        if (currentDoses >= targetDoses) {
            ProjectX.log(Level.INFO,"Already have enough doses ({}), no need to withdraw more", currentDoses);
            return;
        }

        // Only proceed with withdrawal if bank is open
        if (!Rs2Bank.isOpen()) {
            ProjectX.log(Level.INFO,"Bank not open, skipping dose withdrawal");
            return;
        }
        ...
        ProjectX.log(Level.INFO,"After withdrawal check: current={}, target={}", currentDoses, targetDoses);
    }
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (getLastLocation() != null && !getLastLocation().equals(Rs2Player.getWorldLocation())) {
                    ...
                    ProjectX.log(Level.INFO,"Player has not moved for over 1 minute, resetting script state");
                    setLastMove(Instant.now());
                ...
                if (!Rs2Player.isMoving() && !Rs2Player.isInteracting()) {
                    ProjectX.log(Level.INFO,"Checking inventory and run energy");
                ...
                if (state != HerbiboarState.INITIALIZING && state != HerbiboarState.CHECK_AUTO_RETALIATE) {
                    if (needsToBank(config)) {
                        ProjectX.log(Level.INFO,"Need to bank, switching to BANK state");
                        setState(HerbiboarState.BANK);
                    } else if (hasRequiredInventorySetup(config) && isNearBank() && 
                              (getState() == HerbiboarState.START)) {
                        ProjectX.log(Level.INFO,"Returning to island, switching to RETURN_FROM_ISLAND state");
                ...
                switch (state) {
                    case RESET:
                         ...
                         */
                        ProjectX.status = "Resetting...";
                        ProjectX.log(Level.INFO,"Resetting...");
                        attackedTunnel = false;
                        setLastMove(Instant.now());
                        setLastLocation(null);
                        ...
                        if (!reached) {
                            ProjectX.log(Level.INFO, "Failed to reach reset rock, stopping script");
                            Rs2Player.logout();
                            ProjectX.showMessage("Failed to reach reset rock, stopping script");
                            ProjectX.stopPlugin(herbiboarPlugin.getClass());
                            return;
                        }
                        if (ProjectX.getVarbitValue(VarbitID.FOSSIL_HERBIBOAR_ALREADY_CAUGHT_IGNORE_WARNING) != 1) {
                            Rs2GameObject.interact(resetRock, "Toggle warning");
                            sleepUntil(() -> ProjectX.getVarbitValue(VarbitID.FOSSIL_HERBIBOAR_ALREADY_CAUGHT_IGNORE_WARNING) == 1, 5000);
                        }
                        ...
                        break;
                    case INITIALIZING:
                        ProjectX.status = "Starting...";
                        ProjectX.log(Level.INFO,"Initializing...");
                        setState(HerbiboarState.CHECK_AUTO_RETALIATE);
                        break;
                    case CHECK_AUTO_RETALIATE:
                        ProjectX.status = "Checking auto retaliate...";
                        ProjectX.log(Level.INFO,"Checking auto retaliate...");
                        if (ProjectX.getVarbitPlayerValue(172) == 0) {
                            ProjectX.status = "Disabling auto retaliate...";
                    ...
                    case START:
                        ...
                        ProjectX.status = "Finding start location";
                        ProjectX.log(Level.INFO,"Finding start location");
                        if (herbiboarPlugin.getCurrentGroup() == null) {
                            ...
                            if (start != null) {
                                ...
                                LocalPoint localPoint = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), loc);
                        ...
                        break;
                    case TRAIL:
                        ProjectX.status = "Following trail";
                        ProjectX.log(Level.INFO,"Following trail");
                        ...
                        break;
                    case TUNNEL:
                        ProjectX.status = "Attacking tunnel";
                        ProjectX.log(Level.INFO,"Attacking tunnel");
                        if (!attackedTunnel || (ProjectX.getRs2NpcCache().query().withName("Herbiboar").nearestOnClientThread() == null && attackedTunnel)) {
                            int finishId = herbiboarPlugin.getFinishId();
                            if (finishId > 0) {
                                WorldPoint finishLoc = herbiboarPlugin.getEndLocations().get(finishId - 1);
                                LocalPoint localPoint = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), finishLoc);
                            ...
                            Rs2NpcModel herbCheck = ProjectX.getRs2NpcCache().query().withName("Herbiboar").nearestOnClientThread();
                            if (herbCheck != null) setState(HerbiboarState.HARVEST);
                        }
                        break;
                    case HARVEST:
                        ProjectX.status = "Harvesting herbiboar";
                        ProjectX.log(Level.INFO,"Harvesting herbiboar");
                        Rs2NpcModel herb = ProjectX.getRs2NpcCache().query().withName("Herbiboar").nearestOnClientThread();
                        if (herb != null) {
                            ...
                            if (Rs2Player.getWorldLocation().distanceTo(loc) <= 8) {
                                ...
                                if (start != null) {
                                    ...
                                    LocalPoint localPoint = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), startLoc);
                                    if (localPoint == null || Rs2Player.getWorldLocation().distanceTo(startLoc) >= config.interactionDistance()) {
                                        Rs2Walker.walkTo(startLoc);
                                    } else {
                                        Rs2Camera.turnTo(localPoint);
                                        ProjectX.log(Level.INFO,"Searching for next herbiboar");
                                        Rs2GameObject.interact(start, "Search");
                                        Rs2Player.waitForAnimation();
                        ...
                        break;
                    case BANK:
                        ProjectX.status = "Banking items";
                        ProjectX.log(Level.INFO,"Banking items");

                        LocalPoint bankLocalPoint = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), BANK_LOCATION);
                        if (bankLocalPoint == null || Rs2Player.getWorldLocation().distanceTo(BANK_LOCATION) >= 5) {
                            ProjectX.log(Level.INFO,"Walking to bank");
                            Rs2Walker.walkTo(BANK_LOCATION);
                        } else if (!Rs2Bank.isOpen()) {
                            ProjectX.log(Level.INFO,"Opening bank");
                            Rs2Bank.openBank();
                             ...
                             */
                            ProjectX.log(Level.INFO,"Locking configured items");

                            if (ProjectX.getVarbitValue(VarbitID.BANK_SIDE_SLOT_SHOWOP) != 1 ||
                            ProjectX.getVarbitValue(VarbitID.BANK_SIDE_SLOT_IGNOREINVLOCKS) != 0) {
                                ...
                                if (ProjectX.getVarbitValue(VarbitID.BANK_SIDE_SLOT_IGNOREINVLOCKS) != 0) {
                                    Rs2Widget.clickWidget(InterfaceID.BankSideLocks.IGNORELOCKS);
                                    sleepUntil(() -> ProjectX.getVarbitValue(VarbitID.BANK_SIDE_SLOT_IGNOREINVLOCKS) == 0, 2000);
                                }
                                if (ProjectX.getVarbitValue(VarbitID.BANK_SIDE_SLOT_SHOWOP) != 1){
                                    Rs2Widget.clickWidget(InterfaceID.BankSideLocks.EXTRAOPTIONS);
                                    sleepUntil(()->ProjectX.getVarbitValue(VarbitID.BANK_SIDE_SLOT_SHOWOP) == 1, 2000);
                                }
                             ...
                             */
                            ProjectX.log(Level.INFO,"Depositing all items except locked slots: "+ slotsToLock);
                            ...
                            switch (energyOption) {
                                ...
                                case SUPER_ENERGY_POTION:
                                case ENERGY_POTION:
                                    ProjectX.log(Level.INFO,"Withdrawing potions to ensure 24 doses of "+energyOption);
                                    ensureEnergyDoses(energyOption, 24);
                                    potionsWithdrawn = true;
                                    break;
                                case STRANGE_FRUIT:
                                    ProjectX.log(Level.INFO,"Withdrawing strange fruit to ensure 6 in inventory");
                                ...
                                case NONE:
                                default:
                                    ProjectX.log(Level.INFO,"No energy restoration selected");
                                    break;
                            }

                            ProjectX.log(Level.INFO,"Fished banking, status: "+ (potionsWithdrawn ? "Potions withdrawn" : "No potions needed")+" | energy option: "+energyOption);
                    ...
                    case RETURN_FROM_ISLAND:
                        ProjectX.status = "Returning to island";
                        ProjectX.log(Level.INFO,"Returning to island");
                        LocalPoint returnLocalPoint = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), RETURN_LOCATION);
