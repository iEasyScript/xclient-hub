package net.runelite.client.plugins.projectx.magetrainingarena;
...
import net.runelite.client.plugins.projectx.magetrainingarena.enums.*;
...
import net.runelite.client.plugins.projectx.util.magic.*;
...
import static net.runelite.client.plugins.projectx.util.magic.Rs2Magic.getRs2Staff;
import static net.runelite.client.plugins.projectx.util.magic.Rs2Magic.getRs2Tome;
...
public class MageTrainingArenaScript extends Script {
    ...
    public boolean run(MageTrainingArenaConfig config) {
        this.config = config;
        ProjectX.log(String.format("repeatRoom: %s", config.repeatRoom()));
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (!super.run()) return;
                if (mtaPlugin != null && !ProjectX.getPluginManager().isActive(mtaPlugin)) return;

                if (!Rs2Magic.isSpellbook(Rs2Spellbook.MODERN)) {
                    ProjectX.log("Wrong spellbook found...please use the modern spellbook for this script.");
                    sleep(5000);
                    return;
                }

                if (mtaPlugin == null) {
                    if (ProjectX.getPluginManager() == null) return;

                    mtaPlugin = (MTAPlugin) ProjectX.getPluginManager().getPlugins()
                ...
                if (currentRoom == null) {
                    ...
                    if (currentPoints.entrySet().stream().allMatch(x -> getRequiredPoints().get(x.getKey()) * (config.buyRewards() ? 1 : (buyable + 1)) <= x.getValue())) {
                        if (config.buyRewards()) {
                            ...
                            if (config.reward() == Rewards.ALL_ITEMS) {
                                ProjectX.log("Reached required points for Collection Log completion! Goal achieved.");
                    ...
                    if (!missingPoints.isEmpty()) {
                        ...
                        ProjectX.showMessage("MTA: Out of runes! Please restart the plugin after you restocked on runes.");
                        sleep(500);
                        finishWithError("Out of runes - please restock and restart.");
                    }
            } else if (config.repeatRoom()) {
                if (currentRoom != null) {
                    ProjectX.log("Repeating room: " + currentRoom.name());
                    switch (currentRoom) {
                        case ALCHEMIST:
                            handleAlchemistRoom();
                            break;
                ...
                if (currentRoom == Rooms.ENCHANTMENT && Rs2Inventory.contains(ItemID.MAGICTRAINING_ENCHAN_SHAPEORB)) {
                    ProjectX.getRs2TileObjectCache().query().interact(ObjectID.MAGICTRAINING_ENCHA_HOLE, "Deposit");
                ...
                ProjectX.log("MTA Exception: " + ex.getMessage());
            }
    ...
    private void updatePoints() {
        for (var points : currentPoints.entrySet()) {
            ...
            if (widget != null && !ProjectX.getClientThread().runOnClientThreadOptional(widget::isHidden).orElse(false))
    ...
    private void handleEnchantmentRoom() {
        MagicAction enchant;
        var magicLevel = ProjectX.getClient().getBoostedSkillLevel(Skill.MAGIC);
        ...
        boolean successFullLoot = ProjectX.getRs2TileItemCache().query().withId(ItemID.MAGICTRAINING_DRAGONSTONE).interact("Take") && Rs2Inventory.waitForInventoryChanges(5000);
        ...
        Rs2TileObjectModel object = ProjectX.getRs2TileObjectCache().query().withId(bonusShape.getObjectId()).where(obj -> Rs2Camera.isTileOnScreen(obj.getLocalLocation())).nearest();
        if (object == null) {
    ...
    private void handleTelekineticRoom() {
        ...
        if (room.getTarget() != null)
            ...
            if (room.getTarget() == null) {
                ProjectX.log("Something seems wrong, room target was still not found...leaving room to reset.");
                leaveRoom();
                return;
            }
            target = room.getTarget();
        ...
        var localTarget = ProjectX.getClientThread().invoke(() -> LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), target));
        var targetConverted = ProjectX.getClientThread().invoke(() -> WorldPoint.fromLocalInstance(ProjectX.getClient(), Objects.requireNonNull(localTarget)));
        ...
        if (guardian.getWorldLocation().equals(room.getFinishLocation())) {
            ...
            while (!Rs2Player.getWorldLocation().equals(targetConverted)
                    && !Objects.equals(ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalDestinationLocation()), localTarget)) {
            ...
            boolean noTelegrabProjectile = ProjectX.getClientThread()
                    .runOnClientThreadOptional(() -> StreamSupport.stream(ProjectX.getClient().getProjectiles().spliterator(), false)
                            .noneMatch(x -> x.getId() == SpotanimID.TELEGRAB_TRAVEL))
                    .orElse(true);
            var position = ProjectX.getClientThread().invoke(room::getPosition);
    ...
    private void handleGraveyardRoom() {
        ...
        if (Rs2Inventory.contains(ItemID.BANANA, ItemID.PEACH)) {
            var currentHp = ProjectX.getClient().getBoostedSkillLevel(Skill.HITPOINTS);
            var maxHp = ProjectX.getClient().getRealSkillLevel(Skill.HITPOINTS);
    ...
    private void handleAlchemistRoom() {
        ...
        var timer = (AlchemyRoomTimer) ProjectX.getInfoBoxManager().getInfoBoxes().stream()
        ...
        if (room.getSuggestion() == null) {
            ProjectX.getClientThread().invoke(() -> ProjectX.getRs2TileObjectCache().query().withName("Cupboard").interact("Search"));
            sleep(300,600);
    ...
    private boolean areRoomRequirementsInvalid() {
        if (!currentRoom.getRequirements().getAsBoolean()) {
            ProjectX.log("You're missing room requirements. Please restock or fix your staves settings.");
            if (!config.repeatRoom()) {
                sleep(5000);
            }
            return true;
    ...
    private void buyReward(Rewards reward) {
        ...
        if (!Rs2Widget.isWidgetVisible(197, 0)) {
            ProjectX.getRs2NpcCache().query().withId(NpcID.MAGICTRAINING_GUARD_REWARDS).interact("Trade-with");
        ...
        if (Rs2Inventory.contains(config.reward().getItemId())) {
            ProjectX.log("Successfully obtained " + config.reward().toString() + "! Goal achieved.");
    ...
    public static void enterRoom(Rooms room) {
        ...
        ProjectX.getRs2TileObjectCache().query().interact(room.getTeleporter(), "Enter");
        Rs2Player.waitForAnimation();
    ...
    public static void leaveRoom() {
        ...
        ProjectX.getRs2TileObjectCache().query().interact(ObjectID.MAGICTRAINING_RETURNDOOR, "Enter");
        Rs2Player.waitForWalking();
    }
    ...
    private boolean handleFirstTime() {
        if (firstTime) {
            ...
            if (!Rs2Dialogue.isInDialogue())
                ProjectX.getRs2NpcCache().query().withId(NpcID.MAGICTRAINING_GUARD_ENTRANCE).interact("Talk-to");
