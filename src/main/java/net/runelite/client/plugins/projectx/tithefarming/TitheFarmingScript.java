package net.runelite.client.plugins.projectx.tithefarming;
...
import net.runelite.client.plugins.projectx.tithefarming.enums.TitheFarmLanes;
import net.runelite.client.plugins.projectx.tithefarming.enums.TitheFarmMaterial;
import net.runelite.client.plugins.projectx.tithefarming.enums.TitheFarmState;
import net.runelite.client.plugins.projectx.tithefarming.models.TitheFarmPlant;
...
import static net.runelite.client.plugins.projectx.tithefarming.enums.TitheFarmState.*;
import static net.runelite.client.plugins.projectx.util.dialogues.Rs2Dialogue.hasSelectAnOption;
...
public class TitheFarmingScript extends Script {
    ...
    public boolean run(TitheFarmingConfig config) {
        ...
        state = STARTING;
        allPlanted = false;
        ProjectX.log("Tithe farming script started");
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (config.enableDebugging() && plants.stream().anyMatch(x -> x.getGameObject() == null)) {
                    ProjectX.log("There is an empty plant gameobject!");
                }
    ...
    private void coreLoop(TitheFarmingConfig config) {
        ...
        WorldPoint corePlayerLoc = ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation());
        ...
        private void validateInventory() {
            if (!Rs2Inventory.hasItem(ItemID.DIBBER) || !Rs2Inventory.hasItem(ItemID.SPADE)) {
                ProjectX.showMessage("You need a seed dibber and a spade in your inventory!");
                shutdown();
            }
            if (!Rs2Inventory.hasItemAmount("watering can", WATERING_CANS_AMOUNT) && !Rs2Inventory.hasItem(ItemID.ZEAH_WATERINGCAN)) {
                ProjectX.showMessage("You need at least 8 watering can(8) or a Gricoller's can!");
                shutdown();
            }
        }
...
// Helper method to validate run energy and patches
        private void validateRunEnergy() {
            if (ProjectX.getClient().getEnergy() < 4000 && hasAllEmptyPatches() && state != RECHARING_RUN_ENERGY) {
                state = RECHARING_RUN_ENERGY;
                ProjectX.log("Recharging run energy...");
                Rs2Inventory.useRestoreEnergyItem();
            } else if (state == RECHARING_RUN_ENERGY && ProjectX.getClient().getEnergy() >= 4000) {
                state = STARTING;
            }
        }
    ...
    private static boolean interactWithObject(int id, String action) {
        Rs2TileObjectModel model = ProjectX.getRs2TileObjectCache().query()
                .withId(id)
                .nearest();
        if (model == null) {
            ProjectX.log("Object id " + id + " not in scene");
            return false;
        }
        WorldPoint playerLoc = Rs2Player.getWorldLocation();
        if (playerLoc != null && playerLoc.distanceTo(model.getWorldLocation()) > 51) {
            ProjectX.log("Object id " + id + " is " + playerLoc.distanceTo(model.getWorldLocation()) + " tiles away, walking...");
    ...
    private void refillWaterCans(TitheFarmingConfig config) {
        if (TitheFarmMaterial.hasGricollersCan()) {
            ...
            if (gricollerCanCharges < config.gricollerCanRefillThreshold()) {
                ...
                Rs2TileObjectModel barrel = ProjectX.getRs2TileObjectCache().query()
    ...
    private void walkToBarrel() {
        ...
        WorldPoint barrelPlayerLoc = ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation());
        if (barrelLoc.distanceTo2D(barrelPlayerLoc) > DISTANCE_THRESHOLD_MINIMAP_WALK) {
            Rs2Walker.walkMiniMap(barrelLoc, 1);
            sleepUntil(Rs2Player::isMoving);
        }
        sleepUntil(() -> barrelLoc.distanceTo2D(ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation())) < DISTANCE_THRESHOLD_MINIMAP_WALK);
    }
    ...
    private boolean depositSack() {
        if (Rs2Inventory.hasItem(TitheFarmMaterial.getSeedForLevel().getFruitId())) {
            ProjectX.log("Storing fruits into sack for experience...");
            Rs2TileObjectModel sack = ProjectX.getRs2TileObjectCache().query()
                    .withId(ObjectID.TITHE_SACK_OF_FRUIT_EMPTY)
                    .nearest();
            if (sack == null) {
                ProjectX.log("Sack not found in scene");
                return true;
            }
            ...
            if (sackLoc.distanceTo2D(playerLoc) > DISTANCE_THRESHOLD_MINIMAP_WALK) {
                ...
                sleepUntil(Rs2Player::isMoving);
                sleepUntil(() -> sackLoc.distanceTo2D(ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation())) < DISTANCE_THRESHOLD_MINIMAP_WALK);
            }
            sack.click();
