package net.runelite.client.plugins.projectx.revkiller;
...
import net.runelite.client.plugins.projectx.api.player.Rs2PlayerCache;
import net.runelite.client.plugins.projectx.api.tileitem.Rs2TileItemCache;
import net.runelite.client.plugins.projectx.api.tileitem.models.Rs2TileItemModel;
import net.runelite.client.plugins.projectx.api.tileobject.Rs2TileObjectCache;
...
public class revKillerScript extends Script {
    ...
    public boolean run(revKillerConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if(firstRun || weDied) {
                    ProjectX.log("It's our first run or we died!");
                    if(firstRun && ProjectX.getRs2NpcCache().query().withName(config.selectedRev().getName()).nearestOnClientThread() != null){
                        // we're all ready geared and there
                        firstRun = false;
                        ProjectX.log("It's our first run and we're all ready here!");
                        return;
                    }
                ...
                if(!areWeEquipped()||isItTimeToGo()){
                    if(WeAreInTheCaves()){
                        if(goodLootOnGround()){
                            if(areWeEquipped()){
                                if(!isPkerAround()){
                                    ProjectX.log("Returning, there's still loot on the ground");
                                    return;
                                }
                            }
                        }
    ...
    public boolean playerCheck(){
        ...
        List<net.runelite.client.plugins.projectx.api.player.models.Rs2PlayerModel> nearbyPlayers =
                ProjectX.getRs2PlayerCache()
                        .query()
        ...
        if(!nearbyPlayers.isEmpty()){
            if(!Rs2Player.isInCombat()) {
                ProjectX.log("There's another player here hopping.");
                hopToNewWorld();
            }
            return true;
        }
    ...
    public boolean weAreInCombat(){
        ...
        if(!Rs2Player.isAnimating()){
            ProjectX.log("Checking if we're in combat");
    ...
    public boolean goodLootOnGround(){
        ...
        if(ProjectX.getRs2TileItemCache().query().where(it->it.getWorldLocation().distanceTo(Rs2Player.getWorldLocation()) < 10 && it.getTotalGeValue() > 500).nearest() != null) return true;
        return false;
    }

    public Rs2NpcModel revenentKnight() {
        return ProjectX.getRs2NpcCache().query().withId(7939).nearest();
    }
    ...
    public void kiteTheKnight(){
        ...
        if(Rs2Player.getWorldLocation().equals(fifthTile)){
            if(weAreInCombat()) {
                ProjectX.log("We've all ready jammed the knight");
                return;
            } else {
                ProjectX.log("We need to click the rev.");
                if(revenentKnight()!=null && revenentKnight().getWorldLocation().distanceTo(jammedTile)<=2) {
                    if (revenentKnight().click("Attack")) {
                        ProjectX.log("We attacked the knight");
                        return;
                    }
                } else {
                    ProjectX.log("We need to re-jam the knight");
                }
            }
        } else {
                ...
                if(!revenentKnight().getWorldLocation().equals(thirdTile)){
                    ...
                    while(!revenentKnight().getWorldLocation().equals(thirdTile)){
                        ...
                        if(revenentKnight().getWorldLocation().distanceTo(Rs2Player.getWorldLocation())<=1 && !revenentKnight().getWorldLocation().equals(thirdTile)){
                            ProjectX.log("Rev is on a bad tile breaking loop");
                            return;
                        }
                        if(!Rs2Player.getWorldLocation().equals(secondTile)){
                            moveCameraToTile(secondTile);
        ...
        if(Rs2Player.getWorldLocation().equals(fifthTile)) {
            if (revenentKnight().click("Attack")) {
                ProjectX.log("We attacked the knight");
                ProjectX.log("Rev should be locked");
            }
        }
    ...
    public void moveCameraToTile(WorldPoint wp){
        LocalPoint lp = LocalPoint.fromWorld(ProjectX.getClient().getWorldView(ProjectX.getClient().getTopLevelWorldView().getId()), wp);
        Rectangle chatBox = null;
        Point canvasPoint = null;

        if(lp != null) {
            canvasPoint = Perspective.localToCanvas(ProjectX.getClient(), lp, wp.getPlane());
            if(canvasPoint != null) {
    ...
    public void WalkToRevs(){
        if(!WeAreInTheCaves()){
            ...
            if(Rs2Player.getWorldLocation().distanceTo(cave) > 6){
                ...
                if(selectedRev.contains("knight")){
                    ...
                    if(Rs2Walker.walkTo(selectedWP)){
                        ProjectX.log("Teleporting to level 40 rev cave.");
                    }
                } else {
                    if(Rs2Walker.walkTo(cave)){
                        ProjectX.log("Walking to cave. with new method.");
                    }
                }
            } else {
                if(!Rs2Dialogue.isInDialogue()){
                    ProjectX.log("At the cave, clicking.");
                    if(ProjectX.getRs2TileObjectCache().query().interact(31555, "Enter")){
            ...
            if(WeAreInTheCaves()){
                if(Rs2Walker.walkTo(selectedWP)){
                    ProjectX.log("Walking to Revs. with new method.");
                }
            }
        }
    }
    ...
    public void hopWorldsBasedOnTimer(){
        if(useTimedWorldHopper) {
            ...
            if (currentTime - startTime >= howLongUntilHop * 60 * 1000) {
                ProjectX.log("Hopping to a new world, it's been "+howLongUntilHop+" minutes");
                if(Rs2Player.isInCombat()){
                ...
                ProjectX.log("We'll hop to a new world in "+howLongUntilHop+" minutes");
            }
        }
    }
    public void getAwayFromPkerKnight(){
        ProjectX.log("Attempting to logout");
        Rs2Walker.setTarget(null);
        int io = 0;
        int tries = 10;
        if (ProjectX.isLoggedIn()) {
            while (ProjectX.isLoggedIn()) {
                ...
                Rs2Player.logout();
                sleepUntil(() -> !ProjectX.isLoggedIn() || Rs2Player.isInCombat(), Rs2Random.between(1500, 3000));
                if(Rs2Player.isInCombat() && ProjectX.isLoggedIn() || io>=tries){
                    ProjectX.log("We can't log out, running away instead.");
                    break;
                }
                io++;
            }
        ...
        if (isPkerAround() && ProjectX.isLoggedIn()) {
            getAwayFromPker();
            shouldFlee = false;
            return;
        }
        ...
        if (!ProjectX.isLoggedIn()) {
            while (!ProjectX.isLoggedIn()) {
                ...
                if (!ProjectX.isLoggedIn()) {
                    ...
                    sleepUntil(() -> ProjectX.isLoggedIn(), Rs2Random.between(10000, 20000));
                }
            }
            shouldFlee = false;
        }
    ...
    public void getAwayFromPker(){
        ...
        ProjectX.log("Attemping to get away from the PKer.");
        //kill the walker incase we were walking.
        Rs2Walker.setTarget(null);

        if(!Rs2Player.isTeleBlocked()){
            ProjectX.log("At least we're not teleblocked.");
            enablePrayer();
            ...
            if(Rs2Pvp.getWildernessLevelFrom(Rs2Player.getWorldLocation()) >= 20 && Rs2Pvp.getWildernessLevelFrom(Rs2Player.getWorldLocation()) <= 30) {
                while (Rs2Pvp.getWildernessLevelFrom(Rs2Player.getWorldLocation()) >= 20 && Rs2Pvp.getWildernessLevelFrom(Rs2Player.getWorldLocation()) <= 30) {
                    ProjectX.log("Attempting to teleport via glory");
                    if (!super.isRunning()) break;
                    if(!shouldFlee) break;
                    if(weDied){
                        shouldFlee = false;
            ...
            if(Rs2Pvp.getWildernessLevelFrom(Rs2Player.getWorldLocation()) <= 20) {
                while (Rs2Pvp.getWildernessLevelFrom(Rs2Player.getWorldLocation()) <= 20) {
                    ProjectX.log("Attempting to teleport via dueling");
                    if (!super.isRunning()) break;
                    if(weDied){
                        shouldFlee = false;
                        return;
            ...
            ProjectX.log("We're teleblocked! Attempting to run to the bank");
            enablePrayer();
    ...
    public void fightrev(revKillerConfig config){
        Rs2NpcModel Rev = ProjectX.getRs2NpcCache().query().withName(config.selectedRev().getName()).nearest();
        ...
        if(Rev!=null){
            ...
            if(Rev.isInteracting()) {
                if(hoppedWorld) {
                    ProjectX.log("Rev is attacking us attacking back.");
                    if (Rev.click("Attack")) {
                        hoppedWorld=false;
                        sleepUntil(() -> Rev.isDead() || !Rs2Player.isInCombat() || isItTimeToGo() || Rs2Player.getHealthPercentage() <= generateRandomNumber(70, 80), generateRandomNumber(60000, 120000));
                    }
            ...
            if(!Rs2Player.isInCombat()) {
                ProjectX.log("No revs found, hopping");
    ...
    public void startHealthCheck() {
        ProjectX.log("Health detection started");
    ...
    public void startPkerDetection() {
        ProjectX.log("PKer detection started");
    ...
    public void futurePKCheck(){
        try {
            ...
            ProjectX.log("Error during PK check: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void reJfount(){
        ...
        while(Rs2Player.getBoostedSkillLevel(Skill.PRAYER) < rejat || Rs2Player.getRunEnergy() <= runener){
            ...
            if(Rs2Bank.isOpen()){
                ...
                Rs2TileObjectModel rej = ProjectX.getRs2TileObjectCache().query().withId(39651).nearest();

                if(rej == null){
                    return;
                }
                ProjectX.log("Drinking");
                if(rej.click("Drink")){
                    sleepUntil(()-> Rs2Player.isMoving(), Rs2Random.between(1000,3000));
    ...
    public void hopToNewWorld(){
        ...
        World currentWorld = ProjectX.getWorldService().getWorlds().findWorld(ProjectX.getClient().getWorld());

        if(currentWorld == null){
            ProjectX.log("Couldn't find our world");
            return;
        }
        ...
        if(newRandomWorld == 0){
            ProjectX.log("Couldn't find a new random world");
            return;
        }

        ProjectX.log("We're going to world " + newRandomWorld);
        int attempts = 0;
        ...
        while(Rs2Player.getWorld() != newRandomWorld){
            ...
            if(ProjectX.hopToWorld(newRandomWorld)){
                sleepUntil(() -> !ProjectX.isHopping() || isPkerAround() || Rs2Player.getWorld() == newRandomWorld, generateRandomNumber(5000, 10000));
            }
    ...
    public void drinkPotion(){
        if(!Rs2Player.hasRangingPotionActive(potthresh)){
            ProjectX.log("Drinking ranging potion");
    ...
    public void drinkStamPotion(){
        if(!Rs2Player.hasStaminaActive()){
            if(Rs2Player.getRunEnergy() <= generateRandomNumber(20,40)) {
                ProjectX.log("Drinking stamina potion");
    ...
    public void loot(){
        List<Rs2TileItemModel> itemsOnGround = ProjectX.getRs2TileItemCache().query().where(it -> it.getWorldLocation().distanceTo(Rs2Player.getWorldLocation()) < 12 && !it.getName().contains("arrow") && it.getTotalGeValue() > 500).toList();
        List<Rs2TileItemModel> BlightedItemsOnGround = ProjectX.getRs2TileItemCache().query().where(it -> it.getWorldLocation().distanceTo(Rs2Player.getWorldLocation()) < 12
        ...
        if(!itemsOnGround.isEmpty()){
            for (Rs2TileItemModel theItem : itemsOnGround) {
                ...
                if(!Rs2Inventory.isFull()){
                    if(ProjectX.getRs2TileItemCache().query().withId(theItem.getId()).interact("Take")){
    ...
    public void Bankfortrip(){
        ...
        if(!Rs2Bank.isOpen()){
            ...
            if(!WeAreInTheCaves()) {
                ProjectX.log("Walking and using bank");
                OpenTheInv();
                stopTeleSpam();
            ...
            ProjectX.log("Random number: " + howtobank);
            //equipring
            if(howtobank <= 80){
                if(isItTimeToGo() || weHaveLoot()){
                    ProjectX.log("We have loot, depositing all");
                    Rs2Bank.depositAll();
            ...
            if(howtobank <= 40){
                if(Rs2Equipment.get(EquipmentInventorySlot.AMULET).getName().equals("Amulet of glory")){
                    ProjectX.log("Getting a fresh amulet of glory");
                    if(Rs2Bank.get(it->it!=null&&it.getName().contains("Amulet of glory(")) != null){
                        ...
                        ProjectX.log("Out of amulets of glory 6");
                        super.shutdown();
                    }
                    if(Rs2Inventory.contains(it->it!=null&&it.getName().contains("Amulet of glory("))){
            ...
            if(howtobank <= 40){
                if(Rs2Equipment.get(EquipmentInventorySlot.RING)!=null){
                    ...
                    ProjectX.log("Getting the ring of dueling");
                    if(Rs2Bank.count(ItemID.RING_OF_DUELING8)>0){
                        ...
                        ProjectX.log("Out of rings of dueling");
                        super.shutdown();
                    }
                    if(Rs2Inventory.contains(ItemID.RING_OF_DUELING8)){
                ...
                if(Rs2Equipment.get(EquipmentInventorySlot.GLOVES)!=null){
                    if(Rs2Equipment.get(EquipmentInventorySlot.GLOVES).getName().contains("ethereum")){
                        if(Rs2Equipment.get(EquipmentInventorySlot.GLOVES).getId() == ItemID.BRACELET_OF_ETHEREUM_UNCHARGED){
                            ProjectX.log("We need to charge our bracelet");
                            if(Rs2Bank.hasItem("Revenant ether") && Rs2Bank.count("Revenant ether") > 100){
                                ...
                                ProjectX.log("We're out of ether. Stopping.");
                                super.shutdown();
                                return;
                            }
                            if(Rs2Inventory.contains("Revenant ether")){
                        ...
                        if(!Rs2Inventory.contains(selectedArrow)){
                            ...
                            if(selectedArrow == ItemID.BOLT_RACK){
                                ...
                                if(Rs2Equipment.get(EquipmentInventorySlot.WEAPON).getId() == ItemID.KARILS_CROSSBOW_25){
                                    ProjectX.log("Please get a fresh karil's crossbow. Shutting down.");
                                    super.shutdown();
                                    return;
                                }
                            }
            ...
            if(selectedRev.contains("knight")) {
                if (howtobank <= 80) {
                    ProjectX.log("Grabbing rev cave teles");
                    if (!Rs2Inventory.contains("Revenant cave teleport")) {
                        if (Rs2Bank.count("Revenant cave teleport") > 5) {
                            ...
                            ProjectX.log("Rev cave teleports, need at least 5");
                            super.shutdown();
                            return;
                        }
                    }
            ...
            //get stamina pot
            if(howtobank <= 40){
                ProjectX.log("Withdrawing Stamina potion");
                if(!Rs2Inventory.contains(it->it!=null&&it.getName().contains("Stamina"))){
                    if(Rs2Bank.count("Stamina potion(4)") > 0 || Rs2Bank.count("Stamina potion(3)") > 0 || Rs2Bank.count("Stamina potion(2)") > 0){
                        ...
                        ProjectX.log("Out of stamina potions");
                        super.shutdown();
                        return;
                    }
                }
            ...
            if(howtobank <= 40){
                ProjectX.log("Withdrawing Ranging potion");
                if(!Rs2Inventory.contains(it->it!=null&&it.getName().contains("Ranging"))){
                    if(Rs2Bank.getBankItem("Ranging potion(4)") != null || Rs2Bank.getBankItem("Ranging potion(3)") != null){
                        ...
                        ProjectX.log("Out of ranging potions");
                        super.shutdown();
                        return;
                    }
                }
            ...
            if(howtobank <= 40){
                ProjectX.log("Withdrawing Sharks");
    ...
    public boolean isItTimeToGo(){
        ...
        if(value>=config.leaveAtValue()){
            ProjectX.log("We have enough loot");
            return true;
        }
        if(Rs2Inventory.isFull()){
            ProjectX.log("We have enough loot");
        ...
        if(timeToBreak()){
            ProjectX.log("It's time to break");
            return true;
        }
        ProjectX.log("We have "+value+" worth of loot");
        return false;
    }
    ...
    if (Rs2Equipment.get(EquipmentInventorySlot.AMMO) == null) {
        ProjectX.log("We have no ammo!");
        return false;
    }

    if (Rs2Equipment.get(EquipmentInventorySlot.AMMO).getQuantity() < LowOnArrowsCount) {
        ProjectX.log("We don't have enough ammo!");
        return false;
    }

    if (!Rs2Inventory.contains(it -> it != null && it.getName().contains("Stamina"))) {
        ProjectX.log("We have no stam!");
        return false;
    }

    if (!Rs2Inventory.contains(it -> it != null && it.getName().contains("Ranging"))) {
        ProjectX.log("We have no Ranging potion!");
        return false;
    }

    if (!Rs2Inventory.contains(ItemID.SHARK)) {
        ProjectX.log("We're out of sharks!");
        return false;
    }

    if (Rs2Inventory.count(ItemID.SHARK) < 2) {
        ProjectX.log("We have less than 2 sharks!");
        return false;
    }

    if (Rs2Equipment.get(EquipmentInventorySlot.RING) == null) {
        ProjectX.log("ring is null");
        return false;
    }

    if (!Rs2Equipment.get(EquipmentInventorySlot.RING).getName().contains("dueling")) {
        ProjectX.log("We don't have our ring of dueling");
        return false;
    }
    ...
    if(Rs2Equipment.get(EquipmentInventorySlot.GLOVES)!=null) {
        if (Rs2Equipment.get(EquipmentInventorySlot.GLOVES).getName().contains("ethereum")) {
            if (Rs2Equipment.get(EquipmentInventorySlot.GLOVES).getId() == ItemID.BRACELET_OF_ETHEREUM_UNCHARGED) {
                ProjectX.log("Our bracelet doesn't have enough charges.");
                return false;
            }
        }
    }

    ProjectX.log("We're fully equipped and ready to go.");
    return true;
    ...
    public boolean isPkerAround(){
        ...
        for (Rs2PlayerModel player : playerlist) {
            ProjectX.log("There may be a pker around us "+player.getName());
            ...
            ProjectX.log("They have a "+NameOfPlayersWeapon);
            ProjectX.log("They're wearing a' "+nameOfPlayersChestPiece);

            for (String weapon : weapons) {
                if (lowercaseWeapon.contains(weapon) || lowercaseWeapon.equals(weapon)) {
                    ProjectX.log("This player is wielding a " + NameOfPlayersWeapon + " which is used to pk so we're outy.");
                    return true;
                }
            }
            ...
            for (String chestArmor : chestArmors) {
                if (lowercaseChestPiece.contains(chestArmor) || lowercaseChestPiece.equals(chestArmor)) {
                    ProjectX.log("This player is wielding a " + nameOfPlayersChestPiece + " which is used to pk so we're outy.");
                    return true;
                }
            }
            ...
            if ((player.getInteracting() != null) && (player.getInteracting().equals(Rs2Player.getLocalPlayer()))) {
                ProjectX.log(player.getName() + " is attacking us");
                return true;
            }
