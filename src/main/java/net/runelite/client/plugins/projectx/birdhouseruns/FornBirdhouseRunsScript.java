package net.runelite.client.plugins.projectx.birdhouseruns;
...
import net.runelite.client.plugins.projectx.birdhouseruns.FornBirdhouseRunsInfo.states;
import net.runelite.client.plugins.projectx.birdhouseruns.enums.Log;
...
import static net.runelite.client.plugins.projectx.birdhouseruns.FornBirdhouseRunsInfo.*;
...
public class FornBirdhouseRunsScript extends Script {
                    ...
                    switch (botStatus) {
                        ...
                        case FINISHING:
                            ...
                            if (config.startGiantSeaweedAfter()) {
                                ...
                                if (!ProjectX.startPlugin("net.runelite.client.plugins.projectx.GiantSeaweedFarmer.GiantSeaweedFarmerPlugin")) {
    ...
    private boolean dismantleBirdhouse(WorldPoint loc, int varpId) {
        ...
        int varp = ProjectX.getVarbitPlayerValue(varpId);
        if (!isSeeded(varp)) {
        ...
        if (!sleepUntil(() -> isEmpty(ProjectX.getVarbitPlayerValue(varpId)), 10000)) {
            log.warn("Dismantle[{}]: timeout waiting for varp→0 after Empty click; varp={} (player at {})",
                    varpId, ProjectX.getVarbitPlayerValue(varpId), Rs2Player.getWorldLocation());
            return false;
        }
    ...
    private boolean buildBirdhouse(WorldPoint loc, int varpId) {
        ...
        if (!sleepUntil(() -> !isEmpty(ProjectX.getVarbitPlayerValue(varpId)), 15000)) {
        ...
        log.info("Build[{}]: success (varp={})", varpId, ProjectX.getVarbitPlayerValue(varpId));
        return true;
    }
