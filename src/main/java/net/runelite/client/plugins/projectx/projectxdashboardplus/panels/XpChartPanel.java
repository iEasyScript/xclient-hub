package net.runelite.client.plugins.projectx.projectxdashboardplus.panels;
...
public class XpChartPanel extends DashboardSection {
    ...
    private Skill restoreSkill(Skill fallback) {
        try {
            String raw = ProjectX.getConfigManager().getConfiguration(CONFIG_GROUP, K_CHART_SKILL);
    ...
    private int restoreWindowIndex() {
        try {
            String raw = ProjectX.getConfigManager().getConfiguration(CONFIG_GROUP, K_CHART_WINDOW_INDEX);
    ...
    private static void persistString(String key, String value) {
        try {
            ProjectX.getConfigManager().setConfiguration(CONFIG_GROUP, key, value);
