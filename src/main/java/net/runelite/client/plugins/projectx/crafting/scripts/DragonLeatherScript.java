...
import net.runelite.client.plugins.projectx.crafting.enums.DragonLeatherArmour;
...
public class DragonLeatherScript extends Script implements ICraftingScript {
    ...
    private void handleBanking() {
        ...
        if (Rs2Bank.isOpen()) {
            if (config.useCostumeNeedle()) {
                if (!Rs2Inventory.hasItem(costumeNeedle)) {
                    if (!Rs2Bank.hasItem(costumeNeedle)) {
                        ProjectX.showMessage("You don't have a costume needle!");
                        state = CraftingState.STOPPED;
                        shutdown();
                        return;
                    }
                ...
                if (!Rs2Inventory.hasItem(needle) || !Rs2Inventory.hasItem(thread)) {
                    if (!Rs2Bank.hasAllItems(new int[]{needle, thread})) {
                        ProjectX.showMessage("You've run out of materials!");
                        state = CraftingState.STOPPED;
                        shutdown();
                        return;
                    }
            ...
            if (!Rs2Bank.hasItem(armour.getLeatherId())) {
                ProjectX.showMessage("You've run out of leather!");
                state = CraftingState.STOPPED;
                shutdown();
                return;
            }
    ...
    private void handleCrafting() {
        DragonLeatherArmour armour = config.dragonLeatherType();
        ProjectX.status = "Crafting " + armour.getName();
        ...
        if (lastCraftingXp <= 0) {
            lastCraftingXp = ProjectX.getClient().getSkillExperience(Skill.CRAFTING);
        }
        ...
        if (sleepUntil(() -> Rs2Widget.getWidget(17694733) != null, 5000)) {
            ...
            while (super.run() && hasMaterials() && ProjectX.isGainingExp) {
                long currentXp = ProjectX.getClient().getSkillExperience(Skill.CRAFTING);
