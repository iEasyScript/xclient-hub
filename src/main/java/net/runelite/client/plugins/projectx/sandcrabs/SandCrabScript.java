package net.runelite.client.plugins.projectx.sandcrabs;

import net.runelite.api.GameState;
import net.runelite.api.coords.WorldPoint;
import net.runelite.client.plugins.projectx.ProjectX;
import net.runelite.client.plugins.projectx.Script;
import net.runelite.client.plugins.projectx.sandcrabs.enums.State;
import net.runelite.client.plugins.projectx.sandcrabs.models.ScanLocation;
...
public class SandCrabScript extends Script {
            ...
            try {
                ...
                if (!config.DisableInventorySetup()) {
                    if (inventorySetup == null || config.inventorySetup().getName().isEmpty()) {
                        ProjectX.showMessage("Please setup your Inventory Setup for sand crabs! Stopping...");
                        shutdown();
                        return;
                    }
                }
                ...
                if (!inventorySetup.doesEquipmentMatch() && !config.DisableInventorySetup()) {
                    if (Rs2Bank.walkToBankAndUseBank()) {
                        if (inventorySetup.loadEquipment()) {
                            ProjectX.log("We're setup for sand crabs!");
                            return;
                        }
                    }
                }
                ...
                if (currentScanLocation != null && ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation()).distanceTo(currentScanLocation.getWorldPoint()) > 10 && (state != State.RESET_AGGRO && state != State.WALK_BACK && state != State.BANK)) {
                ...
                currentScanLocation = sandCrabLocations.stream()
                        ...
                        .min(Comparator.comparingInt(x -> x.getWorldPoint().distanceTo(ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation()))))
                        .orElse(null);

                if (sandCrabLocations.stream()
                        .noneMatch(x -> x.getWorldPoint()
                                .equals(ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation())))
                ...
                switch (state) {
                    ...
                    case HOP_WORLD:
                        ...
                        boolean isHopped = ProjectX.hopToWorld(world);
                        if (isHopped) {
    ...
    {
        if (ProjectX.getClient() == null || ProjectX.getClientThread() == null)
        {
            return false;
        }
        ...
        List<Rs2NpcModel> npcs = ProjectX.getRs2NpcCache().query().withName("Sandy rocks").toListOnClientThread();
        if (npcs.isEmpty()) {
    ...
    private void scanSandCrabLocations(SandCrabConfig config) {
        ...
        if (currentScanLocation.getWorldPoint().distanceTo(ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation())) > 10) {
