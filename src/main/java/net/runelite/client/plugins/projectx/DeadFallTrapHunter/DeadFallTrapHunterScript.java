package net.runelite.client.plugins.projectx.DeadFallTrapHunter;
...
public class DeadFallTrapHunterScript extends Script {
    ...
    public boolean run(DeadFallTrapHunterConfig config, DeadFallTrapHunterPlugin plugin) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (forceBank) {
                    ProjectX.log("Inventory too full - Banking");
                    handleBanking(config);
                }
                if (hasDied) {
                    ProjectX.log("We died - Shutting down");
                    shutdown();
                }
                if ((Rs2Inventory.count("axe") < 1 && !Rs2Equipment.isWearing("axe")) && plugin.getTraps().isEmpty()) {
                    ProjectX.log("No axe found - Banking");
                    handleBanking(config);
                    return;
                }
                if (!Rs2Inventory.hasItem("Knife")) {
                    ProjectX.log("No knife found - Banking");
                    handleBanking(config);
                    return;
                }
                ...
                if (!isNearArea(deadFallTrapHunting)) {
                    ProjectX.log("Walking to " + deadFallTrapHunting.getName() + " hunting area");
    ...
    private void handleWoodCutting(DeadFallTrapHunterConfig config, DeadFallTrapHunterPlugin plugin) {
        ...
        if (tree != null) {
            ...
            ProjectX.log("No nearby tree found - Walking back to origin");
            WalkBackToHunterArea(config, plugin);
        }
    }
    ...
    private DeadFallTrapHunting getBestDeadFallHuntingCreature() {
        var skillLevel = ProjectX.getClient().getRealSkillLevel(Skill.HUNTER);
        if (skillLevel > 56) {
        ...
        ProjectX.log("Not high enough hunter level for any deadfall creature");
        shutdown();
        return null;
    }
    ...
    private void getBestAxe(DeadFallTrapHunterConfig config) {
        ...
        if (bestAxeOverall != null) {
            ...
            ProjectX.log("No suitable axe found in bank, inventory, or equipment");
            shutdown();
        }
    }
    ...
    private boolean handleExistingTraps(DeadFallTrapHunterPlugin plugin, DeadFallTrapHunterConfig config) {
        ...
        if (!Rs2Player.isAnimating() && !Rs2Player.isMoving()) {
            Rs2TileObjectModel gameObject = ProjectX.getRs2TileObjectCache().query().within(location, 0).first();
            if (gameObject != null) {
    ...
    private void setNewTrap(DeadFallTrapHunting deadFallTrapHunting, DeadFallTrapHunterConfig config) {
        if (ProjectX.getRs2TileObjectCache().query().withId(deadFallTrapHunting.getTrapId()).count() > 0) {
            ProjectX.getRs2TileObjectCache().query().interact(deadFallTrapHunting.getTrapId(), "Set-trap");
