...
public class CharterCrafterScript extends Script {
    ...
    private void bootstrap() {
        ...
        Rs2NpcModel trader = ProjectX.getRs2NpcCache().query().withName(TRADER_NAME).nearestOnClientThread();
        if (trader == null) {
    ...
    private void buyMaterials() {
        ...
        if (remainingSeaweed > 0 || remainingSand > 0) {
            ...
            worldHopAttempts = 0;
            beforeHopWorld = ProjectX.getClient().getWorld();
    ...
    private void worldHop() {
        if (worldHopPending) {
            GameState gs = ProjectX.getClient().getGameState();
        ...
        boolean hopCall = ProjectX.hopToWorld(world);
        if (!hopCall) {
            sleep(800);
        }
    }
