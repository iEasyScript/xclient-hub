...
public class BaggedPlantsScript extends Script {
    ...
    private void calculateState(BaggedPlantsConfig config) {
        ...
        if (inHouse && hasUnnotedPlants) {
            ...
            var builtPlant = ProjectX.getRs2TileObjectCache().query().withId(BUILT_PLANT).nearest();
            if (builtPlant != null) {
                state = BaggedPlantsState.REMOVE_PLANT;
            } else {
                // No built plant, check for empty plant space to build on
                var plantSpace = ProjectX.getRs2TileObjectCache().query().withId(PLANT_SPACE).nearest();
                if (plantSpace != null) {
    ...
    private boolean checkRequiredItems() {
        if (!Rs2Inventory.hasItem(COINS)) {
            ProjectX.showMessage("Missing item Coins, go back and get it!");
            shutdown();
            return false;
        }
        ...
        if (totalWateringCans < 3) {
            ProjectX.showMessage("3 watering cans required - go back and get them!");
            shutdown();
            return false;
        }
        ...
        if (!Rs2Inventory.hasItem(UNNOTED_BAGGED_PLANT) && !Rs2Inventory.hasItem(NOTED_BAGGED_PLANT)) {
            ProjectX.showMessage("No more plants, shutting down");
            shutdown();
            return false;
        }
    ...
    private boolean isInHouse() {
        ...
        return ProjectX.getRs2NpcCache().query().withName("Phials").nearestOnClientThread() == null;
    }
    ...
    private void enterHouse(BaggedPlantsConfig config) {
        var housePortal = ProjectX.getRs2TileObjectCache().query().withId(HOUSE_PORTAL).nearest();
        if (housePortal != null) {
            ...
            ProjectX.showMessage("Can't find house portal!");
            shutdown();
        }
    }
    ...
    private void refillSupplies() {
        ...
        if (!Rs2Inventory.hasItem(UNNOTED_BAGGED_PLANT) && Rs2Inventory.hasItem(NOTED_BAGGED_PLANT)) {
            if (Rs2Widget.getWidget(14352385) == null) {
                if (!Rs2Inventory.isItemSelected()) {
                    ...
                    ProjectX.getClientThread().invoke(() -> ProjectX.getRs2NpcCache().query().withName("Phials").interact("Use"));
        ...
        if (fullWateringCans < 3) {
            ...
            if (wateringCanToRefill != -1) {
                var well = ProjectX.getRs2TileObjectCache().query().withId(SINK).nearest();
                if (well != null) {
                    ...
                    ProjectX.showMessage("Can't find well to refill watering cans!");
                    shutdown();
                    return;
                }
            } else {
                ProjectX.showMessage("Can't find watering can to refill!");
                shutdown();
                return;
            }
        }
