package net.runelite.client.plugins.projectx.pitfallhunter;
...
public class PitfallHunterScript extends Script
{
    ...
    public boolean run(PitfallHunterConfig config)
    {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                if (!super.run() || !ProjectX.isLoggedIn() || !isRunning()) {
                    return;
                }
    ...
    private PitfallDefinition findClosestUsablePitForNpc(Rs2NpcModel npc)
    {
        ...
        PitfallObjectCandidate selectedCandidate = ProjectX.getRs2TileObjectCache().query()
                .within(25)
    ...
    private String getAvailableNpcAction(Rs2NpcModel npc, List<String> possibleActions)
    {
        ...
        NPCComposition composition = ProjectX.getClientThread()
                .runOnClientThreadOptional(() -> ProjectX.getClient().getNpcDefinition(npc.getId()))
                .orElse(null);
    ...
    private boolean cutLogsLocally()
    {
        ...
        while (isRunning() && logCount() < targetLogs && Rs2Inventory.emptySlotCount() > 0) {
            int before = logCount();
            Rs2TileObjectModel tree = ProjectX.getRs2TileObjectCache().query()
                    .withIds(LOCAL_TREE_IDS)
                    .where(this::isReachableTree)
                    .nearestOnClientThread(20);

            if (tree == null) {
                tree = ProjectX.getRs2TileObjectCache().query()
                        .withIds(LOCAL_TREE_IDS)
                        .where(this::isReachableTree)
    ...
    {
        ProjectX.log(level, "[PitfallHunter] " + message);
    }
