package net.runelite.client.plugins.projectx.cooking.scripts;
...
import net.runelite.client.plugins.projectx.cooking.AutoCookingConfig;
...
import static net.runelite.client.plugins.projectx.util.player.Rs2Player.toggleRunEnergy;
...
public class BurnBakingScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                long startTime = System.currentTimeMillis();
                    ...
                    int currentCookingLevel = ProjectX.getClient().getRealSkillLevel(Skill.COOKING);
                    ...
                    if (currentCookingLevel < 41 && currentCookingLevel >= 25 && shouldCookStew() && Rs2Inventory.hasItem("Uncooked stew")) {
                        ...
                        ProjectX.log("You've tried to prepare cake between level 25 and 40 so maybe you wanna make stew. Making stew now.");
                        manageRunEnergy();
                        cookItem("Uncooked stew");
                        ProjectX.naturalMouse.moveOffScreen();
    ...
    private void withdrawAndCheck(String itemName) {
        for (int attempt = 0; attempt < 5 && !Rs2Inventory.contains(itemName); attempt++) {
            if (attempt > 0) {
                ProjectX.log(itemName + " was not withdrawn, retrying... (Attempt " + (attempt + 1) + ")");
            }
        ...
        if (!Rs2Inventory.contains(itemName)) {
            ProjectX.log("Failed to withdraw " + itemName + " after multiple attempts!");
        }
    }
    ...
    public void walkToBanker() {
        ...
        if (bankerLocation == null) {
            ProjectX.log("Failed to get the banker's location.");
            return;
        }

        WorldPoint bankerWorldLocation = WorldPoint.fromLocal(ProjectX.getClient(), bankerLocation);
        WorldPoint playerWorldLocation = ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation());
        ...
        WorldPoint worldLocation = WorldPoint.fromLocal(ProjectX.getClient(), targetLocation);
    ...
    public static List<Rs2NpcModel> getBankerNPCs() {
        List<Rs2NpcModel> bankers = ProjectX.getRs2NpcCache().query()
                .where(npc -> npc.getNpc() != null && npc.getNpc().getComposition() != null
                        && npc.getNpc().getComposition().getActions() != null
                        && Arrays.asList(npc.getNpc().getComposition().getActions()).contains("Bank"))
                .toList();
    ...
    private void prepareBread() {
        ...
        if (currentCookingLevel < 25) {
            ...
            ProjectX.log("You've tried to prepare cake below level 25, so maybe you wanna make bread. Making bread now.");
                ...
                if (Rs2Bank.isOpen()) {
                    ...
                    // Step 4: If we have flour and water in the bank, withdraw them
                        ...
                        if (!Rs2Inventory.hasItem(POT_OF_FLOUR) || !Rs2Inventory.hasItem(BUCKET_OF_WATER)) {
                            ProjectX.log("trying to withdrawX didn't populate inventory, so withdrawing all");
                            Rs2Bank.openBank();
                        ...
                        ProjectX.log("Could not find flour or water in the bank for bread-making.");
                    }
    ...
    public boolean clickWidget(int id) {
        Widget widget = ProjectX.getClientThread().runOnClientThreadOptional(() -> ProjectX.getClient().getWidget(id)).orElse(null);
    ...
    private void prepareStew() {
        ProjectX.log("Entering prepareStew method");
        ...
        ProjectX.log("Completed prepareStew method");
    }
    ...
    private boolean shouldCookStew() {
        ProjectX.log("Entering shouldCookStew method");
    ...
    private void cookItem(String uncookedItemName) {
        ...
        ProjectX.status = "Cooking " + uncookedItemName;
    ...
    public static void manageRunEnergy() {
        int energy = ProjectX.getClient().getEnergy();  // Get current run energy level
        boolean isRunEnabled = ProjectX.getVarbitPlayerValue(173) == 1;  // Check if run is enabled
    ...
    public void findAndInteractWithNearestRange() {
        ...
        net.runelite.client.plugins.projectx.api.tileobject.models.Rs2TileObjectModel rangeObj = ProjectX.getRs2TileObjectCache().query()
    ...
    private void prepareCake() {
        ...
        if (!Rs2Bank.isOpen() && Rs2Inventory.contains(1944) && Rs2Inventory.contains("Bucket of milk") && Rs2Inventory.contains("Pot of flour") && Rs2Inventory.contains("Cake tin")) {
            ...
            if (!Rs2Bank.hasItem(1944)) {
                ProjectX.log("No more egg found in the bank");
            }
        }
    }
    ...
    private void depositAndWithdrawItems() {
        ...
        if (Rs2Bank.isOpen() && Rs2Inventory.isEmpty()) {
            ...
            if (!Rs2Inventory.isFull()) {
                ProjectX.log("inventory not full after attempted withdrawing of 4x of 7 items (28 items)");
                ProjectX.log("Withdrawing ALL of X item(s) IF inventory doesn't have X item");
                ProjectX.log("If you are looping here make sure to have an EVEN NUMBER of EACH ingredient at the start");
