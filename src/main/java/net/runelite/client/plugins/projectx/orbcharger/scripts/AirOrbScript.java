package net.runelite.client.plugins.projectx.orbcharger.scripts;
...
import net.runelite.client.plugins.projectx.orbcharger.OrbChargerPlugin;
...
public class AirOrbScript extends Script {
            ...
            try {
                ...
                switch (state) {
                    case BANKING:
                        ...
                        if (!hasAirStaffEquipped) {
                            ...
                            if (filteredAirStaves.isEmpty()) {
                                ProjectX.showMessage("Missing Air Staff in Bank!");
                                shutdown();
                                return;
                            }
                        ...
                        if (plugin.isUseEnergyPotions()) {
                            if (!Rs2Inventory.hasItem(Rs2Potion.getRestoreEnergyPotionsVariants().toArray(String[]::new))) {
                                if (!Rs2Bank.hasItem(Rs2Potion.getRestoreEnergyPotionsVariants())) {
                                    ProjectX.showMessage("Missing Energy Restore Potions");
                                    shutdown();
                                    return;
                                }
                                ...
                                if (energyRestoreItem == null) {
                                    ProjectX.showMessage("Unable to find Energy Restore Potions");
                                    shutdown();
                                    return;
                                }
                        ...
                        if (plugin.isUseStaminaPotions()) {
                            if (!Rs2Inventory.hasItem(Rs2Potion.getStaminaPotion())) {
                                if (!Rs2Bank.hasItem(Rs2Potion.getStaminaPotion())) {
                                    ProjectX.showMessage("Missing Stamina Potions");
                                    shutdown();
                                    return;
                                }
                        ...
                        if (neededCosmicRunes > 0) {
                            if (!Rs2Bank.hasBankItem(ItemID.COSMICRUNE, neededCosmicRunes)) {
                                ProjectX.showMessage("Missing required cosmic runes in bank.");
                                shutdown();
                                return;
                            }
                        ...
                        if (neededUnpoweredOrbs > 0) {
                            if (!Rs2Bank.hasBankItem(ItemID.STAFFORB, neededUnpoweredOrbs)) {
                                ProjectX.showMessage("Missing required unpowered orbs in bank.");
                                shutdown();
                                return;
                            }
                    ...
                    case CHARGING:
                        ...
                        ProjectX.getRs2TileObjectCache().query().interact(ObjectID.OBELISK_OF_AIR);
                        ...
                        break;
                    case DRINKING:
                        ProjectX.getRs2TileObjectCache().query().interact(ObjectID.POOL_OF_REFRESHMENT);
    ...
    private boolean shouldHandleBankingOrFleeing() {
        ...
        if (shouldBank() && !isTeleportItemEquipped()) {
            ProjectX.log("Banking needed due to missing teleport item.");
            return true;
        }

        if (shouldBank() && !hasRequiredItems()) {
            ProjectX.log("Banking needed due to missing required items.");
            return true;
        }
    ...
    private void handleLogout() {
        ...
        Rs2Player.logout();
        sleepUntil(() -> ProjectX.getClient().getGameState() != GameState.LOGGED_IN, 5000);

        if (ProjectX.getClient().getGameState() == GameState.LOGGED_IN) {
            ProjectX.log("Logout unsuccessful, retrying logout.");
    ...
    private boolean shouldDrinkFromPool() {
        ...
        Rs2TileObjectModel refreshmentPool = ProjectX.getRs2TileObjectCache().query().withId(ObjectID.POOL_OF_REFRESHMENT).nearest();
    ...
    private void handleReplaceTeleportItem() {
        ...
        if (teleportItem == null) {
            ProjectX.showMessage("Missing required teleport item in bank!");
            shutdown();
            return;
        }
        ...
        if (Rs2Inventory.isFull()) {
            ...
            if (Rs2Inventory.hasItem(ItemID.STAFFORB)) {
                ...
                ProjectX.showMessage("Inventory full, but no unpowered orbs to deposit!");
                shutdown();
                return;
            }
        }
