...
import net.runelite.client.plugins.projectx.util.mouse.Mouse;
...
public class BankTabSorterScript extends Script {
    ...
    public boolean run() {
        if (isRunningTask) {
            ProjectX.log("Script is already running. Please wait for the current execution to finish.");
            return false;
        }

        mouse = ProjectX.getMouse(); // Assuming ProjectX.getMouse() is thread-safe or called from client thread
        ...
        mainScheduledFuture = scheduledExecutorService.schedule(() -> {
            try {
                if (!Rs2Bank.isOpen()) {
                    ProjectX.log("Bank is not open. Please open the bank and run the script again.");
                ...
                if (itemsInCurrentTab == null || itemsInCurrentTab.isEmpty()) { // Corrected check
                    ProjectX.log("Failed to retrieve items from the current bank tab, or tab is empty. Please ensure the bank is open and try again.");
                    return; // Exit this scheduled task
                }

                ProjectX.log("Found " + itemsInCurrentTab.size() + " items in the current tab.");
                ProjectX.log("Applying workflow-based sorting logic...");
                long startTime = System.currentTimeMillis();
                List<BankSortItem> sortedItems = sortItemsAndAssignSlots(itemsInCurrentTab);
                long endTime = System.currentTimeMillis();
                ProjectX.log("Item list sorted in " + (endTime - startTime) + "ms. Calculated " + sortedItems.size() + " target positions.");

                int currentBankTab = ProjectX.getVarbitValue(Varbits.CURRENT_BANK_TAB);
                ...
                ProjectX.log("Item list sorted with fuzzy similarity prioritization in " + (endTime - startTime) + "ms. Calculated " + sortedItems.size() + " target positions.");
                ProjectX.log("It's recommended to review the tab. If issues, run again or manually adjust.");
            } catch (Exception e) { // Single catch block for Exception
                log.error("Error during bank sorting script execution:", e);
                ProjectX.log("An unexpected error occurred: " + e.getMessage());
            } finally {
                isRunningTask = false;
            }
        }, 0, TimeUnit.SECONDS);
    ...
    private List<BankSortItem> getItemsInCurrentTab() {
        ...
        if (allBankItemWidgets == null || allBankItemWidgets.isEmpty()) {
            ProjectX.log("Rs2Bank.getItems() returned null or empty list.");
            return Collections.emptyList();
        }

        int currentTabVar = ProjectX.getVarbitValue(Varbits.CURRENT_BANK_TAB);
        ...
        if (currentTabVar == 0) { // "View All" or main tab items
            ...
            itemsInThisTabCount = ProjectX.getVarbitValue(Varbits.BANK_TAB_ONE_COUNT + currentTabVar - 1);
        }
    ...
    private int calculateTabAbsoluteStartIndex(int tabNumber) {
        ...
        for (int i = 1; i < limit; i++) { // Sum counts of tabs *before* the current tab
            startIndex += ProjectX.getVarbitValue(Varbits.BANK_TAB_ONE_COUNT + i - 1);
        }
    ...
    private void rearrangeBankItems(List<BankSortItem> sortedItems, int tabAbsoluteStartIndex) {
        if (sortedItems.isEmpty()) {
            ProjectX.log("No items to rearrange.");
            return;
        }
        ProjectX.log("Rearranging " + sortedItems.size() + " items in bank tab starting at absolute index " + tabAbsoluteStartIndex);
        boolean[] slotCorrect = new boolean[sortedItems.size()];

        for (int i = 0; i < sortedItems.size(); i++) {
            if (!isRunning()) {
                ProjectX.log("Script shutdown requested during rearrangement.");
                break;
            }
            if (!Rs2Bank.isOpen()) {
                ProjectX.log("Bank closed during rearrangement. Stopping.");
                break;
            }
            ...
            if (sourceWidget == null) {
                ProjectX.log("CRITICAL: Could not find source widget for '" + desiredItemForThisSlot.getProcessedName() +
            ...
            ProjectX.log("Moving '" + desiredItemForThisSlot.getProcessedName() + "' from slot " + sourceSlotAbsolute + " to slot " + targetSlotAbsolute);
            ...
            if (targetSlotWidget == null) {
                ...
                ProjectX.log("CRITICAL: Could not get widget for target slot " + targetSlotAbsolute + ". Skipping move for item '" + desiredItemForThisSlot.getProcessedName() + "'.");
            ...
            if (sourcePoint.equals(new Point(-1, -1)) || targetPoint.equals(new Point(-1, -1))) {
                ProjectX.log("Invalid source or target point for drag operation. Source: " + sourcePoint + ", Target: " + targetPoint + ". Skipping move for item '" + desiredItemForThisSlot.getProcessedName() + "' to slot " + targetSlotAbsolute);
                slotCorrect[i] = true;
                continue;
            }
            if (sourcePoint.equals(targetPoint)) {
                ProjectX.log("Source and target points are identical for item '" + desiredItemForThisSlot.getProcessedName() + "' (Slot " + sourceSlotAbsolute + " to " + targetSlotAbsolute + "). Skipping move to prevent issues.");
                slotCorrect[i] = true; // Mark as handled if source and target are same slot effectively
                continue;
            }

            ProjectX.getMouse().drag(sourcePoint, targetPoint);
            sleep(Rs2Random.between(350, 500));
            slotCorrect[i] = true;
        }
        ProjectX.log("Item rearrangement loop finished.");
    }
