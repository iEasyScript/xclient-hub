package net.runelite.client.plugins.projectx.tempoross;
...
import net.runelite.client.plugins.projectx.tempoross.enums.HarpoonType;
...
public class TemporossScript extends Script {
    ...
    private void determineWorkArea() {
        if (workArea == null) {
            LocalPoint playerLocal = ProjectX.getClient().getLocalPlayer() != null
                    ? ProjectX.getClient().getLocalPlayer().getLocalLocation() : null;
            if (playerLocal == null) return;

            List<Rs2NpcModel> forfeitNpcs = ProjectX.getRs2NpcCache().query()
            ...
            List<Rs2NpcModel> ammoCrates = ProjectX.getRs2NpcCache().query()
                    .where(npc -> npc.getNpc() != null && npc.getNpc().getComposition() != null
    ...
    private void finishGame() {
        ...
        Rs2NpcModel exitNpc = ProjectX.getRs2NpcCache().query()
                .where(npc -> npc.getNpc() != null && npc.getNpc().getComposition() != null
        ...
        if (exitNpc != null) {
            ...
            if (emptyBucketCount > 0) {
                if(ProjectX.getRs2TileObjectCache().query().interact(41004, "Fill-bucket"))
    ...
    private void forfeit() {
        LocalPoint playerLocal = ProjectX.getClient().getLocalPlayer() != null
                ? ProjectX.getClient().getLocalPlayer().getLocalLocation() : null;
        if (playerLocal == null) return;
        var forfeitNpc = ProjectX.getRs2NpcCache().query()
                .where(npc -> npc.getNpc() != null && npc.getNpc().getComposition() != null
                        && Arrays.asList(npc.getNpc().getComposition().getActions()).contains("Forfeit"))
                .toList().stream()
                .filter(npc -> npc.getNpc().getLocalLocation() != null)
    ...
    private void fetchMissingItems()
    {
        ...
        if (!hasHarpoon() && harpoonType != HarpoonType.BAREHAND) {
            LocalPoint lp = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), workArea.harpoonPoint);
        ...
        if (needBuckets) {
            LocalPoint lp = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), workArea.bucketPoint);
        ...
        if (!needBuckets && fullBucketCount <= 0) {
            LocalPoint lp = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), workArea.pumpPoint);
            needed.add(new int[]{2, lp != null ? playerLocal.distanceTo(lp) : Integer.MAX_VALUE});
        }

        if (temporossConfig.rope() && !temporossConfig.spiritAnglers() && !Rs2Inventory.contains(ItemID.ROPE)) {
            LocalPoint lp = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), workArea.ropePoint);
        ...
        if (temporossConfig.hammer() && !Rs2Inventory.contains(ItemID.HAMMER)) {
            LocalPoint lp = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), workArea.hammerPoint);
    ...
    private boolean isOnStartingBoat() {
        Rs2TileObjectModel startingLadder = ProjectX.getRs2TileObjectCache().query().withId(ObjectID.ROPE_LADDER_41305).nearest();
    ...
    private void handleEnterMinigame() {
        ...
        Rs2TileObjectModel waterPump = ProjectX.getRs2TileObjectCache().query().withId(ObjectID.WATER_PUMP_41000).nearest();
    ...
    public static void handleWidgetInfo() {
        try {
            Widget energyWidget = ProjectX.getClient().getWidget(InterfaceID.TEMPOROSS, 35);
            Widget essenceWidget = ProjectX.getClient().getWidget(InterfaceID.TEMPOROSS, 45);
            Widget intensityWidget = ProjectX.getClient().getWidget(InterfaceID.TEMPOROSS, 55);
    ...
    public static void updateFireData(){
        List<Rs2NpcModel> allFires = ProjectX.getRs2NpcCache().query()
                .where(npc -> npc.getNpc() != null && npc.getNpc().getComposition() != null
        ...
        LocalPoint exitLocal = workArea != null
                ? LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), workArea.exitNpc) : null;
    ...
    public static void updateAmmoCrateData(){
        LocalPoint mastLocal = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), workArea.mastPoint);
        List<Rs2NpcModel> ammoCrates = ProjectX.getRs2NpcCache().query()
                .where(npc -> npc.getNpc() != null && npc.getNpc().getComposition() != null
                        && Arrays.asList(npc.getNpc().getComposition().getActions()).contains("Fill")
    ...
    public static void updateFishSpotData(){
        fishSpots = ProjectX.getRs2NpcCache().query()
    ...
    private void handleFires() {
        ...
        for (Rs2NpcModel fire : sortedFires) {
            if(isFilling){
                ProjectX.log("Filling, skipping fire");
                return;
            }
    ...
    private void handleStateLoop() {
        temporossPool = ProjectX.getRs2NpcCache().query().withId(NpcID.SPIRIT_POOL)
                .where(npc -> npc.getNpc() != null && npc.getNpc().getComposition() != null
        ...
        switch (state) {
            ...
            case THIRD_CATCH:
                ...
                if (fishSpot != null && fishSpot.getNpc() != null) {
                    ...
                    LocalPoint localPoint = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), workArea.totemPoint);
                    if (localPoint == null) {
            ...
            case THIRD_COOK:
                ...
                if (range != null && rawFishCount > 0) {
                    ...
                    LocalPoint localPoint = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), workArea.rangePoint);
                    if (localPoint == null) {
            ...
            case INITIAL_FILL:
                ...
                LocalPoint fillPlayerLocal = ProjectX.getClient().getLocalPlayer() != null
                        ? ProjectX.getClient().getLocalPlayer().getLocalLocation() : null;
                ...
                if (inCloud(ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getLocalLocation()))) {
    ...
    private static WorldPoint getTrueWorldPoint(WorldPoint point) {
        LocalPoint localPoint = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), point);
        assert localPoint != null;
        return WorldPoint.fromLocalInstance(ProjectX.getClient(), localPoint);
    }
    ...
    private void walkToSafePoint() {
        LocalPoint localPoint = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), workArea.safePoint);
        if (localPoint == null) {
            log("Safe point off-screen, using Rs2Walker");
            Rs2Walker.walkTo(workArea.safePoint);
            return;
        }
        if (Objects.equals(ProjectX.getClient().getLocalDestinationLocation(), localPoint))
            return;
        LocalPoint playerLocal = ProjectX.getClient().getLocalPlayer() != null
                ? ProjectX.getClient().getLocalPlayer().getLocalLocation() : null;
    ...
    private void walkToSpiritPool() {
        LocalPoint localPoint = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), workArea.spiritPoolPoint);
        if (localPoint == null) {
    ...
    private boolean handleCloudDodge() {
        ...
        LocalPoint playerLocal = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), playerLoc);
        if (playerLocal == null) {
            return false;
        }
    ...
    public static boolean inCloud(WorldPoint point, int radius) {
        ...
        LocalPoint playerLocal = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), point);
        if (playerLocal == null) {
            return false;
        }
    ...
    public boolean fightFiresInPath(WorldPoint location) {
        ...
        LocalPoint playerLocal = ProjectX.getClient().getLocalPlayer() != null
                ? ProjectX.getClient().getLocalPlayer().getLocalLocation() : null;
        LocalPoint destLocal = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), location);
