package net.runelite.client.plugins.projectx.hunterKabbits;
...
public class HunterKabbitsScript extends Script {
    ...
    public void run(HunterKebbitsConfig config, HunterKebbitsPlugin plugin) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                if (!ProjectX.isLoggedIn() || !this.isRunning()) return;
                ...
                e.printStackTrace();
                ProjectX.status = "Error: " + e.getClass().getSimpleName() + " - " + e.getMessage();
            }
    ...
    private void handleRetrievingState(HunterKebbitsConfig config) {
        ...
        if (hintNpc != null && VALID_FALCON_NPC_IDS.contains(hintNpc.getId())) {
            Rs2NpcModel model = ProjectX.getRs2NpcCache().query()
            ...
            boolean anyFalconStillActive = ProjectX.getRs2NpcCache().query()
    ...
    private void handleCatchingState(HunterKebbitsConfig config) {
        ...
        Rs2NpcModel kebbit = ProjectX.getRs2NpcCache().query().withName(npcName).nearestOnClientThread();
        if (kebbit != null && kebbit.click("Catch")) {
            boolean falconActive = false;
            for (int i = 0; i < 10; i++) {
                if (!isRunning()) break;

                boolean found = ProjectX.getRs2NpcCache().query()
                        .where(npc -> VALID_FALCON_NPC_IDS.contains(npc.getId()))
                        .count() > 0;
    ...
    private KebbitHunting getKebbit(HunterKebbitsConfig config) {
        int level = ProjectX.getClient().getRealSkillLevel(Skill.HUNTER);
    ...
    private boolean isHintArrowNpcActive() {
        return ProjectX.getClient().getHintArrowNpc() != null;
    }
    ...
    private boolean isFalconWithPlayer() {
        boolean falconVisible = ProjectX.getRs2NpcCache().query()
                .where(npc -> VALID_FALCON_NPC_IDS.contains(npc.getId()))
                .count() > 0;
    ...
    public void shutdown() {
        ...
        ProjectX.status = "Script stopped.";
    }
}
