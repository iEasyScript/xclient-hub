...
public class RewardHandler {
    ...
    public static boolean collectOfferings(TotemLocation totemLocation, TotemProgress progress, GameSession gameSession) {
        try {
            ...
            ProjectX.log("[Offerings] shouldCollect=" + shouldCollect + " rounds=" + gameSession.getTotalRounds() + " freq=" + COLLECTION_FREQUENCY);
            if (!shouldCollect) {
                return false;
            }

            GameObject offerings = GameObjectUtils.findClaimableOfferings(location, OFFERINGS_SEARCH_RADIUS);
            ProjectX.log("[Offerings] findClaimable at " + location + " radius=" + OFFERINGS_SEARCH_RADIUS + " found=" + (offerings != null));
            ...
            ProjectX.log("[Offerings] state=" + (offeringsState != null ? offeringsState.name() : "null"));
