package net.runelite.client.plugins.projectx.arceuusrc;
...
import net.runelite.client.plugins.projectx.arceuusrc.enums.Altar;
...
public class ArceuusRcScript extends Script {
    ...
    private void executeTask() {
        try {
            ...
            hasChippedEssence = Rs2Inventory.hasItem(DARK_ESSENCE_FRAGMENTS);
            ProjectX.log("Error in Arceuus Runecrafter: " + e.getMessage());
        }
    }

    public Altar getAltar() {
        if (config.getAltar() != Altar.AUTO) return config.getAltar();
        final int level = ProjectX.getClient().getRealSkillLevel(Skill.RUNECRAFT);
    ...
    public void useAltar() {
        var altar = ProjectX.getRs2TileObjectCache().query().withName(getAltarName()).within(11).nearestOnClientThread();
        if (altar != null) {
    ...
    public boolean chipEssenceFast() {
        ...
        while (Rs2Inventory.hasItem(ItemID.CHISEL) && blocks > 0) {
            if (!Rs2Inventory.combineClosest(ids[0], ids[1])) {
                ProjectX.log("Failed to combine closest chisel & dark essence block");
                return false;
            }
            reverse(ids);
    ...
    public void useDarkAltar() {
        var darkAltar = ProjectX.getRs2TileObjectCache().query().withName(DARK_ALTAR).within(11).nearestOnClientThread();
    ...
    public void mineEssence() {
        ...
        var runeStone = ProjectX.getRs2TileObjectCache().query().withName(STR_DENSE_RUNESTONE).within(11).nearestOnClientThread();
        if (runeStone == null) {
            ProjectX.log("Cannot find runestone");
            return;
        }
