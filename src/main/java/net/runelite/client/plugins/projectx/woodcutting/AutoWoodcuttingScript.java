package net.runelite.client.plugins.projectx.woodcutting;
...
import net.runelite.client.plugins.projectx.util.inventory.Rs2LogBasket;
...
import net.runelite.client.plugins.projectx.util.skills.fletching.Rs2Fletching;
...
import net.runelite.client.plugins.projectx.woodcutting.enums.*;
import net.runelite.client.plugins.projectx.woodcutting.enums.WoodcuttingTreeLocations;
...
import static net.runelite.client.plugins.projectx.util.player.Rs2Player.getRealSkillLevel;
...
public class AutoWoodcuttingScript extends Script {
    ...
    private boolean preFlightChecks(AutoWoodcuttingConfig config) {
        if (!ProjectX.isLoggedIn()) return true;
        ...
        if (config.hopWhenPlayerDetected() && config.enableForestry()) {
            ProjectX.showMessage("Autohop is not supported with forestry enabled, shutting down.");
            shutdown();
            return true;
        }
        ...
        if (!getActiveTree().hasRequiredLevel()) {
            ProjectX.showMessage("You do not have the required woodcutting level to cut this tree. " + Rs2Player.getRealSkillLevel(Skill.WOODCUTTING));
            shutdown();
            return true;
        }
        ...
        if (!Rs2Inventory.hasItem("axe")) {
            if (!Rs2Equipment.isWearing("axe")) {
                ProjectX.showMessage("Unable to find axe in inventory/equipped");
                shutdown();
                return true;
            }
        }
    ...
    private void burnLog(AutoWoodcuttingConfig config) {
        ...
        if (!isFiremake() && !useCampfire) {
            ...
            ProjectX.log("Sleeping until not animating or no more logs");
