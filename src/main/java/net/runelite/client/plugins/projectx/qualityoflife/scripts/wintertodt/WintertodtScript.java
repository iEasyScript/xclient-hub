package net.runelite.client.plugins.projectx.qualityoflife.scripts.wintertodt;
...
import net.runelite.client.plugins.projectx.qualityoflife.QoLPlugin;
...
public class WintertodtScript extends Script {
    ...
    public static boolean isInWintertodtRegion() {
        ...
        var location = ProjectX.getClientThread().runOnClientThreadOptional(() -> {
            if (ProjectX.getClient().getGameState() != GameState.LOGGED_IN || ProjectX.getClient().getLocalPlayer() == null) {
                return null;
            }

            return ProjectX.getClient().getLocalPlayer().getWorldLocation();
        }).orElse(null);
    ...
    public boolean run(QoLConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                if (!ProjectX.isLoggedIn() || !super.run() || !isInWintertodtRegion()) {
                    return;
                }
                ...
                brokenBrazier = ProjectX.getRs2TileObjectCache().query().withId(ObjectID.WINT_BRAZIER_BROKEN).within(5).nearest();
                unlitBrazier = ProjectX.getRs2TileObjectCache().query().withId(ObjectID.WINT_BRAZIER).within(5).nearest();
                ...
                if (config.wintertodtActions().equals(WintertodtActions.FEED)) {
                    Rs2TileObjectModel fireBrazier = ProjectX.getRs2TileObjectCache().query().withId(ObjectID.WINT_BRAZIER_LIT).nearest();
                    if (fireBrazier != null && fireBrazier.getWorldLocation().distanceTo2D(Rs2Player.getWorldLocation()) < 5) {
                        if (!Rs2Inventory.contains(ItemID.WINT_BRUMA_ROOT) && !Rs2Inventory.contains(ItemID.WINT_BRUMA_KINDLING)) {
                            ...
                            ProjectX.log("No resources found in inventory, cancelling action.");
                            return;
                        }
                        ProjectX.doInvoke(actionToResume, new java.awt.Rectangle(1, 1));
                        qolPlugin.updateWintertodtInterupted(false);
                    }
                }
                ...
                ProjectX.logStackTrace(this.getClass().getSimpleName(), e);
            }
    ...
    public void shutdown() {
        super.shutdown();
        ProjectX.log("Winterdotd script shutting down");
    }

    public void onNpcChanged(NpcChanged event) {
        if (event.getNpc().getId() == 7372) {
            ProjectX.getRs2NpcCache().query().where(n -> n.getNpc().equals(event.getNpc())).nearest();
        } else if (event.getNpc().getId() == 7371) {
            pyromancer = ProjectX.getRs2NpcCache().query().where(n -> n.getNpc().equals(event.getNpc())).nearest();
        ...
        if (event.getNpc().getId() == 7372) {
            if (incapitatedPyromancer != null) {
                if (incapitatedPyromancer.getWorldLocation().distanceTo2D(Rs2Player.getWorldLocation()) > event.getNpc().getWorldLocation().distanceTo2D(Rs2Player.getWorldLocation())) {
                    incapitatedPyromancer = ProjectX.getRs2NpcCache().query().where(n -> n.getNpc().equals(event.getNpc())).nearest();
                    return;
                }
            }
            incapitatedPyromancer = ProjectX.getRs2NpcCache().query().where(n -> n.getNpc().equals(event.getNpc())).nearest();
    ...
    public void onNpcDespawned(NpcDespawned event) {
        if (incapitatedPyromancer != null && event.getNpc().equals(incapitatedPyromancer.getNpc())) {
            incapitatedPyromancer = ProjectX.getRs2NpcCache().query().withName("Incapacitated Pyromancer").nearestOnClientThread();
        }
        if (pyromancer != null && event.getNpc().equals(pyromancer.getNpc())) {
            pyromancer = ProjectX.getRs2NpcCache().query().withName("Pyromancer").nearestOnClientThread();
        }
    }
