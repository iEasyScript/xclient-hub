package net.runelite.client.plugins.projectx.roguesden;
...
import static net.runelite.client.plugins.projectx.roguesden.Obstacles.OBSTACLES;
...
public class RoguesDenScript extends Script {
    ...
    private void initObstacles() {
        ...
        for (Obstacles.Obstacle obstacle : originalObstacles) {
            if (ProjectX.getClient().getRealSkillLevel(Skill.THIEVING) < 80) {
            ...
            try {
                ...
                if (!init) {
                    if (!Rs2Player.getSkillRequirement(Skill.THIEVING, 50)
                        ...
                        ProjectX.showMessage("Rogues Den requires at least 50 thieving and agility. Shutting down.");
                        shutdown();
                        return;
                    }
                ...
                ex.printStackTrace();
                ProjectX.log(ex.fillInStackTrace().getMessage());
            }
    ...
    private boolean clickObstacle() {
        ...
        if (closestIndex == 0 && !Rs2Player.getWorldLocation().equals(currentObstacle.getTile())) {
            ProjectX.getRs2TileObjectCache().query().interact(currentObstacle.getObjectId());
    ...
    private boolean useTileObject() {
        if (Rs2Inventory.hasItem(ItemID.ROGUESDEN_PUZZLE_MOSAIC_TILE1)) {
            ProjectX.log("Handle tile door");
            ProjectX.getRs2TileObjectCache().query().interact(7234, "Open");
    ...
    private boolean useStaminaPotion() {
        if (!Rs2Player.hasStaminaActive() && hasStaminaPotionInBank) {
            ProjectX.log("Looking to withdraw stamina potion...");
            if (Rs2Bank.openBank()) {
                ...
                if (Rs2Bank.hasItem("stamina potion")) {
                    ...
                    ProjectX.log("No stamina potion found in the bank. Continue without it...");
                }
                Rs2Bank.depositAll();
                sleepGaussian(600, 150);
            }
    ...
    private boolean useEnergyPotions() {
        while (Rs2Player.getRunEnergy() < 70 && hasEnergyPotionInBank) {
            ProjectX.log("Looking to withdraw energy potion...");
            if (Rs2Bank.openBank()) {
                if (!Rs2Bank.isOpen()) return true;
                if (Rs2Bank.hasItem("energy potion")) {
                    ...
                    ProjectX.log("No energy potion potion found in the bank. Continue without it...");
                }
                Rs2Bank.depositAll();
                sleepGaussian(600, 150);
            }
    ...
    private void enterMinigame() {
        ...
        if (state == WalkerState.ARRIVED) {
            ProjectX.getRs2TileObjectCache().query().interact(ObjectID.ROGUESDEN_MAZEENTRANCE);
    ...
    private void handleObstacle(Obstacles.Obstacle obstacle) {
        ...
        if ((obstacle != OBSTACLES[OBSTACLES.length - 3]) && (obstacle.getHint().equalsIgnoreCase("run") || obstacle.getHint().equalsIgnoreCase("take")) && Rs2Player.getRunEnergy() < 10) {
            ProjectX.log("Restoring run energy...");
            sleep(60_000, 120_000);
            return;
        }
        if (Rs2Random.between(1, 10) == 7) {
            Rs2Camera.angleToTile(obstacle.getTile());
            ProjectX.log("Rotating camera");
        }

        if (obstacle.getObjectId() != -1) {
            if (obstacle.getObjectId() == 7249) {
                ProjectX.getRs2TileObjectCache().query().interact(obstacle.getObjectId(), "search"); // Handles wall searching
            } else {
                ProjectX.getRs2TileObjectCache().query().interact(obstacle.getObjectId());
            }
