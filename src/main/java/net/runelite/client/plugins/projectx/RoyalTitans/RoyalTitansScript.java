package net.runelite.client.plugins.projectx.RoyalTitans;
...
import static net.runelite.client.plugins.projectx.RoyalTitans.RoyalTitansShared.evaluateAndConsumePotions;
import static net.runelite.client.plugins.projectx.RoyalTitans.RoyalTitansShared.*;
import static net.runelite.client.plugins.projectx.util.antiban.enums.ActivityIntensity.EXTREME;
import static net.runelite.client.plugins.projectx.util.prayer.Rs2Prayer.disableAllPrayers;
...
public class RoyalTitansScript extends Script {
    ...
    public boolean run(RoyalTitansConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                e.printStackTrace();
                ProjectX.log("Exception: " + e.getMessage());
            }
    ...
    private void detectState(RoyalTitansConfig config) {
        if (RoyalTitansShared.isInBossRegion() && state != RoyalTitansBotStatus.FIGHTING) {
            ProjectX.log("Boss region detected - But not in state FIGHTING, changing state");
            state = RoyalTitansBotStatus.FIGHTING;
        }
        if (state == RoyalTitansBotStatus.FIGHTING && !RoyalTitansShared.isInBossRegion()) {
            ProjectX.log("Not in boss region - Changing state to TRAVELLING to instance");
            state = RoyalTitansBotStatus.TRAVELLING;
    ...
    private void handleWaiting(RoyalTitansConfig config) {
        ...
        if (waitingTimeStart != null && teammate == null && Instant.now().isAfter(waitingTimeStart.plusSeconds(config.waitingTimeForTeammate()))) {
            ProjectX.log("Teammate did not show after " + config.waitingTimeForTeammate() + " seconds, shutting down");
            shutdown();
        }
    }
    ...
    private boolean handleEscaping(RoyalTitansConfig config) {
        ...
        var iceTitanDead = ProjectX.getRs2NpcCache().query().withId(ICE_TITAN_DEAD_ID).nearest();
        var fireTitanDead = ProjectX.getRs2NpcCache().query().withId(FIRE_TITAN_DEAD_ID).nearest();
        if (shouldLeave && iceTitanDead != null && fireTitanDead != null && !LootedTitanLastIteration) {
            ProjectX.log("We want to escape, but Titans are dead, lets loot first");
        }
        if (shouldLeave) {
            if (config.emergencyTeleport() != 0) {
                ...
                enrageTile = null;
                ProjectX.getRs2TileObjectCache().query().withId(TUNNEL_ID_ESCAPE).interact("Quick-escape");
                Rs2Bank.walkToBank();
            }
            state = RoyalTitansBotStatus.TRAVELLING;
            travelStatus = RoyalTitansTravelStatus.TO_BANK;
    ...
    private void handlePrayers(RoyalTitansConfig config) {
        ...
        if (ProjectX.getRs2NpcCache().query().where(x -> x.getId() == ICE_TITAN_ID || x.getId() == FIRE_TITAN_ID).count() > 0) {
            Rs2Prayer.toggle(Rs2PrayerEnum.PROTECT_MELEE, true);
            return;
        }
    ...
    private void attackBoss(RoyalTitansConfig config) {
        var iceTitan = ProjectX.getRs2NpcCache().query().withId(ICE_TITAN_ID).nearest();
        var fireTitan = ProjectX.getRs2NpcCache().query().withId(FIRE_TITAN_ID).nearest();
        if (iceTitan == null && fireTitan == null) {
            ProjectX.log("No titans found");
            return;
        }
    ...
    private boolean handleWalls(RoyalTitansConfig config) {
        ...
        if (config.soloMode()) {
            List<Rs2NpcModel> fireWalls = ProjectX.getRs2NpcCache().query().withId(FIRE_WALL).toList();
            List<Rs2NpcModel> iceWalls = ProjectX.getRs2NpcCache().query().withId(ICE_WALL).toList();
            walls = new ArrayList<>();
            walls.addAll(fireWalls);
            walls.addAll(iceWalls);
        } else {
            walls = ProjectX.getRs2NpcCache().query().withId(config.minionResponsibility() == RoyalTitansConfig.Minions.FIRE_MINIONS ? FIRE_WALL : ICE_WALL)
                    .toList();
        }
    ...
    private boolean handleMinions(RoyalTitansConfig config) {
        ...
        if (config.soloMode()) {
            List<Rs2NpcModel> fireMinions = ProjectX.getRs2NpcCache().query().withId(FIRE_MINION_ID).toList();
            List<Rs2NpcModel> iceMinions = ProjectX.getRs2NpcCache().query().withId(ICE_MINION_ID).toList();
            minions = new ArrayList<>();
            minions.addAll(fireMinions);
            minions.addAll(iceMinions);
        } else {
            minions = ProjectX.getRs2NpcCache().query().withId(config.minionResponsibility() == RoyalTitansConfig.Minions.FIRE_MINIONS ? FIRE_MINION_ID : ICE_MINION_ID)
                    .toList();
        }
    ...
    private Rs2NpcModel selectTitanForSoloMode(Rs2NpcModel iceTitan, Rs2NpcModel fireTitan) {
        ...
        if (Rs2Player.isInCombat()) {
            var interacting = ProjectX.getClient().getLocalPlayer().getInteracting();
    ...
    private void handleDangerousTiles() {
        ...
        if (safeTile != null) {
            ...
            if (Rs2Player.getWorldLocation().equals(safeTile)) {
                ProjectX.log("Successfully moved to safe tile: " + safeTile);
            } else {
                ProjectX.log("Trying again to walk to safe tile: " + safeTile);
                Rs2Walker.walkFastCanvas(safeTile); // Try again
            }
        } else {
            ProjectX.log("No safe tiles found nearby!");
        }
    }

    private WorldPoint findSafeTile(WorldPoint playerLocation, Collection<WorldPoint> dangerousWorldPoints) {
        ProjectX.log("Finding safe tile");
        ...
        for (WorldPoint tile : nearbyTiles) {
            ...
            if (!fightArea.contains(tile)) {
                ProjectX.log("Tile is outside the arena, skipping");
                continue;
            }
            if (!dangerousWorldPoints.contains(tile)) {
                ProjectX.log("Found safe tile: " + tile);
                return tile;
            }
        }
            ...
            case TO_INSTANCE:
                ...
                if (isVisible) {
                    if (config.currentBotInstanceOwner()) {
                        ...
                        var teammate = Rs2Player.getPlayers(x -> Objects.equals(x.getName(), config.teammateName())).findFirst().orElse(null);
                        if (teammate != null) {
                            ProjectX.log("Waiting for teammate to enter the instance");
                            return;
                        }
                    ...
                    ProjectX.getRs2TileObjectCache().query().withId(TUNNEL_ID).interact("Enter");
                }
                break;
        }
    }
    ...
    private void handleBanking(RoyalTitansConfig config) {
        ...
        if (equipmentLoaded && inventoryLoaded) {
            ...
            if (!inventorySetup.getAdditionalItems().isEmpty()) {
                ProjectX.log("Pre-potting with additional items");
            ...
            if (inventorySetup.doesInventoryMatch() && inventorySetup.doesEquipmentMatch()) {
                ...
                ProjectX.log("Failed to load inventory or equipment");
                shutdown();
            }
        }
    }
    ...
    public void shutdown() {
        ...
        ProjectX.log("Shutting down Royal Titans script");
    }
}
