package net.runelite.client.plugins.projectx.anonymous;
...
public class AnonymousScript extends Script {
    ...
    public void anonymize(Client client, AnonymousConfig config) {
        if (config.maskTitleName()) ProjectX.getConfigManager().setConfiguration("runelite", "usernameInTitle", false);
    ...
    public void onBeforeRender(Client client, AnonymousConfig config) {
        if (!ProjectX.isLoggedIn() || !super.run() || !isRunning() || client == null) return;
    ...
    public boolean run(AnonymousConfig config, Client client) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                if (!ProjectX.isLoggedIn() || !super.run() || !isRunning() || client == null || config == null) return;
