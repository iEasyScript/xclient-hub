package net.runelite.client.plugins.projectx.varrockcleaner;
...
public class VarrockCleanerScript extends Script {
    ...
    private void takeUncleanedFinds() {
        ...
        if (ProjectX.getRs2TileObjectCache().query().interact(ObjectID.DIG_SITE_SPECIMEN_ROCKS, "Take")) {
    ...
    private void cleanFinds() {
        if (Rs2Inventory.contains("Uncleaned find") && ProjectX.getRs2TileObjectCache().query().interact(ObjectID.SPECIMEN_TABLE_24556, "Clean")) {
    ...
    private void storeFindsInCrate() {
        ...
        if (!Rs2Inventory.contains("Uncleaned find") && ProjectX.getRs2TileObjectCache().query().interact(ObjectID.STORAGE_CRATE, "Add finds")) {
    ...
    public void stop() {
        ProjectX.log("Varrack Cleaner plugin stopped.");
        currentState = State.TAKE_UNCLEANED;
        super.shutdown();
    }
}
