...
public class ConstructionScript extends Script {
    ...
    public Rs2TileObjectModel getClosestTile(List<Integer> objIDs) {
        ...
        return ProjectX.getRs2TileObjectCache().query().withIds(ids).nearest();
    }

    public Rs2NpcModel getButler() {
        return ProjectX.getRs2NpcCache().query().withName("Demon butler").nearestOnClientThread();
    }
    ...
    public boolean run(net.runelite.client.plugins.projectx.construction.ConstructionConfig config) {
    ...
    public void grabPlanksWhileWeBuild(net.runelite.client.plugins.projectx.construction.ConstructionConfig config, int actionDelay){
        if(getButler() != null) {
    ...
    private void calculateState(net.runelite.client.plugins.projectx.construction.ConstructionConfig config) {
        ...
        Rs2TileObjectModel objOnWorkingTile = ProjectX.getRs2TileObjectCache().query()
                .where(o -> o.getWorldLocation().equals(workingTile))
                .nearest();
        if (objOnWorkingTile == null || !objectIDs.contains(objOnWorkingTile.getId())) {
            workingTile = getClosestTile(objectIDs).getWorldLocation();
            objOnWorkingTile = ProjectX.getRs2TileObjectCache().query()
                    .where(o -> o.getWorldLocation().equals(workingTile))
                    .nearest();
        }
        ...
        if (objOnWorkingTile.getId() == objectIDs.get(0)) {
            ...
            ProjectX.getNotifier().notify("Looks like we are no longer in our house.");
            returnToTheHouse();
        }
    }

    private void returnToTheHouse(){
        Rs2TileObjectModel housePortal = ProjectX.getRs2TileObjectCache().query().withName("Portal").nearestOnClientThread();
        if(housePortal != null){
            ...
            ProjectX.getNotifier().notify("Can't find the house portal!");
            shutdown();
        }
    }

    private void buildSpace(net.runelite.client.plugins.projectx.construction.ConstructionConfig config, int actionDelay) {
        Rs2TileObjectModel space = ProjectX.getRs2TileObjectCache().query()
                .where(o -> o.getWorldLocation().equals(workingTile))
                .nearest();
    ...
    private void removeSpace(net.runelite.client.plugins.projectx.construction.ConstructionConfig config, int actionDelay) {
        Rs2TileObjectModel builtObject = ProjectX.getRs2TileObjectCache().query()
                .where(o -> o.getWorldLocation().equals(workingTile))
                .nearest();
    ...
    private void butler(net.runelite.client.plugins.projectx.construction.ConstructionConfig config, int actionDelay) {
