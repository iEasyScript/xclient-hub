...
public class SalamanderScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (config.salamanderHunting().getName().equals("Black salamander") && hasDied) {
                    ProjectX.log("We died - Restocking supplies");
                    handleBanking(config);
                } else if ((Rs2Inventory.count(ROPE) < 1 || Rs2Inventory.count(SMALL_FISHING_NET) < 1) && plugin.getTraps().isEmpty()) {
                    ProjectX.log("Not enough supplies, need ropes and fishing nets");
                    handleBanking(config);
                    return;
                }
                ...
                if (!isNearSalamanderArea(salamanderType)) {
                    ProjectX.log("Walking to " + salamanderType.getName() + " hunting area");
    ...
    private SalamanderHunting getBestSalamander() {
        ...
        ProjectX.log("Not high enough hunter level for any salamander");
        shutdown();
        return null;
    }
    ...
    private boolean handleExistingTraps(SalamanderPlugin plugin, SalamanderConfig config) {
        ...
        if (!Rs2Player.isAnimating() && !Rs2Player.isMoving()) {
            var gameObject = ProjectX.getRs2TileObjectCache().query().within(location, 0).first();
            if (gameObject != null) {
    ...
    private void setNewTrap(SalamanderHunting salamanderType, SalamanderConfig config) {
        if (ProjectX.getRs2TileObjectCache().query().withId(salamanderType.getTreeId()).count() > 0) {
            ProjectX.getRs2TileObjectCache().query().interact(salamanderType.getTreeId(), "Set-trap");
            sleep(config.minSleepAfterLay(), config.maxSleepAfterLay());
        }
    }

    public boolean IsRopeOnTheGround() {
        return ProjectX.getRs2TileItemCache().query().withId(ROPE).within(7).count() > 0 ||
                ProjectX.getRs2TileItemCache().query().withId(303).within(7).count() > 0;
    }
