...
import net.runelite.client.plugins.projectx.util.item.Rs2EnsouledHead;
import net.runelite.client.plugins.projectx.util.magic.Rs2Magic;
import net.runelite.client.plugins.projectx.util.magic.Spell;
...
import static net.runelite.client.plugins.projectx.EnsouledHeadSlayer.EnsouledHeadSlayerScript.*;
import static net.runelite.client.plugins.projectx.util.antiban.enums.ActivityIntensity.MODERATE;
...
public class EnsouledHeadSlayerScript extends Script {
            ...
            try {
                ...
                if (config.useInventorySetup()) {
                    if (config.inventorySetup() == null) {
                        ProjectX.log("Inventory setup is null, please select an inventory setup. If one is already selected, please reselect it.");
                        return;
                    } else {
                    ...
                    case BANKING:
                        if (handleBanking(config)) {
                            if (Rs2Inventory.count("Ensouled") < 1) {
                                ProjectX.log("No ensouled heads found in inventory, stopping script.");
                                shutdown();
                                return;
                            }
    ...
    /*
		...
		4 = NONE
	*/
        if (ProjectX.getVarbitValue(Varbits.SPELLBOOK) != 3) {
            ProjectX.log("On wrong spellbook, switching to Arceuus...");
            ProjectX.getClientThread().invoke(() -> ProjectX.getRs2NpcCache().query().withName("Tyss").interact("Spellbook"));
        }
        ...
        if (ensouledHead == 0 && !Rs2Combat.inCombat() && Rs2Player.getInteracting() == null) {
            ...
            ProjectX.log("No ensouled heads found in inventory, banking...");
            BOT_STATE = EnsouledHeadSlayerStatus.BANKING;
            return;
        }
        if (Rs2Inventory.count(config.food().getName()) < 1 && ProjectX.getClient().getBoostedSkillLevel(Skill.HITPOINTS) < ProjectX.getClient().getRealSkillLevel(Skill.HITPOINTS) / 2) {
            ;
            ProjectX.log("Not enough food & below half health, banking...");
            BOT_STATE = EnsouledHeadSlayerStatus.BANKING;
            return;
        }
        ...
        var enemy = ProjectX.getRs2NpcCache().query()
        ...
        if (enemy.isEmpty() && Rs2Player.getInteracting() == null && ProjectX.getRs2TileObjectCache().query().withId(ENSOULED_GROUND_GRAPHICS).nearest() == null) {
            ...
            if (spell != null) {
                ...
                sleepUntil(() -> ProjectX.getRs2TileObjectCache().query().withId(ENSOULED_GROUND_GRAPHICS).nearest() != null, 4000);
                sleepUntil(() -> ProjectX.getRs2TileObjectCache().query().withId(ENSOULED_GROUND_GRAPHICS).nearest() == null, 4000);
                sleep(3500, 4500);
            }
        } else {
    ...
    private boolean handleBanking(EnsouledHeadSlayerConfig config) {
        ...
        if (Rs2Bank.count("Ensouled") < 1) {
            ProjectX.log("No ensouled heads found in bank, stopping script.");
            shutdown();
            return false;
        }
        ...
        if (!config.useInventorySetup()) {
            if (!getRunesAndEnsouledHeads(config)) {
                ProjectX.log("Failed to withdraw runes or ensouled heads");
            }
            var foodAmount = config.foodAmount() - Rs2Inventory.count(config.food().getId());
            if (foodAmount > 0) {
                ProjectX.log("Withdrawing " + foodAmount + " " + config.food().getName() + " from bank");
    ...
    private boolean getRunesAndEnsouledHeads(EnsouledHeadSlayerConfig config) {
        if (config.ensouledHeads().getName().equals("All")) {
            ...
            if (headCounts.isEmpty()) {
                ProjectX.log("No ensouled heads found in bank");
                return false;
            }
            ...
            if (primaryHead == null) {
                ProjectX.log("Could not identify ensouled head type");
                return false;
            }
            ...
            if (availableSlots <= 0) {
                ProjectX.log("Not enough inventory space");
                return false;
            }
            ...
            if (headCount == 0) {
                ProjectX.log("No " + selectedHead.getName() + " found in bank");
                return false;
            }
            var headsToWithdraw = Rs2Inventory.emptySlotCount() - config.foodAmount();
            ProjectX.log("Withdrawing " + headsToWithdraw + " " + selectedHead.getName() + " from bank");
    ...
    private boolean useInventorySetup(EnsouledHeadSlayerConfig config) {
        if (config.useInventorySetup()) {
            ...
            if (!successEq || !successInv) {
                ProjectX.log("Failed to load inventory setup, trying again...");
                attempts++;
                // Stupid failsafe to handle inventory setups failing the first time
                if (attempts > 2) {
                    ProjectX.log("Failed to load inventory setup after 2 attempts, stopping script.");
                    shutdown();
                    return false;
                }
                return false;
                ...
                attempts = 0;
                ProjectX.log("Loaded inventory setup successfully.");
            }
            Rs2Bank.closeBank();
        }
        return true;
