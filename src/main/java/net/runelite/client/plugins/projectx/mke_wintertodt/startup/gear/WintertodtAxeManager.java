package net.runelite.client.plugins.projectx.mke_wintertodt.startup.gear;
...
public class WintertodtAxeManager {
    ...
    public static AxeDecision determineOptimalAxeSetup() {
        try {
            ...
            ProjectX.log("Bruma torch analysis: " + torchAnalysis.toString());
            ...
            ProjectX.log("Axe decision: " + reasoning);
            ...
            ProjectX.log("Error determining axe setup: " + e.getMessage());
            e.printStackTrace();
    ...
    private static boolean determineOptimalAxePlacement(AxeInfo bestAxe, int attackLevel, BrumaTorchAnalysis torchAnalysis) {
        ...
        if (!bestAxe.canWield(attackLevel)) {
            ProjectX.log("Cannot wield " + bestAxe.name + " (need " + bestAxe.attackLevel + " Attack, have " + attackLevel + ")");
            return false;
        }
        
        // Special case: Bronze axe has no attack requirement, always equip if possible
        if (bestAxe.itemId == ItemID.BRONZE_AXE) {
            ProjectX.log("Bronze axe - equipping (no attack requirement)");
            return true;
        }
        ...
        if (torchAnalysis.hasBrumaTorch) {
            if (torchAnalysis.hasOffhand) {
                ProjectX.log("Optimal setup: " + bestAxe.name + " + bruma torch offhand");
            } else {
                ProjectX.log("Will be optimal after conversion: " + bestAxe.name + " + bruma torch offhand");
            }
            return true;
        }
        ...
        ProjectX.log("No bruma torch available - equipping " + bestAxe.name + " for better inventory efficiency");
        return true;
    }
    ...
    private static AxeInfo findBestAvailableAxe(int wcLevel, int attackLevel) {
        ProjectX.log("Searching for best available axe...");
        ...
        for (AxeInfo axe : AXE_PRIORITY) {
            ...
            if (hasAccess(axe.itemId)) {
                ...
                ProjectX.log("Found: " + axe.name + " (" + canWield + ")");
                return axe;
            }
        }
        
        ProjectX.log("No suitable axe found in inventory/bank/equipment");
        return null;
    }
        ...
        try {
            ...
            if (!analysis.needsConversion) {
                if (analysis.hasOffhand) {
                    ProjectX.log("Bruma torch already in optimal offhand form");
                } else {
                    ProjectX.log("ℹ No bruma torch conversion needed");
                }
                return true;
            }
            
            ProjectX.log("Starting bruma torch conversion to offhand version...");
            
            boolean bankWasOpen = Rs2Bank.isOpen();
            
            // Ensure we have the regular bruma torch in inventory
            if (!ensureBrumaTorchInInventory()) {
                ProjectX.log("Failed to get bruma torch into inventory for conversion");
                return false;
            }
            
            // Close bank for the conversion process
            if (Rs2Bank.isOpen()) {
                ProjectX.log("Closing bank for bruma torch conversion...");
                Rs2Bank.closeBank();
            ...
            if (!Rs2Inventory.hasItem(ItemID.BRUMA_TORCH)) {
                ProjectX.log("Bruma torch disappeared from inventory");
                return reopenBankAndReturn(bankWasOpen, false);
            }
            
            ProjectX.log("Converting bruma torch to offhand version...");
            ...
            if (Rs2Inventory.interact(ItemID.BRUMA_TORCH, "Swap")) {
                ...
                if (converted) {
                    ProjectX.log("Successfully converted bruma torch to offhand version!");
                    return reopenBankAndReturn(bankWasOpen, true);
                } else {
                    ProjectX.log("Bruma torch conversion timed out");
                    return reopenBankAndReturn(bankWasOpen, false);
                }
            } else {
                ProjectX.log("Failed to interact with bruma torch for conversion");
                return reopenBankAndReturn(bankWasOpen, false);
            }
            
        } catch (Exception e) {
            ProjectX.log("Error during bruma torch conversion: " + e.getMessage());
            e.printStackTrace();
            ...
            try {
                ...
                ProjectX.log("Failed to reopen bank after conversion error");
            }
    ...
    private static boolean reopenBankAndReturn(boolean shouldReopenBank, boolean result) {
        if (shouldReopenBank && !Rs2Bank.isOpen()) {
            try {
                ProjectX.log("Reopening bank after conversion...");
                ...
                ProjectX.log("Failed to reopen bank: " + e.getMessage());
            }
        }
        return result;
    }
    ...
    private static boolean ensureBrumaTorchInInventory() {
        try {
            ...
            if (Rs2Inventory.hasItem(ItemID.BRUMA_TORCH)) {
                ProjectX.log("Bruma torch already in inventory");
                return true;
            }
            
            // Check if equipped and unequip
            if (Rs2Equipment.isWearing(ItemID.BRUMA_TORCH)) {
                ProjectX.log("Unequipping bruma torch for conversion...");
                ...
                if (Rs2Inventory.hasItem(ItemID.BRUMA_TORCH)) {
                    ProjectX.log("Successfully unequipped bruma torch");
                    return true;
                } else {
                    ProjectX.log("Failed to unequip bruma torch");
                    return false;
                }
            }
            ...
            if (Rs2Bank.isOpen() && Rs2Bank.hasBankItem(ItemID.BRUMA_TORCH, 1)) {
                ProjectX.log("Withdrawing bruma torch from bank...");
                Rs2Bank.withdrawX(ItemID.BRUMA_TORCH, 1);
                sleepUntilTrue(() -> Rs2Inventory.hasItem(ItemID.BRUMA_TORCH), 100, 3000);
                
                if (Rs2Inventory.hasItem(ItemID.BRUMA_TORCH)) {
                    ProjectX.log("Successfully withdrew bruma torch");
                    return true;
                } else {
                    ProjectX.log("Failed to withdraw bruma torch");
                    return false;
                }
            }
            
            ProjectX.log("Cannot find bruma torch to convert");
            return false;
            
        } catch (Exception e) {
            ProjectX.log("Error ensuring bruma torch in inventory: " + e.getMessage());
            return false;
        }
    }
