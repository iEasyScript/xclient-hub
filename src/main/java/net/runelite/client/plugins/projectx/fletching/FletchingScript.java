package net.runelite.client.plugins.projectx.fletching;
...
import net.runelite.client.plugins.projectx.fletching.enums.FletchingItem;
import net.runelite.client.plugins.projectx.fletching.enums.FletchingMaterial;
import net.runelite.client.plugins.projectx.fletching.enums.FletchingMode;
...
//                    sleep(1000, 60000);
    ...
    private void bankItems(FletchingConfig config) {
        ...
        if (!hasPrimaryInBank && !hasPrimaryInInventory) {
            Rs2Bank.closeBank();
            ProjectX.status = "[Shutting down] - Reason: " + primaryItemToFletch + " not found in the bank.";
            ProjectX.showMessage(ProjectX.status);
            shutdown();
            return;
        }
        ...
        if (!Rs2Bank.hasItem(secondaryItemToFletch)) {
            ...
            Rs2Bank.closeBank();
            ProjectX.status = "[Shutting down] - Reason: " + secondaryItemToFletch + " not found in the bank.";
            ProjectX.showMessage(ProjectX.status);
            shutdown();
            return;
        }
        ...
        if (Rs2AntibanSettings.naturalMouse) {
            ...
            ProjectX.naturalMouse.moveTo(closePoint.getX(), closePoint.getY());
        }
        ...
        if (!hasPrimaryFinal || !Rs2Inventory.hasItem(secondaryItemToFletch)) {
            ProjectX.log("waiting for inventory changes.");
            Rs2Inventory.waitForInventoryChanges(5000);
        }
    ...
    private boolean configChecks(FletchingConfig config) {
        if (config.fletchingMaterial() == FletchingMaterial.REDWOOD && config.fletchingItem() != FletchingItem.SHIELD) {
            ProjectX.getNotifier().notify("[Wrong Configuration] You can only make shields with redwood logs.");
            shutdown();
            return false;
        }
        return true;
    ...
    public void calculateItemToFletch() {
        int level = ProjectX.getClient().getRealSkillLevel(Skill.FLETCHING);
        ...
        if (fletchingMode == FletchingMode.PROGRESSIVE_STRUNG && level < 5) {
            ProjectX.showMessage("Can't String Bows Below Level 5");
            shutdown();
            return;
        }
        if (level < 5) {
