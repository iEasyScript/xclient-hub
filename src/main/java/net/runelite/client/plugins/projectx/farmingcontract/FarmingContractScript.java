...
public class FarmingContractScript extends Script {
            ...
            try {
                ...
                switch (phase) {
                    ...
                    case DONE:
                        ...
                        ProjectX.stopPlugin(ProjectX.getPluginManager()
                            .getPlugins().stream()
    ...
    private void saveContract() {
        if (contract != null) {
            ProjectX.getConfigManager().setRSProfileConfiguration(
                CONFIG_GROUP, CONFIG_KEY_CONTRACT, String.valueOf(contract.getItemID()));
            contractName = contract.getName();
        } else {
            ProjectX.getConfigManager().unsetRSProfileConfiguration(CONFIG_GROUP, CONFIG_KEY_CONTRACT);
            contractName = null;
        }
    }

    private Produce findProduceByContractName(String name) {
        // The contract varbit is authoritative (same lookup as RuneLite's FarmingContractManager);
        // the dialogue name is only a fallback for when the varbit hasn't updated yet.
        int contractVarbit = ProjectX.getVarbitValue(VarbitID.FARMGUILD_CONTRACT_TYPE);
        if (contractVarbit != 0) {
            for (Produce p : Produce.values()) {
                if (p.getContractVarbitValue() == contractVarbit) {
                    return p;
                }
            }
        }

        // Dialogue says e.g. "Redwood tree" or "Potato cactus"; pick the longest produce
        // name it starts with so "Potato cactus" doesn't resolve to POTATO.
        String lower = name.toLowerCase();
        Produce best = null;
        for (Produce p : Produce.values()) {
            if (p.getContractVarbitValue() != 0 && lower.startsWith(p.getName().toLowerCase())
                && (best == null || p.getName().length() > best.getName().length())) {
                best = p;
            }
        }
        return best;
    ...
    private void harvestPatch(TileObject patch, PatchImplementation patchType) {
        ...
        if (patchType == PatchImplementation.FLOWER) {
            String cropName = ProjectX.getClientThread().runOnClientThreadOptional(
                () -> ProjectX.getItemManager().getItemComposition(cropItemId).getName()
            ).orElse("");
            if (!cropName.isEmpty()) {
    ...
    private boolean payGardener(PatchImplementation patchType) {
        ...
        if (gardener != null) {
            ...
            var guildGardener = ProjectX.getRs2NpcCache().query().withName(npcName).nearestOnClientThread();
    ...
    private int countWithNoted(int itemId) {
        ...
        Integer noted = ProjectX.getClientThread().runOnClientThreadOptional(() -> {
            var comp = ProjectX.getItemManager().getItemComposition(itemId);
