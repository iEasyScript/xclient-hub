...
import net.runelite.client.plugins.projectx.util.math.RateCalculator;
...
public class TitheFarmingOverlay extends OverlayPanel {
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        if (plugin.config.enableOverlay()) {
            for (TitheFarmPlant plant : TitheFarmingScript.plants) {
                ...
                final Polygon polygon = Perspective.getCanvasTilePoly(ProjectX.getClient(), plant.getGameObject().getLocalLocation());

                if (polygon != null) {
                    OverlayUtil.renderPolygon(graphics, polygon, Color.CYAN);
                }

                final LocalPoint localLocation = LocalPoint.fromWorld(ProjectX.getClient(), plant.getGameObject().getWorldLocation());
        ...
        if (plugin.config.enableDebugging()) {
            for (TitheFarmPlant plant : TitheFarmingScript.plants) {
                ...
                Point textLocation = Perspective.getCanvasTextLocation(ProjectX.getClient(), graphics, plant.getGameObject().getLocalLocation(), "p: " + plant.isEmptyPatchOrSeedling(), 0);
    ...
    {
        BufferedImage image = ProjectX.getItemManager().getImage(itemID, amount, true);
        return image;
    }
}
