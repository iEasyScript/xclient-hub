package net.runelite.client.plugins.projectx.mke_wintertodt.startup;
...
import net.runelite.client.plugins.projectx.mke_wintertodt.MKE_WintertodtConfig;
...
import net.runelite.client.plugins.projectx.mke_wintertodt.MKE_WintertodtScript;
...
public class WintertodtStartupManager {
    ...
    public WintertodtStartupManager(MKE_WintertodtConfig config) {
        ...
        ProjectX.log("Startup manager created with fresh sub-manager states");
    }
    ...
    public boolean executeStartupSequence() {
        try {
            ProjectX.log("=== Starting Wintertodt Smart Startup Sequence ===");
            
            // Phase 0: Check if already in game room and ready to play
            if (!executeGameRoomCheck()) {
                ProjectX.log("Failed to complete game room check phase");
                return false;
            }
            
            // If we're already ready to play, skip the rest
            if (startupCompleted) {
                ProjectX.log("=== Player already in game room and ready to play! ===");
                return true;
            }
            
            // Phase 1: Location Setup
            if (!executeLocationSetup()) {
                ProjectX.log("Failed to complete location setup phase");
                return false;
            }
            
            // Phase 2: Gear Analysis and Setup
            if (!executeGearSetup()) {
                ProjectX.log("Failed to complete gear setup phase");
                return false;
            }
            
            // Phase 3: Inventory Setup
            if (!executeInventorySetup()) {
                ProjectX.log("Failed to complete inventory setup phase");
                return false;
            }
            
            // Phase 4: Final Positioning
            if (!executeFinalPositioning()) {
                ProjectX.log("Failed to complete final positioning phase");
                return false;
            }
            ...
            ProjectX.log("=== Wintertodt Startup Sequence Completed Successfully ===");
            return true;
    ...
    private boolean executeLocationSetup() {
        try {
            ...
            ProjectX.log("Error in location setup phase: " + e.getMessage());
            return false;
        }
    }
    ...
    private boolean executeGearSetup() {
        try {
            ...
            ProjectX.log("GEAR PHASE: Starting comprehensive gear analysis and setup");
            
            if (gearManager.setupOptimalGear()) {
                statusMessage = "Optimal gear equipped - ready for Wintertodt!";
                ProjectX.log("GEAR PHASE: Completed successfully");
                return true;
            } else {
                statusMessage = "Failed to setup optimal gear - check bank and levels";
                ProjectX.log("GEAR PHASE: Failed to complete gear setup");
                return false;
            }
            
        } catch (Exception e) {
            statusMessage = "Error during gear setup: " + e.getMessage();
            ProjectX.log("GEAR PHASE: Error - " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
    ...
    private boolean executeInventorySetup() {
        try {
            ...
            ProjectX.log("INVENTORY PHASE: Starting comprehensive inventory setup");
            
            if (inventoryManager.setupInventory()) {
                statusMessage = "Inventory setup completed - tools optimally arranged!";
                ProjectX.log("INVENTORY PHASE: Completed successfully");
                return true;
            } else {
                statusMessage = "Failed to setup inventory - check logs for details";
                ProjectX.log("INVENTORY PHASE: Failed to complete inventory setup");
                return false;
            }
            
        } catch (Exception e) {
            statusMessage = "Error during inventory setup: " + e.getMessage();
            ProjectX.log("INVENTORY PHASE: Error - " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
    ...
    private boolean executeFinalPositioning() {
        try {
            ...
            if (Rs2Player.getWorldLocation().distanceTo(WINTERTODT_BANK) > 10) {
                ProjectX.log("Moving to Wintertodt bank for final positioning...");
                Rs2Walker.walkTo(WINTERTODT_BANK, 5);
                
                if (!sleepUntilTrue(() -> Rs2Player.getWorldLocation().distanceTo(WINTERTODT_BANK) <= 10, 
                                   100, 30000)) {
                    ProjectX.log("Failed to reach final position");
                    return false;
                }
            }
            
            ProjectX.log("Final positioning completed - ready to start main script");
            return true;
            
        } catch (Exception e) {
            ProjectX.log("Error in final positioning phase: " + e.getMessage());
            return false;
        }
    }
    ...
    public void reset() {
        ...
        ProjectX.log("Complete startup manager reset - all phases ready for fresh execution");
    }
    ...
    private boolean executeGameRoomCheck() {
        try {
            ...
            if (!WintertodtLocationManager.isInsideGameRoom()) {
                ProjectX.log("Player not in game room, proceeding with full startup sequence");
                return true; // Continue with normal startup
            }
            
            ProjectX.log("Player is inside Wintertodt game room! Checking readiness...");
            
            // Check if we have necessary items and warm gear
            if (isReadyToPlayInGameRoom()) {
                ProjectX.log("Player is ready to play! Skipping banking and setup phases");
                currentPhase = StartupPhase.COMPLETED;
                startupCompleted = true;
                statusMessage = "Ready to play - skipped startup sequence!";
                return true;
            } else {
                ProjectX.log("Player not ready to play, missing items or gear. Need to go bank first.");
                statusMessage = "Missing items or gear - need to bank first";
                return true; // Continue with normal startup to bank
            }
            
        } catch (Exception e) {
            ProjectX.log("Error in game room check phase: " + e.getMessage());
            return false;
        }
    }
    ...
    private boolean isReadyToPlayInGameRoom() {
        try {
            ProjectX.log("Analyzing current gear and inventory for readiness...");
            
            // Check for required tools
            if (!hasRequiredTools()) {
                ProjectX.log("Missing required tools");
                return false;
            }
        ...
        if (config.healingMethod() == HealingMethod.POTIONS) {
                ...
                ProjectX.log("Using rejuvenation potions - no minimum food requirement (can make potions on demand)");
            } else {
                // Regular food requirement
                if (foodCount < minRequired) {
                    ProjectX.log("Not enough food: " + foodCount + "/" + minRequired + " minimum required");
                    return false;
                }
            }
            ...
            if (warmGearCount < 4) {
                ProjectX.log("Not enough warm gear equipped: " + warmGearCount + "/4 minimum required");
                return false;
            }
            
            if (config.healingMethod() == HealingMethod.POTIONS) {
                ProjectX.log("Has required tools, using rejuvenation potions (can make on demand), and " + warmGearCount + " warm gear pieces");
            } else {
                ProjectX.log("Has required tools, " + foodCount + " food items, and " + warmGearCount + " warm gear pieces");
            }
            return true;
            
        } catch (Exception e) {
            ProjectX.log("Error checking readiness: " + e.getMessage());
            return false;
        }
    }
    ...
    private boolean hasRequiredTools() {
        ...
        ProjectX.log("Tools check - Axe (" + axeDecision.getAxeName() + "): " + hasAxe + 
    ...
    private int getWarmGearCount() {
        try {
            ...
            net.runelite.client.plugins.projectx.mke_wintertodt.startup.gear.WintertodtGearDatabase gearDatabase = 
                new net.runelite.client.plugins.projectx.mke_wintertodt.startup.gear.WintertodtGearDatabase();
            
            ProjectX.log("Analyzing equipped gear for warmth using providesWarmth() flags...");
            ...
            for (EquipmentInventorySlot slot : EquipmentInventorySlot.values()) {
                ...
                if (equipped != null) {
                    ...
                    net.runelite.client.plugins.projectx.mke_wintertodt.startup.gear.WintertodtGearItem gearItem = 
                        gearDatabase.findGearItemById(itemId);
                    
                    if (gearItem != null && gearItem.providesWarmth()) {
                        warmCount++;
                        ProjectX.log("Warm gear: " + gearItem.getItemName() + " (provides warmth)");
                    } else if (gearItem != null) {
                        ProjectX.log("Cold gear: " + gearItem.getItemName() + " (no warmth)");
                    } else {
                        ProjectX.log("Unknown gear: " + equipped.getName() + " (not in database)");
                    }
                }
            }
            
            ProjectX.log("Total warm gear count: " + warmCount + "/4 pieces providing warmth");
            return warmCount;
            
        } catch (Exception e) {
            ProjectX.log("Error during warm gear analysis: " + e.getMessage());
            e.printStackTrace();
