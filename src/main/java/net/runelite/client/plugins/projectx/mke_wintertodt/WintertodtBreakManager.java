...
import net.runelite.client.plugins.projectx.accountselector.AutoLoginPlugin;
...
public class WintertodtBreakManager {
    ...
    private void initializeBreakTimer() {
        ...
        ProjectX.log(String.format("Next break scheduled in %d minutes", nextBreakIn / 60));
    }
    ...
    public boolean update() {
        try {
            ...
            if (currentTime >= nextBreakCheck) {
                ...
                if (breakTimeRemaining > 0 && breakActive) {
                    ...
                    if (logoutBreakActive && breakTimeRemaining % 30 == 0) {
                        ProjectX.log("Logout break countdown: " + (breakTimeRemaining / 60) + " minutes remaining");
                    }
                }
                ...
                if (nextBreakIn <= 0 && !breakActive && !lockState) {
                    if (isInSafeLocationForBreak()) {
                        ...
                        if (waitingForSafeSpotSince == 0) {
                            ...
                            ProjectX.log("Started waiting for safe location for break");
                        }
                        ...
                        if (waitingTime >= MAX_WAIT_FOR_SAFE_SPOT_MS || (waitingTime >= MAX_WAIT_FOR_SAFE_SPOT_MS - 180000 && MKE_WintertodtScript.state == State.WAITING)) {
                            if (!isWalkingToSafeSpot) {
                                ProjectX.log("Waiting for safe spot timed out, requesting walk to boss room safe area");
                                isWalkingToSafeSpot = true;
                                return true; // Signal to walk
                            } else if (isInSafeLocationForBreak()) {
                                // We've arrived at the safe spot
                                ProjectX.log("Arrived at safe spot, starting break");
                                waitingForSafeSpotSince = 0;
                                isWalkingToSafeSpot = false;
                                startRandomBreak();
                            }
                            ...
                            if (remainingWait % 30 == 0) { // Log every 30 seconds
                                ProjectX.log("Delaying break - not in safe location (will force walk in " + remainingWait / 60 + " minutes)");
                            }
                        }
                    }
                }
                ...
                if (breakTimeRemaining <= 0 && breakActive) {
                    if (logoutBreakActive) {
                        ProjectX.log("Logout break timer reached zero - ending break now");
                    }
                    endCurrentBreak();
                }
            }
            ...
            ProjectX.log("Error in WintertodtBreakManager: " + e.getMessage());
            e.printStackTrace();
            // Emergency cleanup
            emergencyCleanup();
        }
    ...
    private void startAfkBreak() {
        ...
        originalMousePosition = ProjectX.getMouse().getMousePosition();
        
        // Move mouse offscreen
        ProjectX.naturalMouse.moveOffScreen(90);
        
        // Update window title to show break status
        updateTitle();
        
        ProjectX.log(String.format("Started AFK break for %d minutes", duration));
    }
    ...
    private void startLogoutBreak() {
        ...
        if (!logoutSuccessful) {
            ProjectX.log("Failed to logout after 3 attempts - continuing with break anyway");
        }
        
        // Update window title to show break status
        updateTitle();
        
        ProjectX.log(String.format("Started logout break for %d minutes", duration));
    }
    ...
    private boolean attemptLogoutWithRetries(int maxAttempts) {
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ProjectX.log("Logout attempt " + attempt + "/" + maxAttempts);
                ...
                if (!ProjectX.isLoggedIn()) {
                    ProjectX.log("Logout successful on attempt " + attempt);
                    return true;
                }
                
                ProjectX.log("Logout attempt " + attempt + " failed - player still logged in");
                ...
                ProjectX.log("Error during logout attempt " + attempt + ": " + e.getMessage());
                if (attempt < maxAttempts) {
                    sleep(3000);
                }
            }
    ...
    private void endAfkBreak() {
        ...
        ProjectX.log("AFK break ended - action plan reset, resuming activities");
    }
    ...
    private void endLogoutBreak() {
        ...
        ProjectX.log("Logout break ended - action plan reset, attempting to log back in");
    ...
    private void attemptLogin() {
        try {
            ...
            if (ProjectX.isLoggedIn()) {
                ProjectX.log("Player already logged in");
                return;
            }
            
            ProjectX.log("Attempting to log back in after logout break...");
            ...
            ProjectX.log("All login attempts failed - player will need to log in manually to resume script");
            
        } catch (Exception e) {
            ProjectX.log("Error during login attempt: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private boolean attemptAutoLoginWithRetries(int maxAttempts) {
        if (!isAutoLoginPluginAvailable()) {
            ProjectX.log("AutoLoginPlugin not available - skipping auto login attempts");
            return false;
        }
        
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ProjectX.log("AutoLogin attempt " + attempt + "/" + maxAttempts);
                
                if (ProjectX.isLoggedIn()) {
                    ProjectX.log("Player already logged in during auto login attempt " + attempt);
                    sleep(20000);
                    return true;
                }
                ...
                while (!ProjectX.isLoggedIn() && System.currentTimeMillis() - loginStart < 60000) {
                    sleep(1000);
                }
                
                if (ProjectX.isLoggedIn()) {
                    ProjectX.log("Successfully logged in using AutoLoginPlugin on attempt " + attempt);
                    sleep(20000);
                    return true;
                }
                
                ProjectX.log("AutoLogin attempt " + attempt + " timed out");
                ...
                ProjectX.log("Error during AutoLogin attempt " + attempt + ": " + e.getMessage());
                if (attempt < maxAttempts) {
                    sleep(5000);
                }
            }
    ...
    private boolean attemptManualLoginWithRetries(int maxAttempts) {
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ProjectX.log("Manual login attempt " + attempt + "/" + maxAttempts);
                
                if (ProjectX.isLoggedIn()) {
                    ProjectX.log("Player already logged in during manual login attempt " + attempt);
                    sleep(20000);
                    return true;
                }
                ...
                if (LoginManager.getActiveProfile() == null) {
                    ProjectX.log("No active profile configured for automatic login");
                    return false;
                }
                ...
                ProjectX.log("Attempting manual login to world " + world + " (attempt " + attempt + ")");
                new Login(world);
                
                // Wait for login to complete (up to 30 seconds)
                long loginStart = System.currentTimeMillis();
                while (!ProjectX.isLoggedIn() && !Rs2Widget.isWidgetVisible(378,0) && System.currentTimeMillis() - loginStart < 30000) {
                    sleep(1000);
                }
                
                if (ProjectX.isLoggedIn()) {
                    ProjectX.log("Successfully logged in using manual login on attempt " + attempt);
                    sleep(20000);
                    return true;
                }
                
                ProjectX.log("Manual login attempt " + attempt + " timed out");
                ...
                ProjectX.log("Error during manual login attempt " + attempt + ": " + e.getMessage());
                if (attempt < maxAttempts) {
                    sleep(5000);
                }
            }
    ...
    private boolean isAutoLoginPluginAvailable() {
        try {
            return ProjectX.getPlugin(AutoLoginPlugin.class.getName()) != null;
        } catch (Exception e) {
            return false;
        }
    }
    ...
    private void enableAutoLoginPlugin() {
        try {
            AutoLoginPlugin autoLoginPlugin = (AutoLoginPlugin) ProjectX.getPlugin(AutoLoginPlugin.class.getName());
            if (autoLoginPlugin != null && !ProjectX.isPluginEnabled(autoLoginPlugin.getClass())) {
                ProjectX.getClientThread().runOnSeperateThread(() -> {
                    ProjectX.startPlugin(autoLoginPlugin);
                    return true;
                });
                ProjectX.log("AutoLoginPlugin enabled for break login");
            }
        } catch (Exception e) {
            ProjectX.log("Failed to enable AutoLoginPlugin: " + e.getMessage());
        }
    }
    ...
    private void disableAutoLoginPlugin() {
        try {
            if (isAutoLoginPluginAvailable()) {
                ...
                if (autoLoginPlugin != null && ProjectX.isPluginEnabled(autoLoginPlugin.getClass())) {
                    ProjectX.getClientThread().runOnSeperateThread(() -> {
                        ProjectX.stopPlugin(autoLoginPlugin);
                        return true;
                    });
                    ProjectX.log("AutoLoginPlugin disabled after use");
                }
            }
        } catch (Exception e) {
            ProjectX.log("Failed to disable AutoLoginPlugin: " + e.getMessage());
        }
    }
    ...
    private void onSuccessfulLogin() {
        try {
            ProjectX.log("Login detected");

            // Disable AutoLoginPlugin after successful login
            disableAutoLoginPlugin();
            
            ProjectX.log("Break system resuming normal operation");

            // Ensure we're on a Wintertodt world after login
            ensureWintertodtWorldAfterLogin();
            
        } catch (Exception e) {
            ProjectX.log("Error during post-login cleanup: " + e.getMessage());
        }
    }
        ...
        try {
            ...
            for (int world : wintertodtWorlds) {
                if (currentWorld == world) {
                    ProjectX.log("Already on Wintertodt world " + currentWorld + " after login");
                    return;
                }
            }
            
            // Not on a Wintertodt world, hop to one
            ProjectX.log("Not on Wintertodt world (current: " + currentWorld + ") - hopping after login...");
            
            int targetWorld = wintertodtWorlds[random.nextInt(wintertodtWorlds.length)];
            boolean hopSuccessful = ProjectX.hopToWorld(targetWorld);
            ...
            if (hopSuccessful) {
                ...
                if (Rs2Player.getWorld() == targetWorld) {
                    ProjectX.log("Successfully hopped to Wintertodt world " + targetWorld + " after login");
                } else {
                    ProjectX.log("World hop timeout after login - continuing on world " + Rs2Player.getWorld());
                }
            } else {
                ProjectX.log("Failed to hop to Wintertodt world after login - continuing on world " + currentWorld);
            }
            
        } catch (Exception e) {
            ProjectX.log("Error ensuring Wintertodt world after login: " + e.getMessage());
        }
    }
    ...
    public boolean isInSafeLocationForBreak() {
        try {
            ...
            ProjectX.log("Error checking safe location: " + e.getMessage());
            return false;
        }
    }
    ...
    public void forceBreak(BreakType type) {
        if (breakActive) {
            ProjectX.log("Break already active, ignoring force break request");
            return;
        }
    ...
    public void forceEndBreak() {
        ...
        ProjectX.log("Manually ending break - resetting action plan");
        breakTimeRemaining = 0;
        endCurrentBreak();
    }
    ...
    public void shutdown() {
        ...
        ProjectX.log("Break manager shutdown");
    }
