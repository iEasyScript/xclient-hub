...
import net.runelite.client.plugins.projectx.qualityoflife.enums.WintertodtActions;
import net.runelite.client.plugins.projectx.qualityoflife.managers.CraftingManager;
import net.runelite.client.plugins.projectx.qualityoflife.managers.FiremakingManager;
import net.runelite.client.plugins.projectx.qualityoflife.managers.FletchingManager;
import net.runelite.client.plugins.projectx.qualityoflife.managers.GemCuttingManager;
import net.runelite.client.plugins.projectx.qualityoflife.scripts.*;
import net.runelite.client.plugins.projectx.qualityoflife.scripts.bank.BankpinScript;
import net.runelite.client.plugins.projectx.qualityoflife.scripts.pvp.PvpScript;
import net.runelite.client.plugins.projectx.qualityoflife.scripts.wintertodt.WintertodtOverlay;
import net.runelite.client.plugins.projectx.qualityoflife.scripts.wintertodt.WintertodtScript;
...
import static net.runelite.client.plugins.projectx.qualityoflife.scripts.wintertodt.WintertodtScript.isInWintertodtRegion;
import static net.runelite.client.plugins.projectx.util.Global.awaitExecutionUntil;
...
public class QoLPlugin extends Plugin implements KeyListener {
    ...
    private static final AtomicBoolean loggedProjectXPluginCacheFailure = new AtomicBoolean(false);
    private static final AtomicBoolean loggedProjectXPluginFallbackScan = new AtomicBoolean(false);

    private static final AtomicReference<QoLPlugin> selfRef = new AtomicReference<>();
    private static final AtomicReference<ProjectXPlugin> projectxPluginRef = new AtomicReference<>();
    ...
    private static ProjectXPlugin findProjectXPluginOnce()
    {
        ProjectXPlugin cached = projectxPluginRef.get();
        if (cached != null)
        {
            return cached;
        }

        ProjectXPlugin found = (ProjectXPlugin) ProjectX.getPluginManager().getPlugins().stream()
                .filter(plugin -> plugin instanceof ProjectXPlugin)
                .findAny().orElse(null);
        if (found != null)
        {
            projectxPluginRef.compareAndSet(null, found);
        }
        return found;
    }
    ...
    protected void startUp() throws AWTException {
        selfRef.set(this);
        // Cache ProjectXPlugin once (avoid scanning plugin list in steady-state UI updates).
        try
        {
            findProjectXPluginOnce();
        }
        catch (Exception ex)
        {
            // Best-effort cache; updateUiElements() can still fall back if needed.
            if (loggedProjectXPluginCacheFailure.compareAndSet(false, true))
            {
                log.debug("QoL: failed to cache ProjectXPlugin during startup", ex);
            }
        }
        ...
        if (config.useSpecWeapon()) {
            ProjectX.getSpecialAttackConfigs().setSpecialAttack(true);
            ProjectX.getSpecialAttackConfigs().setSpecialAttackWeapon(config.specWeapon());
            ProjectX.getSpecialAttackConfigs().setMinimumSpecEnergy(config.specWeapon().getEnergyRequired());
        }
        if (config.autoRun()) {
            ProjectX.enableAutoRunOn = true;
        }
        if (config.autoStamina()) {
            ProjectX.useStaminaPotsIfNeeded = true;
            ProjectX.runEnergyThreshold = config.staminaThreshold() * 100;
        }
    ...
    public void onGameTick(GameTick event) {
        ...
        if (config.useSpecWeapon()) {
            if (ProjectX.getSpecialAttackConfigs().getSpecialAttackWeapon() != config.specWeapon()) {
                ProjectX.getSpecialAttackConfigs().setSpecialAttack(true);
                ProjectX.getSpecialAttackConfigs().setSpecialAttackWeapon(config.specWeapon());
                ProjectX.getSpecialAttackConfigs().setMinimumSpecEnergy(config.specWeapon().getEnergyRequired());
            }
    ...
    public void onHitsplatApplied(HitsplatApplied event) {
        if ((config.resumeFletchingKindling() || config.resumeFeedingBrazier())) {
            if (event.getActor() == ProjectX.getClient().getLocalPlayer()) {
    ...
    private void onMenuOptionClicked(MenuOptionClicked event) {
        ...
        if ((config.resumeFletchingKindling() || config.resumeFeedingBrazier()) && isInWintertodtRegion()) {
            if (event.getMenuOption().contains("Fletch") && event.getMenuTarget().isEmpty() && config.resumeFletchingKindling()) {
                ...
                ProjectX.log("Setting action to Fletch Kindle");
            }
        ...
        if (config.smartWorkbench() && event.getMenuOption().contains("Smart Work-at") && event.getMenuEntry().getIdentifier() == ObjectID.WORKBENCH_43754) {
            if (Rs2Inventory.anyPouchEmpty() && Rs2Inventory.hasItem(ItemID.GUARDIAN_ESSENCE)) {
                ...
                ProjectX.getClientThread().runOnSeperateThread(() -> {
                    ...
                    ProjectX.getRs2TileObjectCache().query().interact(ObjectID.WORKBENCH_43754);
                    return null;
                });
        ...
        if (config.smartGotrMine() && event.getMenuOption().contains("Smart Mine") && event.getMenuEntry().getIdentifier() == ObjectID.HUGE_GUARDIAN_REMAINS) {
            if (Rs2Inventory.anyPouchEmpty() && Rs2Inventory.hasItem(ItemID.GUARDIAN_ESSENCE)) {
                ...
                ProjectX.getClientThread().runOnSeperateThread(() -> {
                    Rs2Inventory.fillPouches();
                    ProjectX.getRs2TileObjectCache().query().interact(ObjectID.HUGE_GUARDIAN_REMAINS);
                    return null;
                });
            ...
            if (Rs2Inventory.anyPouchFull()) {
                ProjectX.getClientThread().runOnSeperateThread(() -> {
                    ...
                    Global.sleepUntil(() -> !Rs2Inventory.anyPouchFull(), () -> {
                                ...
                                ProjectX.getClientThread().invoke(() -> ProjectX.getRs2TileObjectCache().query().withName("Altar").interact());
                                Rs2Inventory.waitForInventoryChanges(3000);
                            }
                            , 10000, 200);
                    return null;
    ...
    private void handleMenuOptionClicked(MenuEntry menuEntry, List<NewMenuEntry> menuEntriesList, String... stopOptions) {
        ...
        if (Arrays.stream(stopOptions).anyMatch(menuEntry.getOption()::contains)) {
            recordActions = false;
            ProjectX.log("<col=5F1515>Stopped recording actions</col>");
        }
    }
    ...
    private void onMenuEntryAdded(MenuEntryAdded event) {
        ...
        if (config.useDoLastBank()) {
            if ("Talk-to".equals(option)) {
                for (MenuEntry e : ProjectX.getClient().getMenuEntries()) {
    ...
    private void customHaProfitOnClicked(MenuEntry entry) {
        ...
        ProjectX.getMouse().click(ProjectX.getClient().getMouseCanvasPosition(), highAlch);
        Global.sleep(Rs2Random.randomGaussian(150, 50));
        ProjectX.getMouse().click(ProjectX.getClient().getMouseCanvasPosition(), highAlchItem);
    ...
    public void onConfigChanged(ConfigChanged ev) {
        if ("smoothRotation".equals(ev.getKey()) && config.smoothCameraTracking() && ProjectX.isLoggedIn()) {
        ...
        if (ev.getKey().equals("autoRun")) {
            ProjectX.enableAutoRunOn = config.autoRun();
        }

        if (ev.getKey().equals("autoStamina")) {
            ProjectX.useStaminaPotsIfNeeded = config.autoStamina();
        }
    ...
    public void onBeforeRender(BeforeRender render) {
        if (!ProjectX.isLoggedIn() || !config.smoothCameraTracking()) {
            return;
        }
    ...
    private void customBankingOnClicked(MenuEntry event) {
        ...
        if (bankMenuEntries.isEmpty()) {
            ProjectX.log("<col=5F1515>No actions recorded</col>");
            return;
        }
        ProjectX.log("<col=245C2D>Banking</col>");
    ...
    private void customTrackOnClicked(MenuEntry event) {
        if (Rs2Camera.isTrackingNpc()) {
            ...
            ProjectX.log("<col=5F1515>Stopped tracking old NPC, try again to track new NPC</col>");
            return;
        }
    ...
    private void customFurnaceOnClicked(MenuEntry event) {
        ...
        ProjectX.log("<col=245C2D>Furnace</col>");
    ...
    private void recordNewActions(MenuEntry event) {
        ...
        ProjectX.log("<col=245C2D>Recording actions for: </col>" + option);
    }
    ...
    private void customCookingOnClicked(MenuEntry event) {
        ProjectX.getClientThread().runOnSeperateThread(() -> {
            ...
            ProjectX.log("<col=245C2D>Cooking</col>");
    ...
    private void quickTeleportToHouse(MenuEntry entry) {
        ...
        if (!Rs2Magic.hasRequiredRunes(Rs2Spells.TELEPORT_TO_HOUSE)) {
            ProjectX.log("<col=5F1515>Missing Required Runes</col>");
            return;
        }

        ProjectX.log("<col=245C2D>Casting: </col>Teleport to House");
        ProjectX.getClientThread().runOnSeperateThread(() -> {
    ...
    private void addMenuEntry(MenuEntryAdded event, String option, String target, Consumer<MenuEntry> callback) {
        int index = ProjectX.getClient().getMenuEntries().length;
        ProjectX.getClient().createMenuEntry(index)
                .setOption(option)
                .setTarget(target)
    ...
    private void addLoadoutMenuEntry(MenuEntryAdded event, String option, String target, Consumer<MenuEntry> callback) {
        int index = ProjectX.getClient().getMenuEntries().length - 1;
        ProjectX.getClient().createMenuEntry(index)
                .setOption(option)
                .setTarget(target)
                .setParam0(event.getActionParam0())
    ...
    private boolean updateUiElements() {
        ...
        try {
            ...
            ProjectXPlugin projectxPlugin = findProjectXPluginOnce();
            if (projectxPlugin == null && loggedProjectXPluginFallbackScan.compareAndSet(false, true))
            {
                log.debug("QoL: ProjectXPlugin ref was empty; fell back to plugin manager scan.");
            }

            // If ConfigPlugin is not found, log an error and return false
            if (projectxPlugin == null) {
                ProjectX.log("Config Plugin not found");
                return false;
            }
            projectxPluginRef.set(projectxPlugin);

            // Get the plugin list panel from the ConfigPlugin instance
            JPanel pluginListPanel = getPluginListPanel(projectxPlugin);
            ...
            if (currentPluginList == null) {
                ProjectX.log("Plugin list is null, waiting for it to be initialized");
                return false;
            }
    ...
    private static void restoreUiElements()
    {
        ...
        {
            QoLPlugin self = selfRef.get();
            ProjectXPlugin projectxPlugin = projectxPluginRef.get();
            if (self != null && projectxPlugin != null)
            {
                JPanel pluginListPanel = self.getPluginListPanel(projectxPlugin);
    ...
    private JPanel getPluginListPanel(ProjectXPlugin projectxPlugin) throws ClassNotFoundException {

        Class<?> pluginListPanelClass = Class.forName("net.runelite.client.plugins.projectx.ui.ProjectXPluginListPanel");
        if (projectxPlugin == null)
        {
            throw new IllegalStateException("ProjectXPlugin instance is null");
        }
        return (JPanel) projectxPlugin.getInjector().getProvider(pluginListPanelClass).get();
    }
            ...
            try {
                ...
                if (clipboardText != null && !clipboardText.isEmpty()) {
                    ProjectX.getClientThread().invoke(() -> {
                        ProjectX.getClient().setVarcStrValue(VarClientID.MESLAYERINPUT, clipboardText);
                        ProjectX.getClient().runScript(ScriptID.GE_ITEM_SEARCH);
                    });
                }
            } catch (Exception ex) {
                ProjectX.log("Failed to paste from clipboard: " + ex.getMessage());
            }
        }
    }
