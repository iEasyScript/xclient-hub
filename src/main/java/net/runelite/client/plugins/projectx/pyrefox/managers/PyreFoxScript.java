package net.runelite.client.plugins.projectx.pyrefox.managers;
...
import net.runelite.client.plugins.projectx.pyrefox.PyreFoxConfig;
import net.runelite.client.plugins.projectx.pyrefox.PyreFoxConstants;
import net.runelite.client.plugins.projectx.pyrefox.PyreFoxPlugin;
import net.runelite.client.plugins.projectx.pyrefox.enums.PyreFoxState;
import net.runelite.client.plugins.projectx.pyrefox.helpers.BankHelper;
import net.runelite.client.plugins.projectx.pyrefox.helpers.PlayerHelper;
...
//                System.out.println("Total time for loop " + totalTime);
    ...
    private void _handleCatching()
    {
        ...
        {
            ProjectX.log("No Trap GameObject found.");
            return;
        }
    ...
    private Rs2TileObjectModel _getTrapObjectAtTrapLocation()
    {
        ...
        return ProjectX.getRs2TileObjectCache().query().within(point, 0).first();
    }
    ...
    private WorldPoint _getTrapObjectWorldPoint()
    {
        ...
        {
            var rock = ProjectX.getRs2TileObjectCache().query().withId(PyreFoxConstants.GAMEOBJECT_ROCK_NO_TRAP).within(10).nearest();
            if (rock == null)
            {
                _log("No rock found");
                return null;
        ...
        if (_config.UseMeatPouch())
            if (Rs2Inventory.hasItem(_config.MeatPouch().getClosedItemID()))
                if(Rs2Inventory.interact(_config.MeatPouch().getClosedItemID(), "Open"))
                    ProjectX.log("Opened meat bag.");

        if (!Rs2Inventory.hasItem(ItemID.KNIFE) || !PlayerHelper.playerHasAxeOnHim())
        {
            ProjectX.log("You lack required items - shutting down!");
            shutdown();
        }
