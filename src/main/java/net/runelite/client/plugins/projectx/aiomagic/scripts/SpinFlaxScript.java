        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (!Rs2Player.getSkillRequirement(Skill.MAGIC, 76)) {
                    ProjectX.showMessage("Spin Flax requires 76 Magic.");
                    shutdown();
                    return;
                }
                ...
                switch (state) {
                    case BANKING:
                        ProjectX.status = "Banking: prepare 25 flax";
                        ...
                        if (!hasAstralBank && !hasAstralInv) { ProjectX.showMessage("Astral runes not found in bank."); shutdown(); return; }
                        if (!hasNatureBank && !hasNatureInv) { ProjectX.showMessage("Nature runes not found in bank."); shutdown(); return; }
                        if (!hasAirBank && !hasAirInv) { ProjectX.showMessage("Air runes not found in bank."); shutdown(); return; }
                        ...
                        if (!Rs2Bank.hasBankItem(ItemID.FLAX, 1)) {
                            ProjectX.showMessage("No flax in bank.");
                            shutdown();
                            return;
                        }
                    ...
                    case CASTING:
                        ProjectX.status = "Casting: Spin Flax (" + castsDone + "/5)";
                        ...
                        if (!Rs2Magic.hasRequiredRunes(Rs2Spells.SPIN_FLAX)) {
                            ProjectX.showMessage("Out of runes for Spin Flax");
                            shutdown();
                            return;
                        }
                        ...
                        if (!Rs2Magic.cast(Rs2Spells.SPIN_FLAX)) {
                            ProjectX.log("Unable to cast Spin Flax");
                            state = MagicState.BANKING;
                            break;
                        }
