package net.runelite.client.plugins.projectx.projectxdashboardplus.poller;
...
import net.runelite.client.plugins.projectx.BlockingEventManager;
import net.runelite.client.plugins.projectx.ProjectX;
import net.runelite.client.plugins.projectx.projectxdashboardplus.data.PollSnapshot;
import net.runelite.client.plugins.projectx.projectxdashboardplus.data.XpHistory;
import net.runelite.client.plugins.projectx.projectxdashboardplus.notify.AlertManager;
import net.runelite.client.plugins.projectx.projectxdashboardplus.notify.DiscordNotifier;
...
public class GameStatePoller {
    ...
    /** Package prefix used to identify ProjectX Hub plugins. */
    private static final String PROJECTX_PACKAGE_PREFIX = "net.runelite.client.plugins.projectx.";
     ...
     * (configuration helper), test harnesses, and the bare "ProjectX" core
     * plugin.
     */
    private static final String[] INFRA_NAME_SUBSTRINGS = {
            ...
            "projectx dashboard plus", // the dashboard itself
    ...
    private static boolean isInfrastructurePlugin(String displayName, String simpleClassName) {
        ...
        // Exact equals for bare "ProjectX" (avoid false positives in containing strings).
        if ("projectx".equals(d)) return true;
    ...
    public void start(int pollIntervalSeconds) {
        ...
        executor = Executors.newSingleThreadScheduledExecutor(r -> {
            Thread t = new Thread(r, "ProjectXDashboardPlus-Poller");
            t.setDaemon(true);
            return t;
        });
        scheduledTask = executor.scheduleAtFixedRate(this::tickSafely, 0, this.pollIntervalSeconds, TimeUnit.SECONDS);
        log.info("ProjectXDashboardPlus poller started (interval={}s)", this.pollIntervalSeconds);
    }
    ...
    public void stop() {
        ...
        log.info("ProjectXDashboardPlus poller stopped");
    }
    ...
    private static String profileName() {
        try { return safe(ProjectX.getConfigManager().getRSProfileKey()); }
    ...
    /**
     * Active scripts: enabled ProjectX Hub plugins minus a static exclusion
     * set (Antiban, ProjectX core utility, the dashboard itself). Per-plugin
    ...
    private List<PollSnapshot.ScriptStatus> collectActiveScripts() {
        try {
            ...
            for (Plugin p : ProjectX.getPluginManager().getPlugins()) {
                if (p == null) continue;
                if (!ProjectX.isPluginEnabled(p.getClass())) continue;
                if (!p.getClass().getName().startsWith(PROJECTX_PACKAGE_PREFIX)) continue;
     ...
     * {@link ProjectX#pauseAllScripts} switch, and the registered
    ...
    private PollSnapshot.AntibanState collectAntibanState() {
        try {
            ...
            try { allPaused = ProjectX.pauseAllScripts != null && ProjectX.pauseAllScripts.get(); }
            catch (Throwable t) { log.debug("read pauseAllScripts failed: {}", t.getMessage()); }

            int blockingCount = 0;
            Boolean blockingRunning = null;
            try {
                BlockingEventManager mgr = ProjectX.getBlockingEventManager();
                if (mgr != null) {
                    try {
