...
@PluginDescriptor(
        ...
        description = "ProjectX thieving plugin",
...
public class ThievingPlugin extends Plugin {
    ...
    private void setStartXp() {
        ...
        if (startXp == 0 || (name != null && !name.equals(this.name))) {
            this.name = name;
            startXp = ProjectX.getClient().getSkillExperience(Skill.THIEVING);
        }
    }

    public int xpGained() {
        setStartXp();
        final int currentXp = ProjectX.getClient().getSkillExperience(Skill.THIEVING);
        return currentXp - startXp;
    }

	public int determineMaxCoinPouch() {
		if (ProjectX.getVarbitValue(VarbitID.ARDOUGNE_DIARY_ELITE_COMPLETE) == 1) {
			return 140;
		} else if (ProjectX.getVarbitValue(VarbitID.ARDOUGNE_DIARY_HARD_COMPLETE) == 1) {
			return 84;
		} else if (ProjectX.getVarbitValue(VarbitID.ARDOUGNE_DIARY_MEDIUM_COMPLETE) == 1) {
			return 56;
		} else {
			return 28;
		}
