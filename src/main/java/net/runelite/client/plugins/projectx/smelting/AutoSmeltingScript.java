package net.runelite.client.plugins.projectx.smelting;

import net.runelite.api.Skill;
import net.runelite.api.gameval.ItemID;
import net.runelite.client.plugins.projectx.ProjectX;
import net.runelite.client.plugins.projectx.Script;
import net.runelite.client.plugins.projectx.smelting.enums.Ores;
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (currentSmithing < requiredSmithing) {
                    ProjectX.log("Smithing " + currentSmithing + " is below the " + requiredSmithing
                ...
                Rs2TileObjectModel furnace = ProjectX.getRs2TileObjectCache().query()
                ...
                if (furnace == null) {
                    if (initialPlayerLocation.distanceTo(Rs2Player.getWorldLocation()) > 4) {
                        ...
                        ProjectX.status = "AutoSmelting: no furnace within 20 tiles of start — stand near a furnace and restart";
                    }
                    return;
                }
                ...
                ProjectX.logStackTrace("AutoSmeltingScript", ex);
            }
    ...
    private void withdrawRightAmountOfMaterials(AutoSmeltingConfig config) {
        for (Map.Entry<Ores, Integer> requiredMaterials : config.SELECTED_BAR_TYPE().getRequiredMaterials().entrySet()) {
            ...
            if (!Rs2Bank.hasBankItem(name, totalAmount, true)) {
                ProjectX.log("Bank lacks " + totalAmount + " " + name + ". Shutting down.");
                shutdown();
                return;
            }
            ...
            if (!Rs2Inventory.hasItemAmount(name, totalAmount, false, true)) {
                ProjectX.log("Withdraw of " + totalAmount + " " + name + " didn't settle in 3.5s; retrying next tick.");
                return;
            }
        }
    }
