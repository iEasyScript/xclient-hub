...
import net.runelite.client.plugins.projectx.util.grounditem.Rs2LootEngine;
...
import net.runelite.client.plugins.projectx.util.npc.MonsterLocation;
...
import net.runelite.client.plugins.projectx.slayer.profile.SlayerProfileManager;
import net.runelite.client.plugins.projectx.slayer.profile.SlayerTaskProfileJson;
...
public class SlayerScript extends Script {
    ...
    public boolean run(SlayerConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                // Read slayer points
                int slayerPoints = ProjectX.getVarbitValue(SLAYER_REWARD_POINTS_VARBIT);
    ...
    private void handleGettingTaskState(boolean hasTask, String taskName) {
        ...
        if (hasTask) {
            if (shouldBlockTask(taskName)) {
                ...
                if (shouldSkipTask(taskName)) {
                    ...
                    ProjectX.showMessage("Cannot block/skip task '" + taskName + "' - insufficient slayer points. Logging out.");
                    stopAndLogout();
                } else {
                ...
                ProjectX.showMessage("Cannot skip task '" + taskName + "' - insufficient slayer points. Logging out.");
                stopAndLogout();
            } else {
        ...
        if (distance <= 5) {
            ...
            var masterNpc = ProjectX.getRs2NpcCache().query().withName(master.getName()).nearestOnClientThread();
            if (masterNpc != null) {
    ...
    private void handleSkippingTaskState() {
        ...
        if (slayerPoints < SKIP_TASK_COST || slayerPoints < config.minPointsToSkip()) {
            ...
            ProjectX.showMessage("Cannot skip task '" + taskBeingSkipped + "' - insufficient slayer points. Logging out.");
            resetSkipState();
            stopAndLogout();
            return;
        }
    ...
    private boolean isInPoh() {
        return ProjectX.getRs2TileObjectCache().query().withId(HOUSE_PORTAL_ID).nearest() != null;
    }
    ...
    private boolean useRejuvenationPool() {
        for (int poolId : REJUVENATION_POOL_IDS) {
            if (ProjectX.getRs2TileObjectCache().query().withId(poolId).nearest() != null) {
                return ProjectX.getRs2TileObjectCache().query().interact(poolId, "Drink");
            }
        }
        return false;
    }
    ...
    private void leaveHouse() {
        ...
        if (ProjectX.getRs2TileObjectCache().query().interact(HOUSE_PORTAL_ID, "Enter")) {
    ...
    private boolean teleportViaPortalNexus() {
        ...
        for (int nexusId : PORTAL_NEXUS_IDS) {
            if (ProjectX.getRs2TileObjectCache().query().withId(nexusId).nearest() != null) {
                if (ProjectX.getRs2TileObjectCache().query().interact(nexusId, "Grand Exchange")) {
                    return true;
                }
                if (ProjectX.getRs2TileObjectCache().query().interact(nexusId, "Varrock Grand Exchange")) {
                    return true;
                }
            }
        }
    ...
    private boolean teleportViaMountedGlory() {
        for (int gloryId : MOUNTED_GLORY_IDS) {
            if (ProjectX.getRs2TileObjectCache().query().withId(gloryId).nearest() != null) {
                if (ProjectX.getRs2TileObjectCache().query().interact(gloryId, "Edgeville")) {
                    return true;
                }
            }
        }
    ...
    private boolean teleportViaMountedWealth() {
        if (ProjectX.getRs2TileObjectCache().query().withId(MOUNTED_WEALTH_ID).nearest() != null) {
            if (ProjectX.getRs2TileObjectCache().query().interact(MOUNTED_WEALTH_ID, "Grand Exchange")) {
                return true;
            }
        }
        return false;
    ...
    private void handleBlockingTaskState() {
        ...
        if (slayerPoints < BLOCK_TASK_COST || slayerPoints < config.minPointsToBlock()) {
            ...
            if (shouldSkipTask(currentTask)) {
                ...
                ProjectX.showMessage("Cannot block/skip task '" + currentTask + "' - insufficient slayer points. Logging out.");
                stopAndLogout();
            } else {
    ...
    private net.runelite.client.plugins.projectx.inventorysetups.InventorySetup currentInventorySetup = null;
    ...
    private Rs2TileObjectModel findOccultAltar() {
        for (int altarId : OCCULT_ALTAR_IDS) {
            Rs2TileObjectModel altar = ProjectX.getRs2TileObjectCache().query().withId(altarId).nearest();
            if (altar != null) {
                return altar;
            }
        }
    ...
    private void handlePotions() {
        ...
        int poisonVarp = ProjectX.getClient().getVarpValue(102);
    ...
    private int getRequiredFinishingItemId() {
        ...
        if (taskLower.contains("lizard") && !taskLower.contains("lizardm")
                && ProjectX.getVarbitValue(SLAYER_AUTOKILL_DESERTLIZARDS) == 0) {
            return ItemID.SLAYER_ICY_WATER;
        } else if (taskLower.contains("rockslug")
                && ProjectX.getVarbitValue(SLAYER_AUTOKILL_ROCKSLUGS) == 0) {
            return ItemID.SLAYER_BAG_OF_SALT;
        } else if (taskLower.contains("gargoyle")
                && ProjectX.getVarbitValue(SLAYER_AUTOKILL_GARGOYLES) == 0) {
    ...
    private void handleSlayerFinishingItems() {
        final Player localPlayer = ProjectX.getClient().getLocalPlayer();
        if (localPlayer == null) return;

        Rs2NpcModel npc = ProjectX.getRs2NpcCache().query()
                .where(n -> n.isDead() && java.util.Objects.equals(n.getInteracting(), localPlayer))
                .first();
        if (npc == null) return;

        net.runelite.client.plugins.projectx.util.npc.Rs2NpcModel legacyNpc =
                new net.runelite.client.plugins.projectx.util.npc.Rs2NpcModel(npc.getNpc());

        if (ProjectX.getVarbitValue(SLAYER_AUTOKILL_DESERTLIZARDS) == 0
    ...
    private void handleSuperAntifire() {
        ...
        int superAntifireTimer = ProjectX.getVarbitValue(SUPER_ANTIFIRE_VARBIT);
        int regularAntifireTimer = ProjectX.getVarbitValue(ANTIFIRE_VARBIT);
    ...
    private boolean handleAntivenom() {
        ...
        int poisonVarp = ProjectX.getClient().getVarpValue(102); // VarPlayerID.POISON
        if (poisonVarp <= 1000000) {
    ...
    private boolean handleAntipoison() {
        ...
        int poisonVarp = ProjectX.getClient().getVarpValue(POISON_VARP);
        if (poisonVarp <= 0 || poisonVarp > 1000000) {
    ...
    private void handleCombat() {
        ...
        List<Rs2NpcModel> attackableNpcs = ProjectX.getRs2NpcCache().query()
                ...
                .sorted(Comparator
                        .comparingInt((Rs2NpcModel npc) ->
                                npc.getInteracting() == ProjectX.getClient().getLocalPlayer() ? 0 : 1)
        ...
        if (attackableNpcs.isEmpty()) {
            ...
            List<String> nearbyNpcNames = ProjectX.getRs2NpcCache().query()
                    .where(npc -> !npc.isDead())
                    .where(npc -> npc.getName() != null)
                    .where(npc -> taskDestination == null ||
                            npc.getWorldLocation().distanceTo(taskDestination) <= config.attackRadius())
    ...
    private void hopToNewWorld() {
        try {
            ...
            sleepUntil(() -> ProjectX.getClient().getGameState() == GameState.HOPPING, 5000);
            sleepUntil(() -> ProjectX.getClient().getGameState() == GameState.LOGGED_IN, 10000);
    ...
    private int getHopWorld() {
        ...
        int currentWorld = ProjectX.getClient().getWorld();
    ...
    private boolean isBeingAttacked() {
        return ProjectX.getRs2NpcCache().query()
                .where(npc -> npc.getInteracting() == ProjectX.getClient().getLocalPlayer())
                .count() > 0;
    }
    ...
    private void handleAoeCombat() {
        ...
        List<Rs2NpcModel> nearbyMonsters = ProjectX.getRs2NpcCache().query()
                .where(npc -> npc.getInteracting() == ProjectX.getClient().getLocalPlayer())
                .where(npc -> npc.getName() != null)
                .where(npc -> targetMonsters.stream()
    ...
    private void activateBestRangedPrayer(int prayerLevel) {
        ...
        if (prayerLevel >= 74) {
            boolean rigourUnlocked = ProjectX.getVarbitValue(RIGOUR_UNLOCKED_VARBIT) == 1;
            if (rigourUnlocked) {
    ...
    private void activateBestMagicPrayer(int prayerLevel) {
        ...
        if (prayerLevel >= 77) {
            boolean auguryUnlocked = ProjectX.getVarbitValue(AUGURY_UNLOCKED_VARBIT) == 1;
            if (auguryUnlocked) {
     ...
     */
    private net.runelite.client.plugins.projectx.inventorysetups.InventorySetup getActiveInventorySetup() {
        ...
        if (setupName != null && !setupName.isEmpty()) {
            ...
            if (isUsingCannon && !setupName.toLowerCase().endsWith("-cannon")) {
                ...
                net.runelite.client.plugins.projectx.inventorysetups.InventorySetup cannonSetup = findInventorySetupByName(cannonName);
                if (cannonSetup != null) {
            ...
            net.runelite.client.plugins.projectx.inventorysetups.InventorySetup profileSetup = findInventorySetupByName(setupName);
     ...
     */
    private net.runelite.client.plugins.projectx.inventorysetups.InventorySetup findInventorySetupByName(String name) {
    ...
    private boolean handleCannonPickup() {
        ...
        var cannon = ProjectX.getRs2TileObjectCache().query().withName("Dwarf multicannon").within(Rs2Player.getWorldLocation(), 50).nearestOnClientThread();
        if (cannon == null) {
    ...
    private boolean pickupCannon() {
        var cannon = ProjectX.getRs2TileObjectCache().query().withName("Dwarf multicannon").nearestOnClientThread();
    ...
    private boolean isCannonPlacedNearby() {
        return ProjectX.getRs2TileObjectCache().query().withName("Dwarf multicannon").within(Rs2Player.getWorldLocation(), 10).nearestOnClientThread() != null;
    }
