package net.runelite.client.plugins.projectx.colosseumprayer;
...
public class ColosseumPrayerScript {
    ...
    public void onGameTick(GameTick tick) {
        ...
        if (shouldFlickJavelinWithSerpentShaman(demands, localPlayer)) {
            Client clientForFlick = ProjectX.getClient();
            ...
            if (config.debugLogSignals()) {
                log.info("[ColosseumPrayer] Javelin + Serpent Shaman flick tick={}",
                        ProjectX.getClient() != null ? ProjectX.getClient().getTickCount() : -1);
            }
            return;
        }
    ...
    private boolean hasLineOfSightThreat(ThreatKind kind, Player player) {
        ...
        for (Rs2NpcModel npc : ProjectX.getRs2NpcCache().query().toList()) {
    ...
    List<ColosseumPrayerDemand> gatherDemands() {
        Player player = ProjectX.getClient() != null ? ProjectX.getClient().getLocalPlayer() : null;
