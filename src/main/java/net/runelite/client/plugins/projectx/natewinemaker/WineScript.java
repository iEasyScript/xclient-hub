package net.runelite.client.plugins.projectx.natewinemaker;
...
public class WineScript extends Script {
    ...
    private static int projectedXp() {
        int current = ProjectX.getClient().getSkillExperience(Skill.COOKING);
    ...
    public static int getXpToMax() {
        if (!ProjectX.isLoggedIn()) return 0;
    ...
    public static String getTimeToMax() {
        if (startXp < 0 || !ProjectX.isLoggedIn()) return "-";
    ...
    public boolean run(WineConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (startXp < 0) {
                    startXp = ProjectX.getClient().getSkillExperience(Skill.COOKING);
    ...
    private void logoutAndStop(String reason) {
        ProjectX.status = "[Shutting down] - Reason: " + reason + ".";
        ProjectX.getNotifier().notify("Wine Maker stopping: " + reason);
        if (Rs2Bank.isOpen()) {
            Rs2Bank.closeBank();
            sleepUntil(() -> !Rs2Bank.isOpen());
        }
        Rs2Player.logout();
        sleepUntil(() -> !ProjectX.isLoggedIn(), 10000);
        Plugin wineMakerPlugin = ProjectX.getPluginManager().getPlugins().stream()
                .filter(x -> x.getClass().getName().equals(WinePlugin.class.getName()))
                .findFirst()
                .orElse(null);
        ProjectX.stopPlugin(wineMakerPlugin);
        shutdown();
    }
