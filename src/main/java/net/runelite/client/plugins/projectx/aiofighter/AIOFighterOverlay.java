...
import net.runelite.client.plugins.projectx.aiofighter.combat.AttackNpcScript;
import net.runelite.client.plugins.projectx.aiofighter.model.Monster;
...
import static net.runelite.client.plugins.projectx.aiofighter.combat.AttackNpcScript.filteredAttackableNpcs;
import static net.runelite.client.plugins.projectx.aiofighter.combat.FlickerScript.currentMonstersAttackingUsRef;
...
public class AIOFighterOverlay extends OverlayPanel {
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        GeneralPath lines = linesToDisplay[ProjectX.getClient().getTopLevelWorldView().getPlane()];
        LocalPoint lp = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), config.centerLocation());
        if (lp != null) {
            Polygon poly = Perspective.getCanvasTileAreaPoly(ProjectX.getClient(), lp, config.attackRadius() * 2);
        ...
        // render safe spot
        LocalPoint sslp = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), config.safeSpot());
        if (sslp != null) {
            Polygon safeSpotPoly = Perspective.getCanvasTileAreaPoly(ProjectX.getClient(), sslp, 1);
    ...
    private void renderPath(Graphics2D graphics, GeneralPath path, Color color) {
        LocalPoint playerLp = ProjectX.getClient().getLocalPlayer().getLocalLocation();
        ...
        path = Geometry.filterPath(path, (p1, p2) ->
                Perspective.localToCanvas(ProjectX.getClient(), new LocalPoint((int) p1[0], (int) p1[1]), ProjectX.getClient().getTopLevelWorldView().getPlane()) != null &&
                        Perspective.localToCanvas(ProjectX.getClient(), new LocalPoint((int) p2[0], (int) p2[1]), ProjectX.getClient().getTopLevelWorldView().getPlane()) != null);
        path = Geometry.transformPath(path, coords ->
        {
            Point point = Perspective.localToCanvas(ProjectX.getClient(), new LocalPoint((int) coords[0], (int) coords[1]), ProjectX.getClient().getTopLevelWorldView().getPlane());
            coords[0] = point.getX();
            coords[1] = point.getY();
        });
    ...
    private void calculateLinesToDisplay() {
        ...
        Rectangle sceneRect = new Rectangle(
                ProjectX.getClient().getTopLevelWorldView().getBaseX() + 1, ProjectX.getClient().getTopLevelWorldView().getBaseY() + 1,
    ...
    private void transformWorldToLocal(float[] coords) {
        final LocalPoint lp = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), (int) coords[0], (int) coords[1]);
        if (lp == null) return;
