...
import net.runelite.client.plugins.projectx.util.walker.WalkerState;
import net.runelite.client.plugins.projectx.util.player.Rs2Player;
import net.runelite.client.plugins.projectx.valetotems.handlers.FletchingHandler;
...
public class NavigationHandler {
    ...
    private static void checkPathfinderSettings() {
        try {
            ...
            net.runelite.client.plugins.projectx.shortestpath.pathfinder.PathfinderConfig pathfinderConfig = 
     ...
     */
    public static boolean navigateToTotem(TotemLocation totemLocation, net.runelite.client.plugins.projectx.valetotems.models.GameSession gameSession) {
        try {
            ...
            while (!CoordinateUtils.isAtTotemLocation(totemLocation, ARRIVAL_TOLERANCE) &&
                ...
                if (!isWalkActionOnCooldown()) {
                    LocalPoint localNextCheckpoint = ProjectX.getClientThread().invoke(() ->
                            LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), nextCheckpoint));
    ...
    private static boolean handlePathTransports(List<WorldPoint> path, int currentIndex) {
        try {
            ...
            if (currentIndex < path.size() - 1) {
                ...
                var doors = net.runelite.client.plugins.projectx.ProjectX.getRs2TileObjectCache().query().where(
                    obj -> {
                        if (!obj.getWorldLocation().equals(nextPoint)) {
                            return false;
                        }
                        net.runelite.api.ObjectComposition comp = obj.getObjectComposition();
                        return net.runelite.client.plugins.projectx.util.gameobject.Rs2GameObject.hasAction(comp, "Open") ||
                               net.runelite.client.plugins.projectx.util.gameobject.Rs2GameObject.hasAction(comp, "Close") ||
                               net.runelite.client.plugins.projectx.util.gameobject.Rs2GameObject.hasAction(comp, "Pick-lock");
                    }).toList();
            ...
            java.util.Map<WorldPoint, java.util.Set<net.runelite.client.plugins.projectx.shortestpath.Transport>> allTransports =
                net.runelite.client.plugins.projectx.shortestpath.ShortestPathPlugin.getTransports();
            ...
            if (allTransports != null && !allTransports.isEmpty()) {
                ...
                for (int i = currentIndex; i < Math.min(currentIndex + 15, path.size()); i++) {
                    ...
                    if (allTransports.containsKey(pathPoint)) {
                        ...
                        for (net.runelite.client.plugins.projectx.shortestpath.Transport transport : allTransports.get(pathPoint)) {
                            if (transport.getType().toString().contains("AGILITY") &&
                                ...
                                var agilityObj = net.runelite.client.plugins.projectx.ProjectX.getRs2TileObjectCache().query().withId(transport.getObjectId()).nearest();
                                if (agilityObj != null &&
                                    ...
                                    if (agilityObj != null && agilityObj.click(transport.getAction())) {
                                        ...
                                        while (System.currentTimeMillis() - startTime < timeout) {
                                            ...
                                            if (!Rs2Player.isMoving()) {
                                                int animation = ProjectX.getClientThread().invoke(() -> {
