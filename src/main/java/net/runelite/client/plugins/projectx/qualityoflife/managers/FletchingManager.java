...
import net.runelite.client.plugins.projectx.qualityoflife.enums.FletchingArrow;
import net.runelite.client.plugins.projectx.qualityoflife.enums.FletchingBolt;
import net.runelite.client.plugins.projectx.qualityoflife.enums.FletchingDarts;
import net.runelite.client.plugins.projectx.qualityoflife.enums.FletchingLogs;
...
public class FletchingManager
{
    ...
    private void fletchBrumaRootsOnClick(MenuEntry event)
    {
        ...
        if (brumaRootSlot == -1)
        {
            ProjectX.log("<col=5F1515>Bruma root not found in inventory</col>");
            return;
        }
        ProjectX.log("<col=245C2D>Fletching Kindling</col>");
        NewMenuEntry combined = createWidgetOnWidgetEntry("Fletch", "Bruma root", brumaRootSlot, event.getParam1(), ItemID.WINT_BRUMA_ROOT);
        ProjectX.doInvoke(combined, new Rectangle(1, 1));
    }
    ...
    private void quickFletchLogsOnClick(MenuEntry event)
    {
        ...
        {
            ProjectX.log("<col=5F1515>No fletchable logs found in inventory</col>");
            return;
        }
        Rs2ItemModel logToFletch = fletchableLogs.get(0);
        if (logToFletch.getSlot() == -1)
        {
            ProjectX.log("<col=5F1515>Couldn't get item slot</col>");
            return;
        }

        ProjectX.log("<col=245C2D>Fletching: " + logToFletch.getName() + " To: " + config.fletchingItem().getName() + "</col>");
    ...
    private void quickFletchHeadlessArrowOnClick(MenuEntry event)
    {
        ...
        if (featherSlot == -1) {
            ProjectX.log("<col=5F1515>Feather not found in inventory</col>");
            return;
        }

        ProjectX.log("<col=245C2D>Fletching: Headless Arrow</col>");
    ...
    private void quickFletchDartsOnClick(MenuEntry event, FletchingDarts dart)
    {
        ...
        ProjectX.log("<col=245C2D>Fletching: " + dart.getDartTip() + " To: " + dart.getDart() + "</col>");
        NewMenuEntry combined = createWidgetOnWidgetEntry("Fletch", "Feather", featherSlot, event.getParam1(), ItemID.FEATHER);
        ProjectX.getMouse().click(ProjectX.getClient().getMouseCanvasPosition(), combined);
    }
    ...
    private void quickFletchBoltOnClick(MenuEntry event, FletchingBolt bolt)
    {
        ...
        ProjectX.log("<col=245C2D>Fletching: " + bolt.getBoltTip() + " To: " + bolt.getBolt() + "</col>");
        NewMenuEntry combined = createWidgetOnWidgetEntry("Fletch", "Feather", featherSlot, event.getParam1(), ItemID.FEATHER);
        ProjectX.getMouse().click(ProjectX.getClient().getMouseCanvasPosition(), combined);
    }
    ...
    private void quickFletchArrowOnClick(MenuEntry event, FletchingArrow arrow)
    {
        ...
        if (headlessSlot == -1) {
            ProjectX.log("<col=5F1515>Headless Arrow not found in inventory</col>");
            return;
        }

        ProjectX.log("<col=245C2D>Fletching: " + arrow.getArrow() + "</col>");
