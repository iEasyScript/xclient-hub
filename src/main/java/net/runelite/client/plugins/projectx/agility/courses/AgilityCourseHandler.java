...
public interface AgilityCourseHandler
{
	...
	{
		return ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getTopLevelWorldView().getPlane());
	}

	default int getVarbitValue(int varbitId)
	{
		return ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getVarbitValue(varbitId));
	}
