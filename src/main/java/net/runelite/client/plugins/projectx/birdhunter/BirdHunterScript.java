package net.runelite.client.plugins.projectx.birdhunter;
...
public class BirdHunterScript extends Script {
    ...
    public boolean run(BirdHunterConfig config, BirdHunterPlugin plugin) {
        this.plugin = plugin;
        ProjectX.log("Bird Hunter script started.");
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            ...
            try {
                ...
                if (!hasRequiredSnares()) {
                    ...
                    ProjectX.showMessage("Bird Hunter needs at least " + required
                ...
                if (!isInHuntingArea()) {
                    ProjectX.log("Player is outside the designated hunting area.");
                    walkBackToArea();
                    return;
                }
    ...
    private boolean hasRequiredSnares() {
        ...
        ProjectX.log("Allowed snares: " + allowedSnares + ", Snares in inventory: " + snaresInInventory);
    ...
    private void walkBackToArea() {
        ...
        if (walkableTile != null) {
            ...
            ProjectX.log("No safe walkable tile found inside the hunting area.");
        }
    }
    ...
    private WorldPoint getNearestSafeWalkableTileInArea(WorldArea huntingArea) {
        ...
        for (int x = initialStartTile.getX() - huntingRadius; x <= initialStartTile.getX() + huntingRadius; x++) {
            for (int y = initialStartTile.getY() - huntingRadius; y <= initialStartTile.getY() + huntingRadius; y++) {
                ...
                LocalPoint localPoint = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), candidate);
    ...
    private void handleTraps(BirdHunterConfig config) {
        ...
        successfulTraps.addAll(ProjectX.getRs2TileObjectCache().query().withId(ObjectID.HUNTING_OJIBWAY_TRAP_TRAPPING_JUNGLE).toList());
        successfulTraps.addAll(ProjectX.getRs2TileObjectCache().query().withId(ObjectID.HUNTING_OJIBWAY_TRAP_TRAPPING_COLOURED).toList());
        successfulTraps.addAll(ProjectX.getRs2TileObjectCache().query().withId(ObjectID.HUNTING_OJIBWAY_TRAP_FULL_DESERT).toList());
        successfulTraps.addAll(ProjectX.getRs2TileObjectCache().query().withId(ObjectID.HUNTING_OJIBWAY_TRAP_FULL_WOODLAND).toList());
        successfulTraps.addAll(ProjectX.getRs2TileObjectCache().query().withId(ObjectID.HUNTING_OJIBWAY_TRAP_FULL_POLAR).toList());
        successfulTraps.addAll(ProjectX.getRs2TileObjectCache().query().withId(ObjectID.HUNTING_OJIBWAY_TRAP_FULL_JUNGLE).toList());
        successfulTraps.addAll(ProjectX.getRs2TileObjectCache().query().withId(ObjectID.HUNTING_OJIBWAY_TRAP_FULL_COLOURED).toList());

        List<Rs2TileObjectModel> catchingTraps = new ArrayList<>();
        catchingTraps.addAll(ProjectX.getRs2TileObjectCache().query().withId(ObjectID.HUNTING_OJIBWAY_TRAP_FULL_COLOURED).toList());
        catchingTraps.addAll(ProjectX.getRs2TileObjectCache().query().withId(ObjectID.HUNTING_OJIBWAY_TRAP_TRAPPING_DESERT).toList());
        catchingTraps.addAll(ProjectX.getRs2TileObjectCache().query().withId(ObjectID.HUNTING_OJIBWAY_TRAP_TRAPPING_WOODLAND).toList());
        catchingTraps.addAll(ProjectX.getRs2TileObjectCache().query().withId(ObjectID.HUNTING_OJIBWAY_TRAP_TRAPPING_POLAR).toList());
        catchingTraps.addAll(ProjectX.getRs2TileObjectCache().query().withId(ObjectID.HUNTING_OJIBWAY_TRAP_FULL_JUNGLE).toList());

        List<Rs2TileObjectModel> failedTraps = new ArrayList<>(ProjectX.getRs2TileObjectCache().query().withId(ObjectID.HUNTING_OJIBWAY_TRAP_BROKEN).toList());
        List<Rs2TileObjectModel> idleTraps = new ArrayList<>(ProjectX.getRs2TileObjectCache().query().withId(ObjectID.HUNTING_OJIBWAY_TRAP).toList());
        idleTraps.addAll(ProjectX.getRs2TileObjectCache().query().withId(ObjectID.HUNTING_OJIBWAY_TRAP_FAILING).toList());
        ...
        if (ProjectX.getRs2TileItemCache().query().withId(ItemID.HUNTING_OJIBWAY_BIRD_SNARE).within(20).count() > 0) {
            pickUpBirdSnare();
            return;
        }
    ...
    private void layBirdSnare() {
        ...
        if (Rs2Inventory.interact(birdSnare, "Lay")) {
            ...
            ProjectX.log("Failed to interact with the bird snare.");
        }
    }

    private boolean isGameObjectAt(WorldPoint point) {
        return ProjectX.getRs2TileObjectCache().query().within(point, 0).count() > 0;
    }
    ...
    private WorldPoint getSafeWalkableTile(WorldArea huntingArea) {
        ...
        for (int x = initialStartTile.getX() - huntingRadius; x <= initialStartTile.getX() + huntingRadius; x++) {
            for (int y = initialStartTile.getY() - huntingRadius; y <= initialStartTile.getY() + huntingRadius; y++) {
                ...
                LocalPoint localPoint = LocalPoint.fromWorld(ProjectX.getClient().getTopLevelWorldView(), candidateTile);
    ...
    private void pickUpBirdSnare() {
        if (ProjectX.getRs2TileItemCache().query().withId(ItemID.HUNTING_OJIBWAY_BIRD_SNARE).interact("Take")) {
