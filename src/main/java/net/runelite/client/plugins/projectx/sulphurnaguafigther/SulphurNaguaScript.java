...
public class SulphurNaguaScript extends Script {
    ...
    public boolean run(SulphurNaguaConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                if (!ProjectX.isLoggedIn() || !super.run() || ProjectX.pauseAllScripts.get()) {
                    return;
                }
    ...
    private void determineState(SulphurNaguaConfig config) {
        ...
        if (potionsToPickup > 0 && pickupReady) {
            if (Rs2Inventory.isFull()) {
                ProjectX.log("Inventory is full, cannot pick up remaining potions. Starting to fight.");
        ...
        if (totalOwnedPotions < targetPotions || hasIntermediateIngredients) {
            if (currentState == SulphurNaguaState.FIGHTING) {
                ...
                ProjectX.log("All potions used. Starting preparation for a new batch.");
            }

            if (config.changeInSulphurousEssence() && Rs2Inventory.hasItem(SULPHUROUS_ESSENCE_ID) && totalOwnedPotions == 0) {
                ProjectX.log("No potions left. Exchanging Sulphurous Essence for XP.");
    ...
    private void handlePreparation(SulphurNaguaConfig config) {
        ...
        if (vialsInInv > 0) {
            ...
            if (freeSlots >= grubsToGet) {
                ...
                if (freeSlots > 0) {
                    ...
                    if (potionsToDrop <= 0) {
                        ProjectX.log("Stuck: 0 free slots, 0 potions to drop, but need grubs.");
                        return;
                    }
                    ProjectX.log("Dropping " + potionsToDrop + " potions to make space for grubs.");
        ...
        if (freeSlots > 0) {
            ...
            if (potionsToDrop <= 0 && neededPotionsTotal > 0) {
                ProjectX.log("Stuck: 0 free slots, 0 vials, 0 potions to drop, but need more potions.");
                return;
            }

            ProjectX.log("Dropping " + potionsToDrop + " potions to make space for vials.");
            dropPotions(potionsToDrop);
        }
    }

    private void processAllIngredients() {
        if (Rs2Player.isAnimating() || ProjectX.isGainingExp) {
            return;
        }
        if (Rs2Inventory.hasItem(MOONLIGHT_GRUB_ID)) {
            ProjectX.log("Grinding all available grubs...");
        ...
        if (Rs2Inventory.hasItem(MOONLIGHT_GRUB_PASTE_ID) && Rs2Inventory.hasItem(VIAL_OF_WATER_ID)) {
            ProjectX.log("Mixing all available paste...");
        ...
        if (itemID == VIAL_OF_WATER_ID) {
            ...
            while (Rs2Inventory.count(itemID) < requiredAmount && System.currentTimeMillis() - startTime < 20000) {
                ...
                if (Rs2Dialogue.hasDialogueOption("Take herblore supplies.")) {
                    ...
                    ProjectX.getRs2TileObjectCache().query().interact(SUPPLY_CRATE_ID, "Take herblore supplies");
                }
                sleep(300, 500);
            }
        } else {
            ...
            if (ProjectX.getRs2TileObjectCache().query().interact(GRUB_SAPLING_ID, "Collect-from")) {
    ...
    private void dropPotions(int count) {
        ...
        ProjectX.log("Dropped " + dropped + " potions at " + dropLocation);
    }

    private void pickupDroppedPotions() {
        if (Rs2Inventory.isFull()) {
            ProjectX.log("Inventory is full, cannot pick up.");
            return;
        }
        if (potionsToPickup <= 0) {
            resetPreparationState();
        ...
        for (int potionId : MOONLIGHT_POTION_IDS) {
            if (ProjectX.getRs2TileItemCache().query().withId(potionId).within(8).first() != null) {
                foundPotion = true;
                int potionsBefore = countMoonlightPotions();

                if (ProjectX.getRs2TileItemCache().query().withId(potionId).within(8).interact("Take")) {
        ...
        if (potionsToPickup <= 0) {
            ProjectX.log("Finished picking up all potions.");
            resetPreparationState();
        } else if (!foundPotion) {
            ProjectX.log("Could not find any more dropped potions. Resetting.");
            resetPreparationState();
        }
    }
    ...
    private void cleanupLeftoverIngredients() {
        ProjectX.log("Cleaning up leftover ingredients...");
    ...
    private void handleGettingRunecraftingXp(SulphurNaguaConfig config) {
        if (Rs2Dialogue.isInDialogue()) {
            ProjectX.log("Handling dialogue...");
        ...
        if (!Rs2Inventory.hasItem(SULPHUROUS_ESSENCE_ID)) {
            ProjectX.log("No essence to exchange. Returning to preparation.");
            currentState = isAtLocation(selectedLocation.getPrepArea()) ? SulphurNaguaState.PREPARATION : SulphurNaguaState.WALKING_TO_PREP;
            return;
        }
        ...
        var eytallali = ProjectX.getRs2NpcCache().query().withId(EYTALLALI_ID).nearest();
        if (eytallali == null) {
            ProjectX.log("Waiting for Eytallali to appear...");
            sleep(600, 1000);
            return;
        }

        if (Rs2Player.isAnimating() || ProjectX.isGainingExp) {
            ProjectX.log("Waiting for action to complete...");
            return;
        }

        if (Rs2Inventory.useItemOnNpc(SULPHUROUS_ESSENCE_ID, EYTALLALI_ID)) {
            ProjectX.log("Exchanging essence...");
    ...
    private void handleFighting(SulphurNaguaConfig config) {
        ...
        if (needsNewTarget) {
            ...
            if (combatArea != null && combatArea.contains(Rs2Player.getWorldLocation())) {
                ...
                var attackingNagua = ProjectX.getRs2NpcCache().query()
                ...
                if (attackingNagua != null) {
                    ProjectX.log("Attacking the Nagua that is attacking us!");
                    attackInitiated = attackingNagua.click("Attack");
                } else {
                    attackInitiated = ProjectX.getRs2NpcCache().query()
                            .withName("Sulphur Nagua")
                            .where(n -> !n.isDead())
                ...
                if (attackInitiated) {
                    ...
                    ProjectX.log("Waiting for Sulphur Nagua in the zone...");
                }
            } else {
                ProjectX.log("Outside combat zone, walking back to center...");
                Rs2Walker.walkTo(selectedLocation.getFightAreaCenter());
                sleep(400, 800);
            }
        }
    ...
    private boolean isStackableLootNearby() {
        for (int itemId : dynamicLootIds) {
            if (ProjectX.getRs2TileItemCache().query()
                    .withId(itemId)
                    .within(8)
                    .first() != null) {
                return true;
    ...
    private void handleLooting() {
        ProjectX.log("Looting items...");

        if (ProjectX.getRs2TileItemCache().query().withId(SULPHUR_BLADE_ID).within(8).first() != null) {
            int itemsBefore = Rs2Inventory.count(SULPHUR_BLADE_ID);

            if (ProjectX.getRs2TileItemCache().query().withId(SULPHUR_BLADE_ID).within(8).interact("Take")) {
        ...
        for (int itemId : dynamicLootIds) {
            if (ProjectX.getRs2TileItemCache().query().withId(itemId).within(8).first() != null) {
                int itemsBefore = Rs2Inventory.count(itemId);

                if (ProjectX.getRs2TileItemCache().query().withId(itemId).within(8).interact("Take")) {
