package net.runelite.client.plugins.projectx.chaosaltar;
...
import static net.runelite.client.plugins.projectx.util.walker.Rs2Walker.walkTo;
...
public class ChaosAltarScript extends Script {
    ...
    public boolean run(ChaosAltarConfig config, ChaosAltarPlugin plugin) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                ProjectX.log("Current state: " + currentState);
    ...
    private Rs2TileObjectModel getChaosAltar() {
        return ProjectX.getRs2TileObjectCache().query().withId(CHAOS_ALTAR).nearest();
    }
    ...
    private void dieToNpc() {
        ProjectX.log("Walking to dangerous NPC to die");
        Rs2Walker.walkTo(2979, 3845, 0);
        sleepUntil(() -> ProjectX.getRs2NpcCache().query().withId(CHAOS_FANATIC).nearest() != null, 60000);
        ProjectX.getClientThread().invoke(() -> ProjectX.getRs2NpcCache().query().withName("Chaos Fanatic").interact("Attack"));
        // Wait until player dies
        sleepUntil(() -> ProjectX.getClient().getBoostedSkillLevel(Skill.HITPOINTS) == 0, 60000);
    ...
    private void offerBones() {
        ...
        if (lastBones != null && isRunning()) {
            ...
            sleep(300, 500);
            ProjectX.getRs2TileObjectCache().query().interact(CHAOS_ALTAR);
            sleep(300, 500);
    ...
    private void offerBonesFast() {
        ProjectX.log("Offering bones at altar - IN OFFERBONES");
    ...
    private void handleBanking() {
        ...
        if (!Rs2Bank.hasItem(DRAGON_BONES)) {
            ProjectX.log("NO BONES, SHUTTING DOWN");
            shutdown();
            return;
        }

        if (!Rs2Bank.hasBankItem("Burning Amulet")) {
            ProjectX.log("NO BURNING AMULET, SHUTTING DOWN");
            shutdown();
            return;
        }
        ...
        if (!hasBurningAmulet()) {
            sleep(400);
            ProjectX.log("Withdrawing burning amulet");
    ...
    private State determineState() {
        ...
        if (didWeDie) {
            didWeDie = false;
            ProjectX.log("We died! Going to the bank...");
            return State.BANK;
        }
