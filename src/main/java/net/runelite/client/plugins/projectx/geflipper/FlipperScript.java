package net.runelite.client.plugins.projectx.geflipper;
...
public class FlipperScript extends Script {
            ...
            try {
                ...
                switch (state) {
                    case GOING_TO_GE:
                        ...
                        if (!grandExchangeArea.contains(ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation()))) {
	...
	private Plugin getFlippingCopilot()
	{
		...
		{
			flippingCopilot = ProjectX.getPluginManager()
				.getPlugins()
				.stream()
	...
	private boolean checkAndAbortOrModifyIfNeeded()
	{
		...
		try
		{
			...
			{	
				...
				ProjectX.doInvoke(menuEntry, bounds);
				lastActionTime = System.currentTimeMillis();
				actionCooldown = Rs2Random.randomGaussian(DEFAULT_ACTION_COOLDOWN, ACTION_COOLDOWN_VARIANCE);
				return true;
			}
