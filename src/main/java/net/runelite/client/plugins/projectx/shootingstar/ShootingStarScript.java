package net.runelite.client.plugins.projectx.shootingstar;
...
import net.runelite.client.plugins.projectx.shootingstar.ShootingStarConfig;
import net.runelite.client.plugins.projectx.shootingstar.ShootingStarPlugin;
import net.runelite.client.plugins.projectx.shootingstar.enums.Pickaxe;
import net.runelite.client.plugins.projectx.shootingstar.enums.ShootingStarState;
import net.runelite.client.plugins.projectx.shootingstar.model.Star;
...
public class ShootingStarScript extends Script
{
			...
			try
			{
				...
				switch (state)
				{
					case WAITING_FOR_STAR:
						if (config.useNearestHighTierStar())
						{
							if (!hasSelectedStar())
							{
								...
								if (currentStar == null)
								{
									ProjectX.showMessage("Unable to find a star within your tier range. Consider disabling useNearestHighTierStar until higher mining level.");
									ProjectX.stopPlugin(plugin);
									return;
								}
							}
					...
					case WALKING:
						...
						{
							ProjectX.hopToWorld(currentStar.getWorld());
					...
					case MINING:
						...
						{
							ProjectX.status = "Waiting for star to degrade";
						...
						var starObject = ProjectX.getRs2TileObjectCache().query().withId(currentStar.getObjectId()).nearest();
					...
					case BANKING:
						...
						else
						{
							if (pickaxe == null)
							{
								...
								{
									ProjectX.showMessage("Unable to find pickaxe, please purchase a pickaxe");
									ProjectX.stopPlugin(plugin);
									return;
								}
							}
	...
	{
		boolean isInventorySetupPluginEnabled = ProjectX.isPluginEnabled(MInventorySetupsPlugin.class);
	...
	private ShootingStarState updateStarState()
	{
		...
		if (state == ShootingStarState.MINING)
		{
			var starModel = ProjectX.getRs2TileObjectCache().query()
					.withIds(
