...
public class BlueDragonsScript extends Script {
    ...
    public boolean run(BlueDragonsConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                consecutiveErrors[0]++;
                ProjectX.log("Error in Blue Dragons script: " + ex.getMessage());
                if (consecutiveErrors[0] >= 5) {
                    ProjectX.log("Too many consecutive errors. Stopping script.");
                    shutdown();
                }
            }
    ...
    private void determineStartingState() {
        boolean hasAgilityOrKey = ProjectX.getClient().getRealSkillLevel(Skill.AGILITY) >= 70 || hasDustyKey();
        if (!hasAgilityOrKey) {
            ProjectX.log("Requires Agility level 70 or a Dusty Key. Stopping.");
            shutdown();
            return;
        }

        if (!hasTeleportToFalador()) {
            ProjectX.log("Missing Falador teleport (tab or runes). Stopping.");
            shutdown();
            return;
        }
    ...
    private void handleBanking() {
        ...
        if (foodNeeded > 0) {
            if (!Rs2Bank.hasItem(config.foodType().getName())) {
                ProjectX.log("No " + config.foodType().getName() + " in bank. Stopping.");
                Rs2Bank.closeBank();
                shutdown();
                return;
            }
    ...
    private void handleCombat() {
        ...
        if (config.waitForLoot() && currentTargetId != null) {
            Rs2NpcModel target = ProjectX.getRs2NpcCache().query().withId(currentTargetId).nearestOnClientThread();
    ...
    private void lootItems() {
        ...
        if (config.lootMiscItems() && (!Rs2Inventory.isFull() || config.eatForLoot())) {
            ...
            ProjectX.log("[Loot] Value-based loot pass (threshold=" + config.lootValueThreshold()
    ...
    private void lootItem(String itemName) {
        ...
        if (Rs2GroundItem.lootItemsBasedOnNames(params)) {
            ProjectX.log("[Loot] Picked up: " + itemName);
    ...
    private Rs2NpcModel getAvailableDragon() {
        Rs2NpcModel dragon = ProjectX.getRs2NpcCache().query()
    ...
    private boolean isPlayerAtSafeSpot() {
        return SAFE_SPOT.equals(ProjectX.getClientThread().invoke(() ->
                ProjectX.getClient().getLocalPlayer().getWorldLocation()));
    }
    ...
    private void hopIfPlayerAtSafeSpot() {
        ...
        boolean occupied = players.stream().anyMatch(p ->
                p != null && !p.equals(ProjectX.getClient().getLocalPlayer())
                        && p.getWorldLocation().distanceTo(SAFE_SPOT) <= 1);

        if (occupied) {
            ProjectX.log("Player at safe spot. Hopping worlds.");
            ProjectX.hopToWorld(findRandomWorld());
            sleep(5000);
        }
    }
