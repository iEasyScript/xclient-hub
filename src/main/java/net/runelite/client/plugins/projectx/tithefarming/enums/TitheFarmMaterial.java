package net.runelite.client.plugins.projectx.tithefarming.enums;
...
public enum TitheFarmMaterial {
    ...
    public static TitheFarmMaterial getSeedForLevel() {
        if (ProjectX.getClient().getRealSkillLevel(Skill.FARMING) >= LOGAVANO_SEED.levelRequired)
            return LOGAVANO_SEED;
        if (ProjectX.getClient().getRealSkillLevel(Skill.FARMING) >= BOLOGANO_SEED.levelRequired)
            return BOLOGANO_SEED;
        if (ProjectX.getClient().getRealSkillLevel(Skill.FARMING) >= GOLOVANOVA_SEED.levelRequired)
