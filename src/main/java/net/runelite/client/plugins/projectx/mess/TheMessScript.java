package net.runelite.client.plugins.projectx.mess;
...
public class TheMessScript extends Script {
    ...
    private void tick() {
        try {
            if (!ProjectX.isLoggedIn() || !super.run() || !isRunning()) return;
    ...
    private static void setChatboxAmount(int amount) {
        ProjectX.getClientThread().runOnClientThreadOptional(() -> {
            ...
            ProjectX.getClient().setVarcStrValue(VarClientStr.INPUT_TEXT, String.valueOf(amount));
            return null;
        });
    }
    ...
    private void hopWorld() {
        ...
        ProjectX.hopToWorld(Login.getRandomWorld(true));
        sleepUntil(() -> ProjectX.getClient().getGameState() == GameState.HOPPING, 5000);
        sleepUntil(() -> ProjectX.getClient().getGameState() == GameState.LOGGED_IN, 15000);
        if (ProjectX.getClient().getWorld() == currentWorld) {
