...
import net.runelite.client.plugins.projectx.valetotems.enums.SpiritAnimal;
import net.runelite.client.plugins.projectx.valetotems.enums.TotemLocation;
import net.runelite.client.plugins.projectx.valetotems.models.TotemProgress;
...
public class TotemHandler {
    ...
    public static boolean identifySpiritAnimals(TotemLocation totemLocation, TotemProgress progress) {
        try {
            ...
            while (attemptCount < maxAttempts && !allIdentified) {
                attemptCount++;
                ProjectX.log("Attempting to identify spirit animals (attempt " + attemptCount + "/" + maxAttempts + ")");
                
                // Clear previous identifications for this attempt
                progress.clearIdentifiedAnimals();

                // Search for spirit animals in the area
                List<Rs2NpcModel> nearbyNpcs = ProjectX.getRs2NpcCache().query().toList().stream()
                ...
                ProjectX.log("Found " + nearbyNpcs.size() + " spirit animals in the area");
                ...
                if (!allIdentified && attemptCount < maxAttempts) {
                    ProjectX.log("Not enough spirit animals found or identified. Retrying in 2 seconds...");
                    sleep(2000);
                }
            }
            
            if (!allIdentified) {
                ProjectX.log("Failed to identify all spirit animals after " + maxAttempts + " attempts");
            }
    ...
    public static boolean carveAnimalsIntoTotem(TotemLocation totemLocation, TotemProgress progress) {
        try {
            ...
            ProjectX.log("Carving animals into totem");
        ...
        try {
            if (!sleepUntil(() -> Rs2Widget.hasWidgetText("What animal would you like to carve?",270,5, false), 5000)) {
                ProjectX.getClientThread().invoke(() ->
                        ProjectX.getRs2TileObjectCache().query().withNameContains(GameObjectId.EMPTY_TOTEM.getSearchTerm()).interact("Carve"));
                if (!sleepUntil(() -> Rs2Widget.hasWidgetText("What animal would you like to carve?",270,5, false), 5000)) {
    ...
    public static boolean decorateTotem(TotemLocation totemLocation, TotemProgress progress) {
        try {
            ...
            ProjectX.log("Decorating totem with fletched bows");
            ...
            while (attemptCount < maxAttempts && !decorated) {
                ...
                if (!decorated && attemptCount < maxAttempts) {
                    ProjectX.log("Decoration attempt failed, retrying in 1 second...");
                    sleep(1000);
                }
            }
            
            if (!decorated) {
                ProjectX.log("Failed to decorate totem after " + maxAttempts + " attempts");
                return false;
            }
            ...
            if (decorated) {
                ...
                if (endingBows - startingBows == -4) {
                    ...
                    ProjectX.log("Totem decoration completed!");
                    return true;
                }
            }
    ...
    private static void hoverOverSpiritAnimal(SpiritAnimal animal) {
        ...
        WorldPoint animalLocation = ProjectX.getRs2NpcCache().query().toList().stream()
        ...
        Rs2NpcModel animalNpc = ProjectX.getRs2NpcCache().query().toList().stream()
                .filter(npc -> npc.getId() == animal.getNpcId())
                .findFirst()
                .orElse(null);
