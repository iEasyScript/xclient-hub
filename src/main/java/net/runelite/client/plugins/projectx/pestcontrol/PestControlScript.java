...
import net.runelite.client.plugins.projectx.inventorysetups.InventorySetupsItem;
...
import static net.runelite.client.plugins.projectx.util.prayer.Rs2Prayer.isQuickPrayerEnabled;
import static net.runelite.client.plugins.projectx.util.walker.Rs2Walker.distanceToRegion;
...
public class PestControlScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (initialise && !isInPestControl && !isInBoat) {
                    ProjectX.log("Initialising");
                    if (Rs2Player.getWorld() != config.world()) {
                        ProjectX.hopToWorld(config.world());
                        sleep(1000, 3000);
                        ProjectX.hopToWorld(config.world());
                    ...
                    if (Rs2Player.getWorldLocation().getRegionID() == 10537 && Rs2Player.getWorld() == config.world()) {
                        ...
                        ProjectX.log("Traveling to Pest Island");
                ...
                if (isInPestControl) {
                    ...
                    if (!isQuickPrayerEnabled() && ProjectX.getClient().getBoostedSkillLevel(Skill.PRAYER) != 0 && config.quickPrayer()) {
                        final Widget prayerOrb = Rs2Widget.getWidget(ComponentID.MINIMAP_QUICK_PRAYER_ORB);
                        if (prayerOrb != null) {
                            ProjectX.getMouse().click(prayerOrb.getCanvasLocation());
                            sleep(1000, 1500);
                        }
                    }
                    if (!walkToCenter) {
                    ...
                    if (activity != null && activity.getChild(0).getWidth() <= 20 && !Rs2Combat.inCombat()) {
                        Rs2NpcModel attackableNpc = ProjectX.getClientThread().invoke(() ->
                                ProjectX.getRs2NpcCache().query()
                    ...
                    var brawler = ProjectX.getRs2NpcCache().query().withName("brawler").nearestOnClientThread();
                    ...
                    if (ProjectX.getClient().getLocalPlayer().isInteracting())
                        return;
                    ...
                    Rs2NpcModel portal = ProjectX.getRs2NpcCache().query()
                            .where(n -> n.getName() != null && n.getName().toLowerCase().contains("portal")
                                    && n.getNpc() != null && !n.getNpc().isDead()
                                    && Arrays.stream(ProjectX.getClientThread().runOnClientThreadOptional(() ->
                                        ProjectX.getClient().getNpcDefinition(n.getId()).getActions()).orElse(new String[0]))
                                    .anyMatch(a -> a != null && a.equalsIgnoreCase("attack")))
                            .nearest();
                    if (portal != null) {
                        if (portal.click("Attack")) {
                            sleepUntil(() -> !ProjectX.getClient().getLocalPlayer().isInteracting());
                        }
                    } else {
                        if (!ProjectX.getClient().getLocalPlayer().isInteracting()) {
                            Rs2NpcModel attackableNpc = ProjectX.getRs2NpcCache().query()
                                    .where(n -> n.getNpc() != null && !n.getNpc().isDead() && n.getNpc().getCombatLevel() > 0)
                    ...
                    if (!isInBoat && !initialise) {
                        if (ProjectX.getClient().getLocalPlayer().getCombatLevel() >= 100) {
                            ProjectX.getRs2TileObjectCache().query().interact(ObjectID.GANGPLANK_25632);
                        } else if (ProjectX.getClient().getLocalPlayer().getCombatLevel() >= 70) {
                            ProjectX.getRs2TileObjectCache().query().interact(ObjectID.GANGPLANK_25631);
                        } else {
                            ProjectX.getRs2TileObjectCache().query().interact(ObjectID.GANGPLANK_14315);
                        }
    ...
    private boolean handleInventorySetup() {
        ...
        ProjectX.log("Starting Inv Setup");
        ...
        ProjectX.log("Inv Setup Finished");
        Rs2Bank.closeBank();
    ...
    public boolean isOutside() {
        WorldPoint playerLoc = ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation());
        return playerLoc.distanceTo(new WorldPoint(2644, 2644, 0)) < 20;
    }

    public boolean isInBoat() {
        return ProjectX.getClientThread().runOnClientThreadOptional(
                () -> ProjectX.getClient().getWidget(WidgetInfo.PEST_CONTROL_BOAT_INFO) != null
        ).orElse(false);
    }

    public boolean isInPestControl() {
        return ProjectX.getClientThread().runOnClientThreadOptional(
                () -> ProjectX.getClient().getWidget(WidgetInfo.PEST_CONTROL_BLUE_SHIELD) != null
        ).orElse(false);
    }

    public void exitBoat() {
        if (ProjectX.getClient().getLocalPlayer().getCombatLevel() >= 100) {
            ProjectX.getRs2TileObjectCache().query().interact(ObjectID.LADDER_25630);
        } else if (ProjectX.getClient().getLocalPlayer().getCombatLevel() >= 70) {
            ProjectX.getRs2TileObjectCache().query().interact(ObjectID.LADDER_25629);
        } else {
            ProjectX.getRs2TileObjectCache().query().interact(ObjectID.LADDER_14314);
        }
        sleepUntil(() -> ProjectX.getClient().getWidget(WidgetInfo.PEST_CONTROL_BOAT_INFO) == null, 3000);
    ...
    private static boolean attackPortal() {
        if (!ProjectX.getClient().getLocalPlayer().isInteracting()) {
            Rs2NpcModel npcPortal = ProjectX.getRs2NpcCache().query().withName("portal").nearestOnClientThread();
            if (npcPortal == null) return false;
            NPCComposition npc = ProjectX.getClientThread().runOnClientThreadOptional(() ->
                    ProjectX.getClient().getNpcDefinition(npcPortal.getId())).orElse(null);
            ...
            if (Arrays.stream(npc.getActions()).anyMatch(x -> x != null && x.equalsIgnoreCase("attack"))) {
                ...
                if (localPoint != null && !Rs2Camera.isTileOnScreen(localPoint)) {
                    WorldPoint npcWp = ProjectX.getClientThread().runOnClientThreadOptional(() ->
        ...
        for (Portal portal : portals) {
            if (!portal.isHasShield() && !portal.getHitPoints().getText().trim().equals("0") && closestAttackablePortal == portal) {
                if (!Rs2Walker.isCloseToRegion(distanceToPortal, portal.getRegionX(), portal.getRegionY())) {
                    Rs2Walker.walkTo(WorldPoint.fromRegion(Rs2Player.getWorldLocation().getRegionID(), portal.getRegionX(), portal.getRegionY(), ProjectX.getClient().getTopLevelWorldView().getPlane()), 5);
                    attackPortal();
                } else {
                    attackPortal();
                }
    ...
    private boolean attackSpinner() {
        for (int spinner : SPINNER_IDS) {
            if (ProjectX.getRs2NpcCache().query().withId(spinner).interact("Attack")) {
                sleepUntil(() -> !ProjectX.getClient().getLocalPlayer().isInteracting());
                return true;
            }
        }
    ...
    private boolean attackBrawler() {
        for (int brawler : BRAWLER_IDS) {
            if (ProjectX.getRs2NpcCache().query().withId(brawler).interact("Attack")) {
                sleepUntil(() -> !ProjectX.getClient().getLocalPlayer().isInteracting());
                return true;
            }
        }
    ...
    public void shutdown() {
        ProjectX.log("Pest control about to shutdown");
        initialise = true;
        walkToCenter = false;
        wasInPestControl = false;
        super.shutdown();
