...
import net.runelite.client.plugins.projectx.valetotems.utils.FletchingItemMapper;
...
public class FletchingHandler {
    ...
    private static int getWidgetHotkey(int childId) {
        try {
            ...
            String hotkeyText = ProjectX.getClientThread().invoke(() -> {
    ...
    private static boolean interactWithWidget(int childId, String description) {
        try {
            ...
            if (hotkey != -1) {
                ...
                ProjectX.log("Used keyboard shortcut '" + getKeyName(hotkey) + "' for: " + description);
                return true;
            }

            // Fallback to widget clicking
            Rs2Widget.clickWidget(FLETCHING_INTERFACE_WIDGET_ID, childId);
            ProjectX.log("Used widget click (no hotkey available) for: " + description);
            return true;
    ...
    public static boolean startFletching() {
        try {
            ...
            if (used) {
                ...
                ProjectX.log("Fletching interface opened");
    ...
    public static boolean selectBow(int quantity) {
        try {
            ...
            if (quantity == 1) {
                ...
                ProjectX.log("Selected 1 bow");
            } else if (quantity == 5) {
                interactWithWidget(QUANTITY_5_CHILD_ID, "5 bows");
                ProjectX.log("Selected 5 bows");
            } else if (quantity == 10) {
                interactWithWidget(QUANTITY_10_CHILD_ID, "10 bows");
                ProjectX.log("Selected 10 bows");
            } else if (quantity >= 28) {
                interactWithWidget(QUANTITY_ALL_CHILD_ID, "all bows");
                ProjectX.log("Selected all bows");
            } else {
                String[] widgetInfo = ProjectX.getClientThread().invoke(() -> {
                ...
                if (widgetInfo != null) {
                    ...
                    if (currentText != null) {
                        ...
                        if (Integer.parseInt(currentQuantity) == quantity || hasActions) {
                            ...
                            ProjectX.log("Selected other quantity");
                        } else {
                            ...
                            ProjectX.log("Selected other quantity1");
                        }
                    } else {
                        interactWithWidget(QUANTITY_X_CHILD_ID, "X");
                        sleepUntil(() -> Rs2Widget.getChildWidgetText(162, 42).contains("Enter amount"), 2000);
                        ...
                        sleepGaussian(300,100);
                        Rs2Keyboard.keyPress(KeyEvent.VK_ENTER);
                        ProjectX.log("Selected other quantity2");
                    }
                } else {
                    ProjectX.log("No other quantity widget found");
                }
            }
            ...
            if (!Rs2Widget.handleProcessingInterface(bowAction)) {
                ProjectX.log("Failed to select " + bowAction + " from fletching interface");
                return false;
            }
            ProjectX.log("Selected " + bowAction);
     ...
     * @return true if fletching completed successfully
     */
    public static boolean fletchBows(int quantity, net.runelite.client.plugins.projectx.valetotems.models.GameSession gameSession) {
        try {
     ...
     * @return true if 4 bows were successfully fletched
     */
    public static boolean fletchBowsForOneTotem(net.runelite.client.plugins.projectx.valetotems.models.GameSession gameSession) {
     ...
     * @return true if fletching while walking was initiated
     */
    public static boolean startFletchingWhileWalking(int bowsNeeded, net.runelite.client.plugins.projectx.valetotems.models.GameSession gameSession) {
        try {
