package net.runelite.client.plugins.projectx.giantmole;
...
import net.runelite.client.plugins.projectx.giantmole.enums.DefaultLooterStyle;
import net.runelite.client.plugins.projectx.giantmole.enums.GiantMoleState;
...
import static net.runelite.client.plugins.projectx.util.player.Rs2Player.isMember;
...
public class GiantMoleScript extends Script
{
    ...
    public boolean run(GiantMoleConfig config)
    {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() ->
        {
            try
            {
                ...
                if (!ProjectX.isPluginEnabled(BossTimersPlugin.class))
                {
                    Plugin bossingInfo = ProjectX.getPluginManager().getPlugins().stream()
                            .filter(x -> x.getClass().getName().equals(BossTimersPlugin.class.getName()))
                            .findFirst()
                            .orElse(null);
                    ProjectX.startPlugin(bossingInfo);
                }
    ...
    public void hopWorlds()
    {
        ...
        ProjectX.hopToWorld(randomWorld);
        checkedIfWorldOccupied = false;
        state = GiantMoleState.IDLE;
    }
    ...
    {
        return ProjectX.getRs2TileObjectCache().query().withId(ObjectID.MOLE_HILL).nearest();
    }
    ...
    public static WorldPoint getMoleLocation()
    {
        ...
        return ProjectX.getClientThread()
                .runOnClientThreadOptional(() -> ProjectX.getClient().getHintArrowPoint())
                .orElse(null);
    }
    ...
    {
        return ProjectX.getInfoBoxManager().getInfoBoxes().stream()
    ...
    {
        NPC hintNpc = ProjectX.getClientThread()
                .runOnClientThreadOptional(() -> ProjectX.getClient().getHintArrowNpc())
                .orElse(null);
    ...
    public void attackMole()
    {
        ...
        boolean moleIsDigging = ProjectX.getClientThread()
                .runOnClientThreadOptional(() ->
                {
                    NPC hintNpc = ProjectX.getClient().getHintArrowNpc();
    ...
    public void handlePrayer(GiantMoleConfig config)
    {
        ...
        boolean underAttack = ProjectX.getRs2NpcCache().query().where(Rs2NpcModel::isInteractingWithPlayer).count() > 0 || Rs2Combat.inCombat();
    ...
    {
        boolean usingQoLFood = ProjectX.getConfigManager().getConfiguration("QoL", "autoEatFood", Boolean.class);
    ...
    public boolean needBanking(GiantMoleConfig config)
    {
        ...
        if (Rs2Inventory.isFull())
        {
            ...
            ProjectX.log("Inventory is full, banking...");
            return true;
        }
