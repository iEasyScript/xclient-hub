...
public class FaladorCourse implements AgilityCourseHandler
{
	...
	public boolean waitForCompletion(final int agilityExp, final int plane)
	{
		...
		Global.sleepUntil(() -> ProjectX.getClient().getSkillExperience(Skill.AGILITY) != agilityExp || Rs2Player.getHealthPercentage() < initialHealth, timeoutMs);

		boolean gainedExp = ProjectX.getClient().getSkillExperience(Skill.AGILITY) != agilityExp;
