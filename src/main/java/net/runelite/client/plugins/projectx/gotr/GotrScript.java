package net.runelite.client.plugins.projectx.gotr;
...
import net.runelite.client.plugins.projectx.gotr.data.CellType;
import net.runelite.client.plugins.projectx.gotr.data.GuardianPortalInfo;
import net.runelite.client.plugins.projectx.gotr.data.Mode;
import net.runelite.client.plugins.projectx.gotr.data.RuneType;
...
public class GotrScript extends Script {
    ...
    private void initializeGuardianPortalInfo() {
        ...
        guardianPortalInfo.put(ObjectID.GUARDIAN_OF_COSMIC, new GuardianPortalInfo("COSMIC", 27, ItemID.COSMIC_RUNE, 26896, 4359, RuneType.CATALYTIC, CellType.MEDIUM, ProjectX.getClientThread().runOnClientThreadOptional(() -> Quest.LOST_CITY.getState(ProjectX.getClient())).orElse(null)));
        ...
        guardianPortalInfo.put(ObjectID.GUARDIAN_OF_LAW, new GuardianPortalInfo("LAW", 54, ItemID.LAW_RUNE, 26898, 4362, RuneType.CATALYTIC, CellType.STRONG, ProjectX.getClientThread().runOnClientThreadOptional(() -> Quest.TROLL_STRONGHOLD.getState(ProjectX.getClient())).orElse(null)));
        guardianPortalInfo.put(ObjectID.GUARDIAN_OF_DEATH, new GuardianPortalInfo("DEATH", 65, ItemID.DEATH_RUNE, 26893, 4363, RuneType.CATALYTIC, CellType.OVERCHARGED, ProjectX.getClientThread().runOnClientThreadOptional(() -> Quest.MOURNINGS_END_PART_II.getState(ProjectX.getClient())).orElse(null)));
        guardianPortalInfo.put(ObjectID.GUARDIAN_OF_BLOOD, new GuardianPortalInfo("BLOOD", 77, ItemID.BLOOD_RUNE, 26894, 4364, RuneType.CATALYTIC, CellType.OVERCHARGED, ProjectX.getClientThread().runOnClientThreadOptional(() -> Quest.SINS_OF_THE_FATHER.getState(ProjectX.getClient())).orElse(null)));
    }
            ...
            try {
                ...
                if (!initCheck) {
                    ...
                    if (!Rs2Magic.isSpellbook(Rs2Spellbook.LUNAR)) {
                        ProjectX.log("Lunar spellbook not found...disabling npc contact");
                        useNpcContact = false;
                    }
                    initCheck = true;
                }
                ...
                ProjectX.log("Something went wrong in the GOTR Script: " + ex.getMessage() + ". If the script is stuck, please contact us on discord with this log.");
                ex.printStackTrace();
            }
    ...
    private boolean repairCells() {
        ...
        List<Rs2TileObjectModel> shieldCells = ProjectX.getRs2TileObjectCache().query()
        ...
        if (Rs2Inventory.hasItemAmount(GUARDIAN_ESSENCE, 10)) {
            for (Rs2TileObjectModel shieldCell : shieldCells) {
                if (CellType.GetShieldTier(shieldCell.getId()) < cellTier) {
                    ProjectX.log("Upgrading power cell at " + shieldCell.getWorldLocation());
    ...
    private boolean powerUpGreatGuardian() {
        ...
        Rs2NpcModel guardian = ProjectX.getRs2NpcCache().query().withName("The great guardian").nearest();
        if (guardian == null) {
            return false;
        }
        ...
        if (!Rs2Inventory.hasItem("Uncharged cell")) {
            ...
            if (Rs2Inventory.isFull()) {
                if (Rs2Inventory.drop(ItemID.GUARDIAN_ESSENCE)) {
                    ProjectX.log("Dropped one Guardian essence to make space for Uncharged cell");
                }
            }
    ...
    private boolean usePortal() {
        if (!isInHugeMine() && ProjectX.getClient().hasHintArrow() && Rs2Inventory.count() < config.maxAmountEssence()) {
            if (leaveLargeMine()) return true;
            Rs2Walker.walkFastCanvas(ProjectX.getClient().getHintArrowPoint());
            sleepUntil(Rs2Player::isMoving);
            ProjectX.getRs2TileObjectCache().query().within(ProjectX.getClient().getHintArrowPoint(), 0).interact();
    ...
    private boolean depositRunesIntoPool() {
        ...
        // next round.
        Rs2TileObjectModel pool = ProjectX.getRs2TileObjectCache().query().withId(ObjectID.DEPOSIT_POOL).nearest();
    ...
    private void mineGuardianRemains() {
        if (ProjectX.getClient().hasHintArrow())
            return;
        if (Rs2Inventory.isFull()) {
            shouldMineGuardianRemains = false;
            return;
    ...
    private static void repairWithCordelia() {
        ...
        Rs2NpcModel pouchRepairNpc = ProjectX.getRs2NpcCache().query().withId(NpcID.APPRENTICE_CORDELIA_12180).nearest();
        ...
        ProjectX.log("Repairing pouches...");
    ...
    public  static boolean isInLargeMine() {
        ...
        return Rs2Player.getWorldLocation().getRegionID() == 14484
                && ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation().getX()) >= largeMineX;
    }

    public  boolean isInHugeMine() {
        int hugeMineX = 3594;
        return Rs2Player.getWorldLocation().getRegionID() == 14484
                && ProjectX.getClientThread().invoke(() -> ProjectX.getClient().getLocalPlayer().getWorldLocation().getX()) <= hugeMineX;
    }
    ...
    public ItemManager getItemManager() {
        return ProjectX.getItemManager();
    }

    public boolean isInMiniGame() {
        int parentWidgetId = 48889857;
        Widget elementalRuneWidget = ProjectX.getClient().getWidget(parentWidgetId);
    ...
    public static List<GameObject> getAvailableAltars() {
        ...
        List<GameObject> availableAltars = Rs2GameObject.getGameObjects().stream()
                .filter(x -> {
                    ...
                    if (portalInfo.getRequiredLevel()
                            > ProjectX.getClient().getBoostedSkillLevel(Skill.RUNECRAFT)) {
                        ProjectX.log("Filtered altar " + portalInfo.getName() + " – insufficient RC level");
                        return false;
                    }
                    if (portalInfo.getQuestState() != QuestState.FINISHED) {
                        ProjectX.log("Filtered altar " + portalInfo.getName() + " – quest not complete");
                        return false;
                    }
                    ...
                    ProjectX.log("Adding " + portalInfo.getName() + " to list of available altars");
                    return true;

                })
                .collect(Collectors.toList());

        ProjectX.log("Found " + availableAltars.size() + " active altars after filtering.");

        if (config.Mode() == Mode.POINTS) {
            // Sort by strongest → weakest CellType; if equal, fall back to balancing points
            ProjectX.log("Sorting by CellType (strongest→weakest) for POINTS mode...");
        ...
        if ((config.Mode() == Mode.BALANCED && elementalPoints < catalyticPoints) || config.Mode() == Mode.ELEMENTAL) {
            ProjectX.log(elementalPoints < catalyticPoints
            ...
            ProjectX.log("Sorting for BALANCED/ELEMENTAL mode (" +
        ...
        ProjectX.log("Returning unsorted altars (default mode).");
    ...
    public static void resetPlugin() {
        ...
        activeGuardianPortals.clear();
        greatGuardian = null;
        ProjectX.getClient().clearHintArrow();
    }
    ...
    private static boolean interactObject(int id, String action) {
        return interactObject(ProjectX.getRs2TileObjectCache().query().withId(id).nearest(), action);
    }
    ...
    public static Rs2TileObjectModel findRcAltar() {
        return ProjectX.getRs2TileObjectCache().query().withIds(
