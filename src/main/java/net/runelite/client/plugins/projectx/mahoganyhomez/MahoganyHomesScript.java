package net.runelite.client.plugins.projectx.mahoganyhomez;
...
public class MahoganyHomesScript extends Script {
    ...
    private void log(String format, Object... args) {
        if (plugin.getConfig().logMessages()) {
            ProjectX.log(String.format(format, args));
        }
    }
    ...
    private void fix() {
        ...
        if (Rs2Widget.isWidgetVisible(InterfaceID.PohFurnitureCreation.FRAME)){
            ProjectX.log("Out of plank and furniture creation widget pop up");
            Rs2Bank.walkToBank();
            bank();
            return;
        }
    ...
    private void interactWithObject(GameObject object) {
        ...
        if (ProjectX.getRs2TileObjectCache().query().withId(object.getId()).interact(action)) {
            sleepUntil(() -> {
    ...
    private boolean openDoorToObject(GameObject object, Rs2WorldPoint objectLocation) {
        ...
        for (WorldPoint wp : walkerPath) {
            ...
            var doorModel = ProjectX.getRs2TileObjectCache().query().withId(door.getId()).nearest();
        ...
        List<String> doorNames = doors.stream()
                .map(d -> {
                    var m = ProjectX.getRs2TileObjectCache().query().withId(d.getId()).nearest();
...
//        log("Doors found: %s", doors.size());
        ...
        for (TileObject door : doors) {
            var doorObj = ProjectX.getRs2TileObjectCache().query().withId(door.getId()).nearest();
            ...
            if (actions != null && actions.contains("Open")) {
                ...
                if (ProjectX.getRs2TileObjectCache().query().withId(door.getId()).interact("Open")) {
    ...
    private void tryToUseLadder() {
        ...
        var closestLadder = ProjectX.getRs2TileObjectCache().query().withIds(Arrays.stream(plugin.getCurrentHome().getLadders()).mapToInt(Integer::intValue).toArray()).nearest();
    ...
    private void finish() {
        if (plugin.getCurrentHome() != null
            ...
            var npc = ProjectX.getRs2NpcCache().query().withId(plugin.getCurrentHome().getNpcId()).nearest();
            if (npc == null && Rs2Player.getWorldLocation().getPlane() > 0) {
                ...
                var ladders = ProjectX.getRs2TileObjectCache().query()
    ...
    private void getNewContract() {
        if (plugin.getCurrentHome() == null) {
            ...
            if (contractLocation.distanceTo2D(Rs2Player.getWorldLocation()) > 10) {
                ...
                var npc = ProjectX.getRs2NpcCache().query().withNames("Amy", "Marlo", "Ellie", "Angelo").nearestOnClientThread();
                ...
                if(Rs2Bank.openBank()) {
                    ...
                    if (plugin.getConfig().usePlankSack()) {
                        ...
                        if (additionalPlanksNeeded <= 0) {
                            ...
                            ProjectX.showMessage("Please free up inventory space! Need " + additionalPlanksNeeded + " more planks but only " + freeSlots + " slots available. Stopping script.");
                            shutdown();
                            return;
                        }
                    }
