...
import net.runelite.client.plugins.projectx.agility.courses.PyramidObstacleData.ObstacleArea;
...
public class PyramidCourse implements AgilityCourseHandler {
    ...
    public TileObject getCurrentObstacle() {
        ...
        if (shouldTurnIn) {
            if (pyramidCount > 0) {
                ...
                ProjectX.showMessage("Inventory is full but no pyramid tops found! Clear inventory to continue.");
    ...
    private boolean handlePyramidTurnIn() {
        try {
            ...
            // Try to find Simon
            Rs2NpcModel simon = ProjectX.getRs2NpcCache().query().withName(SIMON_NAME).nearestOnClientThread();
