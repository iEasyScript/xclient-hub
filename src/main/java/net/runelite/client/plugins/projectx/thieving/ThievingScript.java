...
import net.runelite.client.plugins.projectx.thieving.enums.ThievingFood;
import net.runelite.client.plugins.projectx.thieving.npc.ThievingNpcStrategy;
import net.runelite.client.plugins.projectx.thieving.npc.WealthyCitizenStrategy;
...
import net.runelite.client.plugins.projectx.util.models.RS2Item;
...
ThievingNpcStrategy getActiveStrategy() {
    ...
    private Rs2NpcModel getThievingNpcCache() {
        ...
        final Optional<Rs2NpcModel> npcOptional = ProjectX.getRs2NpcCache().query()
    ...
    private <T> T getAttackingNpcs(Function<Stream<Rs2NpcModel>, T> consumer, T defaultValue) {
        final Player me = ProjectX.getClient().getLocalPlayer();
        if (me == null) return defaultValue;

        final Rs2NpcModel[] npcs = ProjectX.getRs2NpcCache().query().toList().toArray(Rs2NpcModel[]::new);
    ...
    private int getMostExpensiveGroundItemId() {
        ...
        return ProjectX.getClientThread().runOnClientThreadOptional(() -> rs2TileItemCache.query()
                .toList()
                .stream()
                ...
                .distinct()
                .map(id -> {
                    final int price = ProjectX.getItemManager().getItemPrice(id);
    ...
    private State getCurrentState() {
        ...
        if (config.THIEVING_NPC() == ThievingNpc.VYRES) {
            ...
            if (ProjectX.getRs2NpcCache().query()
    ...
    private void loop() {
        ...
        switch(currentState) {
            case LOOT:
                ...
                final Rs2TileItemModel item = ProjectX.getRs2TileItemCache().query().withId(id).within(50).nearest();
                if (item == null) {
    ...
    public boolean run() {
        ProjectX.isCantReachTargetDetectionEnabled = true;
        lastAction = System.currentTimeMillis();
    ...
    private void hopWorld() {
        ...
        for (int attempt = 0; attempt < maxAttempts; attempt++) {
            final int currentWorld = ProjectX.getClient().getWorld();
            ProjectX.hopToWorld(Login.getRandomWorld(true, null));
            ...
            sleepUntilWithInterrupt(() -> ProjectX.getClient().getGameState() == GameState.HOPPING, attackedInterrupt, 15_000);
            sleepUntilWithInterrupt(() -> ProjectX.getClient().getGameState() == GameState.LOGGED_IN, attackedInterrupt, 15_000);
            boolean changed = sleepUntilWithInterrupt(() -> ProjectX.getClient().getWorld() != currentWorld, attackedInterrupt, 15_000);
    ...
    public void shutdown() {
        ...
        ProjectX.isCantReachTargetDetectionEnabled = false;
