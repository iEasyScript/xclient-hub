package net.runelite.client.plugins.projectx.plankrunner;
...
import net.runelite.client.plugins.projectx.plankrunner.enums.PlankRunnerState;
                ...
                switch (state) {
                    case BANKING:
                        ...
                        if (!Rs2Bank.hasBankItem(plugin.getPlank().getLogItemId(), logsToWithdraw)) {
                            ProjectX.showMessage("Not enough logs for a full run!");
                            shutdown();
                            return;
                        }
                    ...
                    case RUNNING_TO_SAWMILL:
                        ...
                        if (!isNearSawmill) {
                            ProjectX.status = "Running to Sawmill";
                        ...
                        var sawmillOperator = ProjectX.getRs2NpcCache().query()
                                .where(n -> sawmillNpcs.contains(n.getId()))
                                .nearest();

                        if (sawmillOperator == null) {
                            ProjectX.showMessage("Unable to find Sawmill Operator!");
                            shutdown();
                            return;
                        }

                        sawmillOperator.click("Buy-plank");
                        ProjectX.status = "Buying Planks";
