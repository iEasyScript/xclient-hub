package net.runelite.client.plugins.projectx.lunarplankmake;

import net.runelite.client.plugins.projectx.ProjectX;
import net.runelite.client.plugins.projectx.Script;
import net.runelite.client.plugins.projectx.lunarplankmake.enums.Logs;
...
public class LunarPlankMakeScript extends Script {
    ...
    public boolean run(LunarPlankMakeConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                ProjectX.log("Exception in LunarPlankMakeScript: " + ex.getMessage());
            }
    ...
    private void plankItems(LunarPlankMakeConfig config) {
        ...
        if (lazyMode) {
            ...
            if (waitUntilNoLogsRemaining(config, initialLogQuantity)) {
                ...
                ProjectX.log("Lazy mode: timed out waiting for logs to finish converting.");
        ...
        if (waitForInventoryChange(plankId, initialPlankCount)) {
            ...
            ProjectX.log("Failed to detect plank creation.");
            currentState = State.WAITING;
        }
    }
    ...
    private void bank(LunarPlankMakeConfig config) {
        ...
        if (!Rs2Bank.hasItem(logId)) {
            ProjectX.showMessage("No more " + config.ITEM().getName() + " to plank.");
            shutdown();
            return;
        }
    ...
    private static int gePrice(String itemName) {
        try {
            return ProjectX.getItemManager().search(itemName).get(0).getPrice();
        } catch (Exception e) {
            return 0;
        }
    }
