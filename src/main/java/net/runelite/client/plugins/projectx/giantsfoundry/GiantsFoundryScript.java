...
import net.runelite.client.plugins.projectx.giantsfoundry.enums.CommissionType;
...
import net.runelite.client.plugins.projectx.giantsfoundry.enums.State;
...
import static net.runelite.client.plugins.projectx.util.equipment.Rs2Equipment.get;
...
public class GiantsFoundryScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if ((weapon != null || shield != null) && !weapon.getName().equalsIgnoreCase("preform")) {
                    ProjectX.showMessage(("Please start the script without any weapon or shield in your equipment slot."));
                    sleep(5000);
                    return;
                }
                if (!Rs2Equipment.isWearing("ice gloves") && !Rs2Equipment.isWearing("smiths gloves")) {
                    ProjectX.showMessage(("Please start by wearing ice gloves or smiths gloves."));
                    sleep(5000);
                    return;
                }
    ...
    public boolean hasCommission() {
        CommissionType type1 = CommissionType.forVarbit(ProjectX.getVarbitValue(MouldHelper.SWORD_TYPE_1_VARBIT));
        CommissionType type2 = CommissionType.forVarbit(ProjectX.getVarbitValue(MouldHelper.SWORD_TYPE_2_VARBIT));
    ...
    public void getCommission() {
        if (!hasCommission()) {
            GiantsFoundryState.reset();
            var kovac = ProjectX.getRs2NpcCache().query().withName("kovac").nearestOnClientThread();
    ...
    private boolean hasSelectedMould() {
        return (ProjectX.getVarbitValue(GiantsFoundryState.VARBIT_BLADE_SELECTED) > 0
                && ProjectX.getVarbitValue(GiantsFoundryState.VARBIT_TIP_SELECTED) > 0
                && ProjectX.getVarbitValue(GiantsFoundryState.VARBIT_FORTE_SELECTED) > 0);
    }
    ...
    public void selectMould() {
        ...
        ProjectX.getRs2TileObjectCache().query().withId(MOULD_JIG).interact();

        sleepUntil(() -> Rs2Widget.findWidget("Forte", null) != null, 5000);

        Widget forte = Rs2Widget.findWidget("Forte", null);
        if (forte != null) {
            ProjectX.getMouse().click(forte.getBounds());
            sleep(600, 1200);
        ...
        if (blades != null) {
            ProjectX.getMouse().click(blades.getBounds());
            sleep(600, 1200);
            MouldHelper.selectBest();
            sleep(600, 1200);
        }
        ...
        if (tips != null) {
            ProjectX.getMouse().click(tips.getBounds());
            sleep(600, 1200);
            MouldHelper.selectBest();
            sleep(600, 1200);
            ProjectX.getMouse().click(forte.getBounds());
        ...
        if (setMould != null) {
            ProjectX.getMouse().click(setMould.getBounds());
        }
    }
    ...
    private boolean preconditionsMet() {
        ...
        if (ProjectX.getVarbitValue(MouldHelper.SWORD_TYPE_1_VARBIT) == 0) return false;
        return ProjectX.getVarbitValue(GiantsFoundryState.VARBIT_GAME_STAGE) == 1;
    }
    ...
    private boolean ensureBarsInInventory() {
        ...
        if (Rs2Bank.count(firstBarName) < config.firstBarAmount()
            ...
            ProjectX.log("Insufficient bars / items in bank to continue");
            this.shutdown();
            return false;
        }
    ...
    private void addBarsWithWidget(String barName, SmithableBars key) {
        ...
        ProjectX.getRs2TileObjectCache().query().interact(CRUCIBLE, "Fill");
    ...
    private void addBarsWithInventoryUse(String barName) {
        ...
        ProjectX.getRs2TileObjectCache().query().withId(CRUCIBLE).interact();
    ...
    private void pourCrucible() {
        ProjectX.getRs2TileObjectCache().query().interact(CRUCIBLE, "Pour");
        sleep(5000);
    ...
    public void pickupMould() {
        ...
        if (Rs2Inventory.isEmpty() && GiantsFoundryState.getCurrentStage() == null) {
            ProjectX.getRs2TileObjectCache().query().interact(MOULD_JIG, "Pick-up");
    ...
    public void handleGameLoop() {
        ...
        if(!Rs2Player.isAnimating(3000)) {
            ProjectX.log("Not animating, doAction -> true");
            doAction = true;
        }
        ...
        switch (state) {
            case HEATING:
                ...
                ProjectX.getRs2TileObjectCache().query().interact(LAVA_POOL, "Heat-preform");
            ...
            case COOLING_DOWN:
                ...
                ProjectX.getRs2TileObjectCache().query().interact(WATERFALL, "Cool-preform");
                GiantsFoundryState.heatingCoolingState.stop();
    ...
    private void handIn() {
        ProjectX.getClientThread().invoke(() -> ProjectX.getRs2NpcCache().query().withName("kovac").interact("Hand-in"));
    }
