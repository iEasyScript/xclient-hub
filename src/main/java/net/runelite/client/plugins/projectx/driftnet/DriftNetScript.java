package net.runelite.client.plugins.projectx.driftnet;
...
public class DriftNetScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if(driftnetLogoutWP.equals(Rs2Player.getWorldLocation())
                    ...
                    ProjectX.log("We need to walk to the area");
                    reEnterArea();
                    return;
                }
                ...
                if (!Rs2Inventory.hasItem(ItemID.FOSSIL_DRIFT_NET)) {
                    ProjectX.log("No nets in inventory");
    ...
    public void reEnterArea(){
        ...
        if(Rs2Equipment.get(EquipmentInventorySlot.WEAPON) != null){
            ProjectX.log("Unequipping our weapon");
        ...
        if(driftnetLogoutWP.equals(Rs2Player.getWorldLocation())){
            ProjectX.log("Navigating the tunnel");
        ...
        if(driftnetPastFirstTunnelWP.equals(Rs2Player.getWorldLocation())){
            ProjectX.log("Navigating the Plant door");
            ProjectX.getRs2TileObjectCache().query().withId(30961).interact("Navigate");
            sleepUntil(()-> Rs2Player.isAnimating(), Rs2Random.between(2000,5000));
            sleepUntil(()-> !Rs2Player.isAnimating(), Rs2Random.between(2000,5000));
        }

        if(driftnetPastPlantDoorWP.equals(Rs2Player.getWorldLocation())){
            ProjectX.log("Wielding our weapon");
    ...
    private void fetchNetsFromAnnette() {
        ...
        var maxDriftnets = maxWeight - ProjectX.getClient().getWeight() - 1; // Driftnets are 1kg each; doing - 1 to be safe
        ProjectX.getRs2TileObjectCache().query().withId(ObjectID.FOSSIL_MERMAID_DRIFTNETS).interact("Nets");
        ...
        ProjectX.doInvoke(annetteWithdrawXMenuEntry, Objects.requireNonNull(annetteWidget).getBounds());
    ...
    private void processNets(DriftNetConfig config) {
        for (DriftNet net : DriftNetPlugin.getNETS()) {
            ...
            final Shape netShape = ProjectX.getClientThread().runOnClientThreadOptional(net.getNet()::getConvexHull)
                    .orElse(null);
    ...
    private void chaseNearbyFish(Set<Integer> fishSet) {
        ...
        var fishNpcs = ProjectX.getRs2NpcCache().query().withId(NpcID.FOSSIL_FISH_SHOAL).toList();
        ...
        List<Integer> sortedFish = fishSet.stream()
                .sorted(Comparator.comparingInt(fishIndex -> {
                            ...
                            return npc.getLocalLocation().distanceTo(ProjectX.getClient().getLocalPlayer().getLocalLocation());
                        }
                ))
