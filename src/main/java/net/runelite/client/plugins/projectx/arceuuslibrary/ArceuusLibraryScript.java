package net.runelite.client.plugins.projectx.arceuuslibrary;
...
import net.runelite.client.plugins.projectx.arceuuslibrary.KourendLibraryBridge.BookSnapshot;
import net.runelite.client.plugins.projectx.arceuuslibrary.KourendLibraryBridge.BookcaseSnapshot;
import net.runelite.client.plugins.projectx.arceuuslibrary.KourendLibraryBridge.Solved;
...
public class ArceuusLibraryScript extends Script
{
    ...
    private void tick()
    {
        try
        {
            ...
            boolean loggedIn = ProjectX.isLoggedIn();
            if (tickCount % 10 == 1)
            {
        ...
        {
            ProjectX.log("Arceuus Library tick error: " + e.getMessage());
    ...
    private void handleDeliver(BookSnapshot wanted)
    {
        ...
        Rs2NpcModel customer = activeId > 0
                ? ProjectX.getRs2NpcCache().query().withId(activeId).nearestOnClientThread()
    ...
    private Rs2NpcModel findCustomer(int minDist, boolean excludeLastDelivered, boolean excludeRecentlyAttempted)
    {
        ...
        int bestDist = Integer.MAX_VALUE;
        for (Rs2NpcModel npc : ProjectX.getRs2NpcCache().query()
    ...
    private void ensureInsideLibrary()
    {
        ...
        ProjectX.log("Player not in Arceuus Library (region " + here.getRegionID() + ") — please position manually");
    }
    ...
    private void restrictPathfinderToLocal()
    {
        ...
        ProjectX.getEventBus().post(new PluginMessage("shortestpath", "path", data));
        log.info("Pathfinder restricted to local routes (no teleports/transports)");
    }

    private void clearPathfinderRestrictions()
    {
        ProjectX.getEventBus().post(new PluginMessage("shortestpath", "clear"));
