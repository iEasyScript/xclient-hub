package net.runelite.client.plugins.projectx.valetotems;
...
import net.runelite.client.plugins.projectx.valetotems.enums.TotemLocation.RouteType;
import net.runelite.client.plugins.projectx.valetotems.handlers.*;
...
public class ValeTotemScript extends Script {
    ...
    private void executeMainLoop() {
        ...
        if (now - lastStateLogTime > 5000) {
            ...
            ProjectX.log("[ValeTotem] State: " + currentState.name() + " | Player: " + net.runelite.client.plugins.projectx.valetotems.utils.CoordinateUtils.getPlayerLocation());
        }
    ...
    private void handleBankingState() {
        ProjectX.log("[ValeTotem] handleBankingState: starting unified banking cycle");
        boolean bankingSuccess = BankingHandler.performUnifiedBankingCycle(gameSession);
        ProjectX.log("[ValeTotem] handleBankingState: result=" + bankingSuccess);
        ...
        if (bankingSuccess) {
            ...
            ProjectX.log("Reward collection frequency set to: " + RewardHandler.COLLECTION_FREQUENCY);
            
            // Log the route being used
            ProjectX.log("Using " + currentRouteType.getDescription() + " with " + currentRouteType.getMaxTotems() + " totems");
        } else {
    ...
    private void handleErrorState() {
        ProjectX.log("[ValeTotem] ERROR state. Errors so far: " + gameSession.getErrorMessages().size() + ". Attempting recovery...");
    ...
    public void shutdown() {
        ...
        super.shutdown();
        ProjectX.log("Vale Totems bot stopped.");
    }
