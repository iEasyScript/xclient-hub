package net.runelite.client.plugins.projectx.autofishing;
...
import net.runelite.client.plugins.projectx.autofishing.enums.AutoFishingState;
import net.runelite.client.plugins.projectx.autofishing.enums.Fish;
import net.runelite.client.plugins.projectx.autofishing.enums.HarpoonType;
...
public class AutoFishingScript extends Script {
    ...
    private void handleFishing() {
        if (Thread.currentThread().isInterrupted() || !ProjectX.isLoggedIn()) {
            return;
        }
    ...
    private Rs2TileObjectModel getNearbyFireOrRange() {
        ...
        try {
            return ProjectX.getRs2TileObjectCache().query().withIds(fireIds).within(15).nearestOnClientThread();
        } catch (RuntimeException e) {
            if (e.getMessage() != null && e.getMessage().toLowerCase().contains("interrupted waiting for client thread")) {
                Thread.currentThread().interrupt();
                return null;
