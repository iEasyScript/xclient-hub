package net.runelite.client.plugins.projectx.aiofighter.bank;
...
import net.runelite.client.plugins.projectx.aiofighter.shop.ShopItem;
import net.runelite.client.plugins.projectx.aiofighter.shop.ShopScript;
import net.runelite.client.plugins.projectx.aiofighter.shop.ShopType;
...
public class BankerScript extends Script {
    ...
    public boolean run(AIOFighterConfig config) {
        this.config = config;
        this.inventorySetupsPlugin = ProjectX.getPluginManager().getPlugins().stream()
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (config.bank() && needBanking() && !AIOFighterPlugin.needShopping) {
                    if(handleBanking()){
                        ProjectX.log("Banking handled successfully.");
                    }
                } else if (!needBanking() &&
                    ...
                    if (config.slayerMode()) {
                        ...
                        if (!missingIds.isEmpty()) {
                            ProjectX.log("Missing items: " + missingIds);
                            shouldWalk = handleTeleports(missingIds);
                            ProjectX.log("Should walk(Slayer Mode): " + shouldWalk);
                        }
                        else shouldWalk = true;
                    }
                    else {
                        shouldWalk = true;
                        ProjectX.log("Should walk: " + shouldWalk);
                    }

                    if (shouldWalk && Rs2Bank.closeBank()) {
                         AIOFighterPlugin.setState(State.WALKING);
                        ProjectX.log("Walking to center location: " + config.centerLocation());
    ...
    public boolean needBanking() {
        ...
        if (Rs2Inventory.isFull()) {
            ProjectX.log("Inventory is full, triggering banking.");
        ...
        if ((Rs2Inventory.emptySlotCount() <= config.minFreeSlots() && config.bank()) || needSlayerItems() || inventorySetupChanged) {
            ProjectX.log("Low free slots, missing slayer items, or inventory setup changed, triggering banking.");
            bankingTriggered = true;
            return true;
        }
        ...
        if (setupName == null || !config.useInventorySetup()) {
            ProjectX.log("Inventory setup is disabled, skipping banking.");
            return false;
        }
        ...
        if (needsFood(inventorySetup)) {
            ProjectX.log("Food required but not available, triggering banking.");
            return true;
        }
        ...
        if (needsPotion(inventorySetup, Rs2Potion.getPrayerPotionsVariants())) {
            ProjectX.log("Prayer potion required but not available, triggering banking.");
            return true;
        }
        if (needsPotion(inventorySetup, Rs2Potion.getRangePotionsVariants())) {
            ProjectX.log("Range potion required but not available, triggering banking.");
            return true;
        }
        if (needsPotion(inventorySetup, Rs2Potion.getCombatPotionsVariants())) {
            ProjectX.log("Combat potion required but not available, triggering banking.");
            return true;
        }
        if (needsPotion(inventorySetup, Rs2Potion.getMagicPotionsVariants())) {
            ProjectX.log("Magic potion required but not available, triggering banking.");
            return true;
        }
        if (needsPotion(inventorySetup, Collections.singletonList(Rs2Potion.getStaminaPotion()))) {
            ProjectX.log("Stamina potion required but not available, triggering banking.");
            return true;
        }
        if (needsPotion(inventorySetup, Rs2Potion.getRestoreEnergyPotionsVariants())) {
            ProjectX.log("Restore energy potion required but not available, triggering banking.");
            return true;
        }
        if (needsPotion(inventorySetup, Rs2Potion.getAntiPoisonVariants())) {
            ProjectX.log("Anti-poison potion required but not available, triggering banking.");
            return true;
        }
        if (needsPotion(inventorySetup, Rs2Potion.getAntifirePotionsVariants())) {
            ProjectX.log("Antifire potion required but not available, triggering banking.");
            return true;
        }
    ...
    public boolean hasSlayerItems() {
        ...
        if (!result) {
            ProjectX.log("hasSlayerItems(): Missing slayer task weakness item from inventory.");
        }
        return result;
    }
    ...
    public boolean hasProtectiveSlayerEquipment() {
        ...
        if (needed.equals("Rock hammer") || needed.equals("Rock thrownhammer")) {
            if (Rs2Inventory.contains(needed)) {
                ...
                ProjectX.log("hasProtectiveSlayerEquipment(): Missing " + needed + " in inventory.");
                return false;
            }
        }
        ...
        if (!result) {
            ProjectX.log("hasProtectiveSlayerEquipment(): Missing " + needed);
        }
        return result;
    }
    ...
    public boolean needDesertProtection() {
        ...
        if (result) {
            ProjectX.log("needDesertProtection(): In a Desert slayer location but missing watertskins.");
        }
        return result;
    }
    ...
    public boolean needProtectiveSlayerEquipment() {
        ...
        if (result) {
            ProjectX.log("needProtectiveSlayerEquipment(): In slayer mode but missing protective equipment.");
        }
        return result;
    }
    ...
    public boolean needSlayerItems() {
        ...
        if (result) {
            ProjectX.log("needSlayerItems(): Banking triggered due to slayer item deficiency or equipment issues.");
        }
        return result;
    }
    ...
    public void withdrawUpkeepItems(AIOFighterConfig config) {
        ...
        if (config.useInventorySetup() || config.slayerMode()) {
            ...
            if (setupName == null) {
                ProjectX.log("Cannot load inventory setup - null setup name");
                return;
            }
            
            Rs2InventorySetup inventorySetup = new Rs2InventorySetup(setupName, mainScheduledFuture);
            if (!Rs2Bank.isOpen()) {
                ProjectX.log("Bank didn't open, returning.");
                return;
            }
            if (config.currentInventorySetup() != null) {
                ProjectX.log("Loading equipment for: " + config.currentInventorySetup().getName());
            } else {
                ProjectX.log("Loading equipment for unknown setup (null)");
            }
            ...
            if(needSlayerItems()){
                if (config.slayerHasTaskWeakness()) {
                    if (Rs2Bank.hasBankItem(config.slayerTaskWeaknessItem())) {
                        ...
                        if(Rs2Bank.hasBankItem(config.slayerTaskWeaknessItem(), item.isStackable() ? Rs2Slayer.getSlayerTaskSize() : 1,false)) {
                            ...
                            ItemComposition itemComp = ProjectX.getRs2ItemManager().getItemComposition(Rs2Slayer.getSlayerTaskWeakness());
                        ...
                        ItemComposition item = ProjectX.getRs2ItemManager().getItemComposition(Rs2Slayer.getSlayerTaskWeakness());
                ...
                if (needProtectiveSlayerEquipment()) {
                    if (Rs2Bank.hasBankItem(Rs2Slayer.getSlayerTaskProtectiveEquipment())) {
                        ...
                        ShopScript.shopItems.add(new ShopItem(Rs2Slayer.getSlayerTaskProtectiveEquipment(), 1, ProjectX.getRs2ItemManager().getItemId(Rs2Slayer.getSlayerTaskProtectiveEquipment()), ShopType.SLAYER_SHOP));
                         AIOFighterPlugin.needShopping = true;
    ...
    private void usePoolIfNeeded() {
        ...
        Rs2TileObjectModel pool = ProjectX.getRs2TileObjectCache().query()
