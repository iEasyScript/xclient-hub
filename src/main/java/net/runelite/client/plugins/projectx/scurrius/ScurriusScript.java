package net.runelite.client.plugins.projectx.scurrius;
...
import net.runelite.client.plugins.projectx.scurrius.enums.State;
...
public class ScurriusScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (state != previousState) {
                    ProjectX.log("State changed to: " + getStateDescription(state));
                    previousState = state;
                }

                scurrius = ProjectX.getRs2NpcCache().query().withName("Scurrius").nearestOnClientThread();
                ...
                if (previousInFightRoom == null || isInFightRoom != previousInFightRoom) {
                    ProjectX.log(isInFightRoom ? "Player has entered the boss room." : "Player has exited the boss room.");
                ...
                if (!isScurriusPresent && !hasFood && !hasPrayerPotions) {
                    if (isInFightRoom) {
                        if (Rs2Inventory.hasItem(ItemID.POH_TABLET_VARROCKTELEPORT)) {
                            ...
                            ProjectX.log("No teleport available. Attempting to walk to bank (will likely fail).");
                        }
                    } else {
                        state = State.BANKING;
                    }
                ...
                if (state != State.WAITING_FOR_BOSS) {
                    ...
                    if (isScurriusPresent && !hasFood && ProjectX.getClient().getBoostedSkillLevel(Skill.HITPOINTS) < 25) {
                        state = State.TELEPORT_AWAY;
                    }

                    if (!isScurriusPresent && !isInFightRoom && hasFood && hasPrayerPotions) {
                        if (!hasRequiredSupplies()) {
                            ProjectX.log("Missing supplies, returning to BANKING.");
                            state = State.BANKING;
                        } else {
                ...
                switch (state) {
                    case BANKING:
                        ...
                        if (Rs2Bank.isOpen()) {
                            ...
                            if (!Rs2Bank.withdrawDeficit(config.potionSelection().getItemId(), requiredPotionAmount)) {
                                ProjectX.showMessage("Missing Potion in Bank");
                                shutdown();
                                return;
                            }
                            if (!Rs2Bank.withdrawDeficit(ItemID.POH_TABLET_VARROCKTELEPORT, requiredTeleports)) {
                                ProjectX.showMessage("Missing Teleports in Bank");
                                shutdown();
                                return;
                            }
                    ...
                    case FIGHTING:
                            ...
                            for (WorldPoint worldPoint : dangerousWorldPoints) {
                                if (Rs2Player.getWorldLocation().equals(worldPoint)) {
                                    ...
                                    if (safeTile != null) {
                                        ...
                                        ProjectX.log("Dodging dangerous area, moving to safe tile at: " + safeTile);
                                    }
                                }
                            }
                        }
                        ...
                        if (currentTime - lastEatTime > EAT_COOLDOWN_MS) {
                            ...
                            if (ProjectX.getClient().getBoostedSkillLevel(Skill.HITPOINTS) < randomEatThreshold && !Rs2Player.isAnimating()) {
                                Rs2Player.eatAt(randomEatThreshold);
                                lastEatTime = currentTime;
                                ProjectX.log("Eating food at " + randomEatThreshold + "% health.");
                            }
                        }
                        ...
                        if (currentTime - lastPrayerTime > PRAYER_COOLDOWN_MS) {
                            ...
                            if (ProjectX.getClient().getBoostedSkillLevel(Skill.PRAYER) < randomPrayerThreshold && !Rs2Player.isAnimating()) {
                                Rs2Player.drinkPrayerPotionAt(randomPrayerThreshold);
                                lastPrayerTime = currentTime;
                                ProjectX.log("Drinking prayer potion at " + randomPrayerThreshold + "% prayer points.");
                            }
                        }

                        Rs2NpcModel giantRatModel = ProjectX.getRs2NpcCache().query()
                    ...
                    case WALK_TO_BOSS:
                        if (!hasRequiredSupplies()) {
                            ProjectX.log("Missing supplies, restarting pathfinding and returning to Bank.");
                        ...
                        ProjectX.getRs2TileObjectCache().query().withId(ObjectID.RAT_BOSS_ENTRANCE).interact(interactionType);
                    ...
                    case WAITING_FOR_BOSS:
                        ...
                        if (!hasLoggedRespawnWait) {
                            ProjectX.log("Waiting for Scurris to respawn...");
                            hasLoggedRespawnWait = true;
                            disableAllPrayers();
                        }
                        if (isScurriusPresent) {
                            state = State.FIGHTING;
                            ProjectX.log("Scurris has respawned, switching to FIGHTING.");
                        }
                        break;
                }
    ...
    private WorldPoint findSafeTile(WorldPoint playerLocation, List<WorldPoint> dangerousWorldPoints) {
        ...
        ProjectX.log("No safe tile found!");
        return null;
    }
    ...
    private boolean hasRequiredSupplies() {
        ...
        if (currentFoodCount < foodAmount) {
            ProjectX.log("Not enough food in inventory. Expected: " + foodAmount + ", Found: " + currentFoodCount);
            return false;
        }

        int currentPrayerPotionCount = Rs2Inventory.count(potionItemId);
        if (currentPrayerPotionCount < prayerPotionAmount) {
            ProjectX.log("Not enough prayer potions in inventory. Expected: " + prayerPotionAmount + ", Found: " + currentPrayerPotionCount);
            return false;
        }
    ...
    public void disableAllPrayers() {
        ...
        ProjectX.log("All prayers disabled to preserve prayer points.");
