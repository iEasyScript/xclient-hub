...
public class GauntletHelperScript extends Script {
    ...
    public void checkNPC() {
        TIME_NPC = now;
        Tornado = ProjectX.getRs2NpcCache().query().withId(CG_TORNADO).nearest();
        hunllef = ProjectX.getRs2NpcCache().query().where(npc -> HUNLLEF_IDS.contains(npc.getId())).toList().stream()
                .findFirst()
                .orElse(null);
    ...
    private void checkVitalsStart(){
        int HP = ProjectX.getClient().getBoostedSkillLevel(Skill.HITPOINTS);
        if (HP > STAT_HP) {HP_WENT_UP = true;}
        int PRAY = ProjectX.getClient().getBoostedSkillLevel(Skill.PRAYER);
    ...
    private void checkFood() {
        ...
        int maxHp = ProjectX.getClient().getRealSkillLevel(Skill.HITPOINTS);
