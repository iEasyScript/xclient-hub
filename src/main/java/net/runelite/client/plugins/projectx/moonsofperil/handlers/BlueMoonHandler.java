...
public class BlueMoonHandler implements BaseHandler {
    ...
    public boolean isSpecialAttack1Sequence() {
        return (ProjectX.getRs2NpcCache().query().withId(NpcID.PMOON_BOSS_WINTER_STORM).nearest() != null && Rs2Widget.isWidgetVisible(bossHealthBarWidgetID) && ProjectX.getRs2NpcCache().query().withId(sigilNpcID).nearest() == null);
    }
    ...
    public void specialAttack1Sequence()
    {
        sleep(2_400);
        Rs2Prayer.disableAllPrayers();
        if (debugLogging) {ProjectX.log("Running to safe tile and waiting out the sequence");}
        Rs2Walker.walkFastCanvas(AFTER_TORNADO, true);
        boss.eatIfNeeded();
        boss.drinkIfNeeded();
        if (debugLogging) {ProjectX.log("Sleeping until the special attack sequence is over");}
        sleepUntil(() -> ProjectX.getRs2NpcCache().query().withId(sigilNpcID).nearest() != null || !Rs2Widget.isWidgetVisible(bossHealthBarWidgetID), 35_000);
    }
    ...
    public boolean isSpecialAttack2Sequence() {
        Rs2NpcModel icicle = ProjectX.getRs2NpcCache().query().withId(NpcID.PMOON_BOSS_ICICLE_UNCRACKED).nearest();
        Rs2NpcModel sigil = ProjectX.getRs2NpcCache().query().withId(sigilNpcID).nearest();
        if (icicle != null && sigil == null) {
            if (debugLogging) {ProjectX.log("An icicle has spawned – We've entered Special Attack 2 Sequence");}
            return true;
        }
        return false;
    }
    ...
    public void specialAttack2Sequence(MoonsOfPerilConfig cfg)
    {
        ...
        if (debugLogging) {ProjectX.log(ts.get() + "specialAttack2Sequence() START");}
        ...
        {
            matches = ProjectX.getRs2NpcCache().query().where(n ->
        ...
        if (matches.isEmpty()) {
            if (debugLogging) {ProjectX.log(ts.get() + "Timed-out waiting for animated icicle — aborting");}
            return;
        }
        ...
        if (debugLogging) {ProjectX.log(ts.get() + "Icicle found at " + icicle.getWorldLocation());}

        /* ---------- 2. Attack + dodge loop ------------------------------- */

        long phaseStart = System.currentTimeMillis();

        if (debugLogging) {ProjectX.log(ts.get() + "Entering attack-and-dodge loop (timeout " + PHASE_TIMEOUT_MS + " ms)");}
        Rs2Prayer.disableAllPrayers();
                ...
                System.currentTimeMillis() - phaseStart < PHASE_TIMEOUT_MS)
        {
            ...
            if (debugLogging) {ProjectX.log(ts.get() + "Attack location calculated as: " + attackTile);}
            sleepUntil(() -> net.runelite.client.plugins.projectx.moonsofperil.handlers.BossHandler.inDanger(attackTile) || invSetup.doesEquipmentMatch(),3_000);
            if (invSetup.doesEquipmentMatch()) {
                break;
            }
            if (net.runelite.client.plugins.projectx.moonsofperil.handlers.BossHandler.inDanger(attackTile)) {
                if (debugLogging) {ProjectX.log(ts.get() + "Standing on dangerous tile: " + attackTile);}
                WorldPoint safeTile = Rs2Tile.getSafeTiles(1).get(0);
                if (debugLogging) {ProjectX.log(ts.get() + "Safe tile calculated to be: " + safeTile);}

                if (safeTile != null) {
                    Rs2Walker.walkFastCanvas(safeTile, true);
                    if (debugLogging) {ProjectX.log(ts.get() + "Now standing on safe tile: " + safeTile);}
                    sleepUntil(() -> !BossHandler.inDanger(attackTile));
                    if (debugLogging) {ProjectX.log(ts.get() + "Attack tile now calculated as safe: " + attackTile);}
                }
            }
        }

        /* ---------- 3. Retreat to safespot ------------------------------- */
        if (debugLogging) {ProjectX.log(ts.get() + "Retreating to SAFE_SPOT " + SAFE_SPOT);}
        Rs2Walker.walkFastCanvas(SAFE_SPOT, true);
        boss.eatIfNeeded();
        boss.drinkIfNeeded();

        if (debugLogging) {ProjectX.log(ts.get() + "Waiting for all icicles to despawn…");}
        sleepUntil(() -> ProjectX.getRs2NpcCache().query().withId(ICICLE_NPC_ID).nearest() == null);

        if (debugLogging) {ProjectX.log(ts.get() + "specialAttack2Sequence() COMPLETE");}
    }
