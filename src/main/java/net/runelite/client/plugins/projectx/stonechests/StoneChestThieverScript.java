package net.runelite.client.plugins.projectx.stonechests;
...
public class StoneChestThieverScript extends Script {
    private static void handleThieving() {
        ...
        // Chest ID
        ProjectX.getRs2TileObjectCache().query().interact(34429, "Picklock");
    ...
    public boolean run(StoneChestThieverConfig config) {
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                final int poison = ProjectX.getClient().getVarpValue(VarPlayerID.POISON);
                if (poison > 0) {
                    Rs2Player.drinkAntiPoisonPotion();
                }
                Rs2Player.eatAt(50);
                if (Rs2Inventory.getInventoryFood().isEmpty()) {
                    ProjectX.log("No food in inventory, stopping script.");
                    shutdown();
                    return;
                }
