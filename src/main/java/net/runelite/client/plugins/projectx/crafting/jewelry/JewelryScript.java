package net.runelite.client.plugins.projectx.crafting.jewelry;
...
import net.runelite.client.plugins.projectx.crafting.jewelry.enums.*;
                        ...
                        if (shouldCutGems) {
                            ...
                            if (!Rs2Inventory.hasItem(ItemID.CHISEL)) {
                                if (!Rs2Bank.hasItem(ItemID.CHISEL)) {
                                    ProjectX.showMessage("Missing Chisel in Bank!");
                                    shutdown();
                                    return;
                                }
                        ...
                        if (craftAmount > 0) {
                            ...
                            if (!Rs2Inventory.hasItem(plugin.getJewelry().getToolItemID())) {
                                if (!Rs2Bank.hasItem(plugin.getJewelry().getToolItemID())) {
                                    ProjectX.showMessage("Missing tool item");
                                    shutdown();
                                    return;
                                }
...
//
//                                    if (staffItemID == -1) {
//                                        ProjectX.showMessage("No suitable staff found for enchanting!");
//                                        shutdown();
//                                        return;
//                                    }
//                                } else {
...
//                                            Rs2Random.waitEx(900, 100);
//                                        } else {
//                                            ProjectX.showMessage("Missing required rune: " + runeID);
//                                            shutdown();
//                                            return;
//                                        }
//                                    }
...
//                                
                            case ALCH:
                                ...
                                if (staffItemID == -1 || staffItemID == 0) {
                                    ...
                                    if (staffItemID == -1) {
                                        ProjectX.showMessage("No staff of fire-type found!");
                                        shutdown();
                                        return;
                                    }
                                }
                                ...
                                if (totalAlchJewelry == 0) {
                                    ProjectX.showMessage("All Jewelry has been alched!");
                                    shutdown();
                                    return;
                                }
                                ...
                                if (totalNatureRunes < totalAlchJewelry || (!Rs2Bank.hasItem(staffItemID) && !Rs2Equipment.isWearing(staffItemID))) {
                                    ProjectX.showMessage("Missing required items");
                                    shutdown();
                                    return;
                                }
                                ...
                                if (plugin.isUseRunePouch()) { // Check if the rune pouch is not already in the inventory
                                    if (!Rs2Inventory.hasRunePouch()) { 
                                        if (!Rs2Bank.hasRunePouch()) {
                                            ProjectX.showMessage("Rune Pouch not found! Check your magic settings");
                                            shutdown();
                                            return;
                                        }
                            ...
                            case NONE:
                                ...
                                ProjectX.showMessage("Crafting has been completed!");
                                shutdown();
                                return;
                        }
                    ...
                    case CRAFTING:
                        ...
                        Rs2TileObjectModel furnaceObject = ProjectX.getRs2TileObjectCache().query()
                                .withName("Furnace")
