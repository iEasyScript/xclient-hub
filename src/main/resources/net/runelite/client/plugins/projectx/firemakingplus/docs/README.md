...
Auto Firemaking Plus trains Firemaking on a bank-and-burn loop with two selectable methods, progressive log selection, and a live statistics overlay. It is part of the ProjectX "Plus" suite and shares the same stop-condition system and overlay as the other Plus plugins. It is fully F2P friendly.
...
| **Speed mode** | Disables ProjectX antiban for faster throughput. More detectable, throwaway accounts only. |
...
Auto Firemaking Plus is a from-scratch Plus build by pjmarz. The line-firemaking logic was adapted from the ProjectX leagues firemaking code.
