...
- **Wrong layer:** if the bug turns out to be in the engine (client APIs in `../ProjectX/`), stop and tell the user — this skill does not patch engine code.
...
- Exception stack trace appears in `/tmp/projectx-hub.log`
...
3. tail -30 /tmp/projectx-hub.log
...
- Tail of /tmp/projectx-hub.log (last 30 lines) if it grew
...
1. Restate the bug in one sentence. Identify the target plugin folder (e.g. "PestControl" → `src/main/java/net/runelite/client/plugins/projectx/pestcontrol/`). If the description is ambiguous, list candidates with Grep and pick the one whose `@PluginDescriptor` name/tags match.
...
Open `src/test/java/net/runelite/client/ProjectX.java` and confirm the target plugin class is in `debugPlugins`. If not, add the import and the class entry. `AgentServerPlugin.class` must remain in the list — that's how you talk to the client over HTTP.
...
# Launch (this compiles the Hub plugins + pulls the projectx client JAR)
./gradlew run --args='--debug' > /tmp/projectx-hub.log 2>&1 &
...
../ProjectX/projectx-cli login now --timeout 60
../ProjectX/projectx-cli state

# If welcome screen is still up:
../ProjectX/projectx-cli widgets click --text "Click here to play"
```

Ask Sonnet to verify `gameState == LOGGED_IN` in the final `state` JSON and return that JSON plus the last 30 lines of `/tmp/projectx-hub.log`. If login fails, read the `loginError` field yourself — non-member/banned/bad-creds each need a different response (most often: tell the user, stop).
...
- Stuck script? Tail the log for the script's last `ProjectX.log(...)` line and dump `/state` to see player pos / animation / varbits.
...
```bash
# Start the plugin
curl -sS -X POST -H "X-Agent-Token: $(cat ~/.projectx/agent-token)" -H 'Content-Type: application/json' \
  -d '{"className":"net.runelite.client.plugins.projectx.<pkg>.<Plugin>"}' \
...
# Collect state
curl -sS "http://127.0.0.1:8081/scripts/status?className=net.runelite.client.plugins.projectx.<pkg>.<Plugin>"
...
Consult `docs/PLUGIN_DEBUGGING_NOTES.md` as a checklist when symptoms match. If the bug lives in `Rs2*` calls or pathfinder internals (i.e. `../ProjectX/`), this skill is the wrong tool — stop and say so.

### Stage 5 — Patch *(Opus only)*

Edit the plugin source directly. Follow project rules (CLAUDE.md + `../ProjectX/AGENTS.md`):

- **Never sleep on the client thread.** Never use static sleeps — use `sleepUntil(BooleanSupplier, timeoutMs)`.
- Use `ProjectX.getClientThread().invoke(...)` for widget/varbit/world-view access.
- Use `ProjectX.getRs2XxxCache()` accessors; never instantiate caches.
...
**A. `projectx-cli` wrapper** (preferred — handles auth automatically):

```bash
../ProjectX/projectx-cli scripts reload --name <plugin-jar-name>
../ProjectX/projectx-cli scripts health
```
...
until curl -sS --max-time 2 http://127.0.0.1:8081/state > /dev/null 2>&1; do sleep 2; done
../ProjectX/projectx-cli login now --timeout 60
../ProjectX/projectx-cli widgets click --text "Click here to play"   # if present
```
...
2. No new errors in `/tmp/projectx-hub.log`.
...
- `docs/PROJECTX_CLI.md` — CLI command reference. The CLI is at `../ProjectX/projectx-cli` and wraps most endpoints with auth.
...
- **This skill does not patch engine code.** If root-cause sits in `../ProjectX/` (client APIs, pathfinder, cache), stop and tell the user. They have a separate engine-debug workflow.
- **Token auth:** prefer `../ProjectX/projectx-cli` where available — it reads `~/.projectx/agent-token` automatically. For endpoints the CLI doesn't wrap (`/scripts/deploy*`, `/state`, `/objects`), use curl with `-H "X-Agent-Token: $(cat ~/.projectx/agent-token)"`.
- **The `projectx-cli objects` command ignores `--id` and `--distance`.** For precise filters, curl `/objects` directly (see PLUGIN_DEBUGGING_NOTES.md §1).
