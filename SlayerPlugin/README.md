...
- **Inventory Setups Integration**: Uses ProjectX Inventory Setups to load correct gear per task (via task profiles)
...
- **Break Handler**: Integrates with ProjectX break handler; only allows breaks during safe states (Idle, At Location)
...
|-------|------|---------|-------------|
| `setup` | string | - | Inventory setup name to load from ProjectX Inventory Setups |
...
7. **Traveling** - Uses the ProjectX walker to navigate to the task location (from profile or auto-detected).
...
- ProjectX client version 2.1.11 or higher
- Slayer skill unlocked
- Sufficient combat level for your chosen slayer master
- Food and prayer potions in your bank
- ProjectX Inventory Setups plugin configured with gear setups matching your profile names
...
- **Inventory Setups**: Create named gear setups (e.g., "melee", "ranged", "burst") in the ProjectX Inventory Setups plugin. Your JSON profile `setup` field should match these names.
...
- **Break Handler**: The plugin integrates with the ProjectX break handler and only pauses during safe states (Idle, At Location) to avoid breaking mid-bank or mid-travel.
