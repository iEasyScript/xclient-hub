...
The ProjectX client embeds an HTTP agent server (port 8081, plugin name "Agent Server") that exposes the live scene cache, player state, inventory, NPCs, and dialogue. Two `curl` calls usually beat an hour of staring at code.
...
The CLI wrapper at `../ProjectX/projectx-cli` handles login, inventory, NPCs, dialogue, walking, banking, and script lifecycle. See `docs/PROJECTX_CLI.md` for the command reference.
...
The `projectx-cli objects` command currently ignores `--id` and `--distance` flags and just dumps the raw `/objects` endpoint with defaults. Use `curl` directly when you need precise filtering. The server-side parameters are documented in `docs/AGENT_SERVER.md`; for `/objects` they're `name`, `maxDistance` (default 20), `limit`.
...
The ProjectX wrapper checks `getTopLevelWorldView().getScene().isInstance()` and translates via `WorldPoint.fromLocalInstance(...)`. The raw client call doesn't.
...
```java
boolean instanced = ProjectX.getClient().getTopLevelWorldView().getScene().isInstance();
```
...
if (Rs2Player.getWorldLocation().distanceTo(object.getWorldLocation()) > 51) {
    ProjectX.log("...too far, walking to the object....");
...
private static boolean interactWithObject(int id, String action) {
    ...
    if (playerLoc != null && playerLoc.distanceTo(model.getWorldLocation()) > 51) {
        ProjectX.log("Object id " + id + " too far, walking...");
...
A common ProjectX plugin pattern:
...
## 6. Don't use `ProjectX.showMessage` from script threads

`ProjectX.showMessage` opens a Swing modal via `SwingUtilities.invokeAndWait`. If your script's executor is ticking every 100 ms, the next tick will interrupt the AWT-blocking thread and you'll get a flood of `InterruptedException` traces with no actual message ever shown to the user.

For debug/log indicators, use `ProjectX.log` instead — it's just slf4j, never blocks the AWT thread, and shows up in the same place users already look.
...
1. **Is the script even running?** Add a `ProjectX.log("X script started")` at the top of `run()` and watch the log on the next plugin start.
