...
This is a summary reference for Hub developers. The canonical documentation lives in the ProjectX client repo at `docs/AGENT_SERVER.md`.

## Overview

The Agent Server is an embedded HTTP server (port 8081, localhost only) in the ProjectX client that exposes game state and interactions to external tools. The Hub uses it to control scripts at runtime during automated testing.
...
| `/scripts` | GET | List all projectx plugins |
...
- `docs/PROJECTX_CLI.md` — CLI command reference (this repo)
- `docs/SCRIPT_LIFECYCLE_API.md` — Script lifecycle HTTP API details (this repo)
- `docs/AGENT_SERVER.md` in ProjectX client repo — Full HTTP API reference with examples
