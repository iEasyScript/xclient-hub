# ProjectX Dashboard Plus

![preview](assets/dashboard-top.png)

ProjectX Dashboard Plus is a passive monitoring plugin for the ProjectX RuneLite client. It opens a floating Swing window outside the game canvas with nine live-updating panels covering your session state, plus a compact sidebar panel for quick access. No HTTP server, no port binding, and no Agent Server dependency - it reads game state in-process.
...
| **Active Scripts section** | All enabled ProjectX plugins with per-plugin runtime and a Stop button per row |
...
- ProjectX RuneLite client v2.0.13 or newer
...
1. Enable **ProjectX Dashboard Plus** from the plugin list
...
- Not for vanilla RuneLite users - requires ProjectX client APIs
...
For issues or suggestions, open an issue on the ProjectX Hub repository.
