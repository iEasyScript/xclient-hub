...
The ProjectX client exposes HTTP endpoints that let the Hub start, stop, and monitor plugins at runtime. This is the mechanism for automated testing: the Hub spawns a client with Hub plugins on the classpath, then uses these endpoints to drive execution and collect results.
...
└──────────────────────┬───────────────────────────┘
                       │ HTTP (localhost:8081)
┌──────────────────────▼───────────────────────────┐
│  ProjectX Client (with Agent Server enabled)     │
│                                                  │
...
- ProjectX client built with the Agent Server plugin
...
./projectx-cli scripts start --class "com.hub.MyPlugin"
```
...
Lists all projectx plugins with active/enabled status.
...
```bash
curl -X POST -H 'Content-Type: application/json' \
  -d '{"className":"net.runelite.client.plugins.projectx.aiofighter.AIOFighterPlugin"}' \
...
```bash
curl 'http://127.0.0.1:8081/scripts/status?className=net.runelite.client.plugins.projectx.aiofighter.AIOFighterPlugin'
```
...
```java
import net.runelite.client.plugins.projectx.agentserver.handler.ScriptResultStore;
import java.util.Map;

ScriptResultStore.submit(
    "net.runelite.client.plugins.projectx.myplugin.MyPlugin",
