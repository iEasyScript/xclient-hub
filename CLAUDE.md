...
ProjectX Hub is a community plugin repository for the ProjectX RuneLite client. It maintains a separation between core client functionality and community-contributed plugins, allowing rapid plugin development without affecting client stability. Each plugin is independently built, versioned, and packaged for GitHub Releases.

## Build System Architecture

The build system uses **Gradle with custom plugin discovery and packaging**:

- **Dynamic Plugin Discovery**: `build.gradle` scans `src/main/java/net/runelite/client/plugins/projectx/` for directories containing `*Plugin.java` files
...
# Launch RuneLite debug session with plugins from ProjectX.java
...
Each plugin lives in its own package under `src/main/java/net/runelite/client/plugins/projectx/<pluginname>/`:
...
Matching resources under `src/main/resources/net/runelite/client/plugins/projectx/<pluginname>/`:
...
- `minClientVersion` - Minimum ProjectX client version required
...
If a plugin needs additional libraries beyond the ProjectX client:

1. Create `src/main/resources/net/runelite/client/plugins/projectx/<pluginname>/dependencies.txt`
...
1. Edit `src/test/java/net/runelite/client/ProjectX.java`
...
1. Scans `src/main/java/net/runelite/client/plugins/projectx/` for directories
...
5. For each plugin:
   ...
   - Configures compilation classpath with ProjectX client
...
## ProjectX CLI & Agent Server

The ProjectX client embeds an HTTP server (Agent Server plugin, port 8081) that the Hub uses for automated testing. Reference docs are mirrored in this repo:

- **CLI command reference**: `docs/PROJECTX_CLI.md` — login, script lifecycle, inventory, NPCs, walking, banking, etc.
...
1. **projectx-cli** — bash wrapper for the agent server (`./projectx-cli scripts deploy|reload|undeploy|health|results`)
...
```
edit source → projectx-cli scripts reload → projectx-cli scripts health → observe logs → repeat
```
...
TrialInfo info = ProjectX.getClientThread().invoke(() -> TrialInfo.getCurrent(client));
...
**Always use `ProjectX.getClientThread().invoke()`** when accessing:
...
**Important:** `@Subscribe` event handlers (`onGameTick`, `onClientTick`, `onVarbitChanged`, `onGameObjectSpawned`, etc.) **run on the client thread automatically**. You do NOT need to wrap client API calls in `ProjectX.getClientThread().invoke()` inside these handlers.
...
public class MyScript {
    private final EventBus[AutoRunScript.java](src/main/java/net/runelite/client/plugins/projectx/qualityoflife/scripts/AutoRunScript.java) eventBus;
